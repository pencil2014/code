<template>
  <div class="finance-page">
    ...
    <div class="finance-list-container">
      ...
      <BaseDialog :option="addDialog" ref="addDialog" @close="dialogClose('add', $event)" />
...
// 统一设置规则提醒
const ruleMsg = [{ required: true, message: ' ', trigger: ['blur','change']}] // 请填写内容
...
const formItem = [
  ...
  { label: '业务类型', type: 'select', prop: 'orderBusinessTypes', multiple: true, r: true,
    selectOptions: businsessTypes, filterable: true,
    change: (val, item) => {
      if (item.orderBusinessTypes.includes('*')) {
        item.orderBusinessTypes = ['*']
        businsessTypes.forEach(ele => {
          if (ele.value !== '*') {
            ele.disabled = true
          }
        })
      } else {
        businsessTypes.forEach(ele => ele.disabled = false)
      }
    }
  },
...
export default {
  ...
  async created() {
    ...
    this.tableOption.columns = [
      ...
      {
        label: '业务类型',
        prop: 'orderBusinessTypes',
        type: 'text',
        formatter: ({row}) => {
          let { orderBusinessTypes } = row
          if (orderBusinessTypes.includes('*')) {
            return '全部'
          }
          return businsessTypes.filter(item => orderBusinessTypes.includes(item.value))?.map(ele => ele.label).join('，')
        }
      },
      {
        label: '是否启用',
  ...
  methods: {
    getBusiness () {
      businessServiceList({}).then((res) => {
        let base = res.data.map(item => {
          let { businessType, name } = item
          return {
            label: name,
            value: businessType
          }
        })
        base.unshift({value: '*', label:'全部'})
        businsessTypes.splice( 0, 100, ...base)
        this.getData()
      })
    },
    // 启用、禁用
    ...
    showDialog(type) {
      ...
      this.$nextTick(() => {
        if (type === 'add') {
          this.addDialog.form.enabled = 'yes'
          this.addDialog.form.orderBusinessTypes = ['*']

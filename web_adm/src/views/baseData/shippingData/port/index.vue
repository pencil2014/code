  ...
  <div class="menu">
          ...
          <div class="search-containe">
            <div>
              ...
              <el-input
                ...
                class="input-width"
                size="mini"
                v-model="queryForm.portCode"
                placeholder="代码/中英文模糊查询"
              ></el-input>
              ...
              <el-select
                ...
                clearable
                size="mini"
                v-model="queryForm.portAttribute"
              >
                <el-option
                  v-for="(o, key) in dictMap['portAttribute']"
                  :key="key"
                  :label="$t(`dict.portAttribute.${o.value}`)"
                  :value="o.value"
                ></el-option>
              </el-select>
             <span class="filter-txt">系统航线:</span>
              <el-autocomplete
                          size="mini"
                          v-model="queryForm.sysLineCname"
                          :fetch-suggestions="
                            (queryString, cb) => {
    ...
    <div class="components-container m-t-8">
      ...
      <el-table class="port-table" v-loading="loading" :data="portList" style="margin: 16px 0px">
        ...
        >
           <template slot-scope="scope">
            {{ scope.row.updatedTime ? scope.row.updatedTime.replace('T',' ') : ''}}
          </template>
        </el-table-column>
        <el-table-column
          align="center"
    ...
    >
      ...
      >
        ...
        <el-form-item
          label="所属港口"
          v-if="createItem.portType == 'wharf'"
          prop="parentCname"
        >
          <el-autocomplete
            size="mini"
        ...
        <el-form-item :label="portCode"  prop="portCode">
          <el-input
            style="width: 80%"
            size="mini"
            :disabled="createItem.portId != ''"
            v-model="createItem.portCode"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="航线"  prop="sysLineCname">
          <el-autocomplete
            size="mini"
            v-model="createItem.sysLineCname"
            style="width: 80%"
            :fetch-suggestions="
              (queryString, cb) => {
                sysLineQuery(queryString, cb);
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        portCode: [
          { required: true, message: "代码不能为空", trigger: "blur,change" },
        ],
        cname: [
          {
            required: true,
        ...
        sysLineCname: [
          { required: true, message: "航线不能为空", trigger: "blur,change" },
        ],
        parentId: [
          {
            required: true,
      ...
      logOption: {
        ...
        },
        {
          ...
          label: '更新时间',
          type: 'text',
          formatter: ({ cellValue }) => cellValue ? cellValue.replace('T', ' '): '',
        },
      ],
      }
    };
  ...
  methods: {
    ...
    portSelect(val) {
      this.createItem.parentId = val.portId;
      this.createItem.parentCname = val.value;
    },
    sysLineSelect(val,type) {
      if(type==='query'){
        this.queryForm.sysLineCname = val.cname;
        this.queryForm.sysLineCode = val.sysLineCode;
      }else{
        this.createItem.sysLineCname = val.cname;
    ...
    handleReset() {
      ...
      this.queryForm.portType = undefined;
    ...
    handleEdit(row) {
      ...
      if (row) {
        ...
        this.open = true;
        if (
          this.createItem.parentId &&
          this.createItem.portId != this.createItem.parentId
        ) {
          //初始化 港口数据
          let params = {};
          Object.assign(params, {
    ...
    getList(isSearch) {
      ...
      portList(this.tableQuery).then((res) => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          portSave(this.createItem).then((res) => {

  ...
  <div class="finance-table" :class="{'max__height__table':$attrs.autoMax}" :id="option.id || 'option1'">
    ...
    <div class="finance-el-table" :style="option.style||{}">
      ...
      <vxe-table
        ...
        size="mini"
        :border="option.border === true || 'inner'"
      ...
      >
        ...
        <vxe-column
          ...
          :sortable="option.sortable && item.sortable === true"
        ...
        <!-- :show-overflow="(item.showOverflow === false ? false : true) && item.type !== 'columnTip'" -->
          ...
          <template #default="{ row, column, rowIndex }">
            <el-tooltip placement="top" v-if="item.isTool || (row[item.toolKey] && row[item.toolKey] !== '')">
              <div slot="content" v-if="item.newline">
                <div v-for="(i, k) in row.isTool" :key="k">{{i}}</div>
              </div>
			        <div slot="content">{{ row[item.toolKey] || row.isTool || item.isTool}}</div>
              <!-- <div slot="content" v-else>{{row[item.toolKey]}}</div> -->
...
.finance-table .table-icon_tips {
	display: inline-block;
	width: 12px;
	height: 12px;
	background: url(../../../../assets/icon_tips.png) 0 0 no-repeat;
	vertical-align: middle;
    margin-top: -3px;
	padding-left: 2px;
}

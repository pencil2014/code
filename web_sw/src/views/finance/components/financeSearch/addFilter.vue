<template>
	...
	<div
		class="search-filter"
		:class="{
			dateWith: filterGroups[item.key] && filterGroups[item.key].type === 'daterange',
	...
	>
		...
		<div class="el-row">
			<div class="el-col filter-left">
				...
				<el-select v-model="item.key" size="mini" :placeholder="item.placeholder || '请选择'" filterable @change="handleSelectQuery">
					<el-option v-for="(child, key) in filterGroups" :key="key" :label="child.label" :value="key" :disabled="child.disabledInAdd"></el-option>
				</el-select>
			</div>
			<div class="el-col filter-right" v-if="rightShow" :class="{ scope__select: filterGroups[item.key] && filterGroups[item.key].scope }">
				<el-select v-if="filterGroups[item.key] && filterGroups[item.key].scope" v-model="item.scope">
					<el-option v-for="item in scopeCfg" :key="item.value" :label="item.label" :value="item.value"></el-option>
				</el-select>
				<el-popover
				...
				<div class="multi-select-tag" v-else-if="filterGroups[item.key]">
					...
					<el-autocomplete
						...
						@change="
							value => {
								filterGroups[item.key].change && filterGroups[item.key].change(value, item)
							}
						"
						@select="
							value => {
								filterGroups[item.key].select && filterGroups[item.key].select(value, item)
							}
						"
						@clear="
							item.req = ''
					...
					<el-select
						...
						@change="
							value => {
								filterGroups[item.key].change && filterGroups[item.key].change(value)
							}
						"
						@clear="
							item.value = ''
							filterGroups[item.key].clearFn && filterGroups[item.key].clearFn()
						"
					>
						<!-- 未进行远程请求的，不需自定义过滤方法 -->
					...
					<DateRange v-else-if="['lsDaterange', 'lsDatetimerange'].includes(filterGroups[item.key].type)" :options="filterGroups[item.key]" :item="item" />
					<!-- 只考虑常见date， daterange -->
          ...
          expandTrigger="click"-->
					...
					<!-- 范围值 -->
					<div v-else-if="filterGroups[item.key].type === 'range'" @mouseenter="rangeEnter(item)" @mouseleave="rangeLeave(item)">
						<el-input
							ref="range1"
							size="mini"
						...
						></el-input>
						<i v-if="item.showClose" class="el-icon el-icon-circle-close" style="font-size: 14px; color: #c0c4cc; width: 12px; line: height 20px; cursor: pointer" @click="clearRange(item)"></i>
						<!-- clearable -->
		...
		<!-- 至少保留一个搜索条件 -->
		<i v-if="searchInputGroup.length > 1" class="el-icon-close filter-close" @click="handleDeleteFilter(index)"></i>
	</div>
</template>

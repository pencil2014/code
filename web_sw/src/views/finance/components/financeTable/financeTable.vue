              ...
              <template v-else>
                ...
                >{{scope.row[item.prop]}}</el-button>
							...
							<template v-else-if="option.multiRow">
								<template v-if="item.arrow && item.multi">
									<div v-for="(child, index) in scope.row[option.multiKey]" :key="index" :class="{ noBorder: !scope.row.toggle, pl40: index > 0 }" v-show="index == 0 || scope.row.toggle">
										<i
										v-if="scope.row[option.multiKey].length > 1 && index === 0"
										...
										"
									    ></i>
										{{(item.formatter && item.formatter(child)) || child[item.prop]}}
									</div>
								</template>
								<template v-else-if="item.arrow">
									<i
...
import NewPagination from '../financeTableMass/newPagination'

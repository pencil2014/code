<template>
  ...
  >
    ...
    <el-form-item
      ...
      prop="inviteCode"
    >
      <el-select v-model="form.inviteCode" 
        placeholder="请选择" 
        clearable
        remote
        filterable
        :disabled="bdEmployeeName !== ''"
        :remote-method="(val) => getInviteCodeList(val)"
        @change="changeInviteCode"
      >
        <el-option
          v-for="(item, m) in inviteCodeList"
          :key="m"
          :label="item.label"
          :value="item.value"
        >
    ...
    <el-form-item
      ...
      <template v-if="isCrm && maintainerListCrm.length">
        <el-select 
          ...
          :disabled="bdEmployeeStatus || bdEmployeeName !== '' || form.inviteCode !== ''"
        >
          <el-option
...
import { groupList, inviteCodeList } from '@/api/swcms/index'
export default {
  props: {
    customer: {
      type: Object,
  ...
  created() {
    ...
    if (isCrm) {
      ...
      if (customerBdList?.length) {
        ...
        this.saveMaintainerListCrm = JSON.parse(JSON.stringify(this.maintainerListCrm))
      }
      if (customer.inviteCode && customer.inviteCode !== '') {
        let obj = {
          label: customer.bdEmployeeName,
          value: customer.bdEmployeeId
        }
        this.maintainerListCrm.push(obj)
        this.bdEmployeeStatus = true
  ...
  methods: {
    ...
    maintainerQuerySearch(str = '') {
      if (this.isCrm && str === '') {
        this.maintainerListCrm = JSON.parse(JSON.stringify(this.saveMaintainerListCrm))
      } else {
        let data = {
          name: str,
          state: 'valid',
    ...
    // 邀请码
    getInviteCodeList(val = '') {
      let data = {
        employeeName: val
      }
      inviteCodeList(data).then(res => {
        this.inviteCodeList = res.data.map(item => {
          let { inviteCode, employeeName, employId} = item
          return {
            value: inviteCode,
            label: `${inviteCode}:${employeeName}`,
            employId,
            employeeName
          }
    ...
    // 选中邀请码
    changeInviteCode(val) {
      let item = this.inviteCodeList.find(item => item.value === val)
      if (this.isCrm) {
        this.maintainerListCrm = [{
          value: item.employId,
          label: item.employeeName
        }]
        this.form.bdEmployeeId = item.employId
      } else {
        this.maintainerList = [{
          value: item.employId,
          label: item.employeeName

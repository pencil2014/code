      ...
      <div class="price-box">
        <div class="item-box" v-for="(op, i) in form.priceList" :key="i">
          <div class="item">
            ...
            >
              <el-input v-model="op.nor40Add" placeholder="请输入价格"></el-input>
            </el-form-item>
          </div>
          <!-- <div class="btn-box">
...
import { priceRuleSave, groupList, priceRuleValid } from '@/api/swcms/index'
export default {
  props: {
    option: {
      type: Object,
      ...
      groupList(data)
        .then((res) => {
          ...
          if (this.option.type === 'edit') {
            ...
            // 处理线上历史数据只有4中等级的数据编辑需支持5中等级
            let formList = this.form.priceList.map(item => {
            ...
            Object.assign(this.form, {
              groupPriceRuleId: detailList[0].groupPriceRuleId,
              podCode: detailList[0].podCode,
              podCountryCode: detailList[0].podCountryCode,
              sysLineCode: detailList[0].sysLineCode,
              shipCarrierCode: detailList[0].shipCarrierCode,
              priceList: formList
            })
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          priceRuleSave(data)
            .then((res) => {
              if (res.code === 0) {
                if (res.data && res.data.isEnable) {
                  if (res.data.isEnable === 'yes') {
                    this.$msgErrClose('该加价规则已经存在，请勿重复添加')
                  } else {
                    this.$confirm('该价规则已经存在，状态为失效，请勿重复添加，是否将原加价规则生效', '提示', {
                      confirmButtonText: '确定',
                      cancelButtonText: '取消',
                      type: 'warning'
                    }).then(() => {
                      let groupPriceRuleIds = [res.data.groupPriceRuleId]
                      let data = {
                        groupPriceRuleIds,
                      }
                      priceRuleValid(data)

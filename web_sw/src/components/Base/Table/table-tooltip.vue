...
export default {
  ...
  methods: {
    ...
    showTooltip(cell) {
      ...
      // 宽度小于360加1px的处理原因是：小于满一行显示时可能因为元素宽度不固定的多或者少0.几px，导致tooltip内容换行错误
      let textWidth = textDom.width >= 360 ? textDom.width : textDom.width + 1
      let textHeight = textDom.height
    ...
    hiddenToolPosition(txtWidth, cellWidth, cellLeft, cellTop, textHeight) {
      const toolHight = textHeight,
        toolWidth = txtWidth, //340 < txtWidth ? 360 : txtWidth + 22

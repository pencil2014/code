...
router.beforeEach(async (to, from, next) => {
  ...
  if (hasToken) {
    if (to.path === '/login') {
      ...
      if (hasRoles) {
        ...
        try {
          ...
          await store.dispatch('charge/setPolAllowFinCloseInput')
    ...
    if (whiteList.indexOf(to.path) !== -1) {
      ...
      // 取消后进入登录页面，登录后仍跳转到此forget页面。
      // 原因是：forget操作完可能会跳转到'/'，此时会重新设置 lastUrlNoLogin ，而此时location的值仍处于forget
      // 解决：未登录 & from为forget， to为首页，不设置 lastUrlNoLogin
      if (!(from.path === '/forget' && to.path === '/dashboard')){

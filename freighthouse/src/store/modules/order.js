...
import ColumnFile from '@/views/order/order/components/fileColumn.vue'
...
import ColumnAgree from '@/views/order/order/components/columnAgree.vue'
...
import { basePortListByCodes, baseShipCarrierInfo, getBookParseResult } from '@/api/base'
	...
	columnsBase(handleOrderNoInfo, handleClickBdRemark, handleSaveRemark) {
		return [
			...
			},
			{
				...
				disabled: false,
				formatter: (row, column) => {
					return getDictLabel('releaseCustomsStatus', row.issueStatusStr)
				}
			},
			{
				label: '预计到港时间',
...
const cargoNameList = {
	defaultColumns: ['fileNos', 'status', 'refuseRemark', 'auditTime', 'auditEmployeeName', 'cargoDesc', 'cargoDescEn', 'hscode', 'className', 'chapterName', 'cargoProperty', 'dangerousGrade', 'unNo', 'auditStatus', 'createdName', 'createdTime']
}
const penddingList = {
	defaultColumns: [
		'fileNoList',
...
const state = {
	...
	allServicesMenu: [
		...
		{
			cname: '各类检验',
			ename: 'inspection and detection',
		...
		columnsBase(handleViewSo, handleViewCustName, handleOrderNo, handleToOrderFee, handleArrearAmt, handleOverQuotaAmt, handleViewWorkFlowActList, handleViewFileList) {
			return [
				...
				},
				{
					...
					disabled: false,
					formatter: (row, column) => {
						return getDictLabel('yesNo', row.isOverQuota)
					}
				},
				// {
				// 	label: '超额币种',
				...
				},
				{
					...
					disabled: false,
					formatter: (row, column) => {
						return getDictLabel('yesNo', row.isArrear)
					}
				},
				// {
				// 	label: '超期币种',
	...
	cargoAuditList: {
		// 默认表头
		defaultColumns: cargoNameList.defaultColumns,
		// 自定义表头特殊处理
		specialColumns: {},
		// 表头库
		columnsBase() {
			return [
				...
				},
				{
					...
					width: '150',
					formatter: (row, column) => {
						return getDictLabel('cargoProperty', row.cargoProperty)
					}
				},
				{
					label: '危险等级',
					...
					width: '150',
					formatter: (row, column) => {
						return getDictLabel('dangerousGrade', row.dangerousGrade)
					}
				},
				...
				},
				{
					...
					width: '150',
					formatter: (row, column) => {
						let status = getDictLabel('auditStatus', row.auditStatus)
						let color = row.auditStatus === 'pass' ? '#33B18A' : row.auditStatus === 'refuse' ? '#CD4130' : '4A4A4A'
	...
	cargoNameList: {
		...
		specialColumns: {},
		// 表头库
		columnsBase(handleViewFileList) {
			return [
				{
					label: '申报附件',
					key: 'fileNos',
					...
					width: 100,
					formatter: (row, column) => {
						return row.fileNos ? `<el-button type="text" class="underline">查看</el-button>` : ''
					},
					method: (index, row) => {
						handleViewFileList(index, row)
					}
				...
				},
				{
					...
					width: '150',
					formatter: (row, column) => {
						return row.status === 'valid' ? `<span style='color:#33B18A;'>有效</span>` : `<span style='color:#CD4130;' >无效</span>`
					}
				},
				{
					label: '创建人',
	...
	// 待接单列表
	penddingList: {
		defaultColumns: penddingList.defaultColumns,
		// 自定义表头特殊处理
		specialColumns: {},
		// 表头库
		columnsBase(handleViewFileList) {
			return [
				...
				},
				{
					...
					width: '180',
					formatter: (row, column) => {
						return row.serviceNameList.length > 0 ? row.serviceNameList.join() : ''
					}
				},
				{
					label: '出口单号',
				...
				},
				{
					...
					width: '150',
					formatter: (row, column) => {
						return getDictLabel('importApplyStatus', row.applyStatus?.toLowerCase())
					}
				},
				{
					label: '进口单号',
				...
				{
					label: '接单人',
					key: 'handleEmployeeName',
					prop: 'handleEmployeeName',
					disabled: false,
					width: '150'
				},
				{
...
const mutations = {
	...
	setPolAllowFinCloseInput(state, value) {
		state.finCloseStatusCanEdit = value === 'Y'
	},
...
const actions = {
	...
	setPolAllowFinCloseInput(context, params) {
		if (state.finCloseStatusCanEdit === null) {
			orderConfigString({ key: 'POL_ALLOW_FIN_CLOSE_INPUT' }).then(res => {
				context.commit('setPolAllowFinCloseInput', res.data)
			})
		}
	},
	// 请求下载blob流文件
	...
	compareFile( { state }, params) {
		return new Promise((resolve, reject) => {
			// 托书识别结果
			let { orderNo, fileNo } = params
			if (!orderNo || !fileNo) {
				reject('err')
			}
			console.log('托书识别结果', params)
			getBookParseResult({
				orderNo,
				fileNo
			})
			.then(res => {
				if (res.data && res.data.contrastContent) {
          resolve(res.data || {})
				} else {
					reject('err')
				}

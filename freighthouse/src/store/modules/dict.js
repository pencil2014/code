...
const getDefaultState = () => {
	return {
      ...
      // 前端字典 收款登记类型
      regTypeWeb: [
        { label: '收款登记', value: 'receivable_reg' },
        { label: '预收款', value: 'receivable_prerev' },
      ],
      onlyReceivePay: [
        { label: '应收', value: 'receive' },
        { label: '应付', value: 'pay' },
      ],
		}
	}
...
const actions = {
	...
	basePortList({ commit }, param) {
		let { queryString, portAttribute, state, isEn, systemCode, portType } = param
		return new Promise((resolve, reject) => {
			let fun = systemCode === 'CUST' ? custBlPortList : basePortList
			fun(queryString, portAttribute, state, portType)
				.then(response => {
					let results = []
	...
	querySettleUnit({ commit }, payload, needAll) {
      ...
      console.log(payload, payload.isForbid);
      if (payload.isForbid){
        data[payload.receipayType === 'receive' ? 'flitByRecvFeeForbid' : 'flitByPayFeeForbid'] = 'yes'
      }
	...
	queryFinSupplierList({ commit }, param) {
      ...
      if (param.flitByPayFeeForbid){
        data.flitByPayFeeForbid = 'yes'
      }

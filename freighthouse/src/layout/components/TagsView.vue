<template>
	<div id="tags-view-container" class="tags-view-container">
      ...
      <div id="tags">
			...
			>
				{{ serviceArr.includes(tag.name) ? getTitle(tag) : listPageArr.includes(tag.name) ? getListPageTitle(tag) : tag.title }}
...
export default {
	...
	methods: {
		...
		getListPageTitle(tag) {
			// 订单列表、协同单列表打开的TAB页增加（海）（空）（铁）标识
      let { name, title } = tag
			let prefix = ''
		...
		closeSelectedTag(view) {
			this.$store.dispatch('tagsView/delView', view).then(({ visitedViews }) => {
				if (this.isActive(view)) {
					// this.$router.go(-1)
					if (!this.routerBack('', '', { isFromTagClose: true })) {

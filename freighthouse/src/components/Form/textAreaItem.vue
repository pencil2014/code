...
export default {
  ...
  watch: {
    ...
    speciesStrArr: {
      handler() {
        !this.isMBL && this.inputMethod();
      },
      deep: true,
    },
    switchInfoFlag() {
      if (this.model === this.escapeHtml(this.value)) {
        console.log('切换详情的更新', this.keys)
        this.model = this.escapeHtml(this.value);
        document.getElementById(this.id).textContent = this.model;
        this.textLen = this.getLen(this.model);
        this.blurMethod();
        this.inputMethod();
  ...
  computed: {
    ...
    isMBL() {
      return this.blType && this.blType === 'mbl'
    },
  },
  mounted() {
    this.$nextTick(() => {
          ...
          if (len > this.num) {
            ...
            for (let i = 0; i < forLen; i++) {
              ...
              if (spindex === -1) {
                ...
                if (stayDh) {
                  str[lsIndex + spindex - i] = ',\n'
                } else {
                  str[lsIndex + spindex - i] = str[lsIndex + spindex - i] + '\n'
                }
              }
              item = str.join("");
            }
      ...
      if (len) {
        ...
        for (let i = 0; i < len; i++) {
          if (
            ...
            (this.isMBL || !this.speciesStrArr.includes(result[i]))
          ) {
            val = val.replaceAll(
              result[i],

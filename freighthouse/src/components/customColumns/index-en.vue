<template>
	<BaseDialog :config="dialogConfig" :callback="dialogCallback" ref="baseDialog" class="custom-columns-dialog">
		<Transfer
			class="transferCont-en"
			v-model="customColumns"
			:target-order="'push'"
			ref="transfer"
			id="transfer"
			filterable
			filter-placeholder="Please input"
			:titles="titles"
			:button-texts="['Join the left side', 'Join right']"
		...
		>
			<!-- :data="columnsBase" -->
			<el-button type="primary" class="transfer-footer" size="mini" @click="handleSetTop" slot="right-footer">One Click Top</el-button>
			<span slot="right-footer" class="transfer-footer-tip">Table headers support dragging and sorting</span>
			<div slot-scope="{ option }" class="transfer-list-label">
				{{ option.label }}
				<img class="icon-toTop" :src="require('@/assets/icon_toTop.png')" @click.prevent="handleToTop(option)" title="Sticky" />
				<img class="icon-up" :src="require('@/assets/icon_up.png')" @click.prevent="handleUp(option)" title="Move up" />
				<img class="icon-down" :src="require('@/assets/icon_down.png')" @click.prevent="handleBottom(option)" title="Move down" />
			</div>
		</Transfer>
		<!-- 模板名称浮窗 -->
		<el-popover popper-class="search-template-popover" placement="bottom" width="240" v-model="popVisible" @show="showPopver">
			<div class="flex">
				<span class="span-text">Template Name</span>
				<el-input type="text" size="mini" maxlength="32" show-word-limit v-model="templateName" placeholder="Please enter the name of the template" class="temp-input" style="width: 164px"> </el-input>
			</div>
			<div style="text-align: right; margin: 8px 0 0">
				<el-button size="mini" type="default" @click="handleCancel" class="ui-mini-btn">Cancel</el-button>
				<el-button type="primary" size="mini" @click="handleSaveTemplate" class="ui-mini-btn">Ok</el-button>
			</div>
			<el-button v-if="!hideTemp && showModel" type="primary" class="custom-template-save-btn" slot="reference">Template Save</el-button>
		</el-popover>
		<div v-if="!hideTemp && showModel" class="custom-dash-line"></div>
		<div v-if="!hideTemp && showModel" class="custom-template-list">
			<span class="custom-template-title">Table Header Template:</span>
...
import Transfer from '@/components/transfer-en/main.vue'
...
export default {
	data() {
		return {
			...
			templateName: '',
			dialogConfig: {
				title: this.title ? this.title : 'Customized table headers',
				show: true,
				width: '1000px'
			},
			titles: this.columnsTitle ? this.columnsTitle : ['Optional table header library', 'Customized table headers'],
			// 自定义表头
	...
	methods: {
		...
		handleSaveTemplate() {
			...
			if (!this.templateName) return this.$message.error('Template name cannot be empty!')
      ...
      return arrList.filter(item => key.find(v => v === item))
		...
		handleCustomColumns(done) {
			...
			if (!this.customColumns.length) {
				return this.$message.error('Please select a custom header')
			}
			done()
...
.transferCont-en .el-transfer-panel__header {
	height: 30px;
	line-height: 30px;
	padding-left: 10px;
}
.transferCont-en .el-transfer-panel {
	overflow: visible;
	width: 270px;
	text-align: left;
}
.transferCont-en .el-transfer-panel__body {
	height: 285px;
}
.transferCont-en .el-transfer-panel__list .el-checkbox {
	margin-right: 0;
}
.transferCont-en .el-transfer-panel__list .el-checkbox {
	.icon-up,
...
.transferCont-en .el-transfer-panel__list .el-checkbox:hover {
	background-color: #f4f7ff;
}
// .transferCont-en div:nth-child(3) .el-transfer-panel__list .el-checkbox{
//   cursor: move;
// }
.transferCont-en div:nth-child(3) .el-transfer-panel__list .el-checkbox:hover {
	.icon-up,
	.icon-down,
	.icon-toTop {
		color: #3e80f5;
...
.transferCont-en .el-transfer-panel__list.is-filterable {
	height: 245px;
}
.transferCont-en .el-transfer-panel__filter {
	margin: 10px;
}
.transferCont-en .el-transfer-panel .el-transfer-panel__header .el-checkbox {
	line-height: 30px;
}
.transferCont-en .el-transfer-panel .el-transfer-panel__header .el-checkbox .el-checkbox__label {
	font-size: 14px;
}
.transferCont-en .el-transfer-panel__filter .el-input__inner {
	border-radius: 4px;
	height: 20px;
	line-height: 20px;
}
.transferCont-en .el-transfer-panel__filter .el-input__prefix {
	left: 0;
	line-height: 20px;
}
.transferCont-en .el-transfer-panel__filter.el-input--prefix .el-input__inner {
	padding-left: 30px;
}
.transferCont-en .el-transfer-panel__item {
	padding-left: 10px;
	height: 28px;
	line-height: 28px;
}
.transferCont-en .el-transfer-panel__item.el-checkbox .el-checkbox__label {
	line-height: 28px;
}
.el-checkbox__label {
	font-size: 12px;
}
.transferCont-en .el-checkbox__input.is-disabled + span.el-checkbox__label {
	cursor: pointer;
}
.transferCont-en .el-transfer__buttons {
	padding: 0 50px;
}
.transferCont-en .el-transfer__button {
	// width: 72px;
	// height: 31px;
	// line-height: 31px;
	// display: block;
...
//   }
// }
.transferCont-en .el-transfer-panel .el-transfer-panel__footer {
	bottom: -29px;
	height: 30px;
}
.transferCont-en .transfer-footer {
	margin: 3px 0 0 0;
}
.transferCont-en .transfer-footer-tip {
	font-size: 12px;
	line-height: 16px;
	color: #888;
	text-align: right;
...
.transferCont-en .transfer-list-label {
	position: relative;
	img {
		position: absolute;
		line-height: 12px;

...
export default class {
  constructor(option) {
    ...
    // 财务打印时会出现提示框，未能找到原因。 强制更改元素位置,透明度,宽高看是否仍有问题
    let messageArr = document.querySelectorAll('.el-message')
    if (messageArr && messageArr.length) {
      [...messageArr].forEach(ele => {
        ele.style.position = 'absolute'
        ele.style.top = '0'
        ele.style.left = '0'
        ele.style.zIndex = '-1000 !important'
        ele.style.width = '0'
        ele.style.height = '0'
        ele.style.opacity = '0'
      })
    ...
    // 此方法,仍有延迟,
    // window.forceMessageClose = true
    // ElementUI.Message.closeAll()
    // window.forceMessageClose = false

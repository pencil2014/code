<template>
	<div class="filter-container search-layout">
		<div class="filter">
			<search-filter v-for="(item, index) in config" :key="index" :item="item" :tableQuery.sync="tableQuery" :callback="handleBtn"> </search-filter>
			<!-- 插槽需要自行处理tableQuery-->
			<slot />
			<search-btn :scenesCode="scenesCode" :callback="handleBtn" :searchTemplateShow="searchTemplateShow" :searchConditionHide="searchConditionHide"></search-btn>
		...
		<div class="search-template" v-show="searchTemplateShow && !searchConditionHide">
			<div class="template-list" v-if="searchConditionList.length">
				<span class="label">Select a template：</span>
				<span class="template-li" :class="templateName === item.composeKey ? 'active' : ''" v-for="(item, index) in searchConditionList.filter((item, index) => index < 5)" :key="'condition' + index" @click="handleClickCondition(item, index)">
			...
			<div class="template-operate">
				<span class="mr10" v-show="searchConditionList.length > 5">
					More templates
					<el-select size="mini" v-model="templateName" placeholder="Please select more templates" @change="handleChangeCondition">
				...
				<el-popover popper-class="search-template-popover" placement="bottom" width="240" v-model="visible" @show="showPopver">
					<div class="flex">
						<span class="span-text">Template Name</span>
						<el-input type="text" size="mini" maxlength="32" show-word-limit v-model="composeKey" placeholder="Please enter the name of the template" class="temp-input" style="width: 164px"> </el-input>
					</div>
					<div style="text-align: right; margin: 8px 0 0">
						...
						<el-button type="primary" size="mini" @click="handleSaveAsTemplate" class="ui-mini-btn">Ok</el-button>
					</div>
					<el-button type="text" size="mini" slot="reference">{{ !searchConditionList.length ? 'Save' : 'Save as' }}</el-button>
				</el-popover>
				<el-button type="text" size="mini" @click="handleSaveTemplate" v-show="searchConditionList.length">Save</el-button>
			</div>
		</div>
		<div class="search-fieldValue-cont" v-if="searchValueGroup.length">
			<div class="label-en">Query criteria：</div>
			<div class="filedValue-list" v-for="item in searchValueGroup" :key="item.key">
				<el-tooltip placement="bottom" effect="dark" :content="item.fieldValue.toString()">
					<div slot="content">
						<div v-for="(el, index) in getFieldValues(item)" :key="index">{{ el }}</div>
					</div>
					<div class="value">{{ item.label }}: {{ maptext(item) }}</div>
				</el-tooltip>
	...
	methods: {
		getFieldValues(item) {
			if (typeof item.fieldValue == 'string') {
				...
				let str = item.fieldValue.toString()
				return str.split(';')
			}
		},
		...
		handleSaveTemplate() {
			...
			this.$confirm('Whether to save the template?', 'Warning', {
		...
		getSearchConditionInfo(changeRole, isFirstLoad) {
			...
			searchConditionInfo({
				// 有传scenesCode 优先取scenesCode
				scenesCode: this.scenesCode ? this.scenesCode : this.roleSwitchRouters.includes(this.$route.name) ? `${this.$route.name}_${this.currRole}` : this.$route.name
			}).then(res => {
		...
		setSearchCondition(composeKey, defaultConfigList, changeRole, isFirstLoad) {
			...
			searchItemList = searchItemList.filter(item => { // 过滤掉废弃的搜索项
				return this.filterGroups[item.searchField]
			})
		...
		searchConditionDelete(param) {
			this.$confirm('Whether to confirm the deletion of this search template?', 'Warning', {
				confirmButtonText: 'Ok',
				cancelButtonText: 'Cancel',
				type: 'warning'
			})

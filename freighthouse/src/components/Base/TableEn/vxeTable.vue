<template>
	<div class="table-cont">
		...
		<vxe-table
			ref="tableLayout"
			empty-text="No data was queried"
			resizable
			highlight-hover-row
			...
			:scroll-x="{ mode: 'wheel', enabled: config.options.scrollX ? (config.options.scrollX.enabled === false ? false : true) : true, gt: (config.options.scrollX && config.options.scrollX.gt) || 90 }"
			@cell-click="handleClickRow"
			@selection-change="handleSelectionChange"
			@sort-change="handleSortChange"
			:row-class-name="tableRowClassName"
			:cell-class-name="config.tableCellClassName || ''"
		...
		>
			...
			<vxe-column v-if="config.customColumns && config.customColumns.show" type='customColumns' :width="36" :fixed="'left'" :resizable="true">
				<template #header>
					<el-tooltip :content="'Customized table headers'" placement="top">
						<i class="el-icon-setting icon-custom-columns" @click="config.customColumns && config.customColumns.handleCustomColumns()"></i>
					</el-tooltip>
				</template>
			</vxe-column>
			<!-- 索引 -->
			<vxe-column v-if="isShowTabelIndex && config.isCustomIndex" type="seq" title="Index" align="left" width="46" :resizable="true" fixed="left">
			...
			<vxe-column
				...
				ref="fixedColumn"
				align="left"
				:title="config.operationBtns.title || 'Operations'"
			...
			<template v-if="config.defaultEmpty" #empty>
				<span>
					<p>The data will be displayed after the query</p>
				</span>
			</template>
		</vxe-table>
			...
			if (this.config.options.mutiSelectOnlyOne) {
				...
				if (this.isCheckedRows.length > 1) {
					if (oldSelectArr.length === 1) {
						...
						this.$message({
							message: 'The current operation can only select one piece of data',
							type: 'warning'
						})
		...
		'config.list': {
          ...
          if (!this.config.options?.treeProps?.expandDefaultNot) {
            this.setAllTreeExpand()
					...
					if(this.config.options.treeProps && this.config.options.treeProps.expandAll){
						// 默认展开
						this.$nextTick(() => this.$refs.tableLayout.setAllTreeExpand(true))
					}
				}
			}
		}

<template>
  <div class="upload-input-box" :class="{'upload-disabled':disabled,'upload-is-not-disabled':!disabled}">
    <div class="upload-label" @click="uploadAction">
      <div class="placeholder-text" v-show="!fileName">Please select the file</div>
      <div class="placeholder-text" v-show="fileName" @click="outUploadFile($event)">
        <div class="file-name" title="Preview" @click.stop="handleView" v-show="fileName">
          {{fileName}}
        </div>
        <div v-show="(fileName&&!isAllowDel)&&!disabled" class="close-file" title="Delete" @click.stop="clearFile">
          <i class="el-icon-close"></i>
        </div>
        <div @click.stop="downloadFile" class="download-file" title="Download"><i class="el-icon-download"></i></div>
      </div>
      <!-- 上传图标：没有上传文件或者不展示删除文件时候都需要展示，浏览模式除外 -->
      <div class="upload-icon" @click="uploadFile"><i v-show="!disabled" title="Upload" class="el-icon-upload2"></i></div>
    </div>

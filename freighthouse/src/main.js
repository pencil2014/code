...
ElementUI.Message = function (options) {
  ...
  let instance = Message(options)
  // 只适用于this.$msgSucClose, $msgErrClose, $msgWarnClose, this.$message未能激活， 所以暂不使用
  ...
  this.fields.forEach(field => {
    field.validate('', (message, field) => {
      ...
      if (typeof callback === 'function' && ++count === this.fields.length) {
        if (valid === false && !__timeout__ && location.href.indexOf('/#/finance/') > -1 && !this.$attrs.noCommonValidate){
          __timeout__ = true
          ElementUI.Message({ type: 'error', message:'红框内为必填项，请补充！', showClose: true, onClose:()=>{
            __timeout__ = null
          } })
        }

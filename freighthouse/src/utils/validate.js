...
// 校验毛重，体积 大于0的数字&&小于999999,三位小数 弃货校验
export function checkQuantityWeightVolumeAbandon(max = 999999) {
	return (rule, value, callback) => {
		let reg = /^(([1-9][0-9]*)|([0])|(([0]\.\d{1,3}|[1-9][0-9]*\.\d{1,3})))$/
		if (value) {
			if (!reg.test(value)) {
				ElementUI.Message.error(`请输入大于0的数字,最多包含三位小数`)
				callback(new Error(`请输入不大于${max}的数字`))
			} else if (value > max) {
				ElementUI.Message.error(`请输入不大于${max}的数字`)
				callback(new Error(`请输入不大于${max}的数字`))
			} else {
				callback()
			}
...
// 件数:正整数, 不能大于999999，不能为0
export function validateIntegerQuantityPositive() {
	return (rule, value, callback) => {
		let reg = /^([1-9][0-9]*)$/
		if (value) {
			if (!reg.test(value) || value > 999999 ) {
...
// 件数:正整数, 不能大于999999 弃货校验
export function validateIntegerQuantityAbandon() {
	return (rule, value, callback) => {
		let reg = /^([1-9][0-9]*)$/
		if (value) {
			if (!reg.test(value) || value > 999999) {
				ElementUI.Message.error('请输入1-999999的正整数')
				callback(new Error('请输入1-999999的整数数字'))
			} else {
				callback()
			}
		} else {
...
export function validateOverseasFreightIsOnlyEnchars() {
	// 海外询价只能输入英文、数字、特殊符号；
	return (rule, value, callback) => {
		if (store.getters.overseasFreightIsOnlyEnchars) {
			let reg = /^[a-z_A-Z0-9-\.!@#\$%\\\^&\*\)\(\+=\{\}\[\]\/",'<>~\·`\?:;|]+$/
			if (value) {
				let result = value.replace(/\s+/g, '')
				if (!reg.test(result)) {
					callback(new Error('Please enter English, numbers and English symbols'))
				} else {
					callback()
				}
			} else {

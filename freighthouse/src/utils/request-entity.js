...
const service = axios.create({
  ...
  timeout: 500 * 1000 // request timeout  因为附件信息有视频上传，加了一个零
  // timeout: 5 // 强制超时，用来测试
})
...
service.interceptors.request.use(
  config => {
    ...
    if (moduleArr.includes(config.url.slice(0, 4))) {
      ...
      // 对冲核销打印接口与列表接口共用一个
      // 有些搜索的接口与列表的接口是一致的，搜索会影响列表的pageSize，搜索加入参数 noCachPagesize ，
      if (config.data && config.data.currPage && config.data.pageSize != 10000 && !config.data.noCachPagesize) {
    ...
    if(config.url === '/order/over/enquiry/overSeasList/export'){
...
service.interceptors.response.use(
  ...
  response => {
    ...
    if (res.code !== 0) {
      // 单独处理客户资质认证审核是否通过返回的code,避免抛出错误提示
      if (res.code === 125059) {
        return Promise.resolve({ code: 125059, msg: res.msg })
      }

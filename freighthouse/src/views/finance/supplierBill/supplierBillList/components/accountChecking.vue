        ...
        <div class="money-box-right">
          ...
          <el-button-group>
            <el-button
              ...
              size="mini"
            >自动导入{{supplierBillId?'费用':'单号'}}</el-button>
            <el-button
              class="finance-btn"
              v-if="supplierBillId"
          ...
          <el-button-group>
            <el-button class="finance-btn" @click="allConfirm" size="mini">确认费用</el-button>
          </el-button-group>
          <el-button-group>
            <el-button
              class="finance-btn"
    ...
    <div class="finance-table-double" v-show="!showSettlementDetail">
      <div class="finance-table-container">
        <div class="money-box money-box-overflow">
          <div class="money-box-left">
            <div class="money-title">{{expressBillId?'快递公司':'供应商'}}账单</div>
      ...
      <div class="finance-table-container">
        <div class="money-box money-box-overflow">
          <div class="money-box-left">
            <div class="money-title">{{expressBillId?'寄单':'费用'}}信息</div>
    ...
    getData() {
      if (this.supplierBillId) {
        ...
        return getSupplierBillDetail({ supplierBillId: this.supplierBillId }).then(({ data }) => {
          ...
          this.supplierBillDetailForm.bizDate = data.bizDateStart + '至' + data.bizDateEnd
        })
      }
      this.getDataExpress()
    },
      ...
      companyMethod(this.initData(type)).then((res) => {
        if (this.btnImport) {
          ...
          if (this.supplierBillId && !res.data.list.length) {
            this.btnImport = true
            return this.$msgErrClose('未匹配到订单费用！')
          }
          this.supplierBillId && this.$msgSucClose('导入成功！')
        }
    ...
    compare() {
      if (this.supplierBillId) {
        if (!this.supplierBillDetailForm.statementType) {
          return this.$msgErrClose('请选择比对方式')
        }
        if (!this.importType) {
          return this.$msgErrClose('请导入费用')
        }
      }
      if (!this.option2.data.length) {
        return this.$msgErrClose(`没有${this.supplierBillId ? '公司费用' : '寄单信息'}不能发起对账请求`)
      }

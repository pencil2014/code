    ...
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">月度数据明细</div>
        </div>
        <div class="money-box-right">
          <el-button-group>
            <el-button class="finance-btn" size="mini" @click="cancel">关闭</el-button>
          </el-button-group>
        </div>
      </div>
      <div class="base-info-box">
        <div class="base-info-cnt">
          <div class="base-item">
            <span class="sub-title">月度编号：</span>
            {{ summary.shLienMonNo }}
          </div>

          <!-- <div class="base-item">亏损费用：{{ summary.totalLossAmt }}</div>
          <div class="base-item">币种：{{ summary.currency }}</div>-->
          <div class="base-item">
            <span class="sub-title">航线：</span>
            {{ summary.sysLineName }}
          </div>
          <div class="base-item">
            <span class="sub-title">航线经理：</span>
            {{ summary.sysLineManagerName }}
          </div>
          <div class="base-item">
            <span class="sub-title">所属年月：</span>
            {{ summary.yearMon }}
          </div>
          <!-- <div class="base-item"><span class="sub-title">商务：</span>{{ summary.employeeName }}</div>
          <div class="base-item"><span class="sub-title">工号：</span>{{ summary.employeeNo }}</div>-->
          <div class="base-item flex-item">
            <span class="sub-title">结算单位：</span>
            {{ summary.settleCorpName }}
          </div>
        </div>
      </div>
  ...
  methods: {
    init() {
      ...
      this.selfColumnsBase = [
        ...
        { prop: 'settleCorpName', label: '结算单位', type: 'text', width: 150, st: 2 },
        ...
        { prop: 'pricer', label: '商务', type: 'text', st: 2 },
        ...
        { prop: 'currency', label: '币种', type: 'text', st: 3 },
        { prop: 'feeName', label: '费用名称', type: 'text', st: 2 },
        { prop: 'settleCompName', label: '分公司', type: 'text', width: 150, st: 2 },
        ...
        { prop: 'finDate', label: '财务日期', type: 'text', width: 130, st: 4 },
        ...
        { prop: 'createdName', label: '创建人', type: 'text', st: 2 },
        { prop: 'createdTime', label: '创建时间', type: 'text', width: 130, st: 4 },
        { prop: 'confirmName', label: '确认人', type: 'text', st: 2 },
        { prop: 'confirmTime', label: '确认时间', type: 'text', width: 130, st: 4 },
      ]
      this.feeListColumns.splice(0, 1000, ...this.selfColumnsBase.map((item) => ({ ...item })))
      this.configColumns.push(...this.selfColumnsBase.map((item) => item.prop))
      this.option1.columns = [...this.selfColumnsBase]
      listColumnConfigGet({ scenesCode: 'fin_summary_bill_detail_list' }).then((res) => {
    ...
    customColumnsPopClose(action, value) {
      ...
      if (action === 'Confirm') {
        ...
        listColumnConfigSave({ columnList, scenesCode: 'fin_summary_bill_detail_list' }).then((res) => {
          this.dealCustomColumn(res)
        })
      }
    },
    ...
    getShLienMonInfo() {
      ...
      shLienMonInfo(data)
        .then((res) => {
          ...
          // this.option1.pagination.pageSize = 10
          // this.option1.pagination.totalCount = res.data.feeList.length
        })
        .catch((e) => {
          this.cancel()
        })

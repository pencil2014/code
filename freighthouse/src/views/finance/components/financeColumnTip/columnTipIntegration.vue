...
<div>
  ...
  <template v-else-if="row[item.prop + '_multiValue']">
    <template v-if="row[item.prop + '_multiValue'].length <= 1">
      <div class="finance-column-tip_deep" v-if="item.componentProps.callback" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height: 16px;">
        <el-tooltip :content="row[item.prop + '_multiValue'].filter(v=>v!=='null').join('')" placement="top">
          ...
          <span
            ...
            style="color:#1890ff;"
            @click.stop="item.componentProps.row = row;item.componentProps.callback(subItem, item.componentProps, index, row);"
          >{{subItem}}</span>
        </el-tooltip>
        <!-- <span
          v-for="(subItem, index) in row[item.prop + '_multiValue'].filter(v=>v!=='null')"

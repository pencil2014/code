      ...
      >
        ...
        <vxe-column
          ...
          <template #default="{ row }">
            <div class="td_tag_div" v-if="row._tagArr && row._tagArr.length && (row._tagArr[0].text==='协' || option.tagArrShow)">
              <span
        ...
        >
          <template v-if="option.operationBtns.headerSign" #header>
            <span style="margin-right:4px;">{{ option.operationBtns.title || '操作' }}</span>
  ...
  data() {
    return {
      ...
      COLORFUL: {
        ...
        depositReceipayType:{receive: 'red', pay: 'green', },
        // yesNo 
        yesNo: {yes: 'green', no: 'red', },
        // 审核状态 
        auditStatus: {pass: 'green', refuse: 'red', pending: 'yellow', },
      ...
      if (prop === 'currency') {
        ...
        if (dict && !forceSelection) {
          if(multi){
            ...
            let regx = /^\,*|\,*$/g
            return (cellValue+'').replace(/\b(\w+)\b/g, (item) => ((dict && dict[item]) || (noDict?'':item))).replace(/(,+)/g,',').replace(regx,'')
          }
          // TODO 暂时按需求改，不必全局配置，待全局改再激活配置

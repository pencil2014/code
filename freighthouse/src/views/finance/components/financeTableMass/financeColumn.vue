    ...
    <template v-else-if="item.component === 'ColumnTip'&&item.type !== 'columnTip'">
      <div v-if="!row[item.prop] || row[item.prop].indexOf(',')<0">
        <span
          style="color:#1890ff;"
          class="underline"
          @click.stop="item.componentProps.row = row;item.componentProps.callback(row[item.prop], item.componentProps, 0, row, item);"
        ...
        <template v-if="row[item.prop + '_multiValue'].length <= 1">
          <template v-if="item.componentProps.callback">
            ...
            <span
              ...
              style="color:#1890ff;"
              class="underline"
              @click.stop="item.componentProps.row = row;item.componentProps.callback(subItem, item.componentProps, index, row, item);"
            >{{subItem}}</span>
          </template>
          <template v-else>
            <div

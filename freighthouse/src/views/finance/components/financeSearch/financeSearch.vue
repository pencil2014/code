  ...
  methods: {
    ...
    handleFixCreatedDate() {
      if (this.searchOption.fixCreatedDate) {
        ...
        let fixCreatedSelectArr = this.searchOption.fixCreatedSelectArr
        if (cdFilter) {
          if(fixCreatedSelectArr){
            let cfgObj = {
              '3': {text:'最近三个月', value: 3},
              '6': {text:'最近半年', value: 6},
              '12': {text:'最近一年', value: 12},
              'all': {text:'全部', value: 240},
            }
            cdFilter.pickerOptions = { 
              shortcuts: fixCreatedSelectArr.map(v => {
                let obj = cfgObj[v] || {text: '', value: 0}
                return {
                  text: obj.text,
                  onClick(picker) {
                    const end = new Date();
                    const start = new Date();
                    start.setMonth(start.getMonth() - obj.value);
          ...
          console.log(cdFilter.pickerOptions);
        }
        // this.$nextTick(() => {
    ...
    searchValueGroup() {
      ...
      option.searchInputGroup.forEach((item) => {
        ...
        if (value instanceof Array) {
          ...
          let bakupValArr = [...value]
          let bakupVal = [...value]
          if (['cascader', 'lsCascader', 'diyCascader'].includes(type)) {
            let { value: code, label: name, children, multiple } = cascaderProps
            let result = []
            ...
            if (multiple) {
              let resultArr = []
              bakupValArr.forEach(item => {
                if (item && item.length) {
                  bakupVal = [...item]
                  result = []
                  getLabel(cascaderList, code)
                  resultArr.push(result.map((v) => v[name]).join('/'))
                }
              })
              return query.push({ key, label, value: resultArr.join('，') })
            }
            getLabel(cascaderList, code)

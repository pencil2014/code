    ...
    <div class="money-box">
      <div class="money-box-left">
        <el-button
          ...
          v-if="checkAuth($route.name, 'tab-compareBillReceiveList', 'btn-compare')"
          type="primary"
        >对账</el-button>
      </div>
...
export default {
  ...
  activated() {
    let _compareBillPayList = sessionStorage.getItem('_compareBillReceiveList')
  ...
  created() {
    ...
    this.searchOption = {
      saveDefault: true,
      saveName: 'compareBillReceiveList',
      isNotAutoLoad: true,
      ...
      addFilter: {
        ...
        filterGroups: {
          ...
          // receivePayType: { label: '收付类型', type: 'select', prop: 'receipayType' },
          ...
          compareTime: { label: '对账时间', type: 'daterange', format: 'yyyy-MM-dd' },
        },
      },
    }
    // 表格配置
    ...
    let columns = [
      {
        ...
        operationBtns: {
          ...
          callback: (fn, index, row, option) => {
            this.routerPushTab('CompareBillReceiveResult', { bankAccStatementCompareNo: row.bankAccStatementCompareNo })
          },
        },
        showOverflow: false,
      },
  ...
  methods: {
    ...
    getData(isSearch) {
      ...
      data.query.push({ column: 'receivePayType', type: 'eq', value: 'receive' })
      bankListPage(data)
        .then((res) => {

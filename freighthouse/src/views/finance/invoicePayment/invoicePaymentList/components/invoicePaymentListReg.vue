          ...
          >
            ...
            <div v-show="item.show" class="item-list-invoice-payment">
                ...
                <el-form-item label="发票类型" :prop="`invoicePaymentList[${index}].invoiceType`" :rules="{ required: true, message: ' ', trigger: 'change' }">
                  <el-select v-model="item.invoiceType" placeholder="请选择发票类型" style="width:170px;" @change="invoiceTypeChange($event, index)">
                      <el-option
                ...
                <el-form-item label="发票附件" :prop="`invoicePaymentList[${index}].fileNo`" :rules="fileNoRules[index] || {}">
                  <FinUploadEmbed
                    class=""
...
export default {
  ...
  data(){
    ...
    return {
      ...
      fileNoRules: [{ required: false, message: ' ', trigger: 'change' }],
  ...
  methods:{
    ...
    //提交按钮
    submitAdd() {
      if(this.invoicePaymentAddForm.invoicePaymentList.find(v => !v.fileNo && v.invoiceType.indexOf('electronic') > -1)){
        return this.$msgErrClose('电子发票，发票附件为必填！')
      }
    ...
    invoiceTypeChange(type = '', index){
      if(type.indexOf('electronic') > -1){
        this.fileNoRules[index] = { required: true, message: ' ', trigger: 'change' }
      }else{
        this.fileNoRules[index] = { required: false, message: ' ', trigger: 'change' }
      }
      this.fileNoRules = [...this.fileNoRules]
    }
  },
  computed: {
    isDetail() {

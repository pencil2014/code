...
export const routerMixin = {
  ...
  methods: {
    ...
    routerBack(refresh, close, otherParams = {}) {
      let routeQuery = this.$route.query
      if (routeQuery){
        // 进入详情页时，决定了返回的路径，固定值
        let { toPath, toTab } = routeQuery
        if (toPath){
          this.routerPush(toPath, toTab ? { tabFromOther: toTab } : undefined)
          return
        }
      }
      // console.log('refresh:', refresh)
      let { isFromTagClose } = otherParams
      ...
      if (fromTo) {
        ...
        if (isFromTagClose) {
          // 当isFromTagClose，路由不在已打开的tags的时候，不跳转且返回false
          if (this.visitedViews.some(item => item.name === fromTo.from.name)) {
            this.$router.push({
        ...
        if (isFromTagClose) { // 当isFromTagClose，没有返回路由的时候，返回false
          return false
        }
        if (this.visitedViews.length <= 1) {
          window.close()
          this.$router.push({ // 如果未能关闭，打开首页
            name: 'Dashboard',
            params: {},
            query: {},
          })

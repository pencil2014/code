...
export const mixin = {
  ...
  methods: {
    ...
    // 获取凭证字列表 当做字典使用，去掉重复code
    voucherClassList(queryString, cb) {
      if (VOUCHERCLASS.length) {
        cb && cb(VOUCHERCLASS)
        return Promise.resolve(VOUCHERCLASS)
      }
      return voucherClassList({ name: queryString, }).then(({ data }) => {
        ...
        let setArr = [...new Set(results.map((v) => v.value))]
        results = setArr.map((v) => results.find((item) => v === item.value) || {})
        VOUCHERCLASS.splice(0, 1000, ...results)
        cb && cb(results)
        return results
      })
    },
    ...
    subjectList(queryString, cb, queryString2, item, keyValue) {
      ...
      return subjectList({ currPage: 1, pageSize: size, query, ...data }).then(({ data }) => {

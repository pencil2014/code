  ...
  <el-dialog :title="dialogOption.title" :visible.sync="dialogOption.show" width="90%">
    ...
    >
      <el-form :model="form" ref="form" :rules="rules" label-position="left" inline size="mini">
        <el-form-item label="账簿" prop="acBookId">
          <el-select filterable v-model="form.acBookId" placeholder="请选择">
            <el-option
        ...
        <el-form-item label="凭证字" prop="voucherClass">
          <el-select v-model="form.voucherClass" placeholder="请选择">
            <el-option
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="dimensionSure">确定</el-button>
        <el-button @click="dimensionOption.show = false" size="mini">取消</el-button>
      </span>
    </el-dialog>
  </el-dialog>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      tableOption: {
        columns: [
          { prop: 'id', label: '序号', type: 'text', width: '50px' },
          { prop: 'summaryInfo', label: '摘要', type: 'input', minWidth: 160 },
          {
            prop: 'subjectCode',
            label: '科目编码',
            type: 'remoteSelect',
            ...
            remoteMethod: (query, item) => {
              if (!this.form.acBookId) {
                return this.$msgErrClose('请先选择账簿！')
              }
              query = !query ? [] : [{ column: 'subjectCode', type: 'default', value: query }]
              query.push({ column: 'acBookId', type: 'eq', value: this.form.acBookId })
          ...
          },
          {
            ...
            blur: (row) => {
              ...
              if (row.exRate && Number(row.debitAmt)) {
                ...
                if(!row.exRate && Number(row.debitAmt)){
                  this.$msgWarnClose('当前行无汇率，金额自动清除')
                }
    ...
    addClick(event, row) {
      ...
      this.dimensionOption.formItems = row._dimensions.map((item) => {
        ...
        return {
          ...
          selectOptions: [],
          remoteMethod: (query, item) => {
            searchDimensionFincode({ dimensionCode: item.prop, textValue: query || '', acBookId: this.form.acBookId }).then(
              ({ data }) => {

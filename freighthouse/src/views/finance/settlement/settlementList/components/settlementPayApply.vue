<template>
  <div
    class="finance-page settlement-pay-apply-detail"
    v-loading="loading"
    ...
    <div class="finance-detail-page">
        ...
        <div class="money-box money-box-border-bottom">
          ...
          <div v-if="applyNo" class="money-box-right">
            <el-button-group>
              <el-button
                type="success"
                v-if="!isApproved && checkAuth($route.name, 'btn-settlementPass')"
                class="finance-btn"
            ...
            <el-button-group>
              <el-button
                v-if="!isApproved && checkAuth($route.name, 'btn-settlementRefuse')"
                class="finance-btn"
                @click="refuseDialog"
                type="danger"
                size="mini"
            ...
            <el-button-group>
              <el-button
                v-if="!isStatus && checkAuth($route.name, 'btn-handleWithdrawPayApplyAudit')"
                class="finance-btn"
            ...
            <el-button-group>
              <el-button
                v-if="!isApproved && checkAuth($route.name, 'btn-handleSettleSubmit')"
                class="finance-btn"
      ...
      <div class="settlement-detail-form">
        ...
        <el-form
          v-show="!applyNo"
          label-width="100px"
          :model="payApplyDetailForm"
          ref="payApplyDetailForm"
          :inline="true"
          size="mini"
          :rules="rules"
          class="settlement-detail-form-1"
        >
          ...
          <el-form-item v-if="blackType" label="黑名单类型：">
            <div class="el-form-item-text" v-html="blackType"></div>
          </el-form-item>
          <el-form-item label="结算周期：" prop="settleCycle">
            <div
              class="el-form-item-text"
              v-html="renderNoEdit(payApplyDetailForm, {prop: 'settleCycle', type: 'select'})"
            ></div>
          </el-form-item>
          <el-form-item label="结算币种：" prop="currency">
            <div class="el-form-item-text" v-html="payApplyDetailForm.currency"></div>
          </el-form-item>
          <DetailDiv
            style="width:268px"
            label="应收核销状态："
            :content="renderNoEdit(payApplyDetailForm, {prop: 'recvWriteoffStatus', propInDict: 'writeoffStatus', type: 'select'})"
          />
          <DetailDiv
            style="width:268px"
            label="应收金额："
            :content="payApplyDetailForm.orderReciSumAmt"
            :isTooltip="true"
          />
          <DetailDiv
            style="width:268px"
            label="应收未核销金额："
            :content="payApplyDetailForm.orderReciNoWriteOffAmt"
            :isTooltip="true"
          />
          <DetailDiv
            style="width:268px"
            label="应收已核销金额："
            :content="payApplyDetailForm.orderReciWriteOffAmt"
            :isTooltip="true"
          />
          ...
          <el-form-item label="申请附件：" prop="recvFinishsProveFileNo">
            ...
            <template v-else>
              <div class="seqfile-box">
                <el-button
                  @click="vmShowAttachment(payApplyDetailForm.recvFinishsProveFileNo)"
                  size="mini"
                  type="text"
        ...
        <div v-show="applyNo">
          ...
          <DetailDiv
            label="是否代收付："
            :content="renderNoEdit(payApplyDetailForm, {prop: 'hasAgentPay', propInDict: 'yesNo', type: 'select'})"
          />
          <DetailDiv
            label="结算周期："
            :content="renderNoEdit(payApplyDetailForm, {prop: 'settleCycle', type: 'select'})"
          />
          ...
          >
            ...
            <el-form-item label="申请附件：" prop="recvFinishsProveFileNo">
              <template v-if="!payApplyDetailForm.recvFinishsProveFileNo">
                <div style="width: 147px; display: inline-block; font-size: 12px">无</div>
              </template>
              <template v-else>
                <div class="seqfile-box">
                  <el-button
          ...
          <DetailDiv
            v-if="payApplyDetailForm.payOrderRefuseReason"
            label="出纳拒绝原因："
            :content="payApplyDetailForm.payOrderRefuseReason"
            :isFull="true"
          />
          <!-- 拉黑说明---20221027改为不显示  -->
          ...
          <div class="order-box">
            ...
            <DetailDiv
              label="放单情况："
              :content="renderNoEdit(payApplyDetailForm, {prop: 'putStatus', propInDict: 'finOrderPutStatus', type: 'select'})"
            />
            <DetailDiv
              label="应收金额："
              :content="payApplyDetailForm.orderReciSumAmt"
      ...
      <div v-if="applyNo" class="check-box">
        <div class="title">审核信息 <span v-if="optionCheck.data.find(v => v.approverDesc === '审计')" style="color: red;padding: 0 2px;">(本次付款申请涉及供应商黑名单，需要审计中心审批)</span></div>
      ...
      <div class="finance-table-container" v-for="(item, index) in payInfoList" :key="index">
        <div class="money-box" style="background: #F8F9FD;">
          ...
          <div class="money-box-right">
            <el-button
              v-if="checkAuth($route.name, 'btn-settlementAddInvoice')"
              type="primary"
              class="finance-btn"
              @click="identifyElecInvoice(item,index)"
              size="mini"
            >电子发票识别</el-button>
            <el-button
              v-if="checkAuth($route.name, 'btn-settlementAddInvoice')"
              class="finance-btn"
              @click="addInvoice(item,index)"
              size="mini"
            >发票登记</el-button>
            <el-button
              v-if="checkAuth($route.name, 'btn-settlementRelevance') && (applyNo || payApplyDetailForm.confirmStatus !== 'refuse')"
              class="finance-btn"
              @click="relevance(item)"
              size="mini"
            >关联发票</el-button>
            <el-button type="text" class="collapse-btn" @click="handleCollapse(index)" size="mini">
              {{ item.showTable1 ? '收起' : '展开'}}
              <i
                :class="{'el-icon-arrow-up': item.showTable1, 'el-icon-arrow-down': !item.showTable1}"
              ></i>
            </el-button>
          </div>
        </div>
        <div v-show="item.showTable1" class="detail-info-other">
          ...
          >
            ...
            <el-form-item v-if="isApproved" label="付款方式：" prop="payWay">
              <el-input
                :value="renderNoEdit({payWay: item.payWay}, {prop: 'payWay', type: 'select'})"
                disabled
                placeholder="请输入"
              ></el-input>
            </el-form-item>
            ...
            <el-form-item
              v-if="item.hasCustSpecialFee == 'yes' && !applyNo"
              label="手续费承担人："
            ...
            >
              <el-select
                :disabled="applyNo"
                v-model="item.specialDeductRechtstraeger"
                clearable
                placeholder="请选择手续费承担人"
              >
                <el-option
            ...
            <el-form-item
              v-if="applyNo && item.specialDeductRechtstraeger"
              label="手续费承担人："
              prop="specialDeductRechtstraeger"
            >
              <el-select
            ...
            <el-form-item v-if="isApproved && item.settleMode" label="账户结算方式：" prop="settleMode">
              <el-input
                :value="dictMapObj.bankAccountSettleMode[item.settleMode]"
                disabled
                placeholder="请输入"
              ></el-input>
            </el-form-item>
            <el-form-item
              v-if="item.hasCustSpecialFee==='yes' && item.hasThirdPayChannelCode"
              label="标识码："
        ...
        />
        <FinanceTableMass :option="item.option1" v-show="item.showTable1" @send-multi="sendMulti" />
      </div>
      <!-- 代大掌柜付款 -->
      <div v-if="applyNo" class="finance-table-container">
        <div class="money-box">
          ...
          <div class="money-box-right">
            <el-button
              v-if="checkAuth($route.name, 'btn-handleInternalFee') && (applyNo) && (!isApproved && payInfoList[0].payWay !== 'intrabranch')"
              class="finance-btn"
              @click="handleInternalFee"
              size="mini"
            >代大掌柜付款</el-button>
          </div>
        </div>
        <FinanceTableMass :option="option6" @send-multi="sendMulti" />
      </div>
      <div v-if="payApplyDetailForm.confirmStatus === 'pass'" class="finance-table-container">
        ...
        <FinanceTableMass :option="option4" @send-multi="sendMulti" />
      </div>
      <div class="finance-table-container">
        <div class="money-box">
          <div class="money-box-left">
            <div class="money-title size-12">费用列表</div>
            <div class="money-total" style="margin-right: 0;">应付结算汇总：</div>
            <div
              class="money-total"
        ...
        <DetailFeeSearch
          ...
          ref="detailFeeSearch"
          :list="option3.data"
          :keyObj="{currency:'originalCurrency'}"
          @search="detailFeeSearch"
        />
        <FinanceTableMass :option="option3" @send-multi="sendMulti" />
      </div>
      <el-dialog
        :title="'关联发票'"
        ...
        width="1200px"
      >
        <FinanceSearch :searchOption="searchOption1" @search="searchInvoice" />
        <FinanceTableMass :option="option2" @send-multi="sendMulti" />
        <span slot="footer" class="dialog-footer">
          <el-button
            type="primary"
            :loading="loadingRelationInvoice"
            @click="relevanceConfirm"
            size="mini"
          >确定</el-button>
      ...
      <el-dialog
        ...
        width="1000px"
      >
        <FinanceTableMass :option="option5" @send-multi="sendMulti" />
        <span slot="footer" class="dialog-footer">
      ...
      >
        <el-form :model="internalFeeData" ref="internalFeeData" :inline="true" label-width="110px">
          <div v-for="(item,index) in internalFeeData.list" :key="index">
            <div class="finance-table-container" style="margin:0 0 8px">
              ...
              <el-form-item
                label="代大掌柜付款："
                :prop="`list[${index}].internalFeeCode`"
                :rules="{ required: true, message: ' ', trigger: 'blur' }"
              >
        ...
        <el-button size="mini" @click="handleAddInternalFee" style="width:100%;border: 1px dashed;">
          <i class="el-icon-plus"></i>添加代大掌柜付款信息
        </el-button>
        <span slot="footer" class="dialog-footer">
          <el-button
            type="primary"
            :loading="loadingInternalFeeSubmit"
            @click="internalFeeSubmit"
            size="mini"
          >确定</el-button>
      ...
      <ApplyForPass
        v-if="resultDialogOption.show"
        :option="resultDialogOption"
        @close="applyForPassOptionClose"
        @sure="sure"
      >
        <div slot="descript">
          <el-form
          ...
          >
            ...
            <el-form-item label="付款方式">
              <el-input
                disabled
                :value="renderNoEdit({payWay: payInfoList && payInfoList.length && payInfoList[0].payWay ? payInfoList[0].payWay : ''}, {prop: 'payWay', type: 'select'})"
              ></el-input>
            </el-form-item>
            <el-form-item
              label="收款账号"
...
import {
  ...
  deleteRelationInvoice,
  payInfoRelationInvoice,
  queryUsablePrepayDedueInvoiceList,
  settleDelayInvoice,
  cancelPayApplySettle,
  settleWithdrawPayApplyAudit,
  saveDzgInternalFee,
  deleteDzgInternalFee,
  getIdNoThirdChannelCode,
} from '@/api/fin/pay' // queryPayApplyInfoForSettle
...
export default {
  ...
  created() {
    this.columns0 = [
      { prop: 'recvCorpName', label: '结算单位', type: 'text' },
      {
        ...
        required: true,
        remote: true,
        remoteMethod: (queryString, item, row, index) => {
          this.bankAccountList2(queryString, item, row, index)
        },
        visibleChange: (value, item, row, index) => {
          this.bankAccountList2(value, item, row, index)
        },
        change: (value, item, row, option) => {
          this.select2.change(value, item, row, option)
          row.isRequiredNull = false
        },
        remoteSelectList: [],
      },
      {
        ...
        type: 'text',
        formatter: ({ cellValue }) => {
          return cellValue === 'pub' ? '对公' : cellValue === 'prv' ? '对私' : cellValue
        },
      },
    ...
    this.select2 = {
      change: (value, item, row, option) => {
        let obj = item.remoteSelectList.find((item) => {
  ...
  methods: {
    init() {
      ...
      this.settleOrderIds = this.$route.query.settleOrderIds
        ? JSON.parse(this.$route.query.settleOrderIds)
        : this.$route.query.settleOrderIds
      ...
      queryPayApplySettle(data)
        ...
        .finally(() => {
          if (this.payApplyDetailForm.hasBlack === 'yes') {
            let title =
              this.payApplyDetailForm.hasCustBlack === 'yes'
                ? '该结算单位已在黑名单客户列表被拉黑，请确认是否继续请款？'
                : this.payApplyDetailForm.hasSupplierBlack === 'yes'
                ? '该结算单位已在黑名单供应商列表被拉黑，该付款申请需要审计中心进行审核，请确认是否继续请款？'
                : ''
    ...
    queryPayApplySettleInfo() {
      this.loading = true
      queryPayApplySettleInfo({ applyNo: this.applyNo })
        .then((res) => {
          if (res.code === 0) {
            ...
            this.payInfoList.forEach((item, index) => {
              ...
              let columns8 = [
                ...
                { prop: 'currency', label: '付款币种', type: 'text' },
                { prop: 'realpayAmt', label: '付款金额', type: 'text' }, // balance
                { prop: 'balance', label: '剩余可用金额', type: 'text' },
              ]
    ...
    payWayChange(item, is, index) {
      ...
      if (item.payWay === 'cash') {
        ...
        if (['abroad', 'hk_china', 'chinese_mainland', 'intrabranch'].indexOf(item.payWay) > -1) {
      ...
      let dzgData = { internalFeeCode: '', dzgPayBillNo: '', dzgFeeAmt: '', dzgFeeTaxRate: '' }
      //内部往来清空转大掌柜信息
    ...
    prepayAccountBalance(item) {
      ...
      PREPAY.balance(data).then((res) => {
        if (res.code === 0) {
          if (res.data) {
            item.prepaySum = res.data.reduce((sum, curr) => new LSnum(sum).plus(curr.balance).num, 0)
          } else {
          ...
          this.$confirm('应收未收清，是否继续审核通过？', '提示', {
            ...
            .then(() => {
              if (this.payApplyDetailForm.invoiceRecvAcctEqRest === 'no') {
                this.$confirm(
                  this.payApplyDetailForm.invoiceRecvAcctNotEqWarningMsg || '发票登记账号和付款账号不一致,是否审核通过',
                  '提示',
                  {
                    cancelButtonText: '取消',
                    confirmButtonText: '确定',
    ...
    submit() {
      ...
      for (let i = 0; i < data.payInfoList.length; i++) {
        if (
          data.payInfoList[i].specialDeductRechtstraeger === 'order' &&
          data.payInfoList[i].specialDeductHasApproval === 'yes'
        ) {
          tip =
            '本次付款申请的装车拼箱费扣减手续费后利润不达标，需要总裁办审批。涉及订单号：' +
            data.payInfoList[i].specialProfitLowOrderNo +
            ' 是否确认提交申请？'
        }
      }
      if (hasBlack === 'yes') {
      ...
      for (let i = 0; i < this.payInfoList.length; i++) {
        ...
        if (this.payInfoList[i].payWay === 'cash') {
          ...
          if (option0Data.recvAccountPublicType !== 'prv') {
            this.$msgErrClose('付款方式为现金时，只能选择对私的账号')
            return
          }
        }
        let option0Data = this.payInfoList[i].option0.data[0]
        ...
        if (this.payInfoList[i].hasCustSpecialFee === 'yes' && option0Data.recvAccountPublicType === 'prv') {
          if (!option0Data.recvIdNo || !option0Data.recvMobileNo) {
            // !this.payInfoList[i].recvIdNo || !this.payInfoList[i].recvMobileNo
            this.$msgErrClose('对私账户的必填字段“身份证号”、“手机号”为空，请在CRM核实')
            return
          }
        }
      }
    ...
    relevance(item) {
      ...
      if (this.applyNo) {
        ...
        if (this.payApplyDetailForm.needRelationInvoiceAmt === 0) {
          return this.$message({
            message: '结算付款申请单' + this.payApplyDetailForm.applyNo + '只有押金类费用，不支持关联应付发票',
            type: 'error',
            showClose: true,
          })
        }
    ...
    getInvoiceList() {
      ...
      usableList(data).then((res) => {
        if (res.code === 0) {
          ...
          list.forEach((item) => {
            if (
              this.payInfo.option1.data.every((relevance) => {
    ...
    getListAmtSummary(val) {
      if (val.length) {
        ...
        currencyList.forEach((item) => {
          ...
          let sum = arr.reduce((a, b) => new LSnum(a).plus(b.amt).num, 0)
          if (sum && item) {
          ...
          let sum1 = arr.reduce((a, b) => new LSnum(a).plus(b.balanceAmt).num, 0)
          if (sum1 && item) {
    ...
    relevanceConfirm(selectionOut) {
      ...
      let selection = selectionOut.filter((item) => {
    ...
    handleWithdrawPayApplyAudit() {
      this.$confirmWarn('是否确认撤销付款审核？', () => {
        settleWithdrawPayApplyAudit({ applyNoList: [this.payApplyDetailForm.applyNo] }).then((res) => {
          this.$msgSucClose('撤销付款审核成功')
          this.getData()
        })
      })
    },
    internalFeeChange(item, index) {
      if (
        item.payWay !== 'prepay_dedue' &&
        (item.dzgFeeTaxRate === '' || item.dzgFeeAmt === '' || item.dzgPayBillNo === '' || item.internalFeeCode === '')
      ) {
    ...
    dzgFeeAmtChange(val) {
      ...
      if (this.option6.data.length) {
        ...
        sum = arr.reduce((a, b) => LSnum(a).plus(b).num, 0)
      }
      const applyPayAmts = Number(this.payInfoList[0].applyPayAmts)
      this.payInfoList[0].applyPayAmt = LSnum(sum).plus(applyPayAmts).num
    },
    ...
    identifyElecInvoice(item, index) {
      if (this.applyNo) {
        if (this.payApplyDetailForm.needRelationInvoiceAmt === 0) {
          return this.$message({
            message: '结算付款申请单' + this.payApplyDetailForm.applyNo + '只有押金类费用，不支持应付发票登记',
            type: 'error',
            showClose: true,
          })
        }
    ...
    addInvoice(item, index) {
      if (this.applyNo) {
        if (this.payApplyDetailForm.needRelationInvoiceAmt === 0) {
          return this.$msgErrClose('结算付款申请单' + this.payApplyDetailForm.applyNo + '只有押金类费用，不支持应付发票登记')
        }
      } else {
        for (let i = 0; i < this.payInfoList.length; i++) {
          if (this.payInfoList[i].canInvoiceAmt === 0) {
            return this.$msgErrClose('付款申请单只有押金类费用，不支持应付发票登记')
          }
        }
      }
      //生成reqid
  ...
  computed: {
    ...
    isStatus() {
      let { confirmStatus, status } = this.payApplyDetailForm
      return !(confirmStatus === 'pass' && status !== 'paying' && status !== 'pay_done') && this.applyNo
    },
    blackType() {
          ...
          newVal.forEach((item) => {
            if (item.option1.data && item.option1.data.length) {
              let remainingAmountTotal = item.option1.data.reduce((total, curr) => {
                ...
                if (total <= item.canInvoiceAmt) {
                  // 不够的项的关联金额
                  curr.relevanceAmt = decimalFixed2(curr.remainingAmount)
                } else {
                  // 刚好够的那一项的关联金额
                  curr.relevanceAmt =
                    total - Number(curr.remainingAmount) < item.canInvoiceAmt
                      ? decimalFixed2(item.canInvoiceAmt - (total - Number(curr.remainingAmount)))
                      : 0
                }
                return total
              }, 0)

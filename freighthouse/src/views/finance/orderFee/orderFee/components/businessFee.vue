<template>
  <div class="one-kind-fee"
    ...
    <div class="finance-table-summary">
      <div class="finance-summary-text">
        <div>
          ...
          <el-popover
            v-if="refImportOrders.length"
            placement="bottom-start"
            trigger="hover">
            <div class="relate_profit">
              <div style="font-size: 12px; color: #222222; line-height: 20px; padding-bottom: 10px;">
                所有订单毛利合计：CNY <span style="color: #e63923;padding-right: 6px;">{{refAllOrderProfit.totalCnyProfit}}</span> USD <span style="color: #e63923;padding-right: 20px;">{{refAllOrderProfit.totalUsdProfit}}</span> 毛利率：{{refAllOrderProfit.totalGrossProfit}}
              </div>
              <SimpleToggleTable v-for="(item,index) in refImportOrders" :key="index" :tableOption="item" />
            </div>
            <el-button type="text" slot="reference" style="font-size: 12px;">查看进口单利润</el-button>
          </el-popover>
      ...
      <div v-if="$route.name!=='BookDetail'" class="finance-btn-container">
        <el-button-group v-if="isImport">
          <el-button v-if="checkAuth(pageCode, 'tab-businessFee', 'btn-syncFeesFromExportOrder')" class="finance-btn" size="mini" @click="syncFeesFromExportOrder" type="primary">同步出口单费用</el-button>
        </el-button-group>
        <el-button-group>
          <el-button v-if="checkAuth('PreClose', 'tab-preClose1', 'btn-apply')" class="finance-btn" size="mini" @click="multiApply" type="success" :disabled="losscloseDisable">申请负利润关账</el-button>
        ...
        <el-button-group>
          <el-button v-if="checkAuth(pageCode, 'tab-businessFee', 'btn-applyBad') && mode==='byFee'" class="finance-btn" @click="receiLoss" size="mini" :disabled="precloseDisable">坏账申请</el-button>
        </el-button-group>
        <el-button-group>
    ...
    <div class="finance-table-container business-fee-receive-table">
      <div class="money-box">
        <div class="money-box-left">
          ...
          <div v-if="$route.name!=='BookDetail'" class="money-total" @click="switchMode" style="cursor:pointer;margin-left:-6px;margin-right:12px;width:20px;height:20px;text-align:center;">
          ...
          <div class="money-total" style="cursor:pointer;margin-left:-6px;margin-right:12px;width:20px;height:20px;text-align:center;" @click="receiveExchangeShow = !receiveExchangeShow">
            <el-tooltip content="点击可以展开/折叠折币换算结果" placement="top">
              <img src="@/assets/icon_money_show.png" style="margin-top:2px;"/>
            </el-tooltip>
          </div>
          <div class="money-total">
            <div>
              <div style="display: inline-block;margin-right: 20px;" v-for="(item, index) in moneyTotalReceive" :key="'moneyTotalReceive' + index">
                {{item.currency}}合计：<span :class="{'money-pay-color': moneyTitle1 === 'pay' ? true : false}">{{item.sum}}</span>
              </div>
            </div>
            <div v-if="receiveExchangeShow">
              <div style="display: inline-block;margin-right: 20px;" v-for="(item, index) in moneyTotalReceiveExchange" :key="'moneyTotalReceiveExchange' + index">
                折合{{item.currency}}：<span :class="{'money-pay-color': moneyTitle1 === 'pay' ? true : false}">{{item.sum}}</span>
              </div>
            </div>
          </div>
        </div>
        <div v-if="$route.name!=='BookDetail'" class="money-box-right">
          ...
          <el-button v-if="checkAuth(pageCode, 'tab-businessFee', 'btn-updateNegFol') && mode==='byFee'" class="finance-btn" @click="updateNegativeFollower('receive')" type="warning" size="mini">改负数跟进人</el-button>
    ...
    <div class="finance-table-container" style="position:relative;">
      <div class="money-box">
        <div class="money-box-left">
          ...
          <div class="money-total" style="cursor:pointer;margin-left:-6px;margin-right:12px;width:20px;height:20px;text-align:center;" @click="payExchangeShow = !payExchangeShow">
            <el-tooltip content="点击可以展开/折叠折币换算结果" placement="top">
              <img src="@/assets/icon_money_show.png" style="margin-top:2px;"/>
            </el-tooltip>
          </div>
          <div class="money-total">
            <div>
              <div style="display: inline-block;margin-right: 20px;" v-for="(item, index) in moneyTotalPay" :key="'moneyTotalPay' + index">
                {{item.currency}}合计：<span :class="{'money-pay-color': moneyTitle2 === 'pay' ? true : false}">{{item.sum}}</span>
              </div>
            </div>
            <div v-if="payExchangeShow">
              <div style="display: inline-block;margin-right: 20px;" v-for="(item, index) in moneyTotalPayExchange" :key="'moneyTotalPayExchange' + index">
                折合{{item.currency}}：<span :class="{'money-pay-color': moneyTitle2 === 'pay' ? true : false}">{{item.sum}}</span>
              </div>
            </div>
          </div>
        </div>
        <div v-if="$route.name!=='BookDetail'" class="money-box-right">
          ...
          <el-button v-if="checkAuth(pageCode, 'tab-businessFee', 'btn-confirmAirFeeBook') && mode==='byFee' && isAirOrder" class="finance-btn" @click="confirmAirFee('book')" size="mini" type="primary">确认订舱单价</el-button>
          <el-button v-if="checkAuth(pageCode, 'tab-businessFee', 'btn-confirmAirFeeDeliver') && mode==='byFee' && isAirOrder" class="finance-btn" @click="confirmAirFee('deliver')" size="mini" type="primary">确认出货单价</el-button>
          <el-button v-if="checkAuth(pageCode, 'tab-businessFee', 'btn-confirmAirFeeDetail') && mode==='byFee' && isAirOrder" class="finance-btn" @click="confirmAirFeeDetail('pay')" size="mini" type="primary">查看价格对比</el-button>
          ...
          <el-button v-if="checkAuth(pageCode, 'tab-businessFee', 'btn-updateNegFol') && mode==='byFee'" class="finance-btn" @click="updateNegativeFollower('pay')" type="warning" size="mini">改负数跟进人</el-button>
          <el-button v-if="checkAuth(pageCode, 'tab-businessFee', 'btn-payFeeFinishedPush')" class="finance-btn" @click="payFeeFinishedPush" size="mini">成本已录推送</el-button>
    ...
    <el-dialog
      ...
      width="560px">
      <div>
        <el-input v-model="performanceNum" placeholder="请输入" />
      </div>
      <div style="margin-top:4px;">
        <span style="color:#DCDFE6;">低配置电脑推荐25，配置越低数字越小</span>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="savePerformance" size="mini" type="primary">确定</el-button>
        <el-button @click="performanceVisible = false" size="mini">取消</el-button>
      </span>
    </el-dialog>
    <el-dialog
      v-if="dialogVisible2"
      ...
      <div v-loading="btnLoading2">
        <el-form :model="emailForm" :rules="emailRules" ref="emailForm" label-width="80px" size="mini">
          <el-form-item label="收件人" prop="toEmails">
            <el-input v-model="emailForm.toEmails" placeholder="请输入收件人邮箱（多个以英文逗号分隔）" show-word-limit maxlength="500">
            </el-input>
          </el-form-item>
          <el-form-item label="抄送人" prop="ccEmails">
            <el-input v-model="emailForm.ccEmails" placeholder="请输入抄送人邮箱（多个以英文逗号分隔）" show-word-limit maxlength="500">
            </el-input>
          </el-form-item>
          <el-form-item label="邮件主题" prop="subject">
            <el-input placeholder="请填写邮件主题" v-model="emailForm.subject" type="textarea" :rows="2" show-word-limit maxlength="500">
            </el-input>
          </el-form-item>
          <el-form-item label="邮件内容" prop="emailHtml">
          ...
          <el-form-item label="附件信息" prop="fileNos">
            <FinanceUploadEmbed ref="FinanceUploadEmbed" width="100%" @send-file-list="sendFileList" />
          </el-form-item>
        </el-form>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="sendBillEmail">推送</el-button>
        <el-button @click="dialogVisible2 = false" size="mini">取消</el-button>
      </span>
    </el-dialog>
    <AirCompareDialog :dialogOption="airCompareDialogOption"/>
  </div>
</template>
<script>
import FinanceTableMass from '../../../components/financeTableMass/financeTableMass'
import SimpleToggleTable from '@/views/finance/components/financeTable/simpleToggleTable'
...
import { uploadBusinessFee, saveOrderBizFee, orderFeeList, saveAsTemplate, batchModifySettleUnit, confirmOrderFeeInvoiceStatus, confirmOrderFee,
 ...
 bizFeeSaveVerify, orderFeeListByCorp, syncFeesFromExportOrder, signEmailInfo, confirmAirFee, confirmAirFeeDetail } from '@/api/fin/fee' // getOrderTaxStat不使用
...
import AirCompareDialog from '@/views/finance/orderFee/orderFee/components/components/airCompareDialog'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      moneyTotalReceive: [],
      moneyTotalPay: [],
      receiveExchangeShow: false,
      payExchangeShow: false,
      moneyTotalReceiveExchange: [],
      moneyTotalPayExchange: [],
  ...
  methods: {
    ...
    processDataSU(res, receipayType, dataType) {
      ...
      if (receipayType === 'receive') {
        ...
        this.columns4 = configColumns.length > 0 ? configColumns.filter(v => v !== 'blNos') : this.defaultColumnsSU.filter(v => v !== 'blNos')
    ...
    getOrderProfit() {
      ...
      getOrderProfit(data).then(res => {
        if (res.code === 0) {
          ...
          let refAllOrderProfit = res.data.refAllOrderProfit || {}
          this.refAllOrderProfit = {...refAllOrderProfit, totalGrossProfit: new LSnum(refAllOrderProfit.totalGrossProfit).times(100) + '%'}
          this.setProfitDetail(res.data.profitList)
          this.originalFeeDiff = res.data.originalFeeDiff
          this.refImportOrders = (res.data.refImportOrders || []).map(v => {
            let cnyAmt = v.totalCnyProfit
            let usdAmt = v.totalUsdProfit
            let profit = new LSnum(v.totalGrossProfit).times(100) + '%'
            return {
              title: `订单号：${v.orderNo}　　订单毛利：CNY ${cnyAmt}(USD: ${usdAmt})　　毛利率：${profit}`,
              hasBorder: true,
              disableFixedIndex0: true,
              data: v.profitList,
              columns: [
                { prop: 'ownerCompName', label: '分公司', minWidth: 188, resizable: false, },
                { prop: 'mon', label: '月份', width: 108, resizable: false },
                { prop: 'cnyProfit', label: '人民币毛利', width: 130, resizable: false },
                { prop: 'usdProfit', label: '美元毛利', width: 108, resizable: false },
                { prop: 'grossProfit', label: '毛利率', width: 108, resizable: false, 
                  formatter: ({row}) => new LSnum(row.grossProfit).times(100) + '%', // 此处用的是el-table
                },
              ],
            }
          })
    ...
    addPay(feeItem) {
      ...
      if (feeItem) {
        ...
        // 迭代20230110 ID1009496 订单录入应付费用结算单位，改为跟应收费用一样
        // 产品要求，新增应付费用，恢复原有逻辑，使用 默认订舱代理
    ...
    openBillEmailDialog() {
      signEmailInfo({orderNo: this.orderNo}).then(res => {
        ...
        this.emailForm.subject = `【签费用提醒】工作单号：${res.data.orderNo}，成本已录入，请签应收`
        ...
        this.emailForm.emailHtml = `
          ...
          <p style="padding-left: 30px;">您好，工作单号<a href="${href}" target="_blank" rel="noopener">${res.data.orderNo}</a>，成本已录入，请签应收。</p>
          ...
          <p style="padding-left: 30px;">提醒人：${res.data.reminderName}</p>
          <p style="padding-left: 30px;">提醒人邮箱：${res.data.reminderEmail}</p>
        `
    ...
    confirmAirFee(confirmType) {
      // console.log(this.multipleSelection2);
      let rows = this.multipleSelection2
      if (rows.length !== 1) {
        return this.$msgErrClose('请选择一条费用进行操作！')
      }
      if (rows[0].feeCode !== 'KYF') {
        return this.$msgErrClose('请选择空运费进行操作！')
      }
      confirmAirFeeDetail({orderNo: this.useNo}).then(res => {
        let message, confirmButtonText
        if (confirmType === 'book') {
          if (res.data.bookPriceList && res.data.bookPriceList.length) {
            message = '您已经确认过订舱单价，是否确认进行更新？'
          ...
          if (res.data.deliverPriceList && res.data.deliverPriceList.length) {
            message = '您已经确认过出货单价，是否确认进行更新？'
            confirmButtonText = '更新'
          } else {
            message = '您是否确认出货单价？'
            confirmButtonText = '确认'
        ...
        this.$confirmWarn({
          title: message,
          body: `空运费单价：${rows[0].unitPrice} ${rows[0].currency}`,
          confirm: confirmButtonText,
          customClass: 'air__confirm',
          confirmButtonClass: confirmButtonText === '更新' ? 'el-button--success' : '',
        }, () => {
          confirmAirFee({orderNo: this.useNo, orderFeeId: rows[0].orderFeeId, confirmType}).then(res => {
            this.$msgSucClose(confirmButtonText+'成功！')
          })
        })
      })
    },
    confirmAirFeeDetail() {
      confirmAirFeeDetail({orderNo: this.useNo}).then(res => {
        this.airCompareDialogOption.bookPriceList = res.data.bookPriceList || []
        this.airCompareDialogOption.deliverPriceList = res.data.deliverPriceList || []
        this.airCompareDialogOption.show = true
      })
    },
  },
  watch: {
  ...
  computed: {
    ...
    isAirOrder(){
      let businessType = this.orderInfo.businessType || ''
      return businessType.indexOf('air') > -1
    },
    ... mapState({

    ...
    payFeeFinishedPush() {
      ...
      payFeeFinishedPushCheck(data).then(res => {
        if (res.data === 'repeat') {
          ...
          this.payFeeFinishedPushConfirm('已推送过，是否再次推送？')
        } else {
          this.openBillEmailDialog()
          // this.payFeeFinishedPushConfirm('是否确认成本已录推送？')
        }
      }).finally(() => {
        this.loading = false
      })
    ...
    payFeeFinishedPushConfirm(title, data, callback) {
      if (title === true && data) {
        return payFeeFinishedPush(data).then(res => {
          this.$message({ message: '推送成功', type: 'success', showClose: true })
        }).finally(() => {
          callback && callback()
          this.loading = false
        })

  ...
  created() {
    ...
    baseSystemLineList(name, item, isNotSearch, remoteList) {
      ...
      this.$store.dispatch('dict/baseSystemLineList', param).then(data => {
		...
		// 员工下拉框
    getEmployeeList(queryString, item, param, otherParams) {
	  	let data = {}
      ...
      employeeSelectlist(data).then((res) => {
        ...
        let list = res.data.map((item) => ({ ...item, label: `${item.cname}(${item.employeeNo})`, value: item.employeeId }))
        if (param === 'select') {
          return item.splice(0, 1000, ...list)
        }
    ...
    // 岗位员工下拉框
    listByRole(name = '', item, role, queryArr = []) {
      let query = [...queryArr]
      role && query.push({ column: 'roleCode', type: role.indexOf(',') > -1 ? 'in' : 'eq', value: role }) // column: 'roleCode', type: 'in', value: 'bd,obd'
      name && query.push({ column: 'name', type: 'eq', value: name })
      listByRole({ currPage: 1, pageSize: 30, query }).then(({ data: { list } }) => {
        let remoteSelectList = list.map((v) => ({ ...v, label: `${v.cname}(${v.employeeNo})`, value: v.employeeId }))
        if (this.remoteSelectCommon(item, remoteSelectList)) return
        item.remoteSelectList = remoteSelectList
      })
    },
    ...
    remoteSelectCommon(item, data, cfg = {label:'label', value:'value'}, param = {}) {
      ...
      this.moneyTotalReceiveExchange = ['CNY', 'USD'].map(item => {
        return {
          currency: item,
          sum: receiveData.reduce((sum, cur) => new LSnum(sum).plus(cur[item === 'CNY' ? 'cnyMoneyAmt' : 'usdMoneyAmt']).num, 0)
        }
      })
      ...
      this.moneyTotalPayExchange = ['CNY', 'USD'].map(item => {
        return {
          currency: item,
          sum: payData.reduce((sum, cur) => new LSnum(sum).plus(cur[item === 'CNY' ? 'cnyMoneyAmt' : 'usdMoneyAmt']).num, 0)
        }
      })
		},
      ...
      let { isTab, from, isFromSea, bkgReqNo } = otherParams
          ...
          if (type === 'receive') {
            ...
            if (writeoffType === 'gedging_writeoff' || writeoffType === 'bad_gedging_writeoff') {
                  ...
                  query: { writeoffReceipayNo: no, fromOrder: true}
						...
						} else if (writeoffType === 'bad_gedging_writeoff') { // bad_debt_writeoff是老的类型，暂保留
							if (from === 'OrderFee') {
								let routeUrl = this.$router.resolve({
            ...
            } else if (writeoffType === 'bad_debt_writeoff' || writeoffType === 'bad_pre_writeoff'){
              if (from === 'OrderFee') {
                let routeUrl = this.$router.resolve({
                  name: 'BadOrderDetail',
                  query: { source: 'detail', writeoffNopayNo: no, fromOrder: true }
                ...
                this.$router.push({ name: 'BadOrderDetail', query: { source: 'detail', writeoffNopayNo: no } })
              }
            } else {
    ...
    finCommonSearch(dataQuery = [], searchOption, hms, gle, multipleSelection, otherParams = {}){
      ...
      searchOption.addFilter.searchInputGroup.forEach((item) => {
        ...
        if (range && (range.min || range.max)) {
          ...
          console.log(start);
          if (start || gle === 'rangeMinMax' || key === 'canSettleApplyAmt' || rangeMinMax) {
            range.min && dataQuery.push({ column: start || key + 'Min', type: 'eq', value: range.min })
            range.max && dataQuery.push({ column: end || key + 'Max', type: 'eq', value: range.max })
          }else{
        ...
        if (value) {
          if (value instanceof Array) {
            ...
            if (['cascader', 'lsCascader', 'diyCascader'].includes(filterType)) {
              // 层级多选处理
              if (filterGroup.cascaderProps && filterGroup.cascaderProps.multiple){
                let str = value.map(v => v[v.length-1]).filter(v => v).toString()
                dataQuery.push({ column: key, type: 'in', value: str })
              } else {

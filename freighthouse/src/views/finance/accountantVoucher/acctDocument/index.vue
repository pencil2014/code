...
const nameList = ['writeoffRecvList','writeoffPayList','gedgingWriteOffList','settleOrder',]
//根据组件name属性最后一位数字进行排序 更改页面tab显示顺序需到组件内更改组件name值
const tabsList = nameList.map((v) => List.find((item) => item.name === v)).filter((v) => v)
// console.log(JSON.parse(JSON.stringify(tabsList)));
/******** 自动引入组件  --end  ******** */
export default {

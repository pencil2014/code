...
export default {
  ...
  data() {
    ...
    return {
      ...
      loading: false,
      exportTypeList: [
        { label: '列表', value: 'writeoffPayListExport', voucherValue: 'exAcctWriteoffPayListExport' },
        { label: '明细（工作号汇总）', value: 'writeoffPayDetailExport', voucherValue: 'writeoffPayDetailAccExport' },
        {
          label: '明细表（发票号汇总）',
          value: 'writeoffPayDetailHasInvoiceExport',
          voucherValue: 'writeoffPayDetailHasInvoiceAccExport',
        },
      ],
    }
  },
    ...
    this.searchOption1 = {
      ...
      addFilter: {
        ...
        filterGroups: {
          ...
          // accountName: { label: '付款账户名', type: 'input', showType: '3' },
  ...
  methods: {
    ...
    handleExport(exportType) {
      ...
      // 查询对象的配置编码
      this.$confirm(
        '是否确认导出' + `${exportType.label || '付款核销单'}`,
        `${exportType.label ? '导出' + exportType.label : '付款核销单'}`,
        {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          lockScroll: 'true',

...
export default {
  ...
  data() {
    ...
    return {
      ...
      exportTypeList: [
        { label: '列表', value: 'writeoffRecvListExport', voucherValue: 'writeoffRecvAccListExport' },
  ...
  methods: {
    ...
    handleInvoice() {
      ...
      if (rows.length === 0) {
        return this.$msgErrClose('请勾选需要开票申请的项！')
      }
      if (rows.find((item) => item.noInvoiceAmt == 0)) {
        return this.$msgErrClose('请选择未开票金额大于0的项！')
      }
      if ([...new Set(rows.map((v) => v.settleCorpCode))].length > 1) {
        return this.$msgErrClose('请选择结算单位一致的项！')
      }
    ...
    recheck() {
      ...
      if (!writeoffReceipayIds.length) {
        return this.$msgErrClose('请选择未复核的记录!')
      }
    ...
    manualWriteoff(value) {
      ...
      if (rows.length === 0) {
        this.$msgErrClose('请勾选需要' + (isUnlock ? '取消复核' : isReverse ? '反核销' : '重新核销') + '的项！')
        return
      }
      if (rows.length > 1) {
        this.$msgErrClose('一次只能勾选一个' + (isUnlock ? '取消复核' : isReverse ? '反核销' : '重新核销') + '的项！')
        return
      }
      const writeoffReceipayId = rows[0].writeoffReceipayId
      // 取消复核
      if (isUnlock) {
        this.$confirmWarn({ body: '您是否确认取消复核该核销单?', title: '取消复核' }, () => {
          unlock({ writeoffReceipayId }).then((res) => {
            this.$msgSucClose('取消复核提交成功')
            this.getData()
          })
        })
      } else if (isReverse) {
        // 反核销
        if (rows[0].voucherNo !== '') {
          this.$msgErrClose('已经生成会计凭证的核销，无法进行反核销操作!')
          return
        }
        this.$confirmWarn({ body: '您是否确认对该记录进行反核销?', title: '反核销' }, () => {
        ...
        // 重新核销
        this.$confirmWarn({ body: '是否重新核销。是否继续?', title: '提示', confirm: '是', cancel: '否' }, () => {
          reverse({ writeoffReceipayId }).then((res) => {

<template>
  ...
  >
    <div>
      <div
        class="money-box"
        style="background:#FFFFFF;border-radius:4px;border-bottom:1px solid #e9e9e9;"
      >
        <div class="money-box-left">
          <div class="money-title">应收账单编号：{{fiscalTermNo}}</div>
        </div>
      ...
      <div class="settlement-detail-form settlement-detail-form-own">
        <el-form
          ...
          label-width="100px"
        >
          <DetailDiv label="分公司：" :content="fiscalTermForm.settleCompName" :isTooltip="true" />
          <DetailDiv label="结算单位：" :content="fiscalTermForm.settleCorpName" :isTooltip="true" />
          <DetailDiv label="账单日：" :content="fiscalTermForm.settleDate" />
          <DetailDiv label="结款到期日：" :content="fiscalTermForm.dueDate" />
          <DetailDiv label="宽限天数：" :content="fiscalTermForm.graceDays" />
          <DetailDiv
            label="结清状态："
            :content="dictMapObj['cleanStatus'][fiscalTermForm.cleanStatus]"
          />
        </el-form>
      </div>
    </div>
    ...
    <!-- <div style="height:1px;background:#e9e9e9;margin: 0 8px; "></div> -->
...
export default {
  ...
  created() {
    ...
    this.searchOption = {
      addFilter: {
        ...
        filterGroups: {
          sourceBizNo: { label: '单号', type: 'input', showType: '1' }, // fiscalTermNo
          ...
          crmTimeType: { label: '时间类型', type: 'select', prop: 'settleInfoTimeType', showType: '1' }, // useTimeType: {	label: "时间类型",	type: "select", prop:'settleInfoTimeType',	showType: "1" },
          businessType: { label: '业务类型', type: 'select', prop: 'businessType', showType: '1' },
  ...
  activated() {
    if (!this.$route.meta.isUseCache) {
      ...
      this.fiscalTermForm = Object.assign(this.fiscalTermForm, {
        settleCompName,
        settleCorpName,
        settleDate,
        dueDate,

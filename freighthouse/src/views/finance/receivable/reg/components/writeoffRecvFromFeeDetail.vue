<template>
  ...
  >
    <div class="finance-list-container" @click="saveDzg">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">费用关联{{regInfoForm.receivablePrerevNo?'预':''}}收款核销</div>
        </div>
        <div class="money-box-right">
          <!-- v-if="checkAuth($route.name, 'btn-writeoff')" -->
          <el-button @click="writeoffAction" size="mini" type="primary">核销</el-button>
          ...
          <el-button @click="settleOrderFeeList('isFirst')" size="mini" type="primary">关联费用核销</el-button>
          <el-button @click="openUploadFile" size="mini" type="primary">上传客户付款水单</el-button>
      ...
      <div class="settlement-detail-form">
        ...
        >
          <template v-if="regInfoForm.receivablePrerevNo">
            <h3>预收款信息</h3>
            <DetailDiv label="预收款登记号：" :content="regInfoForm.receivablePrerevNo" />
            <DetailDiv label="预收款金额：" :content="regInfoForm.preceiveAmt" />
            <DetailDiv label="预收款可用金额：" :content="regInfoForm.balance" />
          </template>
          <h3>收款信息</h3>
          <DetailDiv label="收款登记号：" :content="regInfoForm.receivableRegNo" />
          <DetailDiv label="收款金额：" :content="regInfoForm.realAmt" />
          <DetailDiv label="收款方式：" :content="dictMapObj['recvWay'][regInfoForm.recvWay]" />
          <DetailDiv label="收款银行简称：" :content="regInfoForm.accountShortName" />
          <DetailDiv
            label="账户结算方式："
            :content="dictMapObj['bankAccountSettleMode'][regInfoForm.settleMode]"
          />
          <DetailDiv label="所属分公司：" :content="regInfoForm.recvCompName" :isTooltip="true"/>
          <DetailDiv label="银行流水号：" :content="regInfoForm.bankSeqNo" />
          <DetailDiv label="收付日期：" :content="regInfoForm.recvDate" />
          <DetailDiv label="银行流水号：" :content="regInfoForm.bankSeqNo" />
          <DetailDiv label="定价手续费：" :content="regInfoForm.fixedTransferFee" />
          <DetailDiv label="内扣手续费：" :content="regInfoForm.interTransferFee" />
          <DetailDiv label="外扣手续费：" :content="regInfoForm.outerTransferFee" />
          <DetailDiv label="币种：" :content="regInfoForm.currency" />
          ...
          <el-form-item label="核销汇率：" v-if="regInfoForm.exchangeRates.length" class="overshow-item">
            <div>
          ...
          <DetailDiv label="到账水单：" :isSlot="true">
            ...
            >
              <el-tag
                ...
                size="medium"
                @click="vmShowAttachment(regInfoForm.bankseqFileCode)"
              >{{ regInfoForm.bankseqFileName }}</el-tag>
            </el-tooltip>
          </DetailDiv>
          <DetailDiv label="交易备注：" :content="regInfoForm.recvRemark" :isFull="true" />
          ...
          <h3>客户付款信息</h3>
          <DetailDiv label="客户付款账户名：" :content="regInfoForm.payAccountName" />
          <DetailDiv label="客户付款银行：" :content="regInfoForm.payBankName" />
          <DetailDiv
            label="账户类型："
            :content="dictMapObj['accountType'][regInfoForm.payAccountPublicType]"
          />
          <DetailDiv label="客户付款账号：" :content="regInfoForm.payAccountNo" />
          <DetailDiv label="发卡行国家：" :content="regInfoForm.payAccountCountryName" />
          ...
          <DetailDiv label="客户付款水单：" :isSlot="true">
            ...
            >
              <el-tag
                class="detail__content"
                size="medium"
                @click="vmShowAttachment(regInfoForm.payBankseqFileNo)"
              >{{ regInfoForm.payBankseqFileName }}</el-tag>
            </el-tooltip>
          </DetailDiv>
          <DetailDiv label="公司地址：" :content="regInfoForm.payAccountBankCompAddr" :isFull="true" />
        </el-form>
      </div>
    ...
    >
      ...
      <div class="dialog-footer" slot="footer">
        <el-button @click="importFeesAction" size="mini" type="primary">导入费用</el-button>
...
export default {
  ...
  components: { FinanceTable, CustomColumns, FinanceSearch, TransferDzg, TransferFee, FinUploadEmbed, FinanceUpload },
  data() {
    return {

  ...
  >
    ...
    >
      <template>
        ...
        <el-form v-else :model="form" ref="form" label-width="122px" :inline="false" size="mini">
          ...
          <el-form-item
            ...
            prop="isForbidPayFee"
          >
            <el-select style="width: 100%;" v-model="form.isForbidPayFee" placeholder="请选择">
              <el-option
                v-for="item in [{label:'是',value:'no'},{label:'否',value:'yes'}]"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              ></el-option>
          ...
          >
            <el-select style="width: 100%;" v-model="form.isForbidRecvFee" placeholder="请选择">
              <el-option
                v-for="item in [{label:'是',value:'no'},{label:'否',value:'yes'}]"
                :key="item.value"
                :label="item.label"
      ...
      <span slot="footer" v-if="dialogType !== 'serviceCode'">
        <el-button size="mini" type="primary" @click="dialogClose(true)">确定</el-button>
  ...
  created() {
    ...
    this.searchOption = {
      ...
      addFilter: {
        ...
        filterGroups: {
          ...
          unitType: { label: '结算单位类型', type: 'select', prop: 'unitType' },
          ...
          customerBlacklist: { label: '客户是否黑名单', type: 'select', prop: 'yesNo' },
          supplierBlacklist: { label: '供应商是否黑名单', type: 'select', prop: 'yesNo' },
          supplierIsFreeze: { label: '供应商冻结状态', type: 'select', prop: 'yesNo' },
          isForbidPayFee: {
            label: '是否可录应付费用',
            type: 'select',
            selectOptions: [
    ...
    let columns = [
      ...
      },
      {
        ...
        type: 'select',
        // propInDict: 'yesNo',
        formatter: ({ row }) => (row.isTemporary === 'yes' ? `<span class="ls_red">是</span>` : '否'),
      },
...

      {
        ...
        width: 100,
        type: 'text',
        formatter: ({ row }) => (row.state === 'valid' ? '是' : `<span class="ls_red">否</span>`),
      },
      {
        ...
        type: 'select',
        // propInDict: 'yesNo',
        formatter: ({ row }) => (row.customerBlacklist === 'yes' ? `<span class="ls_red">是</span>` : '否'),
      },
      {
        prop: 'supplierBlacklist',
        label: '供应商是否黑名单',
        width: 100,
        type: 'select',
        // propInDict: 'yesNo',
        formatter: ({ row }) => (row.supplierBlacklist === 'yes' ? `<span class="ls_red">是</span>` : '否'),
      },
      {
        prop: 'supplierIsFreeze',
        label: '供应商冻结状态',
        width: 100,
        type: 'select',
        formatter: ({ row }) => (row.supplierIsFreeze === 'yes' ? `<span class="ls_red">是</span>` : '否'),
      },
      ...
      // },
      {
        ...
        width: 130,
        type: 'button',
        formatter: ({ row }) => (row.isForbidPayFee === 'yes' ? `<span class="ls_red underline">否</span>` : '是'),
        callback: (row) => {
      ...
      },
      {
        ...
        width: 130,
        type: 'button',
        formatter: ({ row }) => (row.isForbidRecvFee === 'yes' ? `<span class="ls_red underline">否</span>` : '是'),
        callback: (row) => {

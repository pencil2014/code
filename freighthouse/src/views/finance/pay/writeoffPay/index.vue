        ...
        <div class="money-box" v-if="isVoucherRoute">
          <div class="money-box-left">
            ...
            <el-button
              ...
              type="danger"
            >删除凭证</el-button>
						<el-button-group v-if="checkAuth($route.name, 'tab-writeoffPayList', 'btn-handleExport')">
							<div class="finance-btn select-template-box">
								<el-dropdown trigger="click" @command="handleExport">
									...
									<el-dropdown-menu slot="dropdown">
										<el-dropdown-item v-for="(item, index) in exportTypeList.filter(v => v.voucherValue)" :key="'exportTypeList' + index"
											:command="item">
											导出{{item.label}}
										</el-dropdown-item>
									</el-dropdown-menu>
      ...
      writeoffReceipayNo: '',
        ...
        { prop: 'compareStatus', label: '对账状态', type: 'select', width: 80, propInDict: 'recvPayCompareStatus' },
        {
          ...
          width: 130,
			...
			loading: false,
			exportTypeList: [
				{ label: '列表', value: 'writeoffPayListExport', voucherValue: 'acctWriteoffPayListExport' },
				{ label: '明细（工作号汇总）', value: 'writeoffPayDetailExport', voucherValue: 'writeoffPayDetailAccExport' },
				{ label: '明细表（发票号汇总）', value: 'writeoffPayDetailHasInvoiceExport', voucherValue: 'writeoffPayDetailHasInvoiceAccExport' },
				{ label: '付款核销汇总', value: 'payWriteoffSummaryExport', voucherValue: '' },
			],
		}
	},
	created () {
    ...
    getData(isSearch, customerList) {
      ...
      this.finCommonSearch(data.query, this.searchOption1, '', 'rangeMinMax')
			...
			data.query.push({
				...
				value: this.isVoucherRoute ? exportType.voucherValue : exportType.value
			})
			if(exportType.value !== 'writeoffPayListExport'){
				data.columns = []
			}
			// this.$confirm('是否确认导出'+exportType.label+'付款核销单?', '导出付款核销单', {
        this.$confirm('是否确认导出' +`${exportType.label || '付款核销单'}`, `${ exportType.label ? '导出' + exportType.label : '付款核销单'}`, {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				lockScroll: 'true',
				type: 'warning'
				...
				this.$store
					...
					.then(res => {
						...
						link.href = window.URL.createObjectURL(res)
            link.download = `${ exportType.label ? '导出' + exportType.label : '付款核销单'}` + '.xlsx'
		...
		bankAccountListAll(accountShortName, item) {
			...
			bankAccountListAll(data).then((res) => {
				let data = res.data.map(item=> ({label: `${item.accountShortName}(${item.accountNo})`,value:item.settleUnitBankAcctId}))

<template>
  <div
    class="finance-page prepay-detail"
    v-loading="loading"
    ...
    <div class="order-detail">
      <div class="finance-approve-detail">
        ...
        <DetailDiv label="关联付款单号：" :isSlot="true">
          <span
            ...
            @click="jumpToRoute('OrderDetails', infoData.payOrderNo)"
          >{{infoData.payOrderNo}}</span>
        </DetailDiv>
        <DetailDiv label="我司户名：" :content="infoData.payAccountName" :isTooltip="true" />
        <DetailDiv label="我司账号：" :content="infoData.payAccountNo" />
        <DetailDiv label="银行简称：" :content="infoData.payAccountShortName" :isTooltip="true" />
        ...
        <DetailDiv
          label="付款审批状态："
          :content="dictMapObj.auditStatus[infoData.payOrderConfirmStatus]"
        />
      ...
      <div class="finance-approve-detail" style="border-top: 1px solid #e9e9e9">
        <div class="finance-approve-btns-box">
          ...
          <div class="finance-approve-btns-right"></div>
        </div>
        <DetailDiv label="收款登记号：" :isSlot="true">
          <span
            style="color:#1890ff;cursor:pointer;"
            @click="jumpToRoute('WriteoffRecvDetail', payInfo.receivableRegNo)"
          >{{payInfo.receivableRegNo}}</span>
        </DetailDiv>
        <DetailDiv label="银行流水号：" :content="payInfo.bankSeqNo" :isTooltip="true" />
        <DetailDiv label="收付日期：" :content="payInfo.recvDate" />
        <DetailDiv label="我司户名：" :content="payInfo.recvCompName" :isTooltip="true" />
        <DetailDiv label="我司开户行：" :content="payInfo.bankName" />
        <DetailDiv label="收款银行简称：" :content="payInfo.accountShortName" :isTooltip="true" />
        ...
        <DetailDiv
          label="付款人账户类型："
          :content="dictMapObj.accountType[payInfo.payAccountPublicType]"
        />
        ...
        <DetailDiv label="客户付款水单：" :isSlot="true">
          <span v-if="payBankseqFile.trustFileName" class="detail__content">
            <el-tooltip :content="payBankseqFile.trustFileName" placement="top-start">
              <span
                style="color: #1890ff;"
                @click="bankseqFilePaths(payBankseqFile.trustFileNo)"
              >{{ payBankseqFile.trustFileName }}</span>
            </el-tooltip>
          </span>
        </DetailDiv>
          ...
          <div class="money-box">
            ...
            <div class="money-box-right">
              ...
              >
                {{showTable1 ? '展开' : '收起'}}
                <i
                  :class="{'el-icon-arrow-up': showTable1, 'el-icon-arrow-down': !showTable1}"
                ></i>
              </el-button>
            </div>
          </div>
          <FinanceTable v-if="!showTable1" :option="option1" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.loading = true
      payApplyReceiveRefundInfo({ applyNo: this.applyNo, payApplyReceiveRefundId: this.payApplyReceiveRefundId })
        .then((res) => {
          if (res.code === 0) {
            ...
            let list =
              res.data.receivableRegDetail && res.data.receivableRegDetail.transferFeeAllotList
                ? res.data.receivableRegDetail.transferFeeAllotList
                : []
            let data = []
            list.forEach((item) => {
              data.push(item)

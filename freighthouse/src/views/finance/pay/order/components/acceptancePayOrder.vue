  ...
  <div class="markerror-dialog-pay">
    ...
    >
      ...
      >
        ...
        >
          <DetailDiv
            style="font-size: 12px; width: 256px"
            label="付款单号："
          ...
          >
            <el-select
              ...
              filterable
              remote
              :remote-method="(str) => bankRemote(str, item)"
              @visible-change="(visible) => visible && bankRemote('', item)"
...
export default {
  ...
  methods: {
    ...
    bankRemote(accountShortName, item){
      let hasIntrabranch = "";
      let hasChequeAcct = "";
      let countryOrgType = item.payWay;
      let unitCode = item.payCompCode;
      ...
      this.$store
        ...
        .then((data) => {
          data.forEach(item => item.payBankCode = item.bankCode)
          this.$set(item, 'payAccountShortNameList', data || [])
          this.$forceUpdate()
          // console.log(this.payOrderNoList);
        })
    },

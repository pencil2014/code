<template>
  ...
  >
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        ...
        <div class="money-box-right">
          <el-button
            v-if="checkAuth($route.name, 'btn-refundSubmit')"
      ...
      <div class="order-refund">
        <div class="settlement-detail-form gedging-detail-form">
          <el-form :model="refund" ref="refund" :inline="true" size="mini" label-width="98px">
            <el-form-item
              label="退款至"
              prop="refundType"
            ...
            <el-form-item label="退款金额" prop="applyPayAmt">
              ...
              <el-input
                ...
                size="mini"
                placeholder="请输入退款金额"
                v-lsNumber="{value: 'number', obj: refund, key: 'applyPayAmt',}"
                :maxlength="16"
              ></el-input>
            </el-form-item>
            <el-form-item
            ...
            <el-form-item label="银行流水号" prop="bankSeqNo" :rules="bankSeqNoRule">
              <el-input
            ...
            >
              ...
              <template v-else>
                <el-button size="mini" type="text" @click="handleBankseqFileNo">点击查看</el-button>
                <el-button size="mini" type="text" @click="handleDeleteFileNo">删除</el-button>
              </template>
            </el-form-item>
            <el-form-item
              label="我司银行简称"
        ...
        <div class="finance-table-container">
          <div class="money-box" style="background-color: #F8F9FD;">
            ...
            <div class="money-box-right">
              <!-- 退至供应商不需要手续费；退至我司时需关联收款登记，使用收款登记的手续费，此处不能编辑 -->
              <!-- <el-button
                class="finance-btn"
                @click="add2(null)"
                size="mini"
              ...
              >
                收起
                <i :class="{'el-icon-arrow-up': showTable1, 'el-icon-arrow-down': !showTable1,}"></i>
              </el-button>
            </div>
          </div>
          <FinanceTableMass
            v-if="!showTable1"
            :option="option1"
            class="finance-table-container-finance-table"
          />
        </div>
        <div class="finance-table-container" v-if="regInfo">
          <div class="money-box" style="background-color: #F8F9FD;">
            <div class="money-box-left">
              <div class="money-title">收款登记信息</div>
            </div>
            <div class="money-box-right">
              <el-button
                type="text"
          ...
          <div class="settlement-detail-form" v-if="!showTable3">
            <DetailDiv label="收款登记号：" :content="regInfo.receivableRegNo" />
            <DetailDiv label="收款账户简称：" :content="regInfo.accountShortName" :isTooltip="true" />
            <DetailDiv label="收款账户名：" :content="regInfo.recvCompName" :isTooltip="true" />
            <DetailDiv label="收款账号：" :content="regInfo.accountNo" />
            <DetailDiv label="实收金额：" :content="regInfo.realAmt" />
            <DetailDiv label="收入日期：" :content="regInfo.recvDate" />
            <DetailDiv label="银行流水号：" :content="regInfo.bankSeqNo" />
            <DetailDiv label="银行备注：" :content="regInfo.remark" :isTooltip="true" :isFull="true" />
          </div>
        </div>
        <div class="finance-table-container">
          <div class="money-box" style="background-color: #F8F9FD;">
            ...
            <div class="money-box-right">
              ...
              >
                收起
                <i :class="{'el-icon-arrow-up': showTable2,	'el-icon-arrow-down': !showTable2,}"></i>
              </el-button>
            </div>
          </div>
          <div class="settlement-detail-form" v-if="!showTable2">
            <DetailDiv label="付款单号：" :content="payOrder.payOrderNo" />
            <DetailDiv label="付款单类型：" :content="dictMapObj.payOrderType[payOrder.orderType]" />
            <DetailDiv label="申请单号：" :content="payOrder.sourceApplyNo" />
            <DetailDiv label="受理状态：" :content="payOrder.confirmStatus === 'pass'? '已受理' : '未受理'" />
            <DetailDiv label="付款方式：" :content="dictMapObj.payWay[payInfo.payWay]" />
            <DetailDiv label="付款币种：" :content="payInfo.currency" />
            <DetailDiv label="付款金额：" :content="payInfo.applyPayAmt" />
            <DetailDiv label="付款单状态：" :content="dictMapObj.payStatusType[payOrder.payStatus]" />
            <DetailDiv label="付款人：" :content="payOrder.confirmName" />
            <DetailDiv label="付款时间：" :content="payOrder.confirmTime" />
    ...
    <el-dialog title="关联收款登记" :visible.sync="relateRegDialog" width="1130px">
      <div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="sureRelateReg" size="mini" type="primary">关联</el-button>
        <el-button @click="relateRegDialog = false" size="mini">取消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  created() {
    this.searchOption = {
      columns: [{ type: 'input', label: '收款登记号', prop: 'receivableRegNo' }],
    ...
    this.autocomplete1 = {
      querySearch: (queryString, cb) => {
        ...
        if (!this.refund.recvCorpCode) {
          return this.$msgErrClose('无结算单位')
        }
        alias.bankAccountList({ accountNo: queryString, unitCode: this.refund.recvCorpCode, status: 'effect',receipayType: 'pay', currency: this.payInfo.currency}, cb)
      },
    ...
    this.autocomplete2 = {
      querySearch: (queryString, cb) => {
        ...
        alias2.companyBankAccountList(
          {
            accountNo: queryString,
            currency: this.refund.currency,
            receipayType: 'pay',
            countryOrgType: this.refund.payWay,
            unitCode: this.refund.unitCode,
          },
          cb
        )
      },
    ...
    let operationBtns = Object.assign(
    ...
    this.option1.columns = [
      { prop: 'cutType', label: '内外扣类型', type: 'select', propInDict: 'transferFeeType' },
      { prop: 'currency', label: '币种', type: 'select' },
      { prop: 'amt', label: '金额', type: 'input', key: 'amt', inputType: 'number', maxlength: '16' },
    ]
    ...
    this.option2.columns = [
      { label: '收款登记号', prop: 'receivableRegNo', type: 'text', width: 130 },
      { label: '收款账户简称', prop: 'accountShortName', type: 'text', width: 150 },
      { label: '收款账户名', prop: 'recvCompName', type: 'text', width: 150 },
      { label: '收款账号', prop: 'accountNo', type: 'text', width: 118 },
      { label: '实收金额', prop: 'realAmt', type: 'text', width: 100 },
      { label: '收入日期', prop: 'recvDate', type: 'text', width: 100 },
      { label: '银行备注', prop: 'remark', type: 'text' },
      { label: '银行流水号', prop: 'bankSeqNo', type: 'text', width: 130 },
    ]
  },
  methods: {
    add2() {
    ...
    getRelate() {
      ...
      listReceivableRegForPrePayRefund({ payOrderNo: this.payOrderNo })
        .then((res) => {
          if (!res.data.length) {
            return this.$msgErrClose('没有可以关联的收款登记！')
          }
    ...
    sureRelateReg() {
      if (this.multipleSelection2.length !== 1) {
        return this.$msgErrClose('请勾选1个需要关联的项！')
      }
  ...
  activated() {
    if (!this.$route.meta.isUseCache) {
      ...
      this.$msgWarnClose(`'退至我司银行账户'，请关联收款登记！`)
      this.regInfo = null
      this.receivableRegId = ''
      this.option1.data = []

  ...
  >
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">费用单详情</div>
        </div>
        <div class="money-box-right">
          ...
          >
            <el-button class="finance-btn" size="mini" type="primary" @click="finBillEdit">修改</el-button>
          </el-button-group>
          <el-button-group v-show="isEdit && finBillNo">
            <el-button class="finance-btn" size="mini" type="primary" @click="saveFinBill">保存</el-button>
            <el-button class="finance-btn" size="mini" @click="cancel(true)">取消</el-button>
          </el-button-group> -->
          <el-button-group v-show="feeHasClose==='no'">
            <el-button class="finance-btn" size="mini" type="primary" @click="handleSavePre('both')">保存</el-button>
          </el-button-group>
          <el-button-group
            v-if="checkAuth($route.name, 'btn-createSettle')"
            v-show="finBillNo"
          >
            <el-button class="finance-btn" size="mini" @click="generate">生成结算单</el-button>
          </el-button-group>
          <el-button-group>
    ...
    <div class="settlement-detail-form">
      <el-form :model="formData" ref="form" :rules="rules" :inline="true" size="mini" label-width="100px">
        <DetailDiv label="费用单名称：" :content="formData.finBillName" :isFull="true" />
        <DetailDiv label="费用单号：" :content="finBillNo" />
        <DetailDiv label="费用单类型：" :content="finType" />
        <DetailDiv label="创建人：" :content="formData.createdName" />
        <DetailDiv label="创建时间：" :content="formData.createdTime" />
        <DetailDiv label="分公司：" :content="formData.settleCompName" :isTooltip="true" />
        <DetailDiv v-if="depAndEmpDisabled" label="员工：" :content="formData.employeeName" />
        <el-form-item v-else label="员工：" prop="employeeId">
          <el-select
            :noRemote="true"
            filterable
            remote
        ...
        <DetailDiv label="工号：" :content="formData.employeeNo" />
        <DetailDiv v-if="depAndEmpDisabled" label="部门：" :content="formData.departName" />
        <el-form-item v-else label="部门：" prop="departCode">
          <el-cascader
            v-if="forceRefresh"
            ref="cascader"
            filterable
        ...
        <DetailDiv label="所属年份：" :content="formData.year" />
        <DetailDiv label="所属月份：" :content="(+formData.month)+''" />
        <DetailDiv v-if="formData.sysLineName" label="航线：" :content="formData.sysLineName" />
        <DetailDiv label="备注说明：" :content="formData.remark" :isFull="true" />
      </el-form>
    </div>
    <div class="finance-table-summary">
      <div class="finance-summary-text">
        <div style="display:inline-block;margin-right:16px;">
          ...
          <span>毛利率：{{grossProfit.grossProfitRate}}%</span>
        </div>
        <!-- <el-button type="text" @click="openProfitDetail" size="mini">详情</el-button> -->
        ...
        <!-- <el-button type="text" v-if="!beginRenderTable34" size="mini">详情</el-button> -->
      </div>
      <div
        v-if="checkAuth($route.name, 'btn-close')"
        class="finance-btn-container total-fee-btn-container"
      >
        <el-button class="finance-btn" @click="openFeeClosedModifyFeeDialog" size="mini">关账调费</el-button>
      </div>
    </div>
    ...
    <div class="finance-table-container">
      <div class="money-box">
        ...
        <div class="money-box-right">
          <el-button v-if="checkAuth($route.name, 'btn-addReceive') && !isEditRecei" class="finance-btn" @click="handleEdit('receive')" size="mini" type="warning">修改</el-button>
          <el-button v-if="isEditRecei" class="finance-btn" @click="handleSavePre('receive')" type="primary" size="mini">保存</el-button>
          <el-button v-if="isEditRecei" class="finance-btn" @click="handleCancel('receive')" size="mini">取消</el-button>
          <el-button v-if="checkAuth($route.name, 'btn-addReceive')" class="finance-btn" @click="addReceive(null)" size="mini" type="primary">新增应收费用</el-button>
          <el-button v-if="checkAuth($route.name, 'btn-addReceive')" class="finance-btn" @click="handleCopy('receive')" size="mini">复制</el-button>
          <!-- <el-button class="finance-btn" size="mini" @click="handleCustomColumns">自定义表头</el-button> -->
          <el-button
            class="finance-btn"
          ...
          <!-- <el-button
            ...
            type="primary"
          >新增应收费用</el-button> -->
          <el-button v-if="checkAuth($route.name, 'btn-updateNegFol')" class="finance-btn" @click="updateNegativeFollower('receive')" type="warning" size="mini">改负数跟进人</el-button>
          <el-button v-if="checkAuth($route.name, 'btn-addReceive')" class="finance-btn" @click="handleDelete('receive')" size="mini" type="danger">删除</el-button>
    ...
    <div class="finance-table-container">
      <div class="money-box">
        ...
        <div class="money-box-right">
          <el-button v-if="checkAuth($route.name, 'btn-addPay') && !isEditPay" class="finance-btn" @click="handleEdit('pay')" size="mini" type="warning">修改</el-button>
          <el-button v-if="isEditPay" class="finance-btn" @click="handleSavePre('pay')" type="primary" size="mini">保存</el-button>
          <el-button v-if="isEditPay" class="finance-btn" @click="handleCancel('pay')" size="mini">取消</el-button>
          <el-button v-if="checkAuth($route.name, 'btn-addPay')" class="finance-btn" @click="addPay(null)" size="mini" type="primary">新增应付费用</el-button>
          <el-button v-if="checkAuth($route.name, 'btn-addPay')" class="finance-btn" @click="handleCopy('pay')" size="mini">复制</el-button>
          <el-button
            v-if="checkAuth($route.name, 'btn-addReceive')"
            class="finance-btn"
            size="mini"
          ...
          <!-- <el-button
            ...
            type="primary"
          >新增应付费用</el-button> -->
          <el-button v-if="checkAuth($route.name, 'btn-updateNegFol')" class="finance-btn" @click="updateNegativeFollower('pay')" type="warning" size="mini">改负数跟进人</el-button>
          <el-button v-if="checkAuth($route.name, 'btn-addPay')" class="finance-btn" @click="handleDelete('pay')" size="mini" type="danger">删除</el-button>
    ...
    <!-- <el-dialog title="修改费用备注" :visible.sync="editDialogShow" :destroy-on-close="true" width="604px">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="saveFeeRemark" size="mini">确 定</el-button>
...
export default {
  ...
  created() {
    ...
    this.option1.operationBtns.width = '130px'
  ...
  methods: {
    ...
    getInfo(notGetData) {
      ...
      info({ finBillNo: this.finBillNo })
        .then((res) => {
          ...
          this.getEmployeeList(res.data.employeeName, true)
    ...
    // 分属员工下拉框
    getEmployeeList(queryString, isFirst) {
      ...
      employeeSelectlist(data).then((res) => { // 赵斌加入参deptCodes size
        ...
        //   return 
        // }
        if(isFirst === true && !res.data.length){
          let {employeeId, employeeName, employeeNo} = this.formData
          this.employeeOptions = [{employeeId,cname: employeeName,employeeNo}]
        }else{
    ...
    confirmFee(multipleSelection, btnLoading, option) {
      if (multipleSelection.some((item) => item.edit)) {
        return this.$msgErrClose('请先保存费用，再进行操作！')
      }
      if (option.data && option.data.length === 0) {
        return this.$msgErrClose('无费用可确认！')
      }
      if (multipleSelection.length === 0) {
        return this.$msgErrClose('请选择需要确认的费用！')
      }
      let data = {
        finBillNo: this.finBillNo,
        finBillFeeIdList: multipleSelection.map((item) => item.finBillFeeId),
      }
      this.$confirmWarn('是否确认该费用？', () => {
        this[btnLoading] = true
        confirmFee(data)
          .then((res) => {
            this.$msgSucClose('费用确认成功！')
            this.getData()
          })
    ...
    cancleConfirmFeePrev(receipayType, btnLoading) {
      ...
      if (option.data && option.data.length === 0) {
        return this.$msgErrClose('无费用可撤销确认！')
      }
      if (multipleSelection.length === 0) {
        return this.$msgErrClose('请选择需要撤销确认的费用！')
      } else {
        let feesConfirm = multipleSelection.filter(item => {
          return item.confirmStatus === 'yes'
        })
        if (!feesConfirm.length) {
          return this.$msgErrClose('没有已确认的费用，不能撤销确认')
        }
        ...
        if (!feesNotEnterFin.length) {
          return this.$msgErrClose('费用已进入结算或已关账，不能撤销确认')
        }
      }
      this.$confirmWarn('是否确认撤销该费用？', () => {
        this[btnLoading] = true
    ...
    openFeeClosedModifyFeeDialog() {
      ...
      if (receiveUncloses !== '' || payUncloses !== '') {
        receiveUncloses = receiveUncloses !== '' ? '应收第' + receiveUncloses + '笔费用' : ''
        payUncloses = payUncloses !== '' ? '应付第' + payUncloses + '笔费用' : ''
        let uncloses = receiveUncloses + ' ' + payUncloses
        this.$message({
          message: `选择的${uncloses}未关账，无法进行关账调费！`,
          type: 'error',
          showClose: true,
          duration: 8000,
        })
    ...
    saveFinBill(type, isBoth) { // saveFinBill(operation, $index, option)
      ...
      if (isBoth) {
        ...
        if (
          fbFeeVos.length &&
          !this.validate(fbFeeVos, ['feeName', 'feeCode', 'settleCorpName', 'settleCorpCode', 'unitPrice', 'unitCount'])
        ) {
          return
        }
      ...
      this.$confirm(!needCancel ? '确认保存费用单吗？' : '新增费用有应付亏舱费，请录入应付调整费用！', '提示', {
        ...
        .then(() => {
          ...
          if(departCode) departName = departName || this.formData.departName
    ...
    generate() {
      if (this.isEditPay || this.isEditRecei) {
        return this.$message.error('请先完成费用修改，再生成结算单')
      }
      let feeIds = [...this.multipleSelection1, ...this.multipleSelection2]
      if (!feeIds.length) {
        return this.$message.error('请选择需生成结算单的费用')
      }
        ...
        this.defaultColumns.forEach((value) => {
          ...
          if (obj) {
            ...
            if (obj.prop !== 'serviceCode' && obj.prop !== 'serviceName') {
              let requiredList = [ 'feeName', 'settleCorpName', 'feeUnit', 'unitPrice', 'unitCount', 'currency' ]

<template>
	<div class="soInfo-container">
		<el-table fit style="width: 100%" max-height="160" :data="tableData" class="table-cont" :span-method="objectSpanMethod" @cell-mouse-enter="showTooltip" @cell-mouse-leave="hiddenTooltip">
			<el-table-column label="报关方式" align="center" width="100">
				<template slot-scope="scope">
					{{ scope.row.customsType | customsTypeFilter }}
				</template>
			</el-table-column>
			<el-table-column prop="customsNumber" label="正报几票" align="center" width="100"></el-table-column>
			<el-table-column prop="customsSupplierName" label="意向供应商" align="center" width="100"></el-table-column>
			<el-table-column prop="customsPrice" label="买单意向价格" align="center" width="100">
				<template slot-scope="{row}">
					{{row.customsPrice}} {{row.customsUnit}}
				</template>
			</el-table-column>
			<el-table-column prop="remark" label="报关备注" align="center" width="200"></el-table-column>
			...
			<el-table-column label="操作" align="center" width="80" fixed="right">
				<template slot-scope="scope">
					<div class="operate-group">
						...
						<!-- <el-button class="red" :disabled="isFinish || !isOp || isDisabledEdit" type="text" size="mini" @click="handleDelete(scope.row)">删除</el-button> -->
					</div>
				</template>
			</el-table-column>
		</el-table>
...
export default {
    ...
    */
		...
		handleViewFile(row) {
			...
			this.fileList = (row.fileList || []).filter(item => {
				return item.fileNo
			})
		},
		popFileClose() {

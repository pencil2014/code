<template>
  <BaseDialog :config="dialogConfig" :callback="dialogCallback">
    <el-form ref="uploadFileForm" :model="createdItem" :rules="rules" label-position="right" label-width="70px" class="boxUpload-form">    
  ...
  methods: {
    ...
    handleImport(done) {
      ...
      if (this.sizeLimit) {
        ...
        this.$store.dispatch('user/import', {
          uri: '/order/shipping/inspection/import',
          params: form
        }).then(res => {
    ...
    handleTempDownload() {
      this.$store.dispatch('order/downloadBlobFile', {
        ...
        params: {
          folder: 'excel',
          fileName: this.isFcl ? 'inspectionCiq_temp.xlsx' : 'inspectionCiq_scatter_temp.xlsx '
        }
      }).then(res => {
        console.log(res);
        let link = document.createElement("a")
        link.href = window.URL.createObjectURL(res);
        link.download = this.isFcl ? "整箱清单模板.xlsx" : '拼箱清单模板.xlsx'
        link.click()
      })
    },
  }

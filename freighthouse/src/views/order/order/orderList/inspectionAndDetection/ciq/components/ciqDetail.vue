<template>
	<div class="ciq-container">
		<div class="row-tit h32" id="saveBar">
			<div class="ft12 bold tit">{{ type === 'add' ? '新增CIQ委托' : 'CIQ委托详情' }}</div>
		...
		<el-form ref="ciqSave" :rules="rules" :model="detailData" class="form-body" label-width="90px">
			<el-row class="mt10">
				<el-col :span="6">
					<el-form-item label="CIQ委托编号" prop="intrustNo">
						<el-input size="mini" placeholder="请输入" v-model="detailData.intrustNo" disabled clearable></el-input>
					</el-form-item>
				</el-col>
				<el-col :span="6">
					<el-form-item label="CIQ申报类型" prop="declareType" :show-message="false">
						<el-select style="width: 100%" clearable placeholder="请选择" size="mini" v-model="detailData.declareType" @change="handleSelectDeclareType">
							<el-option v-for="(item, index) in dictMap.ciqDeclareType" :key="index" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</el-form-item>
				</el-col>
				<el-col :span="6">
					<el-form-item label="申报公司" prop="declareCustid">
				...
				<el-col :span="6">
					<el-form-item label="CIQ检测公司" prop="checkSupplierId">
						<el-select
							style="width: 100%"
							clearable
							placeholder="请选择"
				...
				<el-col :span="6">
					<el-form-item label="CIQ联系人" prop="checkContact">
						<el-autocomplete
							style="width: 100%"
							size="mini"
							clearable
							...
							placeholder="请输入CIQ联系人"
							@select="value => handleSelectContact(value)"
						></el-autocomplete>
						<!-- <el-input size="mini" placeholder="请输入CIQ联系人" v-model="detailData.checkContact" clearable></el-input> -->
					</el-form-item>
				</el-col>
				<el-col :span="6">
					<el-form-item label="CIQ联系人电话" prop="checkContactPhone">
						<el-input size="mini" placeholder="请输入CIQ联系人电话" v-model="detailData.checkContactPhone" clearable></el-input>
					</el-form-item>
				</el-col>
				<el-col :span="24">
					<el-form-item label="CIQ检测要求" prop="remark">
			...
			<div class="table-com-layout">
				<div class="row-tit h32" style="border-bottom: 0; padding: 0">
					...
					<div class="row-tit-operate">
						...
						<!-- <el-button size="mini" type="primary" @click="handleAddBox" :disabled="!isOp || isJoint || detailData.status === 'finish'" v-if="!isBulkRo">新增集装箱</el-button> -->
					</div>
				</div>
				<el-table ref="ciqBox" border fit style="width: 100%" :data="boxList" :span-method="objectSpanMethod">
					...
					<el-table-column prop="mark" label="唛头" align="center" width="150">
						<template slot-scope="scope">
					...
					<el-table-column prop="quantity" label="件数" align="center" width="120">
						<template slot-scope="scope">
							<el-input size="mini" placeholder="请输入件数" v-model="scope.row.quantity" clearable @blur="validateQuantityNum('quantity', scope.row)" @input="handleCountTotalPrice(scope.row)" @change="handleCountTotalPrice(scope.row)"></el-input>
						</template>
					</el-table-column>
		...
		<div v-if="fileListShow">
			<CiqFileList :param="fileListQuery" :attachmentList="attachmentList" :isBulkRo="isBulkRo" @close="fileListPopClose" />
		</div>
		<div v-if="boxUploadShow">
			<CiqBoxUpload :intrustNo="detailData.intrustNo" @close="boxUploadPopClose" />
		</div>
...
import CiqFileList from './ciqFileList'
import CiqBoxUpload from './ciqBoxUpload'
...
export default {
	...
	computed: {
		...
		statusTip() {
			if (this.detailData.status === 'finish') {
				return '检测状态为已完成，不允许操作'
			}
		}
	},
	components: {
	...
	watch: {
		createItem: {
			handler(newVal) {
				...
				this.setInitTableData(this.detailData.ciqCnVos)
			},
			deep: true,
			immediate: true
		}
    ...
    */
		...
		getSupplierList(val = '') {
			this.$store.dispatch('dict/querySupplierList', { category: 'serviceCode', value: 'ciq', queryString: val }).then(data => {
      ...
      return arr
		...
		validate() {
			...
			if (!this.boxList.length) {
				this.$message({ type: 'error', message: '请填写整箱清单信息', duration: 1000, onClose: () => {} })
				return false
			}
			if (nameValid) {
				this.$message({ type: 'error', message: '请填写品名', duration: 1000, onClose: () => {} })
				return false
			}
			if (isSameName) {
				this.$message({ type: 'error', message: '单个箱号不能添加多个相同的品名', duration: 1000, onClose: () => {} })
				return false
			}
			if (markValid) {
				this.$message({ type: 'error', message: '请填写唛头', duration: 1000, onClose: () => {} })
				return false
			}
			if (quantityValid) {
				this.$message({ type: 'error', message: '请填写正确的件数', duration: 1000, onClose: () => {} })
				return false
			}
			if (unitPriceValid) {
				this.$message({ type: 'error', message: '请填写正确的单价', duration: 1000, onClose: () => {} })
				return false
			}
			if (weightValid) {
				this.$message({ type: 'error', message: '请填写正确的毛重', duration: 1000, onClose: () => {} })
				return false
			}
			if (volumeValid) {
				this.$message({ type: 'error', message: '请填写正确的体积', duration: 1000, onClose: () => {} })
				return false
			}
			if (currencyValid) {
				this.$message({ type: 'error', message: '请选择正确的币别', duration: 1000, onClose: () => {} })
				return false
			}
			if (packageTypeValid) {
				this.$message({ type: 'error', message: '请选择包装', duration: 1000, onClose: () => {} })
				return false
			}
			return true
		},
		handleSave() {
			this.$refs.ciqSave.validate(valid => {
				if (valid) {

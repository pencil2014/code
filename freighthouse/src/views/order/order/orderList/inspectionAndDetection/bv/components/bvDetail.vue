<template>
	<div class="bv-container">
		<div class="row-tit h32" id="saveBar">
			<div class="ft12 tit">{{ type === 'add' ? '新增BV委托' : 'BV委托详情' }}</div>
		...
		<el-form ref="bvSave" :rules="rules" :model="detailData" class="form-body mt10" label-width="90px">
			<el-row>
				<el-col :span="6">
					<el-form-item label="BV委托编号" prop="intrustNo">
				...
				<!-- <el-col :span="4">
        <el-form-item label="BV检测公司" prop="checkSupplierId">
          <el-select style="width: 100%" clearable placeholder="请选择" size="mini" v-model="detailData.checkSupplierId"
            ...
            <el-option v-for="(item, index) in supplierList" :key="index" :label="item.label" :value="item.value"></el-option>
				...
				<el-col :span="6">
					<el-form-item label="BV联系人" prop="checkContact">
						<el-input size="mini" placeholder="请输入BV联系人" v-model="detailData.checkContact" clearable></el-input>
					</el-form-item>
				</el-col>
				<el-col :span="6">
					<el-form-item label="BV联系人电话" prop="checkContactPhone">
						<el-input size="mini" placeholder="请输入BV联系人电话" v-model="detailData.checkContactPhone" clearable></el-input>
					</el-form-item>
				</el-col>
				<el-col :span="12">
					<el-form-item label="BV检测要求" prop="remark">
			...
			<div class="table-com-layout">
				...
				<el-table ref="bvBox" border fit style="width: 100%" :data="boxList" :span-method="objectSpanMethod">
		...
		<div v-if="fileListShow">
			<BvFileList :param="fileListQuery" :attachmentList="attachmentList" @close="fileListPopClose" />
		</div>
		<div v-if="boxUploadShow">
			<BvBoxUpload :intrustNo="detailData.intrustNo" @close="boxUploadPopClose" />
		</div>
	</div>
</template>
...
import BvFileList from './bvFileList'
import BvBoxUpload from './bvBoxUpload'
...
export default {
	...
	watch: {
		createItem: {
			handler(newVal) {
				...
				this.setInitTableData(this.detailData.bvBlVos)
			},
			deep: true,
			immediate: true
		}
	...
	methods: {
		...
		setInitTableData(values) {
			...
			values.map(item => {
				if (item.bvCnVos && item.bvCnVos.length) {
					item.bvCnVos.map(ele => {
						if (ele.bvBoxVos && ele.bvBoxVos.length) {
							ele.bvBoxVos.map((o, oIndex) => {
								let itemQuery = {}
    ...
    */
		...
		// },
		// getSupplierList(val = '') {
		// 	this.$store.dispatch('dict/querySupplierList', { category: 'serviceCode', value: 'bv', queryString: val }).then(data => {

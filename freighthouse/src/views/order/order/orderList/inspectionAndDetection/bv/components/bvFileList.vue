<template>
	<div class="table-com-layout">
		...
		<div v-if="uploadShow">
			<BvFileUpload :param="param" :attachmentList="attachmentList" @close="uploadPopClose" />
		</div>
	</div>
</template>
<script>
...
import { bvFileList, bvDeleteFile } from '@/api/order/inspectionAndDetection/bv'
import BvFileUpload from './bvFileUpload'
...
export default {
	...
	data() {
		return {
			...
			uploadShow: false,
			staticType: ['bv_box_list', 'bv_application'], // 固定的附件类别
	...
	methods: {
		...
		handleImport(content, row) {
			console.log(content, row, this.param)
			// 根据后台需求数据格式

	...
	<div class="table-com-layout" id="applyFume">
		<div class="mt10 between" style="margin-bottom: 4px">
			...
			<div class="el-button-group" v-if="isOp">
				<ButtonTip
					btnType="primary" 
					:btnDisabled="isBd || isJoint || detailData.intrustStatus === 'finished'" 
					btnText="添加申请人" 
					:tipContent="isOpTruckAndOpTip || isJointTip || isFinishTip || statusTip" 
					class="operateBtn-tip ml10" 
					@click="handleAdd">
				</ButtonTip>
				<!-- <el-button size="mini" type="primary" class="ml10" @click="handleAdd" :disabled="isBd || isJoint || detailData.intrustStatus === 'finished'">添加申请人</el-button> -->
			</div>
		</div>
				...
				<template slot-scope="scope">
					<div class="operate-group">
						<el-upload
							...
							:file-list="fileList"
							:disabled="!isOp || detailData.intrustStatus === 'finished'"
						>
							<ButtonTip
								btnType="text" 
								:btnDisabled="!isOp || isJoint || detailData.intrustStatus === 'finished'" 
								btnText="上传附件" 
								:tipContent="isOpTruckAndOpTip || isJointTip || isFinishTip || statusTip" 
								class="operateBtn-tip" 
								btnClassName="underline">
							</ButtonTip>
							<!-- <el-button type="text" size="mini" style="text-decoration: underline" :disabled="!isOp || isJoint || detailData.intrustStatus === 'finished'">上传附件</el-button> -->
						</el-upload>
						<ButtonTip
							btnType="text" 
							:btnDisabled="!isOp || isJoint || detailData.intrustStatus === 'finished'" 
						...
						<!-- <el-button @click="handleInfo(scope.row, scope.$index)" type="text" size="mini" :disabled="!isOp || isJoint || detailData.intrustStatus === 'finished'">详情</el-button> -->
						<ButtonTip
							btnType="text" 
							:btnDisabled="!isOp || isJoint || detailData.intrustStatus === 'finished'" 
							btnText="删除" 
						...
						<!-- <el-button @click="handleDelete(scope.row, scope.$index)" type="text" size="mini" :disabled="!isOp || isJoint || detailData.intrustStatus === 'finished'">删除</el-button> -->
					</div>
				</template>
			</el-table-column>
		</el-table>
...
export default {
	...
	computed: {
		...
		statusTip() {
			if (this.detailData.intrustStatus === 'finished') {

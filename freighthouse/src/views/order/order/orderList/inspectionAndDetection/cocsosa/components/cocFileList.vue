	...
	<div class="table-com-layout">
		<BaseDialog :config="dialogConfig" :callback="dialogCallback">
			<div class="coc-bewteen">
				<span class="ft12">M单号：{{ param.blNo }}</span>
				...
				<el-table-column label="操作" align="center">
					<template slot-scope="scope">
						<div class="operate-group">
							...
							<!-- 放开固定附件的删除 v-if="scope.row.must != '1'"-->
		...
		<div v-if="uploadShow">
			<CocFileUpload :param="param" :attachmentList="attachmentList" @close="uploadPopClose" />
		</div>
...
import { cocFileList, cocDeleteFile, cocSelectFile } from '@/api/order/inspectionAndDetection/coc'
import CocFileUpload from './cocFileUpload'
...
export default {
	...
	data() {
		return {
			...
			uploadShow: false,
			staticType: ['coc_saso_box_list', 'iso_certification_report', 'internal_inspection_report', 'customs_clearance_invoice', 'idf'], // 固定的附件类别
	...
	methods: {
		...
		getFileList() {
			let data = {
				...
				checkSupplierName: this.param.checkSupplierName
			}
			cocFileList(data).then(res => {
		...
		handleImport(content, row) {
			...
			form.append('checkSupplierName', this.param.checkSupplierName)
			form.append('orderNo', this.$route.query.orderNo)
			this.oQuery.jointNo && form.append('jointNo', this.$route.query.jointNo)

			if (this.sizeLimit) {
				this.$store
					.dispatch('user/import', {
						uri: '/order/shipping/coc/file/upload',
						params: form
					})
					.then(res => {
						this.fileList = []

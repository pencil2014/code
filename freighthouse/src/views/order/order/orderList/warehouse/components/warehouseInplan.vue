	...
	<div id="warehouseInplan" class="edit-row" v-loading="lsLoading" :element-loading-text="eLoadingText" :element-loading-spinner="eLoadingSpinner" :element-loading-background="eLoadingBackground">
		<div class="default-table" id="tableCont">
			<div class="warehouse-btnGroup">
				...
				<ButtonTip btnType="default" :btnDisabled="!isOpTruck || isBd || isDisabledEdit" btnText="新增派车" :tipContent="isOpTruckTip || isFinishTip || statusTip" class="operateBtn-tip ml10" @click="handleClick('car')"> </ButtonTip>
				<!-- <el-button @click="handleClick('car')" type="default" size="mini" :disabled="!isOpTruck || isBd || isDisabledEdit">新增派车</el-button> -->
				<ButtonTip btnType="primary" :btnDisabled="(!isOp && !isOpTruck) || isBd || isDisabledEdit" btnText="新增进仓计划" :tipContent="isOpTruckAndOpTip || isFinishTip || statusTip" class="operateBtn-tip ml10" @click="handleClick('add')">
				</ButtonTip>
			...
			<div class="table-com-layout">
				<el-table class="warehouse-tip-table" fit :data="tableData" @selection-change="handleSelectionChange" align="center" style="width: 100%">
					...
					<el-table-column prop="deliveryCustName" label="客户名称" align="center" show-overflow-tooltip></el-table-column>
					...
					<el-table-column label="操作" width="200" align="center" fixed="right">
						<template slot-scope="scope">
							<div v-if="scope.row.planNo" class="operate-group">
								...
								<ButtonTip
									btnType="text"
									:btnDisabled="!isOpTruck || isBd || isJoint || isDisabledEdit || scope.row.canCreateIn === 'N' || scope.row.status === 'finish'"
									btnText="实际进仓"
									:tipContent="isOpTruckTip || isJointTip || isFinishTip || statusTip || isSyncTip(scope.row) || isStatusFinishTip(scope.row)"
									class="operateBtn-tip"
...
import oneKeyDate from './dialog/realInDate'
...
export default {
	...
	methods: {
		isSyncTip(row) {
			if (row.canCreateIn === 'N') {
				return '该进仓计划已被同步到鼎帆系统，不允许操作'
			}
		},
		isStatusFinishTip(row) {
			if (row.status === 'finish') {
				return '本计划已全部进仓，不可再继续进仓'
			}
		},
		// 日期弹窗关闭
		...
		handleExport() {
			...
			let docType = this.selectedLists[0].canCreateIn === 'N' ? 'xls' : 'doc'
			let data = {
				orderNo: this.$route.query.orderNo,
			...
			this.$store
				...
				.then(res => {
					...
					link.download = `进仓计划${data.orderNo}.${docType}`
					link.click()
				})
		},
		// 获取so列表
		...
		handleClick(type) {
			if (type == 'batchIn') {
				...
				if (this.selectedLists.some(item => item.canCreateIn === 'N')) return this.$message.warning('选中数据中包含被同步到鼎帆系统的数据!')

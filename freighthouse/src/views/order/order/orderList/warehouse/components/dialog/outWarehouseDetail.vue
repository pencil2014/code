		...
		<div class="row-tit" id="saveBar">
			...
			<div class="row-tit-operate">
				...
				<ButtonTip
					...
					btnText="保存"
					:tipContent="isOpTruckTip || isJointTip || isFinishTip || statusTip"
		...
		return {
			...
			tableInfo: {
				...
				titleBtnList: [
					...
					{ label: '添加集装箱', key: 'add', disabled: this.detailData.isCargoEdit === 'y', type: 'primary', tipContent: this.detailData.isCargoEdit === 'y' ? '该货品已编辑过，不允许操作' : '' || this.isFinishTip || this.statusTip },
					{ label: '关联集装箱', key: 'relate', disabled: this.detailData.isCargoEdit === 'y', type: 'primary', tipContent: this.detailData.isCargoEdit === 'y' ? '该货品已编辑过，不允许操作' : '' || this.isFinishTip || this.statusTip }
				]
			},
			// 表格配置项
			tableConfig: {
	...
	watch: {
		detailData: {
			handler(newValue) {
				...
				this.$set(this.inputItem[2], 'isHide', true)
				setTimeout(() => {
					this.$set(this.inputItem[2], 'isHide', false)
				}, 0)
				this.$set(this.inputItem[2], 'options', newValue.whId ? [{ label: newValue.warehouseName, value: newValue.whId }] : [])
				// this.$set(this.inputItem[2], 'remoteMethod', (val, item) => {

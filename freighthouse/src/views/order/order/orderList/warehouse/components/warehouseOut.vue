<template>
	...
	<div id="warehouseInplan" class="edit-row" v-loading="lsLoading"
		...
		<template v-if="showInWarehouse">
			<OutWarehouseDetail :isOpTruckTip="isOpTruckTip" :isJointTip="isJointTip" :isFinishTip="isFinishTip" :statusTip="statusTip" :isFinish="isFinish" :isDisabledEdit="isDisabledEdit" :isJoint="isJoint" :detailData="in_formData" :saveBarFixed="saveBarFixed" @close="close" @handleConfirm="handleSave"></OutWarehouseDetail>
		</template>
		<!-- 附件 -->
		<template v-if="showFilesDialog">
			<FilesDialog @close="closeFilesDialog" :warehouseType="'out'" :warehouseId="warehouseId"></FilesDialog>
		</template>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		statusTip() {
			// 放开已关账、已完成的限制
			// ['cancel'].includes(this.$store.state.order.orderInfoDataObj.orderStatus) || (this.$store.state.order.orderInfoDataObj.finCloseStatus === 'yes' && !this.finCloseStatusCanEdit)
			if(['complete','cancel'].includes(this.$store.state.order.orderInfoDataObj.orderStatus) || (this.$store.state.order.orderInfoDataObj.finCloseStatus === 'yes' && !this.finCloseStatusCanEdit)) {

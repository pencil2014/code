	...
	<div id="warehouseOutplanDetail">
		<div class="row-tit" id="saveBar">
			...
			<div class="row-tit-operate">
				...
				<ButtonTip
					btnType="primary"
					:btnDisabled="(!isOpTruck && !isOp) || isDisabledEdit || formData.canEdit === 'N'"
					btnText="保存"
					:tipContent="isOpTruckAndOpTip || isFinishTip || statusTip || isSyncTip"
	...
	watch: {
		detailData: {
			handler(newValue) {
				...
				this.$set(this.inputItem[1], 'disabled', newValue.canCreateIn === 'N')
				this.$set(this.inputItem[1], 'isHide', true)
				setTimeout(() => {
					this.$set(this.inputItem[1], 'isHide', false)
				}, 0)
				this.$set(this.inputItem[1], 'options', newValue.whId ? [{ label: newValue.warehouseName, value: newValue.whId }] : [])
				// this.$set(this.inputItem[1], 'remoteMethod', (val, item) => {
				...
				// 		// })
				// 		item.disabled = newValue.canCreateOut === 'N'
				// 	}
				// })

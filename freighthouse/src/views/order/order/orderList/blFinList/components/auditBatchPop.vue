			...
			<div class="audit-batch-row" v-for="(item, index) in param.data" :key="index">
				<el-row class="audit-form-row">
					...
					<el-col :span="6">
						<div class="blNo">
							...
							<div class="item-val">
								<i class="icon-hbl" v-if="item.blType">{{ item.blType === 'hbl' ? 'H' : 'M' }}</i
								>{{ item.blNo }}
							</div>
						</div>
					</el-col>
					...
					<el-col v-if="!param.isSpecial && ['original', 'jthdlzb', 'thzb'].includes(item.blMode)" :span="6">
						<div class="item-lab is-origin-pod">是否正本改目放：</div>
						<div class="item-val">{{ item.isOriginalToPod | getOriginPodFilter }}</div>
					</el-col>
				</el-row>
				<el-row class="audit-form-row">
					<el-col :span="6">
				...
				<el-row class="audit-form-row">
					<el-col :span="24">
						...
						<div class="item-val">
							<span style="color: #1890ff; cursor: pointer; margin-right: 10px" @click="handleViewFile(item, index)" v-for="(name, index) in fileNames(item)" :key="index">{{ name }}</span>
						</div>
					</el-col>
				</el-row>
				<div class="audit-form-row warning" v-if="item.applyType === 'pledge' && item.pledgeOrderNos">
					...
					<div class="value-fr">
						{{`提单${item.blNo}(${item.orderNo})选择了提单${item.pledgeBlNo}(${item.pledgeOrderNo})作为押单。`}}
	...
	methods: {
		...
		getFinApplyReason() {
			...
			this.param.data.map(item => {
				let orderBls = [{ orderNo: item.orderNo, blId: item.blId }]

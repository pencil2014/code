		...
		<el-form ref="processing" :rules="rules" :model="createdItem" label-position="right" label-width="75px" class="audit-form">
			<el-row class="audit-form-row">
				<el-col :span="6">
					...
					<div class="item-val">{{ param.orderNo }}</div>
				</el-col>
				<el-col :span="6">
					<div class="blNo">
				...
				<el-col :span="6">
					...
					<div class="item-val">{{ param.blMode | getDictLabelFilter('blMode') }}</div>
				</el-col>
				<el-col v-if="param.isSpecial !== 'Y' && ['original', 'jthdlzb', 'thzb'].includes(param.blMode)" :span="6">
					<div class="item-lab is-origin-pod">是否正本改目放：</div>
					<div class="item-val">{{ param.isOriginalToPod | getOriginPodFilter }}</div>
				</el-col>
			</el-row>
			<el-row class="audit-form-row">
				<el-col :span="6">
					<div class="item-lab">销售：</div>
					<div class="item-val">{{ param.bdEmployeeName }}</div>
				</el-col>
				<el-col :span="6">
					<el-tooltip class="item" effect="dark" placement="top" :content="param.custSettleUnit">
						<div class="settleUnit-tooltip">
							<div class="item-lab">结算单位：</div>
							<div class="item-val">{{ param.custSettleUnit }}</div>
						</div>
					</el-tooltip>
				</el-col>
				<el-col :span="6">
					<div class="item-lab">申请原因：</div>
					<div class="item-val">{{ param.applyType | getDictLabelFilter('blFinApplyType') }}</div>
				</el-col>
			</el-row>
			<el-row class="audit-form-row" v-if="['pledge', 'dzg'].includes(param.applyType)">
				<el-col :span="24" v-if="param.applyType === 'pledge'">
					<div class="item-lab">被押单号：</div>
					<div class="item-val">{{ param.pledgeOrderNo }}</div>
				</el-col>
				<el-col :span="8" v-if="param.applyType === 'dzg'">
					<div class="item-lab">被押大掌柜单：</div>
					<div class="item-val">{{ param.pledgeOrderNo }}</div>
				</el-col>
				<el-col :span="8" v-if="param.applyType === 'dzg'">
					<div class="item-lab">被押T量：</div>
					<div class="item-val">{{ param.pledgeAmount }}</div>
				</el-col>
				<el-col :span="8" v-if="param.applyType === 'dzg'">
					<div class="item-lab">被押提单号：</div>
					<div class="item-val">{{ param.pledgeBlNo }}</div>
				</el-col>
			</el-row>
			<el-row class="audit-form-row">
				<el-col :span="24">
					<div class="item-lab">申请人：</div>
					<div class="item-val">{{ param.applyEmployeeName }}</div>
				</el-col>
			</el-row>
			<div class="audit-form-row">
				<div class="label-fl" style="width: 60px">申请备注：</div>
				<div class="value-fr">{{ param.applyRemark }}</div>
			</div>
			<el-row class="audit-form-row">
				<el-col :span="24">
					<div class="item-lab">附件：</div>
					<div class="item-val">
						<span style="color: #1890ff; cursor: pointer; margin-right: 10px" @click="handleViewFile(item, index)" v-for="(item, index) in fileNames" :key="index">{{ index === fileNames.length - 1 ? item : item + ' ,' }}</span>
					</div>
				</el-col>
			</el-row>
			...
			<div class="audit-form-row warning" v-if="Object.keys(tipsObj).length">
				...
				<div class="value-fr">
					<div v-for="(item, index) in tipsObj" :key="index">{{ item }}</div>
				</div>
			</div>
			<div class="audit-form-row warning" v-if="param.applyType === 'pledge' && pledgeOrderNos">
				<div class="label-fl"></div>
				<div class="value-fr">
					{{`提单${param.blNo}(${param.orderNo})选择了提单${param.pledgeBlNo}(${param.pledgeOrderNo})作为押单。`}}
					<!-- {{ `所选的被押提单，已被工作号${pledgeOrderNos}押了。` }} -->
				</div>
			</div>
			<div class="audit-form-row warning" v-if="param.applyType === 'dzg' && isDzgOrder && dzgBlNos">
				<div class="label-fl"></div>
				<div class="value-fr">
					{{ `该被押工作号，已被提单${dzgBlNos}押了。` }}
				</div>
			</div>
			...
			<el-table border fit highlight-current-row style="width: 100%" ref="settleList" :data="tableData" class="settle-list-table" @cell-mouse-enter="showTooltip" @cell-mouse-leave="hiddenTooltip">
				...
				<el-table-column prop="orginArrearAmt" label="原超期金额" align="center" width="90">
					<template slot-scope="scope">
						<el-button size="mini" type="text" @click="handleArrearAmt(scope.row)">{{scope.row.orginArrearAmt}}</el-button>
					</template>
        </el-table-column>
...
import { finSettList, getFinApplyReason, checkPledgeOrder, checkDzgOrder } from '@/api/order/bl'
...
export default {
	...
	filters: {
		getOriginPodFilter(val) {
			return val === 'Y' ? '是' : val === 'N' ? '否' : ''
		}
	},
	created() {
		console.log('this.param', this.param)
		if (this.param.isSpecial === 'Y') {
			this.auditStatusOptions = [
				{ label: '审核通过', value: 'pass' },
				{ label: '审核拒绝', value: 'refuse' }
			]
			// this.dialogConfig.btns =  [
			//   {label: '审核通过', action: 'Pass', type: 'success'},
			//   {label: '审核拒绝', action: 'Refuse', type: 'danger'},
			//   {label: '取消', action: 'Cancel'}
			// ]
		}
		if (this.param.auditStatus === 'processing') {
			this.createdItem.auditStatus = this.param.auditStatus
			this.createdItem.reason = this.param.auditReason
			this.createdItem.remark = this.param.auditRemark
		}
		this.getFinSettList()
		// 押柜
	...
	computed: {
		...
		fileNames() {
			if (this.param.fileName) {
				return this.param.fileName.split(',')
			} else {
				return []
			}
		}
	...
	methods: {
		// 预览
		handleViewFile(fileName, index) {
			this.$store.dispatch('railway/previewFile', {
				fileNo: this.param.fileNo?.split(',')[index],
				fileName: fileName
			})
		},
		// 打开后有柜的弹窗
		...
		// 获取关联费用的结算单位
		getFinSettList() {
			let { orderNo, blId, applyType } = this.param
			finSettList({
				orderNo,
				blId,
				type: applyType
		...
		getFinApplyReason() {
			...
			getFinApplyReason(data).then(res => {
				let { canPut, applyTypes, tips } = res.data
				this.resApplyReason = {
					canPut,
					applyTypes,
					tips
		...
		dialogCallback(action, done) {
			if (['Pass', 'Processing', 'Refuse', 'Confirm'].includes(action)) {
				this.$refs.processing.validate(valid => {
					if (valid) {
						// // 特殊申请审核需要校验审核结果必填
						// if (this.param.isSpecial === 'Y' && !this.createdItem.auditStatus) {
    ...
    handleArrearAmt(row) {
			this.close('ArrearAmtOriginDetail', row)
    },
	}
}
</script>

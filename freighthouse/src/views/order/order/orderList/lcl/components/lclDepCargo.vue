		...
		<div class="row-flex flex-cargo" v-for="(o, index) in lclDepCargoList" :key="index">
          ...
          <el-select size="mini" v-model="o.hscode" style="width: 100%"  placeholder="请选择海关编号" disabled>
           ...
           <el-input size="mini" v-model="o.ename" clearable placeholder="请输入英文品名" disabled maxlength="1024"></el-input>
			...
			<el-form-item v-if="isWoodenPackageState" class="cargo-item hscode" label="木质包装" :show-message="false" :rules="{ required: false, message: '请选择木质包装', trigger: 'change' }">
			...
			<el-row class="flex-item-commodityDesc">
				<el-form-item class="form-cont" label="唛头" :rules="{ required: isBd && !['st18', 'st19'].includes(createItem.serviceType), message: '请输入唛头', trigger: 'blur' }" style="clear: both">
					<TextAreaItem :disabled="viewState" :required="isBd && !['st18', 'st19'].includes(createItem.serviceType)"  :keys="`mark-${index}`" label="唛头" :rows="1" maxHeightVal="66px" :value.sync="o.mark"></TextAreaItem>
    ...
    isEcs() {
      return this.ordDetRole === 'ecs'
		...
		viewState() {
			...
			if (!this.ordDetRole || this.isOp || this.isEpricing || this.isEcs || this.isDoc || this.isAc || this.isOpTruck || this.isBkg) {
				return true
			}
			return false
		}

							...
							<template v-if="$route.query.source !== 'jointList'">
								...
								<template v-if="isBd">
									...
									<el-button
										...
										:disabled="submitDisabled"
										v-if="createItem.orderStatus === 'pass' && createItem.serviceList.includes('proxy_book') && createItem.submitCancelStatus !== 'APPLIED'"
										>撤回</el-button
									>
								...
								<!-- 操作 海外客服-->
								<template v-if="isEcs || isOp">
									<el-button size="mini" type="primary" class="" @click="handlePush" v-if="['pass'].includes(createItem.orderStatus) && hasDestinationServices" :disabled="submitDisabled">推送海外分公司</el-button>
								</template>
...
如只出MBL（只针对冻柜）, SHIPPER栏不接受ON B长帆显示为实际SHIPPER，只能显示LONGSAIL。"
				...
				<el-form ref="orderSubmit" :model="createItem" :rules="rules" class="order-cont-form" size="mini">
					...
					<div class="ship-cont" ref="shipInfoCont" :class="workAssignShow ? 'abs-toogle' : ''" :style="{ height: `${shipInfoHeight}px` }">
						...
						<OverseasInfo
							...
							:orderStatus="createItem.orderStatus"
						/>
						<SoContainer :containerTotal="containerTotal" :createItem="createItem" :orderStatus="createItem.orderStatus" @emptyFreeTimeSys="emptyFreeTimeSys" @getFreeTimeSys="handleFillFreetimeSys" ref="soContainer" v-if="!isBulkRo" />
						<!-- 货物信息 -->
						<div class="row-cont">
			...
			<div v-if="pushOverseaShow">
				<push-oversea @close="handlePushClose"></push-oversea>
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	computed: {
		showOutBillTip() {
			...
			let isSpecial = this.createItem.soContainerList.some(item => item.containerList.some(subItem => containerTypeList.includes(subItem.containerType)))
			return isSpecial && this.createItem.ship.shipCarrierCode === 'MSK'
		},
		bdId() {
	...
	methods: {
		...
		handlePush() {
			if (!this.shipOrderInfoData.ship?.tdTime) return this.$message.error('请先维护大船实际开船时间')
		...
		getServiceTypes() {
			...
			baseBusinessServiceList(param).then(res => {
				...
				if (this.createItem.serviceType === 'st01') {
					this.serviceCodeOptions = this.serviceCodeOptions.filter(item => item.serviceCode !== 'stuffing')
				}
				console.log('this.serviceCodeOption:', this.serviceCodeOptions)
		...
		handleSave(type) {
			...
			if (type === 'Submit') {
				...
				if (!canSubmit) {
					...
					// 散杂滚装的不用校验箱型箱量里的温度,收货人
					if (!this.isBulkRo) {
						if (!this.temperatureValidate(data)) {
							this.submitDisabled = false
							return this.$message.error('箱型箱量中的最高温度不能低于最低温度')
						}
						// document.getElementById('consigneeTextArea').classList.remove('is-error')
						if (
							this.createItem.serviceType === 'st01' &&
							['VOLTA', 'COSCO'].includes(this.createItem.ship.shipCarrierCode) &&
							this.createItem.payMode === 'collect' &&
							this.createItem.ship.priceType === 'pricing' &&
							['100075000', '100075018', '100074999', '100075011', '100075005', '100075008'].includes(this.createItem.ship.sysLineCode) &&
							!this.createItem.transport.consignee
						) {
							this.submitDisabled = false
							// document.getElementById('consigneeTextArea').classList.add('is-error')
							// toViewPosition('ship-cont', 80)
							return this.$message.error('物流信息中的Consignee(收货人)为必填')
						}
					}

							...
							<template v-if="$route.query.source !== 'jointList'">
								<template v-if="isBd">
									...
									<template v-if="!isBulkRo">
										<el-button size="mini" type="danger" class="ml10" @click="handleCancelRequest" :disabled="submitDisabled" v-if="['pass'].includes(createItem.orderStatus) && createItem.cancelApplyStatus !== 'applied'">{{
									...
									<template v-else>
										<el-button
											...
											v-if="['pass'].includes(createItem.orderStatus) && createItem.cancelApplyStatus !== 'applied' && ['book_release', 'release_custom'].includes(createItem.bkgStatus)"
								...
								<template v-if="isOp">
									...
									<template v-else>
										<el-button
											...
											@click="handleOpCancelRequest('cancel')"
											:disabled="submitDisabled"
											v-if="!['CANCEL', 'BREAK'].includes(createItem.cancelJointHandleStatus) && ['pass'].includes(createItem.orderStatus) && ['book_release', 'release_custom'].includes(createItem.bkgStatus) && !isOrderFeeCount"
											>取消</el-button
										>
										<el-button
											size="mini"
									...
									<div class="pd8 flex-form-item label100">
										...
										<template v-if="isBulkRo">
											...
											<el-form-item label="驳船预计开船时间" prop="barge.etdTime" :show-message="false" v-if="includeFeeder === 'y' || (isOpView && hasInternalTradeBarge)">
												<elDatePickerLimit :field="'bargeEtdTime'" size="mini" v-model="createItem.barge.etdTime" type="date" style="width: 100%" clearable placeholder="请选择" value-format="yyyy-MM-dd" :disabled="setDisabled() && isView" />
											</el-form-item>
										...
										<el-form-item label="大船预计开船时间" prop="ship.etdTime" :show-message="false" class="vessel-time" v-if="isBulkRo">
											<elDatePickerLimit :field="'etdTime'" size="mini" v-model="createItem.ship.etdTime" type="date" style="width: 100%" clearable placeholder="请选择" value-format="yyyy-MM-dd" :disabled="setDisabled() && isView && !isDocEdit" />
											<el-popover placement="top" width="280" trigger="click" class="vessel-time-popover" v-if="isDocEdit || !setDisabled() || !isView">
												<Schedule :vesselTime="vesselTime" :tdTimeDisabled="setDisabled() && isView && !isDocEdit" @updateVesselTime="updateVesselTime" />
										...
										<el-form-item class="timeSpecial" label="预计到港时间" prop="ship.etaTime" :show-message="false" v-if="isOpView && isBulkRo">
											<elDatePickerLimit size="mini" :field="'etaTime'" v-model="createItem.ship.etaTime" type="date" style="width: 100%" clearable placeholder="请选择" value-format="yyyy-MM-dd" :disabled="setDisabled() && isView && !isDocEdit && !isEcsEpricingEdit" />
										</el-form-item>
										<el-form-item class="timeSpecial" label="实际到港时间" prop="ship.taTime" :show-message="false" v-if="isOpView && isBulkRo">
											<elDatePickerLimit
												...
												placeholder="请选择"
												value-format="yyyy-MM-dd"
												:disabled="setDisabled() && isView && !isDocEdit && !isEcsEpricingEdit"
											/>
										</el-form-item>
							...
							<div class="table-td">
								<div class="cont">
									...
									<div class="pd8 flex-form-item label90">
										...
										<el-form-item label="海关是否查验" prop="transport.isCustomsInspection" v-if="isOpView && !isBulkRo">
											<el-radio-group v-model="createItem.transport.isCustomsInspection">
												<el-radio :label="item.value" :disabled="setDisabled() && isView" v-for="(item, index) in yesOrNoOptions" :key="index">{{ item.label }}</el-radio>
											</el-radio-group>
										</el-form-item>
										<el-form-item label="委托备注" prop="intrustRemark" :show-message="false">
											<el-input
												@dblclick.native="showRemarkDetail(createItem.intrustRemark)"
												type="textarea"
												clearable
												rows="4"
												maxlength="512"
												...
												placeholder="请输入"
												show-word-limit
												v-model="createItem.intrustRemark"
												:disabled="setDisabled()"
											></el-input>
										</el-form-item>
...
export default {
	...
	computed: {
		...
		isOpView() {
			return ['pass', 'cancel', 'break'].includes(this.createItem.orderStatus)
		...
		isEcsEpricingEdit() {
			return ['ecs', 'epricing'].includes(this.ordDetRole) && this.createItem.orderStatus === 'pass'
		},
		addState() {
    ...
    */
		...
		fillOrderInfo(data, type) {
			...
			this.createItem = Object.assign({}, this.createItem, data, {
		...
		getMergeRows(row) {
			let result = []
			result = this.soList
				.filter(item => item.soId === row.soId)
				.map(item => {
					return {

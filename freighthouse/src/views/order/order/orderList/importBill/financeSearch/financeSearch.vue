<template>
	<div style="min-height: 36px;" class="finance-search-form" :class="{ borderBottom: $attrs.border, noLabelWidth: $attrs.noLabelWidth }">
		<transition name="ls-fade">
			<el-form ref="form" :inline="true" size="mini">
				<el-form-item v-for="(item, index) in searchOption.columns" :key="'elFormItem' + index" :label="item.label">
					<el-input
						...
						clearable
					></el-input>
					<el-date-picker v-if="item.type === 'datePicker'" :value-format="item.valueFormat || 'yyyy-MM-dd'" v-model="searchOption.data[item.prop]" :placeholder="item.placeholder || '请选择' + item.label"></el-date-picker>
					<el-date-picker
					...
					<el-select
						...
						clearable
					>
						<el-option v-for="(op, j) in item.selectOptions || dictMap[item.propInDict] || dictMap[item.prop]" :key="item.prop + j" :label="op.label" :value="op.value"></el-option>
					</el-select>
					<el-select
						v-if="item.type === 'select' && item.remote"
						v-model="searchOption.data[item.prop]"
						...
						:remote-method="
							queryString => {
								item.remoteMethod && queryString !== '' && item.remoteMethod(queryString, item, searchOption.data)
							}
						"
						@visible-change="
							value => {
								if (value) {
									item.visibleChange && item.visibleChange('', item, searchOption.data)
								}
							}
						"
						@change="
							value => {
								item.change && item.change(value, item, searchOption.data)
							}
						"
						@click.native="remoteClickNative(item)"
					>
						<!-- 部分autocomplete类型更适合用select reomte，原始click事件解决首次点击select列表不触发问题 -->
						<el-option v-for="(optionItem, j) in item.remoteSelectList || []" :key="'select' + item.prop + j" :label="item.labelDIY ? optionItem[item.labelDIY.label] : optionItem.label" :value="optionItem.value">
					...
					<el-autocomplete
						...
						:fetch-suggestions="
							(queryString, cb) => {
								item.querySearch && item.querySearch(queryString, cb)
							}
						"
						@select="
							value => {
					...
					<el-cascader
						...
						expandTrigger="click"
					></el-cascader>
					<SearchFilter v-if="item.type === 'filter'" :item="item.item" :filterGroups="item.filterGroups" :allot-span="item.allotSpan" />
				...
				<template v-if="searchOption.addFilter">
					...
					/>
					<div v-if="searchOption.addFilter.plusShow !== false" class="finance-search-plus" @click="handleAddFilter"></div>
				</template>
				...
				<div class="search-btns" v-if="!searchOption.hideBtns">
					...
					<i :class="`el-icon-arrow-${templateShow ? 'down' : 'up'}`" @click="templateShow = !templateShow" v-if="searchOption.saveShow"></i>
				</div>
			</el-form>
		</transition>
		<!-- 搜索模板 -->
		<div class="search-template" v-show="templateShow && searchOption.saveShow">
			<div class="fl template-list">
				...
				>
					{{ item.composeKey }}
					<i class="el-icon-close" @click.stop="handleConditionDelete(item, index)"></i>
				</span>
			</div>
			<div class="fr template-operate">
				<span class="mr10" v-show="(searchOption.conditions || []).length > 5">
					更多模板
					<el-select style="width: 180px !important" size="mini" v-model="composeKey" placeholder="请选择更多模板" @change="handleChangeCondition">
						<el-option v-for="(item, index) in (searchOption.conditions || []).filter(item => !['defaultSearchSaveKey', 'searchSaveKey'].includes(item.composeKey))" :label="item.composeKey" :value="item.composeKey" :key="'composeKey' + index">
							<span style="float: left">{{ item.composeKey }}</span>
							<i class="el-icon-close" style="float: right; color: #8492a6; font-size: 12px; line-height: 28px" @click.stop="handleConditionDelete(item, index)"></i>
						</el-option>
					</el-select>
				</span>
				...
				<el-popover popper-class="search-template-popover" placement="bottom" width="240" v-model="popoverShow" @show="newComposeKey = ''">
					<div class="flex">
						<span class="span-text">模板名称</span>
						<el-input type="text" size="mini" maxlength="32" show-word-limit v-model="newComposeKey" placeholder="请输入模板名称" class="temp-input" style="width: 164px"></el-input>
					</div>
					...
					<el-button type="text" size="mini" slot="reference">{{ (searchOption.conditions || []).length && composeKey ? '另存为' : '保存' }}</el-button>
				</el-popover>
				<el-button type="text" size="mini" @click="handleSaveTemplate" v-show="(searchOption.conditions || []).length && composeKey">保存</el-button>
			</div>
		</div>
...
export default {
		...
		addFilterSearch() {
			...
			option.searchInputGroup.forEach(({ key: column, value }) => {
				...
				if (dateArr.indexOf(column) > -1) {
					...
					query.push({ column, type: 'like', value })
				}
			})
			return query
		},
		...
		searchConditionSaveKey() {
			...
			searchConditionSave({
				...
				if (this.searchOption.conditions && this.searchOption.conditions[0]) {
					this.searchOption.conditions[0].searchItemList = searchConfigList
				} else {
		...
		searchConditionInfo() {
			searchConditionInfo({ scenesCode: this.searchOption.saveName || this.$route.name }).then(res => {
				...
				if (this.searchOption.conditions.length) {
					...
					this.setSearchInfo(this.searchOption.conditions[0].searchItemList, 'nosave')
				} else {
					this.setSearchInfo('', 'nosave')
				}
			})
		},
		// 设置搜索模板的内容到搜索条件内
		setSearchInfo(searchItemList, nosave) {
		...
		tabFromOtherSearch() {
			...
			for (const key in query) {
				if (filterGroups[key]) {
					...
					if (find) {
						find.value = query[key]
					} else {
						searchInputGroup.unshift({ key, value: filterGroups[key], range: { min: '', max: '' } })
					}
				}
			}
			this.search('btn')
	...
	activated() {
		if (!this.isNotFirstActivated) {
			...
			} else if (this.searchOption.saveShow || this.searchOption.saveDefault) {
				this.searchConditionInfo()
			}

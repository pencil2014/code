<template>
  ...
  >
    <!-- lsrangeWith:filterGroups[item.key] && ['lsDaterange','datetimerange'].includes(filterGroups[item.key].type)}" -->
    <div class="el-row">
      ...
      <div class="el-col filter-right" v-if="rightShow">
        <el-popover
          popper-class="el-popover__search"
          class="multi-select-tag"
          v-if="filterGroups[item.key] && filterGroups[item.key].type === 'datetimerange'"
          placement="bottom"
          trigger="hover"
        ...
        <div class="multi-select-tag" v-else-if="filterGroups[item.key]">
          ...
          >
            ...
            <el-option
              v-for="o in filterGroups[item.key].remoteSelectList"
              :key="filterGroups[item.key].DIY ? o[filterGroups[item.key].DIY.key]+o.label : o.value+o.label"
              :label="o.label"
              :value="o.value"
            ></el-option>
          </el-select>
          ...
          <!-- 日期范围值 -->
          <!-- v-else-if="['lsDaterange','datetimerange'].includes(filterGroups[item.key].type)" -->
          <DateRange
          ...
          <el-date-picker
            style="width:200px"
            size="mini"
            v-else-if="['date', 'month','daterange', 'monthrange', 'datetimerange'].includes(filterGroups[item.key].type)"
            v-model="item.value"
            :type="filterGroups[item.key].type"
            :name="filterGroups[item.key].name"
            :readonly="filterGroups[item.key].readonly"

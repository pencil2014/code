<template>
	<el-dialog @close="handleClose" title="导入账单" :visible="dialogOrderShow" width="952px">
		<div id="si-import-bill" class="si-import-bill">
			<el-form ref="orderImportForm" :model="orderImportForm" :inline="true" label-width="100px" size="mini" :rules="orderImportRegRules">
				...
				<el-form-item label="单号类型:" prop="inputNoType">
					<el-select v-model="orderImportForm.inputNoType" placeholder="请选择" @change="changeLabel">
				...
				<el-form-item label="最晚确认时间:" prop="checkBfDate">
					<elDatePickerLimit :picker-options="pickerOptions" v-model="orderImportForm.checkBfDate" type="date" value-format="yyyy-MM-dd" placeholder="请选择"> </elDatePickerLimit>
				</el-form-item>
				<el-form-item :label="labelName" prop="inputNo">
					<el-autocomplete v-model="orderImportForm.inputNo" :fetch-suggestions="querySearch" placeholder="请输入" @select="handleSelect"></el-autocomplete>
				</el-form-item>
				<el-form-item v-if="orderImportForm.inputNoType === 'SO'" label="所有SO:" prop="soList">
					<el-select @change="changeLabel1" v-model="orderImportForm.soList" collapse-tags multiple placeholder="请选择">
						<el-option v-for="item in soLists" :key="item.value" :label="item.label" :value="item.value"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="附件类型:" prop="dnBillType">
					<el-select @change="changeLabel1" v-model="orderImportForm.dnBillType" filterable placeholder="请选择">
				...
				<el-form-item label="商务:" v-show="orderImportForm.dnBillType === 'DN_IMPORT_BILL'" prop="dnBillType">
					<el-select  v-model="orderImportForm.pricingEmployeeId" filterable placeholder="请选择">
						<el-option v-for="item in pricingInfoList" :key="item.pricingEmployeeId" :label="item.pricingEmployeeName" :value="item.pricingEmployeeId"></el-option>
					</el-select>
				</el-form-item>
...
import { blBillInfo, blAllSoList, blNoList, getFileBatchNo, getPricingInfo } from '@/api/order/bl'
...
export default {
	...
	created() {
		this.orderImportRegRules = {...this.rules1, ...this.rules2 }
		this.initDate()
	},
	mounted() {},
	computed: {
	...
	methods: {
      ...
      this.$message.error(`请先选择${name}`)
		...
		querySearch(queryString, cb) {
			if (!this.orderImportForm.inputNoType) {
				this.disabledSubmit = true
				cb([])
				return this.$message.warning('请先选择单号类型!')
			}
		...
		async handleSelect(item) {
			let { data } = await blBillInfo({ inputNoType: this.orderImportForm.inputNoType, inputNo: this.orderImportForm.inputNo })
			...
			if (this.orderImportForm.inputNoType === 'SO') {
				this.orderImportForm.soList = [this.orderImportForm.inputNo]
				let res = await blAllSoList({ so: this.orderImportForm.inputNo })
				this.soLists = res.data.map(item => {
					return { label: item, value: item }
				})
			}
			this.orderImportForm.orderNo && this.getPricingInfoData()
			this.form = Object.assign({}, data)
		},
		changeLabel1(val) {
			console.log('🚀 ~ val', val)
			if(val === 'DN_IMPORT_BILL') {
				// 附件类型选择了进口账单时，船名和航次非必填
				this.orderImportRegRules = { ...this.rules2 }
				this.$refs['orderImportForm'].clearValidate('vessel')
				this.$refs['orderImportForm'].clearValidate('voyage')
			} else {
				this.orderImportRegRules = {...this.rules1, ...this.rules2 }
			}
			this.orderImportForm.pricingEmployeeId = this.pricingInfoList[0] ? this.pricingInfoList[0].pricingEmployeeId : ''
		},
		getPricingInfoData() {
           getPricingInfo({orderNo: this.orderImportForm.orderNo})
		   .then(res => {
			   this.pricingInfoList = []
			   let obj = res.data
			   if(obj.orgAuditEmployeeId) {
				   this.pricingInfoList.push({
					   pricingEmployeeId: obj.orgAuditEmployeeId,
					   pricingEmployeeName: obj.orgAuditEmployeeName
			       })
			   }
			   if(obj.auditEmployeeId) {
				    this.pricingInfoList.push({
					   pricingEmployeeId: obj.auditEmployeeId,
					   pricingEmployeeName: obj.auditEmployeeName
			       })
			   }
			   if(this.pricingInfoList.length) {
			...
			this.$refs.orderImportForm.validate(valid => {
				if (valid) {
					...
					this.uploadParams = {
						...
						soInfo: this.orderImportForm.soList.join(','),
		...
		closeDialog(val) {
			if (val === 'success') {
				this.$parent.$refs.Info && this.$parent.$refs.Info.getBillList()
				this.$emit('getData')
			}
			this.$emit('close')
		},

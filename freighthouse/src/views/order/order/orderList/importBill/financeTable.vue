<template>
	<div class="finance-table" :class="{ max__height__table: $attrs.autoMax }">
		<div class="finance-el-table finance-el-table-bill" :style="option.style || {}">
			...
			>
				<el-table-column type="selection" width="40px" align="center" :fixed="option.selection.fixed" v-if="option.selection && option.selection.show" :selectable="(row, index) => (['draft', 'refuse'].includes(row.dnCheckStatus) ? true : false)"></el-table-column>
				<el-table-column
				...
				<el-table-column label="操作" :align="option.operationBtns.align || 'center'" :width="option.operationBtns.width" :fixed="option.operationBtns.fixed" v-if="option.operationBtns && option.operationBtns.show">
					<template slot-scope="scope">
						<template v-for="(item, index) in option.operationBtns.data">
							<span :key="'operationBtns' + index" :class="['operationbtns-wrapper', item.class ? item.class : '']" v-if="item.show || item.showFn(scope.row) || (!item.showFn && scope.row.edit)">
		...
		<div class="finance-el-pagination finance-el-pagination-bill" v-if="option.pagination && option.pagination.show">
			<Tips v-if="option.tips" :tips="option.tips" :callback="handleTableClearSelection" />
...
export default {
	...
	methods: {
		...
		handleRowClick(row, column) {
			...
			if (!this.option.selection.show || row._disabled || row.dnCheckStatus != 'draft') {
				return
			}

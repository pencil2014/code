	...
	<div class="customsClearance-container">
		<div class="mb10 between h32 border-bt" id="saveBar">
			...
			<div class="row-tit-operate">
				...
				<ButtonTip btnType="primary" :btnDisabled="isBd || isDisabledEdit" btnText="保存" :tipContent="isDocTip || isJointTip || isFinishTip || statusTip" class="operateBtn-tip ml10" @click="handleSave"> </ButtonTip>
		...
		<el-form ref="customsClearanceSave" :rules="rules" :model="detailData" class="form-body" style="padding: 0 10px" label-width="90px">
			<el-row>
				...
				<el-col v-if="$route.query.isPendding != 'pendding'" :span="6">
			...
			<div class="table-com-layout">
					...
					<el-table-column label="附件信息" align="center">
						<template slot-scope="scope">
							<div class="operate-group">
								<ButtonTip btnType="text" :btnDisabled="isBd || isDisabledEdit" btnText="" :tipContent="isDocTip || isJointTip || isFinishTip || statusTip" class="operateBtn-tip" @click="handleViewFileList(scope.row, scope.$index)">
									点击上传<em v-if="scope.row.fileNum" class="fileNum">({{ scope.row.fileNum }})</em>
								</ButtonTip>
								<!-- <el-button class="underline" @click="handleViewFileList(scope.row, scope.$index)" type="text" size="mini" :disabled="isBd || isDisabledEdit">
				...
				<!-- <el-table ref="customsClearanceBox2" fit style="width: 100%" :data="hblList2" class="comClear-table-border" v-if="createItem.businessType === 'ship_export_lcl'">
...
export default {
	...
	computed: {
		...
		// isBd() {
		// 	return this.ordDetRole === 'bd' || this.ordDetRole === 'obd' || (!this.isDoc && !this.isEpricing && !this.isEcs)
		// },
		viewState() {

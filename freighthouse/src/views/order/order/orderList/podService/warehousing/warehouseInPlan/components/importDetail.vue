		...
		<div class="row-body">
			<el-form ref="form" :rules="rules" :model="formData" class="form-body" label-width="90px">
				<el-row>
					...
					<el-col v-if="['ship_export_lcl'].includes(orderInfoDataObj.businessType)" :span="6">
				...
				<div class="row-tit h32">
					...
					<div class="row-tit-operate">
						<ButtonTip btnType="primary" :btnDisabled="(isBd && !isImportJoint) || isDisabledEdit" btnText="添加进仓货品" :tipContent="isDocTip || isJointTip || isFinishTip || statusTip" class="operateBtn-tip" @click="handleAddCargo"> </ButtonTip>
						<!-- <el-button size="mini" type="primary" @click="handleAddCargo" :disabled="(isBd && !isImportJoint) || isDisabledEdit">添加进仓货品</el-button> -->
					</div>
				</div>
						...
						<el-table-column label="操作" width="150" align="center" fixed="right">
							<template slot-scope="scope">
								...
								<div class="operate-group" v-else>
									<!-- <el-button @click="handleAddCargo(scope.row, item)" type="text" size="mini" :disabled="isBd || detailData.status === 'finish'" v-if="!scope.$index">添加货物箱号</el-button> -->
									<ButtonTip
										btnType="text"
	...
	watch: {
		detailData: {
			handler(newValue) {
				...
				this.tableData = this.formData.cargoInfoList && JSON.parse(JSON.stringify(this.formData.cargoInfoList))
			},
			deep: true,
			immediate: true
		}

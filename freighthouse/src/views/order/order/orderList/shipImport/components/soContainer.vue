			...
			<div v-show="!o.containerListShowState" style="padding: 8px 0 5px">
				<el-form class="row-flex flex-so" v-for="(item, sIndex) in o.containerList" :key="sIndex" :model="createItem.soContainerList[index]" ref="containerList">
					...
					<div class="row-flex-cont">
						<div class="flex">
							<el-form-item class="item-cell" label="箱型" :prop="`containerList.${sIndex}.containerType`" :show-message="false" :rules="{ required: isRequired, message: ' ', trigger: 'change' }">
								<el-select @change="handleTypeChange(item)" filterable size="mini" v-model="item.containerType" style="width: 100px" clearable placeholder="请选择" :disabled="viewState">
									<el-option v-for="item in dictMap.containerTypeShiporder" :key="item.value" :label="item.label" :value="item.value"> </el-option>
								</el-select>
							</el-form-item>
							<el-form-item class="item-cell" label="箱量" :prop="`containerList.${sIndex}.containerNum`" :show-message="false" :rules="[{ required: isRequired, validator: checkContainerNum, trigger: 'blur' }]">
								<el-input size="mini" v-model.number="item.containerNum" clearable placeholder="请输入" style="width: 100px" :disabled="viewState" @blur="validateContainerNum(item)"/>
							</el-form-item>
							<el-form-item class="item-cell" label="单柜件数" :prop="`containerList.${sIndex}.quantity`" :show-message="false">
								<el-input clearable size="mini" v-model="item.quantity" placeholder="请输入" style="width: 100px" :disabled="viewState" @blur="validateQuantityNum(item)"></el-input>
							</el-form-item>
							<el-form-item class="item-cell" label="单柜重量" :prop="`containerList.${sIndex}.weight`" :show-message="false" :rules="[{ required: isRequired, validator: checkWeight, trigger: 'blur' }]">
								<el-input clearable size="mini" v-model="item.weight" placeholder="请输入" style="width: 100px" :disabled="viewState" @blur="validateWeight(item)"><template slot="append">KGS</template></el-input>
							</el-form-item>
							<el-form-item class="item-cell" label="单柜体积" :prop="`containerList.${sIndex}.volume`" :show-message="false" :rules="[{ required: false, trigger: 'blur' }]">
								<el-input clearable size="mini" v-model="item.volume" placeholder="请输入" style="width: 100px" :disabled="viewState" @blur="validateVolume(item)"><template slot="append">CBM</template></el-input>
							</el-form-item>
							<el-form-item class="item-cell" label="包装" :prop="`containerList.${sIndex}.packageInfo`" :show-message="false">
								<el-select size="mini" v-model="item.packageInfo" placeholder="请选择包装" clearable filterable style="width: 100px" :disabled="viewState">
									<el-option v-for="(item, index) in dictMap.packageType" :key="index" :label="item.label" :value="item.value"></el-option>
								</el-select>
							</el-form-item>							
							<el-form-item class="item-cell" label="是否重柜" :prop="`containerList.${sIndex}.isHeavy`" :show-message="false" :rules="{ required: isRequired, message: '请选择是否重柜', trigger: 'change' }">
								<el-select size="mini" v-model="item.isHeavy" style="width: 100px" clearable placeholder="请选择" :disabled="viewState">
									<el-option v-for="ele in yesOrNoOptions" :key="ele.value" :label="ele.label" :value="ele.value"> </el-option>
								</el-select>
							</el-form-item>
							<div class="item-cell">
								<span class="lab">用箱要求</span>
								<el-select size="mini" v-model="item.boxRemark" style="width: 100px;" clearable placeholder="请选择" :disabled="viewState">
						...
						<div class="flex" style="clear: both">
							<el-form-item class="item-cell redStar" label="特种长宽高" v-if="item.containerType && (item.containerType.toUpperCase().indexOf('OT') !== -1 || item.containerType.toUpperCase().indexOf('FR') !== -1)">
								<el-col :span="8">
									<el-form-item :prop="`containerList.${sIndex}.specialLength`" :show-message="false" :rules="[{ required: isRequired, validator: checkWeight, trigger: 'blur' }]">
										<el-input clearable size="mini" v-model="item.specialLength" placeholder="请输入" style="width: 100px; margin-right: 5px" :disabled="viewState" @blur="validateSpecial(item, 'specialLength')">
								...
								<el-col :span="8">
									<el-form-item :prop="`containerList.${sIndex}.specialWidth`" :show-message="false" :rules="[{ required: isRequired, validator: checkWeight, trigger: 'blur' }]">
										<el-input clearable size="mini" v-model="item.specialWidth" placeholder="请输入" style="width: 100px; margin-right: 5px" :disabled="viewState" @blur="validateSpecial(item, 'specialWidth')">
								...
								<el-col :span="8">
									<el-form-item :prop="`containerList.${sIndex}.specialHeight`" :show-message="false" :rules="[{ required: isRequired, validator: checkWeight, trigger: 'blur' }]">
										<el-input clearable size="mini" v-model="item.specialHeight" placeholder="请输入" style="width: 100px;" :disabled="viewState" @blur="validateSpecial(item, 'specialHeight')">
							...
							<div class="item-cell" v-if="item.containerType && (item.containerType.toUpperCase().indexOf('OT') !== -1 || item.containerType.toUpperCase().indexOf('FR') !== -1)">
								...
								<el-select size="mini" v-model="item.gangeType" style="width: 80px;" clearable placeholder="请选择" :disabled="viewState">
							...
							<div class="item-cell" v-if="item.containerType && (item.containerType.toUpperCase().indexOf('OT') !== -1 || item.containerType.toUpperCase().indexOf('FR') !== -1)">
								...
								<el-input clearable size="mini" v-model="item.sizeRight" placeholder="请输入" style="width: 100px;" :disabled="viewState" @blur="validateSpecial(item, 'sizeRight')">
							...
							<el-form-item
								...
								:show-message="false"
								:rules="{ required: isRequired, message: '请选择单位', trigger: 'change' }"
								v-if="item.containerType && item.containerType.toUpperCase().indexOf('RF') !== -1"
							>
								<el-select size="mini" v-model="item.temperatureType" style="width: 100px;" clearable placeholder="请选择" :disabled="viewState">
							...
							<el-form-item
								...
								:show-message="false"
								:rules="[{ required: isRequired, validator: checkTemprature, trigger: 'blur' }]"
								v-if="item.containerType && item.containerType.toUpperCase().indexOf('RF') !== -1"
							>
								<el-input size="mini" v-model="item.temperatureSet" clearable placeholder="请输入" style="width: 100px;" :disabled="viewState" @blur="validateTemprature(item, 'temperatureSet')"></el-input>
							</el-form-item>
							<el-form-item
								class="item-cell"
								label="最低温度"
								...
								v-if="item.containerType && item.containerType.toUpperCase().indexOf('RF') !== -1"
							>
								<el-input size="mini" v-model="item.temperatureMin" clearable placeholder="请输入" style="width: 100px;" :disabled="viewState" @blur="validateTemprature(item, 'temperatureMin')"></el-input>
							</el-form-item>
							<el-form-item
								class="item-cell"
								label="最高温度"
								...
								:show-message="false"
								:rules="[{ required: isRequired, validator: checkTemprature, trigger: 'blur', item: item }]"
								v-if="item.containerType.toUpperCase().indexOf('RF') !== -1"
							>
								<el-input size="mini" v-model="item.temperatureMax" clearable placeholder="请输入" style="width: 100px;" :disabled="viewState" @blur="validateTemprature(item, 'temperatureMax')"></el-input>
							</el-form-item>
							<el-form-item
								class="item-cell"
								label="通风量"
								...
								v-if="item.containerType && item.containerType.toUpperCase().indexOf('RF') !== -1"
							>
								<el-input clearable size="mini" v-model="item.airMete" placeholder="请输入" style="width: 100px;" :disabled="viewState" @blur="validateTemprature(item, 'airMete')">
							...
							<el-form-item
								...
								:show-message="false"
								:rules="{ required: isRequired, message: '请输入湿度要求', trigger: 'blur' }"
								v-if="item.containerType.toUpperCase().indexOf('RF') !== -1"
							>
								<el-input size="mini" v-model="item.humidity" clearable placeholder="请输入" style="width: 100px;" :disabled="viewState"></el-input>
							</el-form-item>
						</div>
					</div>
				</el-form>
...
export default {
	...
	computed: {
		...
		viewState() {
			if(this.$route.query.jointNo || this.$route.query.source === 'jointList') {
				return true
			}
		...
		operBtnShow() {
			if ( ( this.isBd && ['draft', 'refuse'].includes(this.orderStatus) ) || ( this.isOp && ['pass'].includes(this.orderStatus) ) ){
				return true
			}
			return false
		},
		isRequired() {
			return this.ordDetRole === 'bd' || this.ordDetRole === 'obd' || this.ordDetRole === 'op'
		}
	},
	watch: {},
	components: {},

				...
				<div class="cont" :class="this.isOpView ? 'op-cont' : this.isBd ? 'bd-cont' : ''">
					...
					<div class="flex-form-item label90" :class="isTransitPortEdit ? 'pr24' : ''">
							...
							<div class="port-flex-form">
								<div class="port-code">
									<el-select
										...
										filterable
										clearable
										:disabled="(disabledState && isView) || isPendding"
										style="width: 100%"
										placeholder="请输入"
							...
							<div class="port-flex-form">
								<div class="port-code">
									<el-select
										...
										filterable
										clearable
										:disabled="disabledState || isPendding"
										style="width: 100%"
										placeholder="请输入"
						...
						<el-form-item label="中转方式" prop="ship.transitWay" :show-message="false">
							<el-select v-model="createItem.ship.transitWay" size="mini" clearable placeholder="请选择" style="width: 100%" :disabled="disabledState || isPendding">
              ...
              ></el-autocomplete>
						...
						<el-form-item class="timeSpecial" label="驳船实际开船时间" prop="barge.tdTime" :show-message="false" v-if="(isOpView || isDoc || isCsc) && isIncludeFeeder">
							<elDatePickerLimit
								...
								placeholder="请选择"
								value-format="yyyy-MM-dd"
								:disabled="!isDocEdit && disabledState && isView && !isCscEdit"
							/>
						</el-form-item>
						...
						<el-form-item class="timeSpecial vessel-time" label="大船实际开船时间" prop="ship.tdTime" :show-message="false" v-if="isOpView || isDoc || isCsc">
							<elDatePickerLimit
								...
								placeholder="请选择"
								value-format="yyyy-MM-dd"
								:disabled="(!isDocEdit && disabledState && isView && !isCscEdit) || isPendding"
							/>
						...
						<el-form-item class="timeSpecial" label="预计到港时间" prop="ship.etaTime" :required="createItem.serviceType === 'st09' && createItem.bkgStatus === 'book_release'" :show-message="false" v-if="isOpView || isDoc">
							<elDatePickerLimit
								size="mini"
								:field="'etaTime'"
								v-model="createItem.ship.etaTime"
								type="date"
								style="width: 100%"
								clearable
								placeholder="请选择"
								value-format="yyyy-MM-dd"
								:disabled="(!isDocEdit && disabledState && isView) || finIsCloseStatus"
							/>
						</el-form-item>
						<el-form-item class="timeSpecial" label="实际到港时间" prop="ship.taTime" v-if="isOpView || isDoc">
							<elDatePickerLimit
								:field="'taTime'"
								size="mini"
								v-model="createItem.ship.taTime"
								type="date"
								style="width: 100%"
								clearable
								placeholder="请选择"
				...
				<div class="cont" :class="this.isOpView ? 'op-cont' : this.isBd ? 'bd-cont' : ''">
					...
					<div class="flex-form-item label125">
						<el-form-item label="贸易条款" prop="transport.tradeTermsCode" :show-message="false">
							<el-select v-model="createItem.transport.tradeTermsCode" size="mini" clearable placeholder="请选择" style="width: 100%" :disabled="disabledState || isPendding">
						...
						<el-form-item :required="true" label="运输条款" prop="transport.transportTerm" :show-message="false">
							<el-select v-model="createItem.transport.transportTerm" size="mini" clearable placeholder="请选择" style="width: 100%" :disabled="disabledState || isPendding">
						...
						<el-form-item :required="true" label="付款方式" prop="payMode" :show-message="false">
							<el-select v-model="createItem.payMode" size="mini" clearable placeholder="请选择" style="width: 100%" :disabled="disabledState || isPendding">
						...
						<el-form-item label="出单方式" prop="ship.blMode" :show-message="false">
							<el-select v-model="createItem.ship.blMode" size="mini" clearable placeholder="请选择" style="width: 100%" :disabled="(disabledState && isView) || isPendding">
                ...
                <el-radio :label="item.value" :disabled="disabledState && isView" v-for="(item, index) in yesOrNoOptions" :key="index">{{ item.label }}</el-radio>
						...
						>
							<el-input
								...
								placeholder="请输入"
								clearable
								v-model="createItem.transport.takeContactName"
								:disabled="disabledState && isView && !opEditIsBreak"
								maxlength="32"
								@blur="handleClearEmptyString('takeContactName', $event)"
							></el-input>
						</el-form-item>
						<el-form-item
							:required="(isBd || (ordDetRole === 'op' && ['pass', 'break'].includes(createItem.orderStatus))) && (createItem.serviceList.includes('truck') || createItem.serviceList.includes('pol_ltl'))"
						...
						>
							<el-input
								...
								placeholder="请输入"
								clearable
								v-model="createItem.transport.takeContactPhone"
								:disabled="disabledState && isView && !opEditIsBreak"
								maxlength="32"
								@blur="handleClearEmptyString('takeContactPhone', $event)"
							></el-input>
						</el-form-item>
						<el-form-item
							:required="(isBd || (ordDetRole === 'op' && ['pass', 'break'].includes(createItem.orderStatus))) && (createItem.serviceList.includes('truck') || createItem.serviceList.includes('pol_ltl'))"
						...
						<el-form-item :required="isBd || (ordDetRole === 'op' && ['pass', 'break'].includes(createItem.orderStatus))" label="分单数" prop="transport.splitNum" :show-message="false">
							<el-input
								@blur="validateSplitNum('splitNum', createItem.transport)"
								size="mini"
								placeholder="请输入"
								clearable
								v-model.number="createItem.transport.splitNum"
								:disabled="(disabledState && isView && !opEditIsBreak)"
							></el-input>
						</el-form-item>
						<el-form-item
							:required="(isBd || (ordDetRole === 'op' && ['pass', 'break'].includes(createItem.orderStatus))) && createItem.serviceList.includes('pol_declaration')"
							label="发货人是否有出口权"
							prop="transport.isExportRight"
							:show-message="false"
						>
						...
						<el-form-item
							:required="(isBd || (ordDetRole === 'op' && ['pass', 'break'].includes(createItem.orderStatus))) && createItem.serviceList.includes('pod_comm_clearance')"
							label="收货人是否有进口权"
							prop="transport.isImportRight"
							:show-message="false"
						>
						...
						<el-form-item label="AMS B/L NO." prop="transport.amsBlNo" :show-message="false">
							<el-input size="mini" placeholder="请输入" clearable v-model="createItem.transport.amsBlNo" :disabled="(disabledState && isView && !isEcsEdit) || isJointList" maxlength="128"></el-input>
						</el-form-item>
						<el-form-item label="ISF NO." prop="transport.isfNo" :show-message="false">
							<el-input size="mini" placeholder="请输入" clearable v-model="createItem.transport.isfNo" :disabled="(disabledState && isView && !isEcsEdit) || isJointList" maxlength="128"></el-input>
						</el-form-item>
						<el-form-item label="CY LOCATION" prop="ship.pierCyCode" :show-message="false">
							<div class="port-flex-form">
								<div class="port-code">
									<el-select
										size="mini"
										v-model="createItem.ship.pierCyCode"
										filterable
										clearable
										:disabled="(disabledState && isView && !isEcsEdit) || isJointList"
										style="width: 100%"
										placeholder="请输入"
										:filter-method="pierCyLocationFilterMehod"
										@visible-change="pierCyLocationVisibleChange"
										@change="handleChangePierCyLocation"
									>
										<div class="cyLocation-select-tit">
											<div class="code">Code</div>
											<div class="name">Location Name</div>
											<div class="iata">IATA(Fims)</div>
										</div>
										<el-option v-for="item in pierCyLocationList" :key="item.value" :label="item.value" :value="item.value">
											<div class="cyLocation-select-list">
												<div class="code">{{ item.cyCode }}</div>
												<div class="name">{{ item.nameEn }}</div>
												<div class="iata">{{ item.iataFirms }}</div>
											</div>
										</el-option>
									</el-select>
								</div>
								<PortPopover :portItem="pierCyLocationItem" />
							</div>
						</el-form-item>
						<el-form-item label="I.T. NO. & PLACE" prop="transport.itNoAndPlace" :show-message="false">
							<el-input size="mini" placeholder="请输入" clearable v-model="createItem.transport.itNoAndPlace" :disabled="(disabledState && isView && !isEcsEdit) || isJointList" maxlength="128"></el-input>
						</el-form-item>
						<el-form-item class="timeSpecial" label="I.T. DATE" prop="transport.itDate" :show-message="false">
							<elDatePickerLimit
								size="mini"
								:field="'itDate'"
								v-model="createItem.transport.itDate"
								type="date"
								style="width: 100%"
								clearable
								placeholder="请选择"
						...
						<el-form-item label="审单备注" prop="auditRemark" :show-message="false" v-if="isOpView">
							<el-input
								@dblclick.native="showRemarkDetail(createItem.auditRemark)"
								type="textarea"
								clearable
								rows="2"
								size="mini"
								...
								maxlength="128"
								show-word-limit
								v-model="createItem.auditRemark"
								:disabled="disabledState"
							></el-input>
						</el-form-item>
						<el-form-item label="订舱备注" prop="bkgRemark" :show-message="false" v-if="isOpView">
							<el-input @dblclick.native="showRemarkDetail(createItem.bkgRemark)" type="textarea" clearable rows="2" size="mini" placeholder="请输入" v-model="createItem.bkgRemark" :disabled="disabledState"></el-input>
						</el-form-item>
						<el-form-item label="备注" prop="bdRemark" :show-message="false" v-if="$route.query.source !== 'jointList'">
							<el-input
								@dblclick.native="showRemarkDetail(createItem.bdRemark)"
...
import { bizdateChange, orderCyList } from '@/api/order/list'
...
export default {
	...
	created() {
		...
		this.fillPierCyLocation(this.pierCyCode)
	},
	mounted() {
	...
	computed: {
		...
		isEcsEdit() {
			// 海外客服角色，审核通过可以编辑
			return this.isEcs && ['pass'].includes(this.orderStatus)
		},
		isView() {
	...
	watch: {
		...
		pierCyCode: {
			handler(newVal) {
				this.fillPierCyLocation(newVal)
			},
			deep: true
 		},
		sysLineCname: {
	...
	methods: {
		...
		fillPierCyLocation(code) {
			if (code) {
				...
				orderCyList(data).then(res => {
					let {list} = res.data
					list = list || []
					this.pierCyLocationList = list.map(item => {
						return {...item, label: item.nameEn, value: item.cyCode, ename: item.nameEn, countryEname: item.iataFirms}
					})
					this.pierCyLocationItem = this.pierCyLocationList.find(item => item.cyCode === code) || {}
				})
			}
		},
		// 驳船收货地
		...
		pierCyLocationSearch(queryString,) {
			let data = {
		...
		pierCyLocationVisibleChange(val) {},
		pierCyLocationFilterMehod(val) {
			this.pierCyLocationSearch(val)
		},
		handleChangePierCyLocation(val) {
			let findItem = this.pierCyLocationList.find(item => item.cyCode === val) || {}
			this.createItem.ship.pierCyLocation = findItem.nameEn || ''
			this.createItem.ship.pierFirmsCode = findItem.iataFirms || ''
			this.pierCyLocationItem = findItem
		},
		initSystemLine(val) {
			if (val) {

						...
						<template v-else>
							<template v-if="$route.query.source !== 'jointList'">
								...
								<template v-if="(isDoc || isOp) && hasDestinationServices">
									<el-button size="mini" type="primary" class="" @click="saMailShow = true" v-if="['pass'].includes(createItem.orderStatus) && $route.query.isPendding != 'pendding'" :disabled="submitDisabled">发送SA</el-button>
								</template>
								<!-- 操作 -->
								<template v-if="isOp">
									...
									<el-button
										...
										:disabled="submitDisabled"
										v-if="
											(createItem.serviceType === 'st09' && ['pass'].includes(createItem.orderStatus) && createItem.bkgStatus === 'book_release' && isOrderFeeCount && !['CANCEL', 'BREAK'].includes(createItem.cancelJointHandleStatus)) ||
...
export default {
	...
	computed: {
		...
		opCancelBtnShow() {
			return (
				(this.createItem.serviceType === 'st09' && ['pass'].includes(this.createItem.orderStatus) && this.createItem.bkgStatus === 'book_release' && !this.isOrderFeeCount && !['CANCEL', 'BREAK'].includes(this.createItem.cancelJointHandleStatus)) ||
		...
		opBreakBtnShow() {
			return (
				(['pass'].includes(this.createItem.orderStatus) && this.createItem.bkgStatus === 'book_release' && this.isOrderFeeCount && !['CANCEL', 'BREAK'].includes(this.createItem.cancelJointHandleStatus)) ||
	...
	methods: {
		...
		fillOrderInfo(data, type) {
			...
			if (data.ship) {
				let ship = { ...data.ship }
				let { polPortCode, podPortCode, transitPortCode, transitPortCode1, transitPortCode2, unloadingPortCode, pierCyCode } = ship
				...
				// 获取CY LOCATION
				// this.$refs.orderBookInfo.setCyLocationValues()
				console.log('ref.orderBookInfo', this.$refs.orderBookInfo)
			}
		...
		handleJointEditSave() {
			this.$refs['orderSubmit'].validate(valid => {
				...
				Promise.all([orderJointSave(jointData), orderImportEdit(editData)])
					.then(res => {
		...
		getValues() {
			...
			return {
				...
				ship: {
					...
					pierCyCode: this.createItem.ship.pierCyCode,
					pierCyLocation: this.createItem.ship.pierCyLocation,
					pierFirmsCode: this.createItem.ship.pierFirmsCode,
					pierTel: this.createItem.ship.pierTel
				},
				...
				transport: {
					...
					amsBlNo: this.createItem.transport.amsBlNo,
					isfNo: this.createItem.transport.isfNo,
					itNoAndPlace: this.createItem.transport.itNoAndPlace,
					itDate: this.createItem.transport.itDate
				},
				workAssign: workArr,

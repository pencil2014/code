<template>
  <el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
    ...
    <el-table-column label="报关时间" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.sortValue}}</span>
      </template>
    </el-table-column>
    <el-table-column label="委托状态" align="left"> 
      <template slot-scope="scope">
        <span>{{getStatus(scope.row)}}</span>
      </template>
    </el-table-column>
    <el-table-column label="SO号" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.so}}</span>
      </template>
    </el-table-column>
    <el-table-column label="报关方式" align="left">
      <template slot-scope="scope">
        <span v-if="scope.row.infoData">{{scope.row.infoData.customsType | getDictLabelFilter('customsType')}}</span>
        <span v-else></span>
      </template>
    </el-table-column>
    <el-table-column label="正报几票" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.customsNumber}}</span>
      </template>
    </el-table-column>
     <el-table-column label="截关时间" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.cyCutOff}}</span>
      </template>
    </el-table-column>
  </el-table>
</template>
...
export default {
  ...
  methods: {    
    ...
    */
    ...
    getStatus(row) {
      let statusCn = ''
      if (row.matterCode === 'pol_declaration') { // 报关
        if (row.infoData && row.infoData.status) {
          statusCn = getDictLabel('customsIntrustStatus', row.infoData.status)
        } else {
          statusCn = '未委托'
        }
      } else if (row.matterCode === 'pol_inspection') { // 报检
        if (row.infoData && row.infoData.status) {
          statusCn = getDictLabel('inspectionStatus', row.infoData.status)
        } else {
          statusCn = '未委托'
        }
      }

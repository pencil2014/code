<template>
  <el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
    ...
    <el-table-column prop="cyCutOff" label="截关时间" align="left"></el-table-column>
    <el-table-column prop="matterName" label="待办事项" align="left"></el-table-column>
    <!-- <el-table-column label="放舱要求" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.issueMode}}</span>
      </template>
    </el-table-column> -->
    <el-table-column label="放舱给客户" align="left"> 
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.status === 'y' ? '是' : '否'}}</span>
      </template>
    </el-table-column>
  </el-table>
</template>

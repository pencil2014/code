<template>
  <el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
    ...
    <el-table-column label="箱号" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.cn}}</span>
      </template>
    </el-table-column>
    <el-table-column label="截VGM时间" align="left">
      <template slot-scope="scope">
        <span>{{vgmCutOffFormat(scope.row)}}</span>
      </template>
    </el-table-column>
    <el-table-column label="申报状态" align="left"> 
      <template slot-scope="scope">
...
export default {
  ...
  methods: {    
    ...
    */
    ...
    getStatus(infoData) {
      let statusCn = ''
      if (!infoData || (infoData && !infoData.status)) return '未申报'
      statusCn = getDictLabel('vgmStatus', infoData.status)
      return statusCn
    },
    vgmCutOffFormat(row) {
      let result = ''

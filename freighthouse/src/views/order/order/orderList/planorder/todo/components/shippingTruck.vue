<template>
	<el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
		...
		<el-table-column label="委托编号" align="left">
			<template slot-scope="scope">
				<span>{{ scope.row.infoData && scope.row.infoData.intrustNo }}</span>
			</template>
		</el-table-column>
		<el-table-column prop="isDangerous" label="是否危险品" align="left" >
			<template slot-scope="scope">
				{{ scope.row.isDangerous === 'yes' ? '是' : '否' }}
			</template>
		</el-table-column>
		<el-table-column label="委托装柜时间" align="left">
			<template slot-scope="scope">
				<span>{{ scope.row.infoData && scope.row.infoData.EDeliveryTime }}</span>
			</template>
		</el-table-column>
		<el-table-column prop="matterStatus" label="委派状态" align="left">
			<template slot-scope="scope">
				<span>{{ getStatus(scope.row.infoData) }}</span>
			</template>
		</el-table-column>
		<el-table-column prop="matterStatus" label="SO号" align="left">
			<template slot-scope="scope">
				<span>{{ scope.row.infoData && scope.row.infoData.so }}</span>
			</template>
		</el-table-column>
		<el-table-column prop="matterStatus" label="箱型箱量" align="left">
			<template slot-scope="scope">
				<span>{{ scope.row.infoData && scope.row.infoData.containerInfo }}</span>
			</template>
		</el-table-column>
		<el-table-column prop="matterStatus" label="起运港" align="left">
			<template slot-scope="scope">
				<span>{{ scope.row.infoData && scope.row.infoData.polPortName }}</span>
			</template>
		</el-table-column>
		<el-table-column prop="matterStatus" label="装货详细地址" align="left">
			<template slot-scope="scope">
				<span>{{ scope.row.infoData && scope.row.infoData.truckAddress }}</span>
			</template>
		</el-table-column>
	</el-table>
</template>
...
export default {
    ...
    */
		...
		getStatus(infoData) {
			let statusCn = ''
			if (!infoData || (infoData && !infoData.status)) return '未委托'
			statusCn = getDictLabel('truckIntrustStatus', infoData.status)
			return statusCn
		}
	}
}

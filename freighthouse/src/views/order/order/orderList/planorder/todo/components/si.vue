<template>
  <el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
    ...
    <el-table-column prop="blNo" label="提单号" align="left">
      <template slot-scope="scope">
    ...
    <el-table-column label="截补料时间" align="left">
      <template slot-scope="scope">
        <span>{{siCutOffFormat(scope.row)}}</span>
      </template>
    </el-table-column>
    <el-table-column label="是否补料" align="left">
      <template slot-scope="scope">
  ...
  methods: {    
    ...
    */
    ...
    siCutOffFormat(row) {
      let result = ''
      if (row.siCutOff) {
        result = row.siCutOff.slice(0, 13)
      }
      return result
    }
  }

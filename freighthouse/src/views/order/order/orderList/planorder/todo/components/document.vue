<template>
  <el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
    ...
    <el-table-column label="贸易单证类别" align="left">
      <template slot-scope="scope">
        <span>{{getDocName(scope.row.infoData)}}</span>
      </template>
    </el-table-column>
    ...
    <el-table-column label="取单状态" align="left"> 
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.status === 'y' ? '已取单' : '未取单'}}</span>
      </template>
    </el-table-column>
  </el-table>
</template>
...
export default {
  ...
  methods: {
    ...
    */
    ...
    getDocName(infoData) {
      let result = ''
      if (infoData && infoData.docName) {
        result = getDictLabel('tradeDocName', infoData.docName)
      }
      return result
    }
  }

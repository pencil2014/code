<template>
  <el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
    ...
    <el-table-column label="熏蒸委托编号" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.intrustNo}}</span>
      </template>
    </el-table-column>
    <el-table-column label="熏蒸日期" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.fumeTime}}</span>
      </template>
    </el-table-column>
    <el-table-column label="熏蒸状态" align="left"> 
      <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    */
    ...
    getStatus(infoData) {
      let statusCn = ''
      if (!infoData || (infoData && !infoData.status)) return '未开始'
      // if (infoData && !infoData.status) statusCn = '未开始'
      statusCn = getDictLabel('fumeIntrustStatus', infoData.status)
      return statusCn
    }
  }
}

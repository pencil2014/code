<template>
  <el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
    ...
    <el-table-column label="清关委托编号" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.intrustNo}}</span>
      </template>
    </el-table-column>
    <el-table-column label="预计到港时间" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.etaTime}}</span>
      </template>
    </el-table-column>
    <el-table-column label="清关状态" align="left"> 
      <template slot-scope="scope">
  ...
  methods: {
    ...
    */
    ...
    getStatus(infoData) {
      ...
      statusCn = getDictLabel('podCommClearanceStatus', infoData.status)
      return statusCn
    }
  }
}

<template>
  <el-table fit class="plan-table" style="width: 100%;" :data="tableData" :span-method="objectSpanMethod">
    ...
    <el-table-column label="进仓事项" align="left">
      <template slot-scope="scope">
        <span>{{getMatter(scope.row.infoData)}}</span>
      </template>
    </el-table-column>
    <el-table-column prop="etaTime" label="预计到港时间" align="left">
    </el-table-column>
    <el-table-column label="进仓备注" align="left">
      <template slot-scope="scope">
        <span>{{scope.row.infoData && scope.row.infoData.whMatterRemark}}</span>
      </template>
    </el-table-column>
    <el-table-column label="完成状态" align="left"> 
      <template slot-scope="scope">
  ...
  methods: {    
    ...
    */
    ...
    getStatus(infoData) {
      ...
      statusCn = getDictLabel('inspectionStatus', infoData.status)
      return statusCn
    },
    getMatter(infoData) {
      let matterCn = ''
      if (infoData && infoData.whMatterCode) matterCn = getDictLabel('whMatterType', infoData.whMatterCode)
      return matterCn
    }
  }
}

...
export default {
  ...
  methods: {
    ...
    */
    ...
    getStatus(row) {
      let statusCn = ''
      if (row.matterCode === 'ciq') { // ciq
        if (row.infoData && row.infoData.status) {
          statusCn = getDictLabel('inspectionStatus', row.infoData.status)
        } else {
          statusCn = '未委托'
        }
      } else if (row.matterCode === 'coc_saso') { // coc_saso
        if (row.infoData && row.infoData.status) {
          statusCn = getDictLabel('cocSasoInspectionStatus', row.infoData.status)
        } else {
          statusCn = '未委托'
        }
      } else if (row.matterCode === 'bv') { // bv
        if (row.infoData && row.infoData.status) {
          statusCn = getDictLabel('inspectionStatus', row.infoData.status)
        } else {
          statusCn = '未委托'

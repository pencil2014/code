...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.podWarehouseList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.podWarehouseList.defaultColumns,
			specialColumns: state => state.followItems.podWarehouseList.specialColumns,
			columnsBase: state => state.followItems.podWarehouseList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.podWarehouseList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.podWarehouseList.columnsBase(this.handleClickRefreshList)
		},
		handleOpPlan(row) {

...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.planTodoList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.planTodoList.defaultColumns,
			specialColumns: state => state.followItems.planTodoList.specialColumns,
			columnsBase: state => state.followItems.planTodoList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.planTodoList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn)
			this.tableConfig.allColumns = store.state.followItems.planTodoList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn)
		},
		handleOpPlan(row) {
      ...
      return (list.find(item => {
		...
		getAscOrDescData() {
          ...
          if (this.sortClickFlag) {
						...
						let ascColumns = [
							{ key: 'submit_plan_todo', value: ['etd'] },
							{ key: 'audit_plan_todo', value: ['etd'] },
						]
    ...
    // 表格操作回调
		...
		handleClickRefreshListAndColumn() {
			this.$emit('callBack', 'column')
		}
  }
}
</script>

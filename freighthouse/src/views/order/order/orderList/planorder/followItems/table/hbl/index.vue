...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.hblList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.hblList.defaultColumns,
			specialColumns: state => state.followItems.hblList.specialColumns,
			columnsBase: state => state.followItems.hblList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.hblList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.hblList.columnsBase(this.handleClickRefreshList)
		},
		handleOpPlan(row) {
      ...
      return (list.find(item => {
		...
		getAscOrDescData() {
          ...
          if (this.sortClickFlag) {
						...
						let ascColumns = [
							{ key: 'bl_confirm_hbl', value: ['siCutOff'] },
							{ key: 'take_hbl', value: ['td'] },
							{ key: 'put_hbl', value: ['etd'] },
							{ key: 'release_agree_hbl', value: ['etd'] },
						]

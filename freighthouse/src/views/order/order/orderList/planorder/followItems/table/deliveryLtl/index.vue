  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.deliveryLtlList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.deliveryLtlList.defaultColumns,
			specialColumns: state => state.followItems.deliveryLtlList.specialColumns,
			columnsBase: state => state.followItems.deliveryLtlList.columnsBase(),
  ...
  methods: {
      ...
      let routeUrl = this.$router.resolve({
       ...
       window.open(routeUrl.href, '_blank')
		...
		init() {
			this.tableConfig.columns = store.state.followItems.deliveryLtlList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.deliveryLtlList.columnsBase(this.handleClickRefreshList)
		},
      ...
      return (list.find(item => {
		...
		getAscOrDescData() {
          ...
          if (this.sortClickFlag) {
						...
						let ascColumns = [
							{ key: 'pod_ltl_delivery', value: ['etd'] },
						]

...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.saList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.saList.defaultColumns,
			specialColumns: state => state.followItems.saList.specialColumns,
			columnsBase: state => state.followItems.saList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.saList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn)
			this.tableConfig.allColumns = store.state.followItems.saList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn)
		},
		handleOpPlan(row) {

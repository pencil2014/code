...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.polLtlList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.polLtlList.defaultColumns,
			specialColumns: state => state.followItems.polLtlList.specialColumns,
			columnsBase: state => state.followItems.polLtlList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.polLtlList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.polLtlList.columnsBase(this.handleClickRefreshList)
		},
		handleOpPlan(row) {
      ...
      return (list.find(item => {
		...
		getAscOrDescData() {
          ...
          if (this.sortClickFlag) {
						...
						let ascColumns = [
							{ key: 'pol_ltl', value: ['etd'] },
							{ key: 'pod_ltl_delivery_entrust', value: ['etd']}
						]

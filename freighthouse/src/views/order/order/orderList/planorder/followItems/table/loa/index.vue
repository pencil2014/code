  ...
  <div>
    ...
    <!-- 自定义表头 -->
		...
		<div v-if="uploadPopShow">
			<FileUpload @close="uploadPopClose" :orderNo="orderNo" />
		</div>
...
import { fileAudit } from '@/api/order/list'
...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.loaList.columnsBase(this.handlePreview, this.handleClickRefreshList),
				operationBtns: {
					minWidth: '200',
					fixed: 'right',
					show: true,
					callback: (action, $index, row, item) => {
						...
						if (action === 'Pass') {
							this.handleAudit(row, 'PASS')
						}
						if (action === 'Refused') {
							this.handleAudit(row, 'REFUSE')
						}
						if (action === 'OpPlan') {
					...
					data: [
						...
						},
						{
							...
							type: 'text',
							show: (item, row) => {
								return row.name && row.auditStatus === 'SUBMIT' && row.auditEmployeeId === this.userId
							},
							action: 'Pass'
						},
						{
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.loaList.defaultColumns,
			specialColumns: state => state.followItems.loaList.specialColumns,
			columnsBase: state => state.followItems.loaList.columnsBase(),
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.loaList.columnsBase(this.handlePreview, this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.loaList.columnsBase(this.handlePreview, this.handleClickRefreshList)
		},
		handleOpPlan(row) {
    ...
    // 表格操作回调
      ...
      if (type === 'REFUSE') {
        this.auditPopShow = true
				...
				this.$confirm(`确定要通过该条${row.typeName}吗?`, '提示', {
          ...
          .then(() => {
            let data = {
							orderNo: this.auditData.orderNo,
							fileNo: this.auditData.fileNo,

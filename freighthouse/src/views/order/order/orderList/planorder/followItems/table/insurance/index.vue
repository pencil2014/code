...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.insuranceList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.insuranceList.defaultColumns,
			specialColumns: state => state.followItems.insuranceList.specialColumns,
			columnsBase: state => state.followItems.insuranceList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.insuranceList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.insuranceList.columnsBase(this.handleClickRefreshList)
		},
		handleOpPlan(row) {
      ...
      return (list.find(item => {
		...
		getAscOrDescData() {
          ...
          if (this.sortClickFlag) {
						...
						let ascColumns = [
							{ key: 'insurance', value: ['etd'] },
							{ key: 'put_insurance', value: ['eta'] },
						]

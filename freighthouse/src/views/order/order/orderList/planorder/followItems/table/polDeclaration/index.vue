...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.polDeclarationList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.polDeclarationList.defaultColumns,
			specialColumns: state => state.followItems.polDeclarationList.specialColumns,
			columnsBase: state => state.followItems.polDeclarationList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.polDeclarationList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.polDeclarationList.columnsBase(this.handleClickRefreshList)
		},
		handleOpPlan(row) {

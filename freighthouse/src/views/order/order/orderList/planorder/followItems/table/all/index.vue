...
import { getOrderFollowListStatus  } from '@/api/order/followItems'
const defaultPage = {
	pageSize: -1,
	currPage: 1
}
...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.allList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.allList.defaultColumns,
			specialColumns: state => state.followItems.allList.specialColumns,
			statusColumns: state => state.followItems.allList.statusColumns,
			columnsBase: state => state.followItems.allList.columnsBase(),
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.allList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.allList.columnsBase(this.handleClickRefreshList)
		},
		handleOpPlan(row) {
      let routeUrl = this.$router.resolve({
       ...
       query.query.map(item => {
        ...
        let { data } = await columnWidthConfigInfo({
          ...
          columnsObj = { columns: [] }
			...
			getOrderFollowListStatus(this.tableQuery)
				.then(response => {
       ...
       let arr = []
			 ...
			 this.tableConfig.tableCellClassName = ({ row, column, rowIndex, columnIndex }) => {
				let obj = this.statusColumns.find(item => {
					return item.key === column.property
				})
				if (obj && obj.key) {
					rowStatusVal = arr[rowIndex][column.property]
					mapList = store.state.dict.dictMap[obj.mapValue] || []
				  if (rowStatusVal && mapList.length) {
            if (rowStatusVal === mapList[0].value) {
							return ''
						} else if (rowStatusVal === mapList[mapList.length - 1].value) {
							return 'td-finish'
						} else if (this.hasRefuseStatus(column.property, rowStatusVal)) {
              return 'td-refuse'
						} else {
							return 'td-doing'
						}
		...
		hasRefuseStatus(property, rowStatusVal) {
       let arr = [
				{ property: 'loa', statusList: ['audit_refuse_all'] }
			 ]
			 return arr.some(item => item.property === property && item.statusList.includes(rowStatusVal))
		},
		// 过滤显示自定义的表头

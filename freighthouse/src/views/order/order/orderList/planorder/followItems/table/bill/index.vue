...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.billList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.billList.defaultColumns,
			specialColumns: state => state.followItems.billList.specialColumns,
			columnsBase: state => state.followItems.billList.columnsBase(),
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.billList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn)
			this.tableConfig.allColumns = store.state.followItems.billList.columnsBase(this.handleClickRefreshList, this.handleClickRefreshListAndColumn)
		},
		handleOpPlan(row) {
      ...
      return (list.find(item => {
		...
		getAscOrDescData() {
          ...
          if (this.sortClickFlag) {
						...
						let ascColumns = [
							{ key: 'send_factory_bill', value: ['etd'] },
							{ key: 'send_agent_bill', value: ['etd'] },
						]

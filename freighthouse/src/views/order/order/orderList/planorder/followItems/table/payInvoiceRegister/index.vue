...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.payInvoiceRegisterList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.payInvoiceRegisterList.defaultColumns,
			specialColumns: state => state.followItems.payInvoiceRegisterList.specialColumns,
			columnsBase: state => state.followItems.payInvoiceRegisterList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.payInvoiceRegisterList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.payInvoiceRegisterList.columnsBase(this.handleClickRefreshList)
		},
		handleOpPlan(row) {

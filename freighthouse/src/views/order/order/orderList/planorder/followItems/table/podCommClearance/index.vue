...
export default {
  ...
  data() {
    return {
      ...
      // table复选框多选的值
        ...
        noShowStripe: true,
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.followItems.podCommClearanceList.columnsBase(this.handleClickRefreshList),
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
					show: true,
	...
	computed: {
		...mapState({
			...
			defaultColumns: state => state.followItems.podCommClearanceList.defaultColumns,
			specialColumns: state => state.followItems.podCommClearanceList.specialColumns,
			columnsBase: state => state.followItems.podCommClearanceList.columnsBase()
		})
	},
	created() {
		this.init()
  ...
  methods: {
		init() {
			this.tableConfig.columns = store.state.followItems.podCommClearanceList.columnsBase(this.handleClickRefreshList)
			this.tableConfig.allColumns = store.state.followItems.podCommClearanceList.columnsBase(this.handleClickRefreshList)
		},
		handleOpPlan(row) {
      ...
      return (list.find(item => {
		...
		getAscOrDescData() {
          ...
          if (this.sortClickFlag) {
						...
						let ascColumns = [
							{ key: 'pod_comm_clearance', value: ['etd'] },
						]

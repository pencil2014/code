<template>
	<div class="lcl-dept-cargo-form">
		<div class="row-flex flex-cargo" v-for="(o, index) in lclDepCargoList" :key="index">
			...
			<el-form-item class="cargo-item hscode" label="HS Code" :show-message="false" :rules="{ required: isBd, message: '请选择海关编号', trigger: 'change' }">
          ...
          <el-select size="mini" v-model="o.hscode" style="width: 100%"  placeholder="请选择海关编号" disabled>
			...
			<el-form-item class="cargo-item cname cname-ename" label="中文品名" :show-message="false" :rules="{ required: isBd, message: '请输入中文品名', trigger: 'blur' }">
           ...
           <el-input size="mini" v-model="o.cname" clearable placeholder="请输入中文品名" disabled maxlength="1024"></el-input>
				...
				<el-input v-else size="mini" v-model="o.cname" clearable placeholder="请输入中文品名" :disabled="viewState" maxlength="1024"></el-input>
			</el-form-item>
			<el-form-item class="cargo-item cname cname-ename" label="英文品名" :show-message="false" :rules="{ required: isBd, message: '请输入英文品名', trigger: 'blur' }">
           ...
           <el-input size="mini" v-model="o.ename" clearable placeholder="请输入英文品名" disabled maxlength="1024"></el-input>
				...
				<el-input v-else size="mini" v-model="o.ename" clearable placeholder="请输入英文品名" :disabled="viewState" maxlength="1024"></el-input>
			</el-form-item>
			...
			<el-form-item class="cargo-item cname" label="货物性质" :show-message="false" :rules="{ required: isBd, message: '请选择货物性质', trigger: 'change' }">
			...
			<el-form-item class="cargo-item cname" label="包装" :show-message="false" :rules="{ required: isBd, message: '请选择包装', trigger: 'change' }">
			...
			<el-form-item class="cargo-item cname" label="长" :prop="`cargoList.${index}.length`" :show-message="false" :rules="{ required: isBd, message: '请输入长', trigger: 'blur' }">
				<el-input @blur="validateNum(o, 'length')" size="mini" v-model="o.length" clearable placeholder="请输入长" :disabled="viewState">
					<template slot="append">m</template>
				</el-input>
			</el-form-item>
			<el-form-item class="cargo-item cname" label="宽" :prop="`cargoList.${index}.width`" :show-message="false" :rules="{ required: isBd, message: '请输入宽', trigger: 'blur' }">
				<el-input @blur="validateNum(o, 'width')" size="mini" v-model="o.width" clearable placeholder="请输入宽" :disabled="viewState">
					<template slot="append">m</template>
				</el-input>
			</el-form-item>
			<el-form-item class="cargo-item cname" label="高" :prop="`cargoList.${index}.height`" :show-message="false" :rules="{ required: isBd, message: '请输入高', trigger: 'blur' }">
				<el-input @blur="validateNum(o, 'height')" size="mini" v-model="o.height" clearable placeholder="请输入高" :disabled="viewState">
					<template slot="append">m</template>
				</el-input>
			</el-form-item>
			<el-form-item class="cargo-item hscode" label="危险等级" v-if="o.property === 'dangerous'" :show-message="false" :rules="{ required: isBd, message: '请选择危险等级', trigger: 'change' }">
			...
			<el-form-item class="cargo-item cname" label="国际危码" :show-message="false" v-if="o.property === 'dangerous'" :rules="{ required: isBd, message: '请选择国际危码', trigger: 'change' }">
			...
			<el-form-item class="cargo-item hscode" label="木质包装" :show-message="false" :rules="{ required: false, message: '请选择木质包装', trigger: 'change' }">
			...
			<el-row class="flex-item-commodityDesc">
				<el-form-item class="form-cont" label="唛头" :rules="{ required: false, message: '请输入唛头', trigger: 'blur' }" style="clear: both">
				...
				<el-form-item class="form-cont" label="货物描述" :show-message="false" :rules="{ required: false, message: '请输入货物描述', trigger: 'blur' }">
...
export default {
	...
	methods: {
		...
		handleSelectCustid(val, o) {
			...
			// let findItem = this.shipperCustOptions.find(item => item.value === val)
			// o.shipperCustid = findItem ? findItem.custid : ''
		},
		// 获取委托单位
		getCustList(val = '') {
		...
		getDeptCode() {
			getValueByKey('lcl_company_code').then(res => {
				...
				console.log('this.isSameCode22222222222', this.isSameCode)
			})
		},
		// 增加一行货物信息
		handleAddCargo() {
			this.lclDepCargoList.push({})
		},

		// 删除一行货物信息
		handleDeleteCargo(index) {
			this.lclDepCargoList = this.lclDepCargoList.filter((item, sindex) => {
				return sindex !== index
			})
		},
...
.lcl-dept-cargo-form .row-flex.flex-cargo {
	.btn-minus-search,
	.btn-plus-search {
		margin-top: 2px;
	}

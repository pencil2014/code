<template>
	<div class="order-info lcl-order-import-info">
		<div class="flex between nowrap">
			<div class="orderNo">
              ...
              <div class="port-popover">
                <div>
                  <span>{{$route.query.orderNo}}({{ businessTypeName }})</span>
                  <em @click="handleCopyorderNo($route.query.orderNo, $event)">复制工作号</em>
                </div>
              </div>
              <div slot="reference" class="item-label bold">工作号:{{$route.query.orderNo}}({{ businessTypeName }})</div>
            </el-popover>
					</div>
					<div class="item" :class="$route.query.jointNo ? 'per40' : 'per50'">
						<span class="item-label cust-item"
							...
							<!-- <el-tooltip
								v-if="getTextSize('12px', ($route.query.custid || createItem.custid) + (createItem.custName ? `(${createItem.custName})` : '')) >= 107"
								effect="dark"
								:content="($route.query.custid || createItem.custid) + (createItem.custName ? `(${createItem.custName})` : '')"
								placement="top-start"
							>
							...
							<i class="el-icon-edit" @click="handleCustUpdaShow" v-if="isBd && createItem.isDraftCust === 'Y' && ['draft', 'submit', 'pass', 'refuse'].includes(orderStatus)"></i> -->
						</span>
					</div>
...
.order-info.lcl-order-import-info {

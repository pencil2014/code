<template>
  <div class="row-cont" style="margin-bottom:10px">
    ...
    <div v-if="showQuotationSchemePop">
      <QuotationSchemePop @close="closeQuotationScheme" :schemeNumber="schemeNumbers" :bdId="orderInfoDataObj.bdEmployeeId" :porCode="porCode" :polCode="polCode" :podCode="podCode" :businessType="businessType"></QuotationSchemePop>
    </div>
    <div v-if="addFeePop">
  ...
  computed: {
    porCode(){
      if(this.createItem.barge && this.createItem.barge.porPortCode){
        return this.createItem.barge.porPortCode
      }else{
        return ''
      }
    },
    showCreateFee() {
      if (
        this.orderInfoDataObj.businessType === 'ship_export_fcl' ||
        this.orderInfoDataObj.businessType === 'ship_export_lcl'
      ) {
        if (
    ...
    isPricing() {
      ...
      return this.userEditRoles.includes('pricing')
    },
    isBd() {
      return (
        this.ordDetRole === 'bd' ||

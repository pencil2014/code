...
import { contactList, contactSave, contactDelete } from '@/api/order/list'
...
export default {
  ...
  computed: {
    ...
    showTaxCode() {
      // 发货人 或者 （海运进口，空运进口 的收货人）
      return this.csnType === 'shipper' || this.isConsignee
    },
    businessType() {
      return (this.shipOrderInfoData && this.shipOrderInfoData.businessType) || (this.airOrderInfoData && this.airOrderInfoData.businessType) || ''
    },
    isConsignee() {
      return this.csnType === 'consignee' && this.businessType && this.businessType.includes('import')
    },
  },
  watch: {},
  components: {
  ...
  methods: {
    ...
    contactSave(data, index) {
      let custid = JSON.parse(sessionStorage.getItem('custStr')).custid
      let obj = {}
      if (this.showTaxCode) {
        obj = {
      ...
      contactSave(params).then((res) => {
        ...
        // this.$set(this.list[index],'isNewData',false)
        this.contactList()
      })
    },

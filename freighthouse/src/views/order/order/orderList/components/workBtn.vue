...
export default {
  ...
  watch: {
    'workAssignShow'(val) {
      if(val) {
        ...
        if (this.createItem.workAssign && this.createItem.workAssign.length) {
        ...
        // 先取obd部门code，再取下单助理部门code最后再取bd部门code
				let findObdItem = this.orderWorkOptions.find(item => item.jobCode === 'obd')
				let findAsstItem = this.orderWorkOptions.find(item => item.jobCode === 'asst')
				let findBdItem = this.orderWorkOptions.find(item => item.jobCode === 'bd')
  ...
  methods: {
    // 获取订舱协助审核人
		getJointAuditWork() {
      if (this.jointAuditWorkData && this.jointAuditWorkData.length) return
			jointAuditWork({
    ...
    // 获取协同岗位分配
		getJointWork() {
      if (this.jointWorkData && this.jointWorkData.length) return
			jointWork({
				orderNo: this.$route.query.orderNo
			}).then(res => {

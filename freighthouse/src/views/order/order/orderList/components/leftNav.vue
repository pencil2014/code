...
export default {
	...
	methods: {
		getServiceMenu(data) {
			...
			if (['st01', 'st15', 'st03', 'st18', 'st09', 'st11', 'st24', 'st25'].includes(this.orderInfo.serviceType)) {
			...
			// 20230117版本-后端在海运进出口整箱、拼箱业务都放开了装箱的服务项，前端在这里过滤掉装箱stuffing这个服务项
			if (['ship_export_fcl', 'ship_export_lcl', 'ship_import_fcl', 'ship_import_lcl'].includes(this.orderInfo.businessType)) {
				this.serviceMenus = this.serviceMenus.filter(item => item.component !== 'YardBox')
			}

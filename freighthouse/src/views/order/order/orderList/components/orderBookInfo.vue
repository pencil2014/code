        ...
        <div class="cont" :class="this.isOp ? 'op-cont' : this.isBd ? 'bd-cont' : ''">
          ...
          <div class="flex-form-item">
            ...
            <el-form-item :required="true" label="船公司" prop="ship.shipCarrierCode" :show-message="false" class="vessel-time">             
              ...
              >
                <el-option v-for="item in shipCarrierOptions" :key="item.value" :label="item.label" :value="item.value"> </el-option>
              </el-select>
            ...
            <el-form-item class="timeSpecial vessel-time" label="大船实际开船时间" prop="ship.tdTime" :show-message="false" v-if="isOpView || isDoc || isCsc || !ordDetRole">
              <!-- 暂时取消已关账的限制(includeFeederState==='y' && finIsCloseStatus) -->
              <elDatePickerLimit noCanFuture :field="'tdTime'" size="mini" v-model="createItem.ship.tdTime" type="date" style="width: 100%" clearable placeholder="请选择" value-format="yyyy-MM-dd" :disabled="(!isDocEdit && disabledState && isView && !lineCsJointIsEdit  && !isCscEdit) || (includeFeederState==='y' && finIsCloseStatus) " />
            ...
            <el-form-item class="timeSpecial" label="预计到港时间" prop="ship.etaTime" :show-message="false" v-if="isOpView || isDoc || !ordDetRole">
              <elDatePickerLimit size="mini" :field="'etaTime'" v-model="createItem.ship.etaTime" type="date" style="width: 100%" clearable placeholder="请选择" value-format="yyyy-MM-dd" :disabled="!(isDocEdit && !isJoint) && !(isEcsEpricingEdit && !isJoint)  && disabledState && isView" />
            </el-form-item>
            <el-form-item class="timeSpecial" label="实际到港时间" prop="ship.taTime" :show-message="false" v-if="isOpView || isDoc || !ordDetRole">
              <elDatePickerLimit noCanFuture :field="'taTime'" size="mini" v-model="createItem.ship.taTime" type="date" style="width: 100%" clearable placeholder="请选择" value-format="yyyy-MM-dd" :disabled="!(isDocEdit && !isJoint) && !(isEcsEpricingEdit && !isJoint) && disabledState && isView" />
            </el-form-item>
      ...
      <div class="table-td">
        <div class="cont" :class="this.isOp ? 'op-cont' : this.isBd ? 'bd-cont' : ''">
          ...
          <div class="flex-form-item label100">
            ...
            <el-form-item label="海关是否查验" prop="transport.isCustomsInspection" v-if="(isOpView || !ordDetRole) && !isBulkRo">
            ...
            <el-form-item label="审单备注" prop="auditRemark" :show-message="false" v-if="isOpView">
              <el-input @dblclick.native="showRemarkDetail(createItem.auditRemark)" type="textarea" clearable rows="2" size="mini" placeholder="请输入" maxlength="128" show-word-limit v-model="createItem.auditRemark" :disabled="disabledState"></el-input>
            </el-form-item>
...
export default {
  ...
  computed: {
    ...
    // 海外客服和海外商务可以编辑 预计到港时间和实际到港时间
		isEcsEpricingEdit() {
			return ['ecs', 'epricing'].includes(this.ordDetRole) && this.orderStatus === 'pass'
		},
    isOpEdit() {
  ...
  methods: {
    ...
    noDropSendMailClick() {
      this.orderInfoData = {
        ...
        isWhiteCust: this.orderInfoDataObj.isWhiteCust=='Y'?'是':'否',
        porPortName: this.createItem.barge.porPortName,
        pricingEmployeeId: this.getEmployeeId('pricing'),
        bdId: this.getEmployeeId('bd'),
        obdId: this.getEmployeeId('obd'),
        asstId: this.getEmployeeId('asst'),
...
.orderbook-info-table {
  ...
  .table-td {
    ...
    .flex-form-item {
      ...
      &.label80 .el-form-item .el-form-item__label {
        width: 80px;
      }
      &.label100 .el-form-item .el-form-item__label {
        width: 100px;

<template>
	<div class="row-transport ml10">
		...
		<el-row>
			...
			<div class="flex-item-double oneThird" id="consigneeTextArea">
				<div class="flex-class">
					<div class="item-label">
						<span class="lab">
							<!-- 满足以下条件必填：海运出口整箱-委托订舱；船司为VOLTA，或者 COSCO；系统航线为欧洲，地西，地东，黑海，北非，俄罗斯的其中一个；付款方式为：到付FREIGHT COLLECT；拿价类型为：商务自拿价 -->
							<em
								class="red"
								v-if="
									isBd &&
									createItem.serviceType === 'st01' &&
									['VOLTA', 'COSCO'].includes(createItem.ship.shipCarrierCode) &&
									createItem.payMode === 'collect' &&
									createItem.ship.priceType === 'pricing' &&
									['100075000', '100075018', '100074999', '100075011', '100075005', '100075008'].includes(createItem.ship.sysLineCode)
								"
								>*</em
							>
							Consignee(收货人)</span
	...
	methods: {
		...
		getBlackUrlData() {
			if (this.createItem.ship && this.createItem.ship.shipCarrierCode) {
				getBlackUrl(this.createItem.ship.shipCarrierCode).then(res => {

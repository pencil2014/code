...
export default {
		...
		currRole: {
			handler: function (val) {
				if (val === 'bd') {
					...
					this.tableConfig.columns = this.tableConfig.columns.filter(el => el.key !== 'bkgAgentSupplierName')
				} else {
					this.tableConfig.columns = store.state.order.orderList.columnsBase(this.handleOrderNoInfo, this.handleClickBdRemark, this.handleSaveRemark)
	...
	methods: {
		...
		getListQuery() {
			...
			} else if (this.tableQuery.orderStatus === 'addStatusIsSubmit') {
				statusQuery = [
					{ column: 'addStatus', type: 'eq', value: 'submit' },
		...
		async handleInfo(row) {
			...
			let { data: pushData } = await importApplyList({ orderNo: row.orderNo })
			...
			let routeUrl = this.$router.resolve({
				...
				query: {
					...
					showRelate: (isSameCode && ['st01', 'st15', 'st09'].includes(row.serviceType)) || (data && data.length) || (pushData && pushData.length) ? true : false
				}
			})

...
import { listUnBindContainer } from '@/api/order/list'
...
export default {
	...
	methods: {
		...
		getContainerList() {
			...
			listUnBindContainer(data).then(res => {
				let oids = this.bindContainerList.map(item => item.containerId)
				this.containerList = res.data.filter(item => !oids.includes(item.oid))
			})
		},

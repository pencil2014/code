		...
		<el-form ref="mailOnlyForm" :rules="rules" :model="createItem" label-position="right" label-width="90px" class="custBlSiPush-form">
			...
			<el-form-item label="附件">
				<el-row>
					...
					<el-col :span="24" v-if="copyFileNameList.length">
						<div class="init-file" v-for="(item, index) in copyFileNameList" :key="index">
...
import { custBlContract, custBlMailTitle, blFileList, custBlSiCopyConfirmInfo } from '@/api/order/bl'
...
export default {
	data() {
		return {
			...
			exportFileTypeOptions: [{ label: 'mbl_copy', txt: 'M单COPY件' }],
			// 补料详情富文本编辑器配置
			tinymceConfig: {
	...
	methods: {
		...
		getMailTitle() {
			custBlMailTitle({
				orderNo: this.$route.query.orderNo,
				blId: this.custBlCopyPushQuery.blId
			}).then(res => {
				this.createItem.mailTitle = `【提单copy】${res.data}`
			})
		},
		// 设置默认邮件内容
		async setDefaultContent() {
			...
			let res = await deptEmployeeInfo(data)
			...
			let copyConfirmRes = await custBlSiCopyConfirmInfo(copyConfirmData)
			let { fileList, url } = copyConfirmRes.data
			this.copyFileNameList = fileList
			this.createItem.fileNoList = fileList.map(item => item.fileNo)
			this.copyConfirmUrl = `${window.location.origin}/${url}`
			// await deptEmployeeInfo(data)
			// 	.then(res => {
			// 		if (res.code === 0) {
			...
			this.createItem.mailContent = `
				...
				<p style="font-size: 12px;padding-left:24px">请查收附件提单copy，请务必在收到后1个工作日内确认提单，逾期将默认提单。确认提单后，除船司出错外任何更改将产生改单费，如后续产生费用将由贵司自行承担，请知悉，谢谢！</p>
				<p style="font-size: 12px;padding-left:24px"> 若提单确认无误，可直接点击<a href="${this.copyConfirmUrl}" target="_blank">“确认无误”</a>；若提单有误，可直接联系  ${email}。</p>
		...
		// 		this.copyFileNameList = res.data
		// 		// this.createItem.mblCopyFileNo = fileNo
		// 	})
		// },
		getCopyConfirmInfo() {
			let data = {
			...
			custBlSiCopyConfirmInfo(data).then(res => {
				let { fileList, url } = res.data
				this.copyFileNameList = fileList
				// this.copyFileNameList = [{
				// 	fileName: copyFile.fileName,
				// 	fileNo: copyFile.fileNo
				// }]
				this.createItem.fileNoList = fileList.map(item => item.fileNo)
				this.copyConfirmUrl = `${window.location.origin}/${url}`
			})
		...
		handleToMail() {
			...
			if (this.sizeLimit) {
				...
				let { fileType, mailTo, mailCc, mailTitle, mailContent, mailSign, fileNoList } = this.createItem
				let params = {
					data: {
						orderNo: this.$route.query.orderNo,
						blId: this.custBlCopyPushQuery.blId,
				...
				this.$store
					.dispatch('order/soUpload', {
						uri: '/order/cust/bl/si/copy/push',
						params
					})
					.then(res => {

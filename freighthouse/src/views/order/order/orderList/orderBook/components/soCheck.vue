...
import { orderInfo, orderJointInfo, soCheck, orderFileList } from '@/api/order/list'
	...
	methods: {
		...
		getFileList() {
			orderFileList({ orderNo: this.$route.query.orderNo }).then(res => {
				let fileBook = res.data.find(item => ['booking', 'ls_booking', 'cust_booking'].includes(item.typeCode))
				if (!fileBook) {
					this.fileBookUrl = ''
					return
				}
				let { fileNo, name } = fileBook
				this.$store.dispatch('order/getPreViewFile', { fileNo, fileName: name })
				.then(url => {
					this.fileBookUrl = url
				})
			})
		},
		// 取消返回放舱详情
		handleCancel() {
			...
			delete routeQuery.soFileUrl
			delete routeQuery.so
			delete routeQuery.containerInfo
			this.$router.push({

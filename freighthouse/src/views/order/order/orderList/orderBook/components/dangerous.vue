					...
					<el-table-column label="操作" width="120" align="center" fixed="right">
						<template slot-scope="scope">
							<div class="operate-group">
								...
								<ButtonTip
									btnType="text" 
									:btnDisabled="isBd || scope.row.declareStatus === 'declared'" 
									btnText="删除" 
									:tipContent="isBdTip || rowDeclaredTip(scope.row)" 
									class="operateBtn-tip ml10"
									btnClassName="red"
		...
		<div class="agentDetail mt10" v-show="showDetail">
			<div class="row-tit" id="saveBar">
				...
				<div class="row-tit-operate">
					...
					<ButtonTip
						btnType="primary" 
						:btnDisabled="isBd || createdItem.declareStatus === 'declared'" 
						btnText="保存" 
						:tipContent="isBdTip || declaredTip" 
						class="operateBtn-tip ml10"
						@click="handleSave('createdItem')">
					</ButtonTip>
							...
							<template slot-scope="scope">
								<div class="operate-group">
									<ButtonTip
										...
										class="operateBtn-tip ml10"
										btnClassName="red"
										@click="handleDeleteContainer(scope)">
									</ButtonTip>
									...
									<ButtonTip
										...
										:tipContent="isBdTip || declaredTip" 
										class="operateBtn-tip ml10"
										@click="handleAddEnclosure(scope)">
									</ButtonTip>
...
export default {
    ...
    isDoc() {
		...
		declaredTip() {
			if (this.createdItem.declareStatus === 'declared') {
				return '申报状态为已申报，不允许该操作'
			}
		}
	},
	methods: {
		rowDeclaredTip(row) {
			if (row.declareStatus === 'declared') {
				return '申报状态为已申报，不允许该操作'
			}
		},
		// hscode模糊查询

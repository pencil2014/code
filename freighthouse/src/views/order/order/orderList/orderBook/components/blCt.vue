		...
		<div class="edit-container so" id="blTableCont">
			<div class="row-tit so">
				...
				<div class="row-tit-operate">
					<ButtonTip btnType="default" :btnDisabled="isBd || isView || blListIncludesNotCscCreated" btnText="草单确认" :tipContent="isBdTip || isViewTip || notCscCreatedTip" class="mr5" @click="handleBatchBlDraftConfirm"></ButtonTip>
		...
		<div class="component-cont blInfo-cont" v-if="showDetail">
			<div class="blInfo-between" id="saveBar">
				...
				<div class="el-button-group">
					...
					<ButtonTip
						btnType="primary" 
						:btnDisabled="isBd || isView || hblIsDraftConfirm || createdByIsNotUserId" 
						btnText="保存" 
						:tipContent="isBdTip || isViewTip || hblIsDraftConfirmTip || createdByIsNotUserIdTip" 
	...
	computed: {
		...
		isBd() {
			...
			if(['st26','st27','st28'].includes(this.orderInfoDataObj.serviceType)){
				...
				return !this.ordDetRole || this.ordDetRole === 'bd' || this.isAc || this.isOp || this.isLclOrderList || this.isOpTruck || this.isEpricing || this.isEcs || (!this.userEditRoles.includes('obd') && !this.userEditRoles.includes('asst') && !this.userEditRoles.includes('doc') && !this.isCsc)
			}
		...
		hblIsDraftConfirmTip() {
			if (this.hblIsDraftConfirm) {
				return '该单草单已确认或该单仅够系统信息更新原因撤回了，不允许该操作'
			}
		},
	},
	components: {

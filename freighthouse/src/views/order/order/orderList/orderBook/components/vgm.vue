			...
			<div class="row-form mt10 table-com-layout">
				...
				>
					...
					<el-table-column label="操作" width="120" align="center" fixed="right">
						<template slot-scope="scope">
							<div class="operate-group">
								<ButtonTip btnType="text" :btnDisabled="isBd" btnText="详情" :tipContent="isBdTip" class="operateBtn-tip" @click="handleDetail(scope.row)"> </ButtonTip>
			...
			<el-form ref="form" label-position="top" label-width="80px">
				<div class="row-tit" id="saveBar">
					...
					<div class="row-tit-operate">
						...
						<ButtonTip btnType="primary" :btnDisabled="isBd || createItem.status === 'declared'" btnText="保存" :tipContent="isBdTip || declaredTip" class="operateBtn-tip ml10" @click="handleSave"> </ButtonTip>
			...
			<el-form label-position="left" label-width="94px" ref="createItem" :model="createItem" :rules="rules">
				<div class="row-flex ml20">
					...
					<el-form-item label="VGM邮箱" prop="chargeEmail" class="flex-item" :show-message="false">
						<el-input :validate-event="false" size="mini" placeholder="请输入" v-model="createItem.chargeEmail" clearable></el-input>
					</el-form-item>
...
export default {
	...
	data() {
		...
		var validatorVgmMail = (rule, value, callback) => {
	...
	computed: {
		...
		isBd() {
			let isBd = !this.ordDetRole || this.ordDetRole === 'bd' || this.ordDetRole === 'obd' || this.isAc || this.isDoc || this.isOpTruck || !this.isOp
		...
		isBdTip() {
			if (this.isBd) {
				return '非本单操作人员、或服务项进度为已完成、或服务项已被协同，不允许该操作'
			}
		},
		declaredTip() {
			if (this.createItem.status === 'declared') {
				return 'VGM申报状态为已申报，不允许该操作'
			}
		}
	},
	methods: {

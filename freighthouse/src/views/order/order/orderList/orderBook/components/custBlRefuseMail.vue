...
import { custBlContract, custBlMailTitle, blFileList } from '@/api/order/bl'
...
export default {
	...
	methods: {
		...
		getMailTitle() {
			custBlMailTitle({
				orderNo: this.$route.query.orderNo,
				blId: this.custBlRefuseQuery.blId
			}).then(res => {
				this.createItem.mailTitle = `【补料拒绝】请修改 ${res.data}`
			})
		},
		// 设置默认邮件内容
		async setDefaultContent() {
			...
			this.createItem.mailContent = `
				...
				<p style="font-size: 12px;padding-left:24px">贵司提交到我司系统的补料因资料不齐/相关信息有误无法完成补料，具体原因如下，</p>
				<p style="font-size: 12px;padding-left:24px">请您在截补料时间前完成修改，如有问题可直接联系${email}进行确认，谢谢 。</p>
				<p style="font-size: 12px;padding-left:24px">了解更多详情，请点击<a href="https://www.sowoll.com" target="_blank">https://www.sowoll.com</a></p>
		...
		handleToMail() {
			...
			if (this.sizeLimit) {
				...
				let { fileType, mailTo, mailCc, mailTitle, mailContent, mailSign, mblCopyFileNo } = this.createItem
				let params = {
					data: {
						orderNo: this.$route.query.orderNo,
						blId: this.custBlRefuseQuery.blId,

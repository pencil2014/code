<template>
	<div class="table-filter">
		<div class="table-filter-left">
			...
			<el-button size="mini"  @click="handleAddBl('AddBl')" plain>
				新增提单
			</el-button>
		...
		<div class="table-filter-right">
			<div class="pulish-status">
				<el-radio-group v-model="tableQuery.siStatus" size="mini" @change="handleSearchByStatus">
...
export default {
	...
	computed: {
		...
		statusOptions() {
			return {
				all: '全部',
				siCheck: '待补料',
				copySiCheck: 'copy件单证待确认',
				copyCheckStatus: 'copy件客户待确认',
				obdCheck: '待收OBD提单',
				dnGet: '待收费用单',
				dnGetY: '商务待签单',
				dnFeeInput: '费用待录入',
				takeApplyStatus: '已申请取单',
				isCancel: '已撤回',
				state: '已取消'
			}
		}
	...
	methods: {
		...
		handleSearchByStatus(value) {
			...
			console.log(this.tableQuery.siStatus)

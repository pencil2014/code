<template>
	<div>
		<div class="bookInfo-cont">
			<div class="table-td">
				...
				<div class="pd8 flex-form-item portInfo" :class="!disabledState ? 'pr24' : ''">
					<el-form-item label="船东参考号" prop="ship.shipRefNo" :show-message="false" v-if="isBkg">
						<el-input
							clearable
							size="mini"
							placeholder="请输入"
							v-model="bookDetailForm.shipRefNo"
							:disabled="disabledState && bkgEditState"
							@mouseenter.native="showTooltip($event, bookDetailForm.shipRefNo)"
							@mouseleave.native="hiddenTooltip"
						></el-input>
					</el-form-item>
					<el-form-item label="驳船收货地" prop="barge.porPortCode" :show-message="false" v-if="isIncludeFeeder" :required="bookDetailForm.businessType === 'ship_export_fcl'">
					...
					<el-form-item label="中转港1" prop="ship.transitPortCode" :show-message="false">
						<div class="port-flex-form">
							<div class="port-code">
								<el-popover placement="top" width="100%" trigger="hover" class="port-ename" :disabled="isPricing || !bookDetailForm.ship.transitPortCode" popper-class="port-ename-popover">
									<div class="port-popover">{{ bookDetailForm.ship.transitPortCode }}<em @click="handleCopyPort(bookDetailForm.ship.transitPortCode, $event)">复制</em></div>
									<el-select
										slot="reference"
										key="transitPortCode"
										size="mini"
										v-model="bookDetailForm.ship.transitPortCode"
										filterable
										clearable
										:disabled="disabledState && !isChangeEdit"
										style="width: 100%"
							...
							<PortPopover :portItem="transitPortItem" :isShowCopy="isBkg ? true : false" />
					...
					<el-form-item label="中转港2" prop="ship.transitPortCode1" :show-message="false" v-if="transitPortOneShow">
						<div class="port-flex-form">
							<div class="port-code">
								<el-popover placement="top" width="100%" trigger="hover" class="port-ename" :disabled="isPricing || !bookDetailForm.ship.transitPortCode1" popper-class="port-ename-popover">
									<div class="port-popover">{{ bookDetailForm.ship.transitPortCode1 }}<em @click="handleCopyPort(bookDetailForm.ship.transitPortCode1, $event)">复制</em></div>
									<el-select
										slot="reference"
										key="transitPortCode1"
										size="mini"
										v-model="bookDetailForm.ship.transitPortCode1"
										filterable
										clearable
										:disabled="disabledState && !isChangeEdit"
										style="width: 100%"
										placeholder="请输入"
										:filter-method="transitPortOneFilterMehod"
										@visible-change="transitPortOneVisibleChange"
							...
							<PortPopover :portItem="transitPortItem1" :isShowCopy="isBkg ? true : false" />
					...
					<el-form-item label="中转港3" prop="ship.transitPortCode2" :show-message="false" v-if="transitPortTwoShow">
						<div class="port-flex-form">
							<div class="port-code">
								<el-popover placement="top" width="100%" trigger="hover" class="port-ename" :disabled="isPricing || !bookDetailForm.ship.transitPortCode2" popper-class="port-ename-popover">
									<div class="port-popover">{{ bookDetailForm.ship.transitPortCode2 }}<em @click="handleCopyPort(bookDetailForm.ship.transitPortCode2, $event)">复制</em></div>
									<el-select
										slot="reference"
										key="transitPortCode2"
										size="mini"
										v-model="bookDetailForm.ship.transitPortCode2"
										filterable
										clearable
										:disabled="disabledState && !isChangeEdit"
										style="width: 100%"
										placeholder="请输入"
										:filter-method="transitPortTwoFilterMehod"
										@visible-change="transitPortTwoVisibleChange"
							...
							<PortPopover :portItem="transitPortItem2" :isShowCopy="isBkg ? true : false" />
					...
					<el-form-item required label="系统航线" prop="ship.sysLineCode" :show-message="false">
						<el-input clearable size="mini" placeholder="请输入" v-model="bookDetailForm.ship.sysLineName" disabled></el-input>
					</el-form-item>
					<el-form-item label="国家" :show-message="false" v-if="['ship_import_fcl'].includes(bookDetailForm.businessType)">
					...
					<el-form-item label="委托备注" prop="intrustRemark" :show-message="false" v-if="isBkg && $route.query.orderNo">
						<el-input 
							@dblclick.native="showRemarkDetail('intrustRemark', '委托备注', true)"
							size="mini" type="textarea" rows="3" placeholder="请输入" v-model="bookDetailForm.intrustRemark" :disabled="true" @mouseenter.native="showTooltip($event, bookDetailForm.intrustRemark)" @mouseleave.native="hiddenTooltip">
						</el-input>
					</el-form-item>
					<template v-if="isBkg">
						<el-form-item label="审单备注" prop="auditRemark" :show-message="false">
							<el-input
								@dblclick.native="showRemarkDetail('auditRemark', '审单备注', disabledState && !isChangeEdit, 512)"
								size="mini"
								type="textarea"
								rows="3"
								placeholder="请输入"
								maxlength="512" show-word-limit
								v-model="bookDetailForm.auditRemark"
								:disabled="disabledState && !isChangeEdit"
								@mouseenter.native="showTooltip($event, bookDetailForm.auditRemark)"
								@mouseleave.native="hiddenTooltip"
							>
							</el-input>
						</el-form-item>
						<el-form-item label="订舱要求" prop="bookingRequire" :show-message="false" class="vessel-time">
							<div slot="label">
								<span>订舱要求</span>
							...
							<el-input
								@dblclick.native="showRemarkDetail('bookingRequire', '订舱要求', isAddCont || isChange || isBkg || (isPricing && bookDetailForm.bkgStatus !== 'submit'), 512)"
								size="mini"
								type="textarea"
								rows="3"
								placeholder="请输入"
								maxlength="512"
								show-word-limit
								v-model="bookDetailForm.bookingRequire"
								:disabled="isAddCont || isChange || isBkg || (isPricing && bookDetailForm.bkgStatus !== 'submit')"
							>
							</el-input>
						</el-form-item>
						<el-form-item label="订舱备注" prop="bkgRemark" :show-message="false">
							<el-input 
								@dblclick.native="showRemarkDetail('bkgRemark', '订舱备注', disabledState && bkgEditState)"
								size="mini" type="textarea" rows="3" placeholder="请输入" v-model="bookDetailForm.bkgRemark" :disabled="disabledState && bkgEditState"> </el-input>
						</el-form-item>
					</template>
				</div>
			</div>
			<div class="table-td">
				...
				<div class="pd8 flex-form-item">
					...
					<el-form-item class="timeSpecial" label="驳船预计开船时间" prop="ship.bargeEtdTime" :required="bookDetailForm.businessType === 'ship_export_fcl'" :show-message="false" v-if="isIncludeFeeder">
					...
					<el-form-item class="timeSpecial" label="大船截关时间" prop="ship.cyCutOff" :required="bookDetailForm.businessType === 'ship_export_fcl'" :show-message="false">
						<elDatePickerLimit
							size="mini"
							:field="'cyCutOff'"
					...
					<template v-if="!isBulkRo">
						...
						<el-form-item label="运输条款" prop="transport.transportTerm" :show-message="false" v-if="isPricing">
					...
					<el-form-item label="免用免堆信息" prop="ship.freetimeSys" :show-message="false" :required="isPricing">
						<el-input
							type="textarea"
							clearable
							rows="4"
							...
							@input="saveTempFreetimeSys"
							@blur="handleClearEmptyString('freetimeSys', $event)"
			...
			<div class="table-td">
				...
				<div class="pd8 flex-form-item cargoInfo">
					...
					<template v-if="isBulkRo && isPricing">
						<el-form-item label="付款方式" prop="payMode" :show-message="false" v-if="isPricing">
							<el-select v-model="bookDetailForm.payMode" size="mini" clearable placeholder="请选择" style="width: 100%" :disabled="true">
								<el-option v-for="(item, index) in dictMap.payMode" :label="item.label" :value="item.value" :key="index"> </el-option>
							</el-select>
					...
					<el-form-item label="船司订单号" prop="shipCarrierOrderNo" :show-message="false" :required="isBkg && bookDetailForm.isOnline === 'Y' && ['COSCO', 'OOCL'].includes(bookDetailForm.ship.shipCarrierCode)" v-if="!isBulkRo">
						<el-input size="mini" type="text" placeholder="请输入" v-model="bookDetailForm.shipCarrierOrderNo" :disabled="disabledState && !(isBkg && ['pass','booking','booked','book_release'].includes(bkgStatus))" maxlength="128" clearable> </el-input>
					</el-form-item>
					<el-form-item label="委托备注" prop="intrustRemark" :show-message="false" v-if="isPricing && $route.query.orderNo">
						<el-input @dblclick.native="showRemarkDetail('intrustRemark', '委托备注', true)" size="mini" type="textarea" rows="3" placeholder="请输入" v-model="bookDetailForm.intrustRemark" :disabled="true"> </el-input>
					</el-form-item>
					<template v-if="isPricing">
						<el-form-item label="审单备注" prop="auditRemark" :show-message="false" class="vessel-time">
							<el-input 
								@dblclick.native="showRemarkDetail('auditRemark', '审单备注', disabledState && (!isAddCont || bookDetailForm.bkgStatus !== 'submit'), 512)" 
								size="mini" type="textarea" rows="3" placeholder="请输入" maxlength="512" show-word-limit v-model="bookDetailForm.auditRemark" 
								:disabled="disabledState && (!isAddCont || bookDetailForm.bkgStatus !== 'submit')"> 
							</el-input>
						...
						<el-form-item label="订舱要求" prop="bookingRequire" :show-message="false" class="vessel-time" v-if="!isBulkRo">
							...
							<el-input
								@dblclick.native="showRemarkDetail('bookingRequire', '订舱要求', disabledState && (!isAddCont || bookDetailForm.bkgStatus !== 'submit'), 512)"
								size="mini" type="textarea" rows="3" placeholder="请输入" maxlength="512" show-word-limit v-model="bookDetailForm.bookingRequire" :disabled="disabledState && (!isAddCont || bookDetailForm.bkgStatus !== 'submit')"> 
							</el-input>
						</el-form-item>
					</template>
				</div>
      ...
      <RemarkDialog :showWordLimit="remarkDialogParam.showWordLimit" :remark="bookDetailForm[remarkDialogParam.prop]" :title="remarkDialogParam.title" :isDisabled="remarkDialogParam.isDisabled" @close="remarkDialogClose" />
    </div>
	</div>
</template>
...
export default {
	data() {
		return {
			...
			saveFreeTimeSys: '',	// 保存运价免用免堆接口返回的值
	...
	methods: {
		saveTempFreetimeSys(val) {
			// console.log('val', val)
			this.$emit('updateFreeTimeSys', val)
		},
		showRemarkDetail(prop, title, isDisabled, maxlength) {
			this.remarkDialogParam = {
				prop,
				title,
				isDisabled,
				showWordLimit: maxlength ? true : false
			}
			this.remarkVisible = true
    },
    remarkDialogClose(val) {
			this.remarkVisible = false
			this.bookDetailForm[this.remarkDialogParam.prop] = val
    },
		// 船期查询结果直接填入订单详情
    ...
    handleClearEmptyString(char, e) {
      if (['destinationArea', 'deliveryPlace'].includes(char)) {
        this.$set(this.bookDetailForm.ship, char, e.target.value.replace(/^\s+|\s+$/g, ''))
      } else {
        this.$set(this.bookDetailForm.ship, char, e.target.value.replace(/^\s+|\s+$/g, ''))
      }
    },
      ...
      clip(val, e)
		...
		handleChangePolPortCode(val) {
			...
			if (val && this.isPricing) {
				...
				if (this.orderIsJoint === 'Y') {
					...
					this.$emit('getWorkAssign', param, '大船起运港')
				} else {
					// 变更bkg岗位分配信息
			...
			if (val) {
				portSystemLineInfo(val).then(response => {
					...
					if (systemLineList.length > 0) {
						...
						this.bookDetailForm.ship.sysLineName = systemLine.cname
		...
		handleChangeShipCarrier(val) {
			...
			this.bookDetailForm.isOnline = (this.isPricing && !this.isChange && !this.isAddCont && val === 'MAERSK') ? 'Y' : this.oIsOnline
			...
			if (val && this.isPricing) {
				...
				this.$emit('getWorkAssign', param, '船公司')
			}
			this.handleFillFreetimeSys()
		},
    ...
    handleFillFreetimeSys(notIncludeBarge) {
      ...
      mainRouteFreeTime(param).then(res => {
			...
			if (val) {
				if (this.isPricing) {
					if (this.orderIsJoint === 'Y') {
						...
						this.$emit('getWorkAssign', param, '订舱代理')
					} else {
						this.updateBkg('订舱代理')
					}
				}
		...
		handleChangeExternal(val) {
			...
			if (this.isPricing && !this.isChange && !this.isAddCont && val === 'y') {
				this.bookDetailForm.isOnline = 'N'
			}
		},
		// // 拿价人

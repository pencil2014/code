...
import { blInfoList, blInfoExport, transferCreate } from '@/api/order/bl'
...
export default {
	...
	data() {
		return {
			...
			tableConfig: {
				...
				list: [],
				configColumns: [],
				columns: this.$store.state.order.blList.columnsBase(this.handleOrderNoInfo),
				allColumns: this.$store.state.order.blList.columnsBase(this.handleOrderNoInfo),
				// 操作按钮组
				operationBtns: {
					minWidth: '100',
					fixed: 'right',
	...
	computed: {
		...
		...mapState({
			...
			defaultColumns: state => state.order.blList.defaultColumns,
			specialColumns: state => state.order.blList.specialColumns,
			columnsBase: state => state.order.blList.columnsBase()
		}),

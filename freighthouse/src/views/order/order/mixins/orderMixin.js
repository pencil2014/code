...
export const orderMixin = {
	...
	created() {
		this.getFinCloseStatusCanEdit()
	},
	mounted() {},
	methods: {
		...mapMutations('order', ['setServiceList', 'setServiceInfoList']),
		getFinCloseStatusCanEdit() {
			this.$store.dispatch('order/setPolAllowFinCloseInput')
		},
		// 获取订单详情
		getOrderInfo() {
			this.lsLoading = true
			...
			if (data.jointNo) {
				...
				if (!userEditRoles.length) {
					...
					if (['draft', 'submit', 'refuse', 'close'].includes(data.orderStatus)) {
						...
						// 进口拼箱-
						if (['ship_import_fcl', 'ship_import_lcl'].includes(data.businessType)) {

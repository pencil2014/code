<template>
	<div class="app-container" id="putApply">
		<el-tabs v-model="activeName" @tab-click="handleClick" class="put-el-tabs">
			<el-tab-pane label="放单申请" name="Apply"></el-tab-pane>
			<el-tab-pane label="放单审核" name="Audit"></el-tab-pane>
		</el-tabs>
		<keep-alive>
...
import ApplyList from '@/views/order/order/orderList/blFinApplyList/index'
import AuditList from '@/views/order/order/orderList/blFinList/index'
	...
	methods: {
		...
		init() {
			if (this.$route.query.tab === 'audit') {
				...
				this.$nextTick(() => {
					this.$refs.Apply.externalSearch()
				})
			}
		},
		refresh() {

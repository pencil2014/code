...
<script>
import General from './emc/general'
import Transport from './emc/transport'
import Particulars from './emc/particulars'
import Totals from './components/totals'
import Charges from './components/charges'
import Bl from './emc/bl'
...
export default {
  ...
  methods: {
      ...
      blDetail(data)
        .then((res) => {
          if (res.code === 0) {
            ...
            if (blContact) {
              const { shipper, consignee, notify1, notify2 } =
                blContact
              if (shipper.length) {
    ...
    submitOriginal(type) {
      ...
      let particularsType = this.particularsOption.map(item => item.containers).flat(Infinity).some(ele => !ele.sn || !ele.cn)
      if (particularsType) {
        document.getElementsByClassName('particulars-cnt')[0].scrollIntoView()
        return this.$msgErrClose('The Container Number / Seal Number is Required')
      }

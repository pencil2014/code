<template>
  <div class="packing-cnt">
    <div class="cnt-box">
      ...
      <div class="sub-title" v-if="showAdditional">Additional</div>
      <div class="control items" v-if="showAdditional">
        <Additional
          :additionalOption.sync="additionalOption"
          :transportOption.sync="transportOption"
          ref="additional"
         />
      </div>
      ...
      <div class="bl items">
        <Bl :blOption.sync="blOption" ref="blForm" :replaceChinese="replaceChinese"/>
      </div>
...
<script>
import General from './hmm/general'
import Transport from './components/transport'
import Additional from './components/additional'
import Particulars from './emc/particulars'
import Totals from './components/totals'
import Charges from './components/charges'
import Bl from './components/bl'
import Notification from './components/notification'
import Preview from './components/preview'
import { shipId, siOriginalHmm, dictAll, siDetail } from '@/api/order/createBooking'
...
export default {
  ...
  computed: {
    showAdditional() {
      return this.transportOption &&
             ...
             ['IN', 'EG'].includes(this.transportOption.pod.locationCode.countryCode)
    },
    showAdditionalIN() {
      return this.transportOption &&
             this.transportOption.pod &&
  ...
  methods: {
    ...
    getDataSource (shipId) {
      ...
      siDetail(data).then(res => {
        ...
        if (this.showAdditional) {
          for (let param in this.additionalOption) {
            this.additionalOption[param] = source[param] || ''
          }
        }
          ...
          if (res.code === 0) {
            ...
            if (blContact) {
              ...
              if (shipper.length) {
                ...
                this.generalOption.shipper.name = arr.join('\n')
              }
              if (consignee.length) {
                const arr = consignee.split(/\r?\n/)
                this.generalOption.consignee.name = arr.join('\n')
              }
              if (notify1.length) {
                const arr = notify1.split(/\r?\n/)
                this.generalOption.notifyParty.name = arr.join('\n')
              }
              if (notify2.length) {
                const arr = notify2.split(/\r?\n/)
                this.generalOption.notifyParty2.name = arr.join('\n')
              }
              if (notify3.length) {
                const arr = notify3.split(/\r?\n/)
                this.generalOption.notifyParty3.name = arr.join('\n')
              }
    ...
    submitOriginal(type) {
      ...
      // additional
      if (this.showAdditional) {
        const addition = this.$refs.additional.validateForm()
        if (!addition) {
          document.getElementsByClassName('additional-cnt')[0].scrollIntoView()
          return this.$msgErrClose('Additional is Required')
        }
      }
    ...
    submitOriginalFn(type) {
      ...
      consignee.address = ''
      notifyParty.address = ''
      notifyParty2.address = ''
      notifyParty3.address = ''
      shipTo.address = ''
      ...
      if (this.showAdditional) {
        if (this.showAdditionalIN) {
          additionalOption = {
            gstNo: this.additionalOption.gstNo,
            iecNo: this.additionalOption.iecNo,
            ioeId: this.additionalOption.ioeId,
            panNo: this.additionalOption.panNo,
            invokeCurrencyCode: this.additionalOption.invokeCurrencyCode,
            invokeValue: this.additionalOption.invokeValue,
          }
        } else if (this.showAdditionalEG) {
          additionalOption = {
            exporterRegisterNo: this.additionalOption.exporterRegisterNo,
            importerVatNo: this.additionalOption.importerVatNo,
            acidNo: this.additionalOption.acidNo,
          }
        }
      }
      const data = {

...
<script>
import General from './leyuTsl/general'
import Transport from './leyuPil/transport'
import Particulars from './leyuTsl/particulars'
import Totals from './components/totals'
import Charges from './leyuPil/charges'
import Bl from './leyuEsl/bl'
import Notification from './components/notification'
import Preview from './components/preview'
import { shipId, siOriginalLeyu, dictAll, siDetail } from '@/api/order/createBooking'
...
export default {
  ...
  data() {
    return {
      ...
      notificationOption: {
        notifyEmails: [{ name: this.$store.state.user.userInfo.email }],
  ...
  methods: {
    ...
    getDataSource (shipId) {
      ...
      siDetail(data).then(res => {
        ...
        localPaymentPlace && Object.assign(this.chargesOption,{
          localPaymentPlace: {
            locationCode: {
              cityUnCode: localPaymentPlace.locationCode,
              cityName: localPaymentPlace.locationName,
              countryName: localPaymentPlace.country,
              countryCode: localPaymentPlace.countryCode
            }
          }
        })
        bookingOffice && Object.assign(this.chargesOption,{
          bookingOffice: {
            locationCode: {
          ...
          if (res.code === 0) {
            ...
            if (blCargos.length) {
              blCargos.forEach((item) => {
                const { containerType, quantity, sn, cn, volume, weight } = item
                containers.push({
                  containerType,
                  quantity,
                  volume,
            ...
            this.blOption.blType = '706' || blMode
            ...
            if (blContact) {
              ...
              this.generalOption.mainBlNo = blNo
            }
            if (orderInfo) {
              const {
                vessel,
    ...
    submitOriginal(type) {
      ...
      if (
        reg.test(JSON.stringify(this.$refs.generalForm.$data.form.carrierScac))
      ) {
        document.getElementsByClassName('general-cnt')[0].scrollIntoView()
        return this.$msgErrClose(
      ...
      if (particularsError) {
        ...
        return this.$msgErrClose(
          'The Cargo Description or Marks  in Particulars is incorrect'
        )
      }
      ...
      let flag = true
      let list = this.$refs.particularsForm.$data.form.list
      list.forEach((item) => {
        const cargoQ = Number(item.cargos[0].quantity)
        const contatinerQ = item.containers.reduce(
          (pre, cur) => pre + Number(cur.quantity),
          0
        )
        if (cargoQ !== contatinerQ) {
          flag = false
        }
      })
      if (!flag) {
        document.getElementsByClassName('particulars-cnt')[0].scrollIntoView()
        return this.$msgErrClose(
          'Package count must be equal to the sum of container quantity'
        )
      }
      // Freight Charges
    ...
    submitOriginalFn(type) {
      ...
      const { blType, remarks } = this.blOption
      const {
      ...
      const data = {
        ...
        localChargeTerm,
        localPaymentPlace: {
          locationCode: localPaymentPlace.locationCode.cityUnCode,
          locationName: localPaymentPlace.locationCode.cityName,
          country: localPaymentPlace.locationCode.countryName,
          countryCode: localPaymentPlace.locationCode.countryCode,
        },
        bookingOffice: {
      ...
      data.cc.forEach((item) => {
        ...
        item.cargos.forEach((ele) => {
          ele.hsCode = ele.hsCode ? ele.hsCode.replaceAll('\t', ' ') : ''

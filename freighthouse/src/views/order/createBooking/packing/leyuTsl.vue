...
import Transport from './leyuTsl/transport'
import Particulars from './leyuTsl/particulars'
import Totals from './components/totals'
import Charges from './leyuTsl/charges'
import Bl from './leyuTsl/bl'
  ...
  methods: {
    ...
    submitOriginal(type) {
      ...
      if (!sta3) {
        document.getElementsByClassName('particulars-cnt')[0].scrollIntoView()
        return this.$msgErrClose(
          'Cargo and container are required. The number of cargo must be equal to the total number of containers'
        )
      }
    ...
    submitOriginalFn(type) {
      ...
      const { chargeType, chargeTerm } = this.chargesOption

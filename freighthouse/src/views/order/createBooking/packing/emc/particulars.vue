<!--
 * @Author: Sivan
 * @Date: 2022-05-23 14:43:14
 * @LastEditTime: 2022-06-16 11:18:22
 * @LastEditors: Sivan
-->
<template>
        ...
        <div class="cnt-left">
          <div class="description-box">
            <div class="cargo-box" v-for="(option, i) in item.cargos" :key="i">
              <div class="cargo-cnt">
                <el-form-item
                  label="Cargo Description  (35 X 999 lines)"
                ...
                >
                  <el-input
                    ...
                    @blur="handleTextareaDes(index, i, 'description', 35, 999)"
                    :class="{ 'is-exceed': descriptionErr[index+`${i}`] }"
                  ></el-input>
                  <p class="error-tips" v-show="descriptionErr[index+`${i}`]">
                    This field can accept only 999 lines of 35 characters
                  </p>
                </el-form-item>
                <el-form-item
                  :label="`Marks & Numbers (${markLenLimit} X 999 lines)`"
                ...
                >
                  <el-input
                    ...
                    @blur="handleTextareaMark(index, i, 'marks', 999)"
                    :class="{ 'is-exceed': marksErr[index+`${i}`] }"
                    :autosize="{ minRows: 7, maxRows: 7 }"
                    
                  ></el-input>
                  <p class="error-tips" v-show="marksErr[index+`${i}`]">
                    {{`This field can accept only 999 lines of ${markLenLimit} characters`}}
                  </p>
                </el-form-item>
                <el-row :gutter="20">
                  <el-col :span="6">
...
export default {
  ...
  methods: {
    checkChinese() {
      ...
      this.form.list.forEach((item1, index) => {
        (item1.cargos || []).forEach((item2, i) => {
          item2.description && this.handleTextareaDes(index, i, 'description', 35, 999)
          item2.marks && this.handleTextareaMark(index, i, 'marks', 999)
        })
      })
    },
    // 搜索编码
    ...
    handleTextareaDes(index, i, type, num, line) {
      this.handleTextarea(index, i, type, num, line)
    },
    handleTextareaMark(index, i, type, line) {
      let num = Number(this.markLenLimit)
      this.handleTextarea(index, i, type, num, line)
    },
    ...
    // textarea格式化
    handleTextarea(index = 0, i = 0, type, num = 35, line = 999) {
      let res = []

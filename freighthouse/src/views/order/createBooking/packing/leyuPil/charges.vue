<template>
  <div class="charges-cnt">
    ...
    >
      ...
      <el-form-item label="Local Payment Term" prop="localChargeTerm">
        <el-select
          class="large-select"
          clearable
          placeholder="Select One"
          size="mini"
          v-model="form.localChargeTerm"
        >
          <el-option
            :key="item.value"
            :label="item.label"
      ...
      <el-form-item
        label="Local Payment Office"
        prop="localPaymentPlace.locationCode"
      >
        <el-select
          :loading="loading"
          :remote-method="getCityList"
          ...
          reserve-keyword
          size="mini"
          v-model="form.localPaymentPlace.locationCode"
          value-key="cityUnCode"
        >
          <el-option
            :key="item.cityUnCode"
      ...
      <el-form-item label="​PIL Sales" prop="sales">
        <el-select
          v-model="form.sales"
          placeholder="Select One"
          filterable
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        'localPaymentPlace.locationCode': {
          required: true,
          message: 'Required Field',
          trigger: 'change',
        },
    ...
    dictAll: {
      handler(val) {
        if (val.dictList.length) {
          ...
          // 初始化sales
          const source3 = val.dictList.filter(
            (item) => item.groupCode === 'leyu_pil_sales'
          )[0]
          this.salesList = source3.dictItems.map((item) => {
  ...
  methods: {
    ...
    initData() {
      ...
      if (shipmentId) {
        ...
        this.form.localPaymentPlace.locationCode && this.cityList.push(this.form.localPaymentPlace.locationCode)
        this.form.bookingOffice.locationCode && this.cityList.push(this.form.bookingOffice.locationCode)
  ...
  mounted() {
    this.$EventBus.$on('changePol', (val) => {
      const find = this.cityList.find(
        (item) => item.cityUnCode === val.cityUnCode
      )
      if (!find) {
        this.cityList.push(val)
      }
      this.form.releaseOffice.locationCode = Object.assign({}, val)
    })
  },
}
</script>

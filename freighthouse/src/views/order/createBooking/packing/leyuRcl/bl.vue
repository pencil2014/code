<template>
  <div class="bl-cnt">
    ...
    >
      ...
      <div class="count-box">
        <el-form-item
          label="Telex release office"
          prop="telexReleaseOffice.locationCode"
          :required="form.blType === '710'"
        >
          <el-select
            :loading="loading"
...
export default {
      ...
      handler(val) {
        if (val.dictList.length) {
          ...
          let dictItems = source.dictItems.filter(
            (ele) => ele.descpt !== 'TLX RELEASE'
          )
          this.blTypeList = dictItems.map((i) => {
            const { descpt, itemCode } = i
            return {
              label: `${descpt}`,
              value: itemCode,
            }
  ...
  methods: {
    ...
    initData() {
      ...
      let source = this.blTypeList.find((item) => item.value === blType)
      if (source) {
        this.form.blType = source.value
      } else {
        this.form.blType = '706'
        this.$nextTick(() => {

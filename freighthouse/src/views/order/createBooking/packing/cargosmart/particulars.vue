        ...
        <div class="cnt-left">
          <div class="description-box">
            <div class="cargo-box" v-for="(option, i) in item.cargos" :key="i">
              <div class="cargo-cnt">
              <el-form-item
                label="Cargo Description  (30 X 99 lines)"
              ...
              >
                <el-input
                  ...
                  @blur="handleTextarea(index, i, 'description', 30, 99)"
                  :class="{ 'is-exceed': descriptionErr[index+`${i}`] }"
                ></el-input>
                <p class="error-tips" v-show="descriptionErr[index+`${i}`]">
                  This field can accept only 99 lines of 30 characters
                </p>
              </el-form-item>
              <el-form-item
                label="Marks & Numbers (15 X 99 lines)"
              ...
              >
                <el-input
                  ...
                  @blur="handleTextarea(index, i, 'marks', 15, 99)"
                  :class="{ 'is-exceed': marksErr[index+`${i}`]}"
                  :autosize="{ minRows: 7, maxRows: 7 }"
                ></el-input>
                <p class="error-tips" v-show="marksErr[index+`${i}`]">
                  This field can accept only 99 lines of 15 characters
                </p>
              </el-form-item>
              <el-row :gutter="20">
                <el-col :span="6">
                  <el-form-item label="HS Code" class="middle-item">
                    <el-select
                      :loading="loading"
          ...
          <div class="info-box">
            ...
            >
                    ...
                    <el-row :gutter="20">
                      <el-col :span="12">
                        ...
                        >
                          ...
                          ></el-input>
                          <el-select
                            :remote-method="getContainerTypeList"
              ...
              <div class="btn-box">
                <i
                  @click="addItem(index, item.containers)"
...
export default {
  ...
  watch: {
    ...
    'form.list': {
      handler(val) {
        if (val.length) {
          val.forEach(item => {
            if (item.cargos.length > 1) {
              item.containers[0].volume = item.cargos.reduce((pre,next) => {
                return pre += Number(next.volume)
              }, 0)
              item.containers[0].weight = item.cargos.reduce((pre,next) => {
                return pre = new LSnum(pre).plus(next.weight).toNum(3)
              }, 0)
               item.containers[0].quantity = item.cargos.reduce((pre,next) => {
  ...
  methods: {
    checkChinese() {
      ...
      this.form.list.forEach((item1, index) => {
        (item1.cargos || []).forEach((item2, i) => {
          item2.description && this.handleTextarea(index, i, 'description', 30, 99)
          item2.marks && this.handleTextarea(index, i, 'marks', 15, 99)
        })
      })
    },
    // 搜索编码
    ...
    handleTextarea(index = 0, i = 0, type, num = 35, line = 99) {
      ...
      // this[`${type}Err`][index+`${i}`] = res.length > line
      this.form.list[index].cargos[i][type] = res.join('\n')
      if ( type==='marks' && res.some(ele => !ele) && i===0) {
    ...
    showGoodslDialog(index, y) {
      ...
      this.dialogGoodsOptions.form =
        this.form.list[index].cargos[y].dangerousDto
    ...
    showContainerDialog(index, y) {
      ...
      this.dialogContainerOptions.form =
        this.form.list[index].containers[y].reeferDto
      this.dialogContainerOptions.show = true
    },
    // 提交弹窗
    submitContainerDialog(data) {
      const [x, y] = this.dialogContainerOptions.index
      this.form.list[x].containers[y].reeferDto = data

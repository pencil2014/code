  ...
  >
    <div class="cnt-box">
      ...
      <div class="reference items">
        <Reference :referenceOption.sync="referenceOption" ref="referenceForm" />
      </div>
      <div class="sub-title">
      ...
      <div class="sub-title">
        ...
        <span class="tips">
          **NOTE: The sum of all Cargo Weights reflects the Gross Weight of the Cargo (excluding
          Tare) for the entire Booking. Inaccurate declaration of cargo weight may endanger crew,
          port worker, and vessel safety. Please verify the reasonable accuracy of this information
          prior to submission.
        </span>
      </div>
      ...
      <div class="container items">
        <Container :containerOption.sync="containerOption" ref="containerForm" />
      </div>
    ...
    <Schedule
      v-if="scheduleOptions.show"
      :scheduleOptions="scheduleOptions"
      @cancle="closeSchedule"
...
<script>
import Overall from './msk/overall'
import Remarks from './components/remarks'
import Reference from './components/reference'
import Payment from './emc/payment'
import Container from './msk/container'
import Goods from './msk/goods'
import Transport from './components/transport'
import Interested from './emc/interested'
import { shipId, booker, bkgOriginal, dictAll, bkgDetail } from '@/api/order/createBooking'
...
export default {
  ...
  methods: {
    ...
    getDataSource(shipId) {
      ...
      bkgDetail(data)
        .then((res) => {
          ...
          let bkoffice = {
            cityUnCode: bookingOffice.locationCode,
            cityName: bookingOffice.locationName,
            countryName: bookingOffice.country,
            countryCode: bookingOffice.countryCode,
          }
          this.overallOption = Object.assign(this.overallOption, {
            ...
            bookingNo,
            bookingOffice: {
              locationCode: bkoffice,
              locationNmae: '',
              country: '',
              countryCode: '',
            },
          })
          this.$refs.overallForm.$data.cityList = [bkoffice]
          ...
          this.$refs.paymentForm.initData()
      ...
      bookInfo(data)
        .then((res) => {
          if (res.code === 0) {
            ...
            const { bkgRemark, ship, soContainer, cargoList, transport, Barge } = source
            ...
            this.transportOption.placeOfReceipt.locationCode = porPortCode || polPortCode
            ...
            if (soContainer.length) {
              const containerList = soContainer.map((item) => item.containerList).flat(Infinity)
    ...
    submitOriginal(type) {
      ...
      if (!sta2) {
        document.getElementsByClassName('intereste-cnt')[0].scrollIntoView()
        return this.$msgErrClose('Shipper and Booking Agent is required')
      }
      if (reg.test(JSON.stringify(this.$refs.interestedForm.$data.form))) {
        document.getElementsByClassName('intereste-cnt')[0].scrollIntoView()
        return this.$msgErrClose('Chinese characters and punctuation are not allowed for Parties')
      }
      // 校验运输
      ...
      if (reg.test(JSON.stringify(this.$refs.transportForm.$data.form))) {
        document.getElementsByClassName('transport-cnt')[0].scrollIntoView()
        return this.$msgErrClose('Chinese characters and punctuation are not allowed for Transport')
      }
      // 货物
      ...
      if (reg.test(JSON.stringify(this.$refs.goodsForm.$data.form))) {
        ...
        return this.$msgErrClose('Chinese characters and punctuation are not allowed for Cargo')
      }
      // 集装箱
      ...
      if (sta6) {
        return this.$msgErrClose('Charge Type and Payment Term must be completed together')
      }
      // 邮箱
      const sta7 = this.$refs.remarksForm.validateForm()
      if (!sta7) {
        return this.$msgErrClose('BKG Requestor Emails is required')
      }
    ...
    submitOriginalFn(type) {
      this.isLoading = true
      const { carrierName, carrierScac, contractNo, bookingOffice } = this.overallOption
      const { shipper, consignee, notifyParty, notifyParty2, notifyParty3, forwarder } =
        this.interestedOption
      const { freightForwardNo, blNo } = this.referenceOption
      const { transport, placeOfDelivery, placeOfReceipt, etd, eta, pol, pod, transportDetail } =
        this.transportOption
      const { chargeType, chargeTerm, paymentPlace, releaseOffice } = this.paymentOption
      ...
      data.notifyParty = JSON.stringify(data.notifyParty) === noticeStr ? null : data.notifyParty
      data.notifyParty2 = JSON.stringify(data.notifyParty2) === noticeStr ? null : data.notifyParty2
      data.notifyParty3 = JSON.stringify(data.notifyParty3) === noticeStr ? null : data.notifyParty3
      ...
      data.consignee = JSON.stringify(data.consignee) === consigneeStr ? null : data.consignee
      data.shipper = JSON.stringify(data.shipper) === consigneeStr ? null : data.shipper

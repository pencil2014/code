...
import Transport from './leyuRcl/transport'
import Payment from './leyuRcl/payment'
import Goods from './leyuTsl/goods'
...
export default {
  ...
  methods: {
    ...
    submitOriginal(type) {
      ...
      if (!sta4) {
        return this.$msgErrClose('Cargo Outer Package Count/Type is required')
      }
      ...
      //支付
      const { payLocation, chargeTerm } = this.paymentOption
      const sta6 = !payLocation || !chargeTerm
      if (sta6) {
        return this.$msgErrClose(
    ...
    submitOriginalFn(type) {
      ...
      const { chargeTerm, payLocation, bookingOffice } = this.paymentOption
      ...
      const data = {
        ...
        contractNo,
        bookingOffice: {
          locationCode: bookingOffice?.cityUnCode,
          locationName: bookingOffice?.cityName,
          country: bookingOffice?.countryName,

    ...
    dictAll: {
      handler(val) {
        if (val.dictList.length) {
          ...
          const descpts = ['CY-CY', 'CY-DOOR', 'CY-FREE OUT']
          const dictItems = source.dictItems.filter((item) =>
          ...
          this.blTypeList = source2.dictItems.map((item) => {
            const { descpt, itemCode } = item
            return {
              label: `${descpt}`,
              value: itemCode,
  ...
  methods: {
    ...
    changePol(val = '') {
      if (typeof val === 'object') {
        const find = this.releaseCityList.find(
          (item) => item.cityUnCode === val.cityUnCode
        )
        if (!find) {
          this.releaseCityList.push(val)
        }
        this.form.releaseLocation.locationCode = Object.assign({}, val)
      }
    },
    // 港口下拉

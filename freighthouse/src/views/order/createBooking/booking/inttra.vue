  ...
  >
    <div class="cnt-box">
      ...
      <div class="sub-title">
        ...
        <span class="tips">Shipper or Forwarder is required.</span>
      </div>
...
import Payment from './inttra/payment'
...
export default {
  ...
  methods: {
      ...
      bkgDetail(data)
        .then((res) => {
          ...
          let {
            ...
            shipFrom,
            shipTo
          } = JSON.parse(res.data.requestData) 
          ...
          payLocation && Object.assign(this.paymentOption, {
              payLocation: {
                locationCode: {
                  cityUnCode: payLocation.locationCode,
                  cityName: payLocation.locationName,
                  countryName: payLocation.country,
                  countryCode: payLocation.countryCode,
                },
              },
            })
          this.$refs.paymentForm.initData()
          this.containerOption = containers.map(item => {
            if (!item.reeferDto) {
              item.reeferDto = {
                humidity: '',
                airFlow: '',
          ...
          this.goodsOption = cargos.map(item => {
    ...
    submitOriginal(type) {
      ...
      if (!sta2) {
        document.getElementsByClassName('intereste-cnt')[0].scrollIntoView()
        return this.$msgErrClose('Shipper or Forwarder is required')
      }
      ...
      //   )
      // }
      const sta6 = this.$refs.paymentForm.validateForm()
      if (!sta6) {
        return this.$msgErrClose('Charge Type and Payment Term is required')
      }
      // 邮箱
    ...
    submitOriginalFn(type) {
      this.isLoading = true
      const { carrierName, carrierScac, contractNo, bookingOffice } =
        this.overallOption
      const {
        shipper,
        forwarder,
      ...
      const { chargeType, chargeTerm, payLocation } = this.paymentOption

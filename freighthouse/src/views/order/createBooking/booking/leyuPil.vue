<template>
  ...
  >
    ...
    <div class="general-details items">
      <General
    ...
    <div class="transport items">
      <Transport :transportOption.sync="transportOption" ref="transportForm" @showSchedule="showSchedule" />
    </div>
    <div class="sub-title">
...
<script>
import General from './leyuPil/general'
import Interested from './leyuRcl/interested'
import Transport from './leyuPil/transport'
import Payment from './leyuPil/payment'
import Goods from './leyuPil/goods'
import Container from './leyuRcl/container'
...
export default {
  ...
  methods: {
      ...
      bookInfo(data)
        .then((res) => {
          if (res.code === 0) {
            ...
            const { consignee, notify1, notify2, notify3 } = transport
    ...
    submitOriginal(type) {
      ...
      if (
        reg.test(JSON.stringify(this.$refs.overallForm.$data.form.carrierScac))
      ) {
        document.getElementsByClassName('overall-cnt')[0].scrollIntoView()
      ...
      if (!sta2) {
        document.getElementsByClassName('intereste-cnt')[0].scrollIntoView()
        return this.$msgErrClose('Shipper, Consignee, Notify Party is required')
      }
      ...
      if (!sta4) {
        return this.$msgErrClose('Cargo is required')
      }
      let { cargoName, cargoCname, mark } = this.$refs.goodsForm.$data.form[0]
      if (reg.test(cargoName) || reg.test(mark)) {
        document.getElementsByClassName('goods-cnt')[0].scrollIntoView()
        return this.$msgErrClose(
          'Chinese characters and punctuation are not allowed for Cargo Name and Marks'
        )
      }
      if (!reg.test(cargoCname)) {
        document.getElementsByClassName('goods-cnt')[0].scrollIntoView()
        return this.$msgErrClose(
          'The commodity (CN) of the Cargo must be entered in Chinese'
        )
      }
      // 集装箱
      ...
      if (sta6) {
        return this.$msgErrClose(
          'Payment Location and Payment Term  is required'
        )
      }
      // 邮箱
    ...
    submitOriginalFn(type) {
      ...
      const data = {
        ...
        blType,
        releaseLocation: {
          locationCode: releaseLocation.locationCode.cityUnCode,
          locationName: releaseLocation.locationCode.cityName,
          country: releaseLocation.locationCode.countryName,
          countryCode: releaseLocation.locationCode.countryCode,
        },
        placeOfDelivery: {
        ...
        chargeTerm,
        payLocation: {
          locationCode: payLocation.locationCode.cityUnCode,
          locationName: payLocation.locationCode.cityName,
          country: payLocation.locationCode.countryName,
          countryCode: payLocation.locationCode.countryCode,
        },
        localChargeTerm,
        localPayLocation: {
          locationCode: localPayLocation.locationCode.cityUnCode,
          locationName: localPayLocation.locationCode.cityName,
          country: localPayLocation.locationCode.countryName,
          countryCode: localPayLocation.locationCode.countryCode,
        },

  ...
  <div class="particulars-cnt">
    <el-form :inline="true" :model="form" label-position="top" ref="form" size="mini" class="form">
      <div class="Containers-item-title">
        ...
        <div>
          <el-button type="text" size="mini" @click="addCargosItemOuter">+ Add Cargos</el-button>
        </div> 
      </div>
       <div
        class="container-box"
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-form-item
                :prop="`list.${index}.cargos.${scope.$index}.hsCode`"
                :show-message="false"
                class="small-item"
                :rules="getFiedleConfigRequired('cargos.hsCode')"
              >
                <el-select
                  :loading="loading"
          ...
          <el-table-column
            prop="description"
            :label= "'Cargo Description' + getLinesTip('cargos.description', 'label')"
            >
            <template slot="header">
              <span>
                  <span class="red" v-if="getFiedleConfigRequired('cargos.description')">*</span> 
                  <span class="label-title-class">{{ 'Cargo Description' + getLinesTip('cargos.description', 'label')}}</span>
              </span>
            </template>
            <template slot-scope="scope">
              <el-form-item
                :prop="`list.${index}.cargos.${scope.$index}.description`"
                :show-message="false"
                 style="width:100%;"
                :rules="getFiedleConfigRequired('cargos.description')"
              >
                <el-input
                   style="width:100%;"
                  ...
                  @blur="handleTextarea('description', 'cargos.description', scope.$index )"
                ></el-input>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column
            prop="marks"
            :label="'Marks & Numbers' + getLinesTip('cargos.marks', 'label')"
            >
            <template slot="header">
              <span>
                  <span class="red" v-if="getFiedleConfigRequired('cargos.marks')">*</span> 
                  <span class="label-title-class">{{ 'Marks & Numbers' + getLinesTip('cargos.marks', 'label')}}</span>
              </span>
            </template>
            <template slot-scope="scope">
              <el-form-item
                :prop="`list.${index}.cargos.${scope.$index}.marks`"
                :show-message="false"
                 style="width:100%;"
                :rules="getFiedleConfigRequired('cargos.marks')"
              >
                <el-input
                  style="width:100%;"
                  ...
                  @blur="handleTextarea('marks', 'cargos.marks', scope.$index)"
                  :autosize="{ minRows: 4, maxRows: 4 }"
                ></el-input>
              </el-form-item>
            </template>
          ...
          >
           <template slot-scope="scope">
             <el-button type="text" size="mini" @click="editDangerous(scope)">Dangerous</el-button>
             <el-button type="text" size="mini" @click="editReefer(scope)">Reefer</el-button>
             <el-button class="red" type="text" size="mini" v-if="item.cargos.length > 1" @click="removeCargosItemOuter(index, scope.$index)">Delete</el-button>
           </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    />
    <DangerousDialog 
      v-if="dialogDangerousOptions.show"
      :dialogDangerousOptions="dialogDangerousOptions"
      :shipCarrierCode="shipCarrierCode"
      :dictAll="dictAll"
      @close="dialogDangerousClose"
    />
    <ReeferDialog 
      v-if="dialogReeferOptions.show"
      :dialogReeferOptions="dialogReeferOptions"
      :shipCarrierCode="shipCarrierCode"
      :dictAll="dictAll"
      @close="dialogReeferClose"
    />
  </div>
</template>
...
import DangerousDialog from './dangerousDialog'
import ReeferDialog from './reeferDialog'
export default {
  ...
  methods: {
    ...
    checkChinese() {
      ...
      this.form.list.forEach((item1, index) => {
        (item1.cargos || []).forEach((item2, i) => {
          item2.description && this.handleTextarea('description', 'cargos.description', i)
          item2.marks && this.handleTextarea('marks', 'cargos.marks', i)
        })
      })
    },
    // 搜索编码
    ...
    // textarea格式化
    handleTextarea(type, fieldName, ind) {
      let res = []
      // const reg = new RegExp('(.{' + num + '})', 'g')
      const val = this.form.list[0].cargos[ind][type]
      const source = this.replaceChinese(val)
      ...
      this.form.list[0].cargos[ind][type] = res.join('\n')
      // 汉字检测
      if (this.isCheck) {
        // 汉字校验
        const reg =
    ...
    removeCargosItemOuter(index, y) {
      this.$confirmWarn('确定删除此数据吗？', () => this.form.list[index].cargos.splice(y, 1))
    },
    editDangerous(scope) {
      this.dialogDangerousOptions.show = true
      this.dialogDangerousOptions.options = JSON.parse(JSON.stringify(scope.row.dangerous))
      this.dialogDangerousOptions.index = scope.$index
    },
    dialogDangerousClose(action, dangerous) {
      this.dialogDangerousOptions.show = false
      if (action === 'Confirm') {
        let data = this.form.list[0].cargos[this.dialogDangerousOptions.index]
        this.form.list[0].cargos.splice(this.dialogDangerousOptions.index, 1, {
          ...data,
          dangerous,
        })
      }
    ...
    editReefer(scope) {
      this.dialogReeferOptions.show = true
      this.dialogReeferOptions.options = JSON.parse(JSON.stringify(scope.row.reefer))
      this.dialogReeferOptions.index = scope.$index
    },
    dialogReeferClose(action, reefer) {
      this.dialogReeferOptions.show = false
      if (action === 'Confirm') {
        let data = this.form.list[0].cargos[this.dialogReeferOptions.index]
        this.form.list[0].cargos.splice(this.dialogReeferOptions.index, 1, {
          ...data,
          reefer,
        })
      }

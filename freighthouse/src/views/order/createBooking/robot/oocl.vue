...
<script>
import General from './Oocl/general'
import Transport from './Oocl/transport'
import Particulars from './Oocl/particulars'
import Totals from './Oocl/totals'
import Charges from './Oocl/charges'
import Bl from './Oocl/bl'
import Notification from './Oocl/notification'
import Preview from './Oocl/preview'
import { shipId, siOriginal, dictAll, siDetail } from '@/api/order/createBooking'
import { getFieldConfig, getSiInfo, saveSiInfoOocl, updateSiInfoOocl, startSiTask, cancelSiTask, checkAuthInfo } from '@/api/order/robotSi'
...
export default {
  ...
  methods: {
      ...
      blDetail(data)
        .then((res) => {
          if (res.code === 0) {
            ...
            if (blContact) {
              const { shipper, consignee, forwarder, notify1, notify2, notify3 } =
                blContact
              if (shipper.length) {
              ...
              if (forwarder && forwarder.length) {
                const arr = consignee.split(/\r?\n/)
                if (arr.length > -1) {
                  this.generalOption.forwarder.name = arr.slice(0, 1).join('')
                  this.generalOption.forwarder.address = arr.slice(1).join('\n')
                } else {
                  this.generalOption.forwarder.name = forwarder
                }
              }
              if (notify1.length) {
    ...
    submitOriginalFn(type) {
      ...
      let requestMethod = this.oid ? updateSiInfoOocl : saveSiInfoOocl

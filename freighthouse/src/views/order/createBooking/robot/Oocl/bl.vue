<template>
  <div class="bl-cnt">
    ...
    >
      ...
      <div class="type-box">
        <el-form-item label="blReceiveType" prop="blReceiveType">
          <el-select
            class="large-select"
            placeholder="Select One"
            clearable
            v-model="form.blReceiveType"
          >
            <el-option
              :key="item.value"
              :label="item.label"
              :value="item.value"
              v-for="item in blReceiveTypeList"
            ></el-option>
          </el-select>
        </el-form-item>
      </div>
  ...
  methods: {
    getDictData() {
      const source = this.dictAll.dictList.filter(
        (item) => item.groupCode === 'ai_cosco_bl_type'
      )[0]
      ...
      const source2 = this.dictAll.dictList.filter(
        (item) => item.groupCode === 'ai_oocl_receiveType'
      )[0]
      this.blReceiveTypeList = source2.dictItems.map((item) => {
        const { descpt, itemCode, baseItemCode } = item
        return {
          label: `${itemCode}-${descpt}`,
          value: itemCode,

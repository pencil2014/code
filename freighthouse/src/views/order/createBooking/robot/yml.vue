<template>
  <div class="packing-cnt">
    <div class="cnt-box">
      ...
      <div class="sub-title">Handling Instructions</div>
      <div class="notification items">
        <Handling
          v-if="dictAll.dictList.length && !fieldConfigRequesting"
          :handlingOption.sync="handlingOption"
          ref="handlingForm"
          :fieldConfigObj="fieldConfigObj"
          :fieldConfigList="fieldConfigList"
        />
...
<script>
import General from './Yml/general'
import Transport from './Yml/transport'
import Particulars from './Yml/particulars'
import Totals from './Yml/totals'
import Charges from './Yml/charges'
import Bl from './Yml/bl'
import Handling from './Yml/handling'
import Notification from './Yml/notification'
import Preview from './Yml/preview'
import { shipId, siOriginal, dictAll, siDetail } from '@/api/order/createBooking'
import { getFieldConfig, getSiInfo, saveSiInfoYml, updateSiInfoYml, startSiTask, cancelSiTask, checkAuthInfo } from '@/api/order/robotSi'
  ...
  methods: {
    ...
    getSiInfoData(resData) {
      ...
      if(resData.oid) {
        ...
        this.handlingOption = Object.assign(this.handlingOption, {
          isSplit,
          issuePlaceType,
          blOriginalCountType,
          blOriginalCount,
          ...
          blCopyCount,
          blCopyUnfreightCount,
          issuePlace: issuePlace ? issuePlace : { locationName : ''},
        })
        ...
        this.transportOption = Object.assign(this.transportOption,{
            ...
            transportDetail,
            indiaData: indiaData || this.transportOption.indiaData,
            egyptData: egyptData || this.transportOption.egyptData,
            usData: usData || this.transportOption.usData,
          })
          if (containers) {
            this.particularsOption[0].containers = containers
          }
          if (cargos) {
            this.particularsOption[0].cargos = (cargos || []).map(item => {
              if (!item.dangerous) {
                item.dangerous = {
                  classCode: '',
                  imdgUnNo: '',
        ...
        this.chargesOption = Object.assign(this.chargesOption,{
          chargeTerm,
          paymentPlace: paymentPlace ? paymentPlace : { locationName : ''},
        })
    ...
    submitOriginal(type) {
      ...
      const handlingForm = this.$refs.handlingForm.validateForm()
      if (!handlingForm) {
        return this.$msgErrClose('Handling Instructions is Required')
      }
      if (reg.test(JSON.stringify(this.$refs.handlingForm.$data.form))) {
        return this.$msgErrClose(
          'Chinese characters and punctuation are not allowed for Handling Instructions'
        )
      }
      // Notification Emails
    ...
    submitOriginalFn(type) {
      ...
      const data = {
        ...
        egyptData,
        usData,
        containers: particularsOption[0].containers,
        cargos: particularsOption[0].cargos,
        controlTotal: {
          totalContainer,
          totalPackage,
          totalWeight,
      ...
      let requestMethod = this.oid ? updateSiInfoYml : saveSiInfoYml

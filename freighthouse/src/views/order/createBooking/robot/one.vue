    ...
    <div class="cnt-box">
      ...
      <div class="particulars items">
        <Particulars
          ...
          :fieldConfigObj="fieldConfigObj"
          :fieldConfigList="fieldConfigList"
          :podCountryCode="transportOption.pod.locationCode && transportOption.pod.locationCode.countryCode"
        />
      </div>
      ...
      <div class="sub-title">B/L Split Information</div>
      <div class="notification items">
        <BlSplit
          v-if="dictAll.dictList.length && !fieldConfigRequesting"
          :blSplitOption.sync="blSplitOption"
          ref="blSplitForm"
          :fieldConfigObj="fieldConfigObj"
          :fieldConfigList="fieldConfigList"
        />
...
<script>
import General from './One/general'
import Transport from './One/transport'
import Particulars from './One/particulars'
import Totals from './One/totals'
import Charges from './One/charges'
import Bl from './One/bl'
import Notification from './One/notification'
import BlSplit from './One/blSplit'
import Preview from './One/preview'
import { shipId, siOriginal, dictAll, siDetail } from '@/api/order/createBooking'
import { getFieldConfig, getSiInfo, saveSiInfoOne, updateSiInfoOne, startSiTask, cancelSiTask, checkAuthInfo } from '@/api/order/robotSi'
...
export default {
  ...
  data() {
    return {
      ...
      blOption: {
        blType: '',
        remarks: this.remarksDefault,
      },
      remarksDefault:
       `1.BL type：
2.FREIGHT PAID AT SHENZHEN PLACE OF ISSUE:SHENZHEN
3.SHOW THE AGENT DETAILS OF DESTINATION   
4.PLS DON'T SHOW OCEAN FREIGHT ON THE BILL 
5.PLS DON'T SHOW FORWARDER AGENT'S DETAILS ON THE BILL
6.HS CODE :--NOT SHOW `,
      notificationOption: {
  ...
  methods: {
      ...
      if(resData.oid) {
        ...
        let content = JSON.parse(resData.content)
          ...
          this.blOption = Object.assign(this.blOption,{
            blType,
            remarks: remarks || this.remarksDefault,
          })
          this.blSplitOption = {
            typeOfBl,
            splitSequence,
          ...
          this.transportOption = Object.assign(this.transportOption,{
              ...
              transportDetail: Object.assign(transportDetail, {
                iecCode,
                cargoValue,
                cargoValueCurrency,
                acidNo,
        ...
        .then((res) => {
          if (res.code === 0) {
            ...
            if (orderInfo) {
              ...
              this.transportOption.transportDetail = Object.assign(
            ...
            console.log('this.shipCarrierCode', this.shipCarrierCode)
            this.generalOption = Object.assign(this.generalOption,{
    ...
    submitOriginal(type) {
      ...
      const particularsForm = JSON.parse(JSON.stringify(this.$refs.particularsForm.$data.form))
      const particularsFormData = Object.assign(particularsForm.data, { files: [] })
      if (reg.test(JSON.stringify(Object.assign(particularsForm, { data: particularsFormData })))) {
        document.getElementsByClassName('particulars-cnt')[0].scrollIntoView()
        return this.$msgErrClose(
      ...
      const blSplitForm = this.$refs.blSplitForm.validateForm()
      if (!blSplitForm) {
        return this.$msgErrClose('B/L Split Information is Required')
      }
      if (reg.test(JSON.stringify(this.$refs.blSplitForm.$data.form))) {
        return this.$msgErrClose(
          'Chinese characters and punctuation are not allowed for B/L Split Information'
        )
      }
    ...
    submitOriginalFn(type) {
      ...
      const data = {
        ...
        files: this.particularsOption2.files,
        controlTotal: {
          totalContainer,
          totalPackage,
          totalWeight,
      ...
      let requestMethod = this.oid ? updateSiInfoOne : saveSiInfoOne

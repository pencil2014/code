<template>
  <div class="charges-cnt">
    ...
    >
      ...
      <el-form-item label="Freight Payer" prop="cmaFreightPayer">
          <el-select
            class="large-select"
            placeholder="Select One"
            clearable
            v-model="form.cmaFreightPayer"
          >
            <el-option
              :key="item.value"
              :label="item.label"
              :value="item.value"
              v-for="item in cmaFreightPayerLocationList"
            ></el-option>
          </el-select>
        </el-form-item>
      <el-form-item label="Payment Location " prop="paymentPlace.locationName">
          <el-select
            class="large-select"
            clearable
            :loading="loading"
  ...
  methods: {
    getDictData() {
      ...
      const source2 = this.dictAll.dictList.filter(
        (item) => item.groupCode === 'ai_cma_freight_term'
      )[0]
      ...
      const source = this.dictAll.dictList.filter(
        (item) => item.groupCode === 'ai_cma_freight_payer'
      )[0]
      this.cmaFreightPayerLocationList = source.dictItems.map((item) => {
        const { descpt, itemCode, baseItemCode } = item
        return {
          label: `${descpt}`,
          value: itemCode,

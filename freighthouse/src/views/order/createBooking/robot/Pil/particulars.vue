    ...
    <el-form :inline="true" :model="form" label-position="top" ref="form" size="mini" class="form">
      <div class="description-box">
        <div class="cargo-box">
          ...
          <div class="cargo-cnt2">
            <el-form-item
              label= "Package Statement"
              prop="data.basePackageStatement"
              :rules="getFiedleConfigRequired('basePackageStatement')"
            >
             <el-select
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.basePackageStatement"
              >
                <el-option
                  :key="option.value"
                  :label="option.label"
                  :value="option.value"
                  v-for="option in basePackageStatementList"
                ></el-option>
              </el-select>
            </el-form-item>
            <el-form-item
              v-if="form.data.basePackageStatement === 'PIL_WoodOthers'"
              label= "Package Handle"
              prop="data.basePackageHandle"
              :rules="getFiedleConfigRequired('basePackageHandle')"
            >
             <el-input
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.basePackageHandle"
              >
              </el-input>
            </el-form-item>
            <el-form-item
              label= "HS Code"
              prop="data.baseHsCode"
              :rules="getFiedleConfigRequired('baseHsCode')"
            >
             <el-select
                :loading="loading"
            ...
            <el-form-item
              ...
              prop="data.baseCommodity"
              :rules="getFiedleConfigRequired('baseCommodity')"
            >
             <el-input
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.baseCommodity"
              >
              </el-input>
            </el-form-item>
          </div>
          <div class="cargo-cnt2">
            <el-form-item
              label= "IsDag"
              prop="data.cntDag.isDag"
              :rules="getFiedleConfigRequired('cntDag.isDag')"
            >
             <el-select
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.cntDag.isDag"
              >
                <el-option
                  :key="option.value"
                  :label="option.label"
                  :value="option.value"
                  v-for="option in yesOrNoList"
                ></el-option>
              </el-select>
            </el-form-item>
            <el-form-item
              v-if="form.data.cntDag.isDag"
              label= "UnCode"
              prop="data.cntDag.unCode"
              :rules="getFiedleConfigRequired('cntDag.unCode')"
            >
             <el-input
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.cntDag.unCode"
              >
              </el-input>
            </el-form-item>
            <el-form-item
              v-if="form.data.cntDag.isDag"
              label= "ClassName"
              prop="data.cntDag.className"
              :rules="getFiedleConfigRequired('cntDag.className')"
            >
             <el-input
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.cntDag.className"
              >
              </el-input>
            </el-form-item>
            <el-form-item
              v-if="form.data.cntDag.isDag"
              label= "EmergencyTel"
              prop="data.cntDag.emergencyTel"
              :rules="getFiedleConfigRequired('cntDag.emergencyTel')"
            >
             <el-input
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.cntDag.emergencyTel"
              >
              </el-input>
            </el-form-item>
            <el-form-item
              v-if="form.data.cntDag.isDag"
              label= "DG PIC"
              prop="data.cntDag.dgPic"
              :rules="getFiedleConfigRequired('cntDag.dgPic')"
            >
             <el-input
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.cntDag.dgPic"
              >
              </el-input>
            </el-form-item>
            <el-form-item
              v-if="form.data.cntDag.isDag"
              label= "PSA Group"
              prop="data.cntDag.psaGroup"
              :rules="getFiedleConfigRequired('cntDag.psaGroup')"
            >
             <el-input
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.cntDag.psaGroup"
              >
              </el-input>
            </el-form-item>
            <el-form-item
              v-if="form.data.cntDag.isDag"
              label= "Chemical Name"
              prop="data.cntDag.chemicalName"
              :rules="getFiedleConfigRequired('cntDag.chemicalName')"
            >
             <el-input
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.cntDag.chemicalName"
              >
              </el-input>
            </el-form-item>
            <el-form-item
              v-if="form.data.cntDag.isDag"
              label= "Packaging"
              prop="data.cntDag.packaging"
              :rules="getFiedleConfigRequired('cntDag.packaging')"
            >
             <el-select
                class="large-select"
                placeholder
                clearable
                size="mini"
                v-model="form.data.cntDag.packaging"
              >
               <el-option 
                 v-for="(item, index) in packagingList"
                 :key="index"
                 :label="item.label"
                 :value="item.value+''"
               ></el-option>
            ...
            <el-form-item
              v-if="form.data.cntDag.isDag"
              label= "Flash Point"
              prop="data.cntDag.flashPoint"
              :rules="getFiedleConfigRequired('cntDag.flashPoint')"
            >
             <el-input-number
                class="large-select"
                placeholder
                ...
                :max="9999.9"
                :controls="false"
                v-model="form.data.cntDag.flashPoint"
              >
              </el-input-number>
            </el-form-item>
          </div>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot="header">
              <span>
                  <span class="red">*</span> 
                  <span class="label-title-class">IsFull</span>
              </span>
            </template>
            <template slot-scope="scope">
              <el-form-item
                :prop="`list.${index}.containers.${scope.$index}.isFull`"
                :show-message="false"
                class="small-item"
                :rules="{
                  required: true,
          ...
          <el-table-column
            ...
            <template slot="header">
              <span>
                  <span class="label-title-class">SOC</span>
              </span>
            </template>
            <template slot-scope="scope">
              <el-form-item
                :prop="`list.${index}.containers.${scope.$index}.soc`"
                :show-message="false"
                class="small-item"
              >
               <el-checkbox size="mini" v-model="scope.row.soc"></el-checkbox>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column
            ...
            <template slot="header">
              <span>
                  <!-- <span class="red">*</span>  -->
                  <span class="label-title-class">OverWeight</span>
              </span>
            </template>
            <template slot-scope="scope">
              <el-form-item
                :prop="`list.${index}.containers.${scope.$index}.overWeight`"
                :show-message="false"
                class="small-item"
              >
                <el-checkbox size="mini" v-model="scope.row.overWeight"></el-checkbox>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column
  ...
  methods: {
    getDictData() {
      const source = this.dictAll.dictList.filter(
        (item) => item.groupCode === 'ai_pil_package_statement'
      )[0]
      this.basePackageStatementList = source.dictItems.map((item) => {

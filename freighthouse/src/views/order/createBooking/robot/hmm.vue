<template>
  <div class="packing-cnt">
    <div class="cnt-box">
      ...
      <div class="sub-title">Additional</div>
      <div class="control items">
        <Additional
          v-if="!fieldConfigRequesting"
          :additionalInformation.sync="additionalInformation"
          :fieldConfigObj="fieldConfigObj"
          :fieldConfigList="fieldConfigList"
          ref="additional"
         />
      </div>
      <div class="sub-title">Mark & Description</div>
      ...
      <div class="control items">
        <Totals 
           v-if="!fieldConfigRequesting" 
           :totalsOption.sync="totalsOption" 
           :totalsOption2.sync="totalsOption2" 
           :fieldConfigObj="fieldConfigObj"
           ref="totalsForm" 
         />
      </div> -->

      <div class="sub-title">Ocean Freight</div>
      <div class="control items">
        <Charges 
          v-if="dictAll.dictList.length &&!fieldConfigRequesting"
...
import { getFieldConfig, getSiInfo, saveSiInfoHmm, updateSiInfoHmm, startSiTask, cancelSiTask, checkAuthInfo } from '@/api/order/robotSi'
  ...
  methods: {
    ...
    getSiInfoData(resData) {
      ...
      if(resData.oid) {
        ...
        this.additionalInformation = Object.assign(this.additionalInformation, additionalInformation)
        ...
        alsoNotifyParty && Object.assign(this.generalOption,{alsoNotifyParty})
        ...
        this.totalsOption2 = Object.assign(this.totalsOption2,{
          totalContainerDescription,
          commodityForDeclaration,
        }) 
    ...
    submitOriginal(type) {
      ...
      if (reg.test(JSON.stringify(this.$refs.transportForm.$data.form))) {
       ...
       // additional
      const additionalValid = this.$refs.additional.validateForm()
      if (!additionalValid) {
        document.getElementsByClassName('additional-cnt')[0].scrollIntoView()
        return this.$msgErrClose('Additional is Required')
      }
      if (reg.test(JSON.stringify(this.$refs.additional.$data.form))) {
        document.getElementsByClassName('additional-cnt')[0].scrollIntoView()
        return this.$msgErrClose(
          'Chinese characters and punctuation are not allowed for additional'
        )
      }
      // Mark & Description
      ...
      if (particularsError) {
        ...
        return this.$msgErrClose(
          'The Cargo Description or Marks & Numbers in Mark & Description is incorrect'
        )
      }
      ...
      if (!sta3) {
        document.getElementsByClassName('particulars-cnt')[0].scrollIntoView()
        return this.$msgErrClose('Mark & Description is Required')
      }
      if (reg.test(JSON.stringify(this.$refs.particularsForm.$data.form))) {
        document.getElementsByClassName('particulars-cnt')[0].scrollIntoView()
        return this.$msgErrClose(
          'Chinese characters and punctuation are not allowed for Mark & Description'
        )
      }
      // Ocean Freight
      const sta4 = this.$refs.chargesForm.validateForm()
      if (!sta4) {
        return this.$msgErrClose('Ocean Freight is Required')
      }
      if (reg.test(JSON.stringify(this.$refs.chargesForm.$data.form))) {
        return this.$msgErrClose(
          'Chinese characters and punctuation are not allowed for Ocean Freight'
        )
      }
      // B/L Type
    ...
    submitOriginalFn(type) {
      ...
      const data = {
        ...
        totalContainerDescription: this.totalsOption2.totalContainerDescription,
        commodityForDeclaration: this.totalsOption2.commodityForDeclaration,
      ...
      let requestMethod = this.oid ? updateSiInfoHmm : saveSiInfoHmm

  ...
  <div class="particulars-cnt">
    <el-form :inline="true" :model="form" label-position="top" ref="form" size="mini" class="form">
      <div class="description-box">
        ...
        <el-button @click="fileUploadMethod" class="flie-upload-class" type="text" size="mini">附件</el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 删除数据
    // 删除数据
    removeItem(index, y, i) {
      this.$confirmWarn('确定删除此数据吗？', () => this.form.list[index].containers[y].cargos.splice(i, 1))
    },
    ...
    changeCargosPackageType(index, y, i) {
      // if (!this.form.list[index].containers[y].cargos[i].printPackageType) {
      //   const source = this.form.list[index].containers[y].cargos[i].packageType
      //   this.form.list[index].containers[y].cargos[i].printPackageType = source.descpt
      // }
    },
    fileUploadMethod() {
      this.dialogFileOptions.show = true
    },
    closeFileUploadDialog(action, value) {
      this.dialogFileOptions.show = false
      if (action === 'Confirm') {
        this.form.data.files = value
      }
    },
    // 数据验证
    validateForm() {

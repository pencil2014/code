    ...
    >
      <div class="address-box">
        ...
        <div class="item-box">
          ...
          <div class="item-box">
            <div class="cnt-info">
              <el-form-item label="Also Notify Party" prop="alsoNotifyParty.name">
                <div class="label-box" slot="label">
                  <div class="title">Also Notify Party</div>
                  <div class="btn-box">
                    <el-button
                      @click="clearData('alsoNotifyParty')"
                      class="btn-text"
                      icon="el-icon-close"
                      size="mini"
                      type="text"
                ...
                <el-input
                  ...
                  type="textarea"
                  spellcheck="false"
                  v-model="form.alsoNotifyParty.name"
                  :autosize="{ minRows: 0, maxRows: 2 }"
                  
                  @blur="handleTextarea('alsoNotifyParty', 'name', 'alsoNotifyParty.name')"
                ></el-input>
                <p class="error-tips" v-show="alsoNotifyPartynameError">
                  {{ getErrTip('alsoNotifyParty.name')}}
                </p>
              </el-form-item>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      alsoNotifyPartynameError: false,
  ...
  methods: {
    ...
    // textarea格式化
    handleTextarea(type, key, fieldName) {
      let fieldConfig = this.fieldConfigList.find(item => {
        return item.fieldName === fieldName
      }) || {}
      let num = fieldConfig.maxLength || null
      ...
      let source = ''
      if (key) {
        source = this.replaceChinese(this.form[type][key])
      } else {
        source = this.replaceChinese(this.form[type])
      }
    ...
    initData() {
      ...
      this.form.alsoNotifyParty.name &&
        this.handleTextarea('alsoNotifyParty', 'name', 'alsoNotifyParty.name')
      this.isCheck = true
    },
  },
}

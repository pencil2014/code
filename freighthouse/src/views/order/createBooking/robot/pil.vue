<template>
  <div class="packing-cnt">
    <div class="cnt-box">
      ...
      <div class="sub-title">Service Information</div>
      <div class="control items">
        <Charges 
          v-if="dictAll.dictList.length &&!fieldConfigRequesting"
...
<script>
import General from './Pil/general'
import Transport from './Pil/transport'
import Particulars from './Pil/particulars'
import Totals from './Pil/totals'
import Charges from './Pil/charges'
import Bl from './Pil/bl'
import Notification from './Pil/notification'
import Preview from './Pil/preview'
import { shipId, siOriginal, dictAll, siDetail } from '@/api/order/createBooking'
import { getFieldConfig, getSiInfo, saveSiInfoPil, updateSiInfoPil, startSiTask, cancelSiTask, checkAuthInfo } from '@/api/order/robotSi'
...
export default {
  ...
  data() {
    return {
      ...
      chargesOption: {
        ...
        notifyEmails: this.$store.state.user.userInfo.email,
        notifyName: 'Longsail',
      },
      blOption: {
        blType: '',
  ...
  methods: {
    ...
    submitOriginal(type) {
      ...
      if (!sta4) {
        return this.$msgErrClose('Service Information is Required')
      }
      if (reg.test(JSON.stringify(this.$refs.chargesForm.$data.form))) {
        return this.$msgErrClose(
          'Chinese characters and punctuation are not allowed for Service Information'
        )
      }
      // B/L Type
    ...
    submitOriginalFn(type) {
      ...
      let requestMethod = this.oid ? updateSiInfoPil : saveSiInfoPil

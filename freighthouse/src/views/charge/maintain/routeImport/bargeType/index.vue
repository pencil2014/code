<template>
  <div class="ship-type">
    ...
    <el-table v-loading="loading" :data="tabledata" border>
      <el-table-column min-width="55" align="center" type='index' label="序号">
      </el-table-column>
      <el-table-column min-width="189" prop="routeType" align="center" label="类型">
      </el-table-column>
      <el-table-column min-width="220" prop="filePath" align="center" label="附件">
          <template slot-scope="scope">
              <span>{{scope.row.fileName}}</span>
              <el-button @click="handleExport(scope.row)" type="text" size="mini" >下载</el-button>
          </template>
      </el-table-column>
      <el-table-column min-width="80" prop="totalRows" align="center" label="总共条数" >
      </el-table-column>
      <el-table-column min-width="80" align="center" label="成功数">
          <template slot-scope="scope">
              <span style="color:#7BD269">{{scope.row.importSuccessRows}}</span>
          </template>
      </el-table-column>
      <el-table-column min-width="80" align="center" label="失败数">
          <template slot-scope="scope">
              <span style="color:#E63923">{{scope.row.importFailedRows}}</span>
          </template>
      </el-table-column>
      <el-table-column min-width="150" prop="importTime" align="center" label="导入时间">
      </el-table-column>
...
import { routeImportList, fileImportDelete, fileExportErrorList } from '@/api/charge/routeImport'
...
const defaultTableQuery = {
  query: [{"column":"routeType","type":"default","value":"feeder"}], 
}
export default {
    ...
    handleCurrentChange(val) {
      ...
      handleToPublish(row) {
        sessionStorage.setItem('routeImportToFeederRoute', true)
        this.$router.push({
          path:'/charge/maintain/feederRoute',
          query: {
    ...
    handleDelete(row){
      ...
      // 下载按钮操作 
      handleExport(row) {
          this.downloadErrorData(row.fileNo,row.fileName);
      },
      //下载失败

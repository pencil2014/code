<template>
  <div class="filter-container search-layout" id="chargeSearch">
    <div class="filter">
    ...
    <div class="search-fieldValue-cont" v-if="searchValueGroup.length">
      ...
      <template v-for="(item, index) in searchValueGroup">
        <template v-if="item.saveList && item.saveList.length">
          <template v-for="(ele, sIndex) in item.saveList">
            <div class="filedValue-list" :key="'field'+index+sIndex">
              <el-tooltip placement="bottom" effect="dark">
                <div slot="content">{{item.label}}: {{ele.label}}</div>
                <div class="value">{{item.label}}: {{ele.label}}</div>
              </el-tooltip>
              <i class="el-icon-close filter-close" @click="handleDeleteMultiFiledValue(item, ele)"></i>
            </div>
          </template>
        </template>
        <template v-else>
          <div class="filedValue-list" :key="'fieldValue'+index">
            <el-tooltip placement="bottom" effect="dark">
              <div slot="content">{{item.label}}: {{item.fieldValue}}</div>
              <div class="value">{{item.label}}: {{item.fieldValue}}</div>
            </el-tooltip>
...
import SearchBtn from '@/components/Base/Search/btn';
...
export default {
  ...
  computed: {
    ...
    searchValueGroup() {
      // console.log('this.searchFiledGroup', this.searchFiledGroup)
      let result = []
      result = this.searchFiledGroup.filter(item => item.value && (''+item.value)).map(item => {
        ...
        item.saveList = searchItemObj.saveList
  ...
  methods: {
    // 清除多选条件,ele.value为要删除的值
    handleDeleteMultiFiledValue(item, ele) {
      console.log('item', item.value, ele.value)
      if (isArray(item.value)) {
        let tempVal = item.value.filter(t => t !== ele.value) || []
        let tempSaveList = item.saveList.filter(t => t.value !== ele.value) || []
        this.searchFiledGroup.map(o => {
          if (o.key === item.key) {
            o.value = tempVal
          }
          return {...o}
        })
        this.$set(this.filterGroups[item.key], 'saveList', tempSaveList)
      } else {
        this.searchFiledGroup.map(o => {
          if (o.key === item.key) {
            o.value = ''
    ...
    callbackAction(action, param) {
      if (typeof this.callback !== 'function') return
      this.callback(action, param)
    },
    // 重置
    handleReset(action) {
    ...
    setSearchCondition(composeKey, defaultConfigList, changeRole) {
        ...
        if (!item.searchValue) {
          ...
          if (this.isJSON(item.searchValue)) {
            ...
            if (searchValue && typeof(searchValue) === 'object') {
              ...
              if (['remoteSelect', 'select', 'cascader'].includes(this.filterGroups[item.searchField].type)) {
      ...
      this.callbackAction('Update', searchFiledGroup)
      // console.log('搜索条件反显')
      changeRole && this.callbackAction('Search')
    },
    // 保存搜索条件
...
<style lang="scss">
@import '@/components/Base/less/search.scss';
</style>

...
import { cityList, shippingLineList, extraChargeFeeList, sapidList, bookInfo } from '@/api/exec/cosco'
...
export default {
    ...
    // 缓存搜索条件
    let shipLineSearch = window.sessionStorage.getItem('shipLineSearchCosco')
    if (shipLineSearch) {
	...
	methods: {
		...
		//搜索按钮
		handleSearch() {
      window.sessionStorage.setItem('shipLineSearchCosco',JSON.stringify(this.queryForm))

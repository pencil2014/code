				...
				<el-table-column header-align="center" align="left" label="出发地" width="150px">
					<template slot-scope="scope">
						<div v-if="scope.$index % 2 === 1">
							<div class="tableTitle">
								<span v-for="(item, i) in scope.row.scheduleData.legs" :key="i">
									<b>
										{{ item.vessel ? item.vessel.vesselName : '' }}{{ item.externalVoyageNumber ? item.externalVoyageNumber : '' }}/{{ item.service ? item.service.serviceCode : '' }}
										<span v-if="i < scope.row.scheduleData.legs.length - 1">|</span>
									</b>
								</span>
						...
						<div class="shippingLines" v-else>
							...
							{{ scope.row.estimatedTransitTimeInDays }}天
							<!-- 运价价格 -->
							<div class="symbol"></div>
							<div>
								...
								<div class="result-price-footer">
									<div style="position: relative">
										<el-popover placement="bottom-start" popper-class="extraTableClass" trigger="click">
											...
											<el-table :data="extraFeeList" v-loading="loading2">
												...
												<!-- <el-table-column width="200" header-align="center" align="left" label="预付-到付">
													<template slot-scope="scope">
														<el-radio-group @change="handleChange('extraFeeList', scope.row)" v-model="scope.row.paymentTerm">
															<el-radio v-if="scope.row.paymentTerms.includes('P')" label="P">预付</el-radio>
															<el-radio v-if="scope.row.paymentTerms.includes('C')" label="C">到付</el-radio>
														</el-radio-group>
													</template>
												</el-table-column> -->
											</el-table>
...
import { cityList, shippingLineList, extraChargeFeeList, sapidList, bookInfo } from '@/api/exec/oocl'
...
export default {
	...
	filters: {
		billPriceFilter(row) {
			...
			if (row.paymentTerm === 'P') {
				return row.priceTicket.P && (row.priceTicket.P.price === 0 ? 'included' : `${row.priceTicket.P.currency} ${row.priceTicket.P.price}`)
			} else {
				return row.priceTicket.C && (row.priceTicket.C.price === 0 ? 'included' : `${row.priceTicket.C.currency} ${row.priceTicket.C.price}`)
			}
		},
		price20GpFilter(row) {
			if (!row.price20Gp) return
			if (row.paymentTerm === 'P') {
				return row.price20Gp.P && (row.price20Gp.P.price === 0 ? 'included' : `${row.price20Gp.P.currency} ${row.price20Gp.P.price}`)
			} else {
				return row.price20Gp.C && (row.price20Gp.C.price === 0 ? 'included' : `${row.price20Gp.C.currency} ${row.price20Gp.C.price}`)
			}
		},
		price40GpFilter(row) {
			if (!row.price40Gp) return
			if (row.paymentTerm === 'P') {
				return row.price40Gp.P && (row.price40Gp.P.price === 0 ? 'included' : `${row.price40Gp.P.currency} ${row.price40Gp.P.price}`)
			} else {
				return row.price40Gp.C && (row.price40Gp.C.price === 0 ? 'included' : `${row.price40Gp.C.currency} ${row.price40Gp.C.price}`)
			}
		},
		price40HqFilter(row) {
			if (!row.price40Hq) return
			if (row.paymentTerm === 'P') {
				return row.price40Hq.P && (row.price40Hq.P.price === 0 ? 'included' : `${row.price40Hq.P.currency} ${row.price40Hq.P.price}`)
			} else {
				return row.price40Hq.C && (row.price40Hq.C.price === 0 ? 'included' : `${row.price40Hq.C.currency} ${row.price40Hq.C.price}`)
			}
		}
	},
	// table基础组件
    ...
    // 缓存搜索条件
    let shipLineSearch = window.sessionStorage.getItem('shipLineSearchOocl')
    if (shipLineSearch) {
      Object.assign(this.queryForm, JSON.parse(shipLineSearch))
    }
		this.getList()
	},
	computed: {
	...
	methods: {
		...
		//搜索按钮
		handleSearch() {
      window.sessionStorage.setItem('shipLineSearchOocl',JSON.stringify(this.queryForm))
      ...
      if (typeof val !== 'object') {
        return
			...
			if (type === 'por') {
				...
				this.queryForm.porName = val.cityFullNameCn + '-' + val.cityFullNameEn
			} else {
				this.queryForm.fndCityId = val.id
				this.queryForm.fndName = val.cityFullNameCn + '-' + val.cityFullNameEn
			}
		},
		getList() {
      ...
      let {porName, porCityId, fndName, fndCityId} = this.queryForm
      if (porName && !porCityId) {
        return this.$msgErrClose('请选择正确的出发地')
      }
      if (fndName && !fndCityId) {
        return this.$msgErrClose('请选择正确的目的地')
      }
			this.loading = true
			this.initQueryForm()
			shippingLineList(this.tableQuery).then(res => {
				let { totalCount = 0 } = res.data || {}
			...
			for (let i = 0, len = data.length; i < len; i++) {
				...
				if (newEle.routeProductPricingList.length > 0) {
					...
					newEle.routeProductPricingList.map(item => {
						...
						containerOceanFeeMap.set(item.cntrType, pricePrefix + item.tradePrice)
					})
		...
		getExtraFeeList(id) {
			//获取附加费明细数据
      this.loading2 = true
			extraChargeFeeList({ productId: id, sapid: this.queryForm.sapid }).then(res => {
				...
				bookInfo({ productId: id, sapid: this.queryForm.sapid }).then(res => {

<template>
	<div class="edit-container-exec" v-loading="loading">
		<div class="edit-row">
			<div class="row-tit">
						...
						<span
							...
							{{
								detailInfo.orderTime
									? detailInfo.orderTime.substring(0, 10)
									: ""
							}}</span
						>
					</div>
				...
				<section class="orderSection">
					<div class="bookingTable">
						<el-table :data="containerSumTableList" border>
							<el-table-column
								v-for="(item, index) in containerSumTableOptions"
								:key="item.prop + index"
								:label="item.label"
								:align="'center'"
								:header-align="'center'"
			...
			<div class="orderCard">
				<section class="order-section">
					...
					<div class="container-detail">
						<div
							class="container-detail-box"
							v-for="(item, key) in detailInfo.bookingContainers"
							:key="key"
						>
							...
							>
								<template slot-scope="scope">
									<div class="shippingLines">
											...
											<el-row class="time-display">
												<el-col :span="6">
													<div>
														<div class="horizontal-content-div">
															<span
																v-if="
																	scope.row.sailingProduct.scheduleData.legs
																		.length > 1
																"
																>ETD</span
															>
														...
														<div class="horizontal-content-div">
															<span
																...
																"
																>{{
																	scope.row.sailingProduct.scheduleData.legs[0]
																		.pol.etd
																		? scope.row.sailingProduct.scheduleData.legs[0].pol.etd.substring(
																				0,
																				10
																		  )
																		: ""
												...
												<el-col :span="6">
													...
													>
														...
														<div class="horizontal-content-div">
															{{
																scope.row.sailingProduct.scheduleData.legs[1]
																	.pol.etd
																	? scope.row.sailingProduct.scheduleData.legs[1].pol.etd.substring(
																			0,
																			10
																	  )
																	: ""
												...
												<el-col :span="6">
													...
													<div class="horizontal-content-div">
														{{
															scope.row.sailingProduct.eta
																? scope.row.sailingProduct.eta.substring(0, 10)
																: ""
														}}
													</div>
												</el-col>
										...
										<!-- 有效期 -->
										{{
											scope.row.sailingProduct.effectiveStartDate
												? scope.row.sailingProduct.effectiveStartDate.substring(
														0,
														10
												  )
												: ""
										...
										<br />
										{{
											scope.row.sailingProduct.effectiveEndDate
												? scope.row.sailingProduct.effectiveEndDate.substring(
														0,
														10
												  )
												: ""
										...
										{{ scope.row.sailingProduct.scheduleData.voyageNo
										}}{{ scope.row.sailingProduct.scheduleData.direction }}/{{
											scope.row.sailingProduct.scheduleData.serviceCode
										}}
									</div>
								</template>
							</el-table-column>
			...
			<div class="orderCard" style="padding: 0 0 18px 0">
				<!-- <section class="order-section">
					...
					<div
						...
						v-if="
							transferServices.cityFullNameCn || transferServices.cityFullNameEn
						"
					>
						<span style="padding: 0 18px 0 0"
							>目的地：{{
								transferServices.cityFullNameCn ||
								transferServices.cityFullNameEn
							}}</span
						>
						<span style="padding: 0 18px 0 0"
							>运输天数：{{ transferServices.transitTime }}</span
						>
						<span style="padding: 0 18px 0 0"
							>运输条款：{{ transferServices.transportTerms }}</span
						>
						<span style="padding: 0 18px 0 0"
							>转运方式：{{ transferServices.transportMode }}</span
						>
					</div>
				</section> -->
			</div>
...
import { orderDetailInfo } from '@/api/exec/oocl'
import {
...
export default {
    ...
    getData () {
      ...
      this.loading = true
      orderDetailInfo({ orderNo: this.$route.query.orderNo,sapid }).then((res) => {

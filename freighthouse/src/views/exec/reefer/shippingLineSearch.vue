	...
	<div class="menu cosco-page">
			...
			<div class="filter-containe search-layout">
				<div class="filter">
					<div class="search-containe">
						<div>
              <div class="branch-company-wrap filter-item">
								产品类型:
								<el-select default-first-option filterable size="mini" v-model="queryForm.cargoCategory" placeholder="请选择产品类型">
									<el-option v-for="item in dictMap['coscoCargoCategory']" :key="item.value" :label="item.label" :value="item.value"> </el-option>
								</el-select>
							</div>
							<div class="branch-company-wrap filter-item">
								分公司:
		...
		<div class="m-t-8 cosco-box">
			<el-table :data="shippingLineList" border v-loading="loading" :span-method="arraySpanMethod" class="orderTable cosco-table">
				<el-table-column header-align="center" align="left" label="出发地" width="150px">
					<template slot-scope="scope">
						...
						<div class="shippingLines" v-else>
							...
							<div style="width:150px;text-align:center">{{ scope.row.estimatedTransitTimeInDays }}天</div>
							<!-- 运价价格 -->
							<!-- <div class="symbol"></div> -->
							<div>
								<div class="product-price-wrapper">
									<div style="display: flex">
										...
										<div class="priceGroup">
											<div class="priceItem">
												<div class="ectPrice">
													<span>{{ scope.row.containerOceanFeeMap.get('20RF') }} </span>
												</div>
											</div>
										</div>
										<div class="priceGroup">
											<div class="priceItem">
												<div class="ectPrice">
													<span>{{ scope.row.containerOceanFeeMap.get('40RQ') }} </span>
												</div>
											</div>
										</div>
									</div>
									...
									<div style="position: relative">
										<el-popover placement="bottom-start" popper-class="extraTableClass" trigger="click">
											...
											<el-table :data="extraFeeList">
												...
												<el-table-column width="100" property="20RF" label="20RF">
													<template slot-scope="scope">
														{{ scope.row | price20RfFilter }}
													</template>
												</el-table-column>
												<el-table-column width="100" property="40RQ" label="40RQ">
													<template slot-scope="scope">
				...
				<el-table-column header-align="center" align="left" label="预期交货时间" width="150px"> </el-table-column>
				<el-table-column header-align="center" align="left" label="20RF" width="200px"> </el-table-column>
				<el-table-column header-align="center" align="left" label="40RQ" width="200px"> </el-table-column>
...
import { cityList, shippingLineListReefer, extraChargeFeeListReefer, sapidList, bookInfoReefer } from '@/api/exec/cosco'
...
export default {
	...
	methods: {
		...
		//搜索取消按钮
		handleReset() {
      this.queryForm.cargoCategory = 'REEFER'
      ...
      if (fndName && !fndCityId) {
        return this.$msgErrClose('请选择正确的目的地')
			...
			this.loading = true
			this.initQueryForm()
			shippingLineListReefer(this.tableQuery).then(res => {
		...
		getExtraFeeList(id) {
			//获取附加费明细数据
			extraChargeFeeListReefer({ productId: id, sapid: this.queryForm.sapid }).then(res => {
				...
				bookInfoReefer({ productId: id, sapid: this.queryForm.sapid }).then(res => {
...
.menu {
	...
	.filter-containe {
      ...
      margin-right: 10px;
      margin-bottom: 5px;
      .el-input--mini .el-input__inner,.el-input__inner{
        height: 20px;
        line-height: 20px;
      }
		}

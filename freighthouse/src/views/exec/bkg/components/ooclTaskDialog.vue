<template>
	<el-dialog :title="title" :visible.sync="dialogVisible.show" :lock-scroll="true" width="1100px" @close="dialogClose" class="cosco-add">
		<el-form ref="createItemForm" :rules="rules" :model="createItem" label-suffix=":" :inline="true" label-width="110px">
          ...
          <el-option :label="item.label" :value="item.value" :key="item.value" v-for="item in taskTypeList"></el-option>
				</el-select>
			</el-form-item>
        ...
        >
          <el-option v-for="(item, index) in cityListArr2" :key="index" :label="`${item.cityFullNameCn} - ${item.cityFullNameEn}`" :value="item.id"> </el-option>
			...
			<el-form-item label="箱量" prop="quantity">
				<el-input style="width: 100%" type="number" size="mini" placeholder="请输入组织中文全称" v-model="createItem.quantity"></el-input>
			</el-form-item>
...
import { ooclTaskSave } from '@/api/exec/autobook'
import { cityOoclList, dingList, ooclUserList } from '@/api/exec/cosco'
	...
	methods: {
		...
		submitForm(formName) {
			this.$refs[formName].validate(valid => {
				if (valid) {
					let formData=JSON.parse(JSON.stringify(this.createItem))
					//提交代码封装表单属性
					for (var p in this.taskBookDto) {
						this.taskBookDto[p] = formData[p]
					}
					...
					if (companyObj) {
						formData.companyName = companyObj.companyCname
					}
					ooclTaskSave(formData).then(res => {
		...
		cityQuery(queryString, cb) {
			...
			cityOoclList(params).then(response => {

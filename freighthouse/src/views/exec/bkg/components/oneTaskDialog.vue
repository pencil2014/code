<template>
  <el-dialog
    :lock-scroll="true"
    :title="`${form.taskId ? '查看' : '新增'}one任务`"
    :visible.sync="oneData.show"
    class="one-add"
    width="1000px"
  >
    <el-form
...
export default {
  ...
  computed: {
    ...
    oneContainerType () {
      return this.dictMap['oneContainerType'].map(item => {
        return {
          label: item.label,
          value: item.label
        }
  ...
  methods: {
    ...
    // 查询支付地点onePayLocations
    // getPayLocations(str) {
    //   const data = {
    //     keywords: str
    //   }
    //   onePayLocations(data)
    //     .then(res => {
    //       if (res.code === 0) {
  ...
  mounted() {
    if (this.oneData.source) {
      this.form = Object.assign({}, this.form, this.oneData.source)
    }
  }
}
</script>

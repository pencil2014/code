<template>
	<div class="finance-page flex-page">
		...
		<div class="finance-list-container">
			...
			<FinanceTable :option="tableOption" @send-multi="sendMulti" class="list-table" />
		</div>
		<configTaskDialog :dialog-visible="dialogVisible" ref="configTaskDialog" @relateSuc="getData" />
...
import configTaskDialog from '@/views/exec/bkg/components/configTaskDialog'
import TaskLog from './taskLog'
import { coscoTaskList, coscoTaskDelete } from '@/api/exec/autobook'
import { mixin } from '@/views/finance/orderFee/orderFee/mixin'
import { mixin as mixin2 } from '@/views/finance/voucher/mixin'
import { dateFormat } from '@/views/finance/utils/finance'
import { cityList, setHp, cancelHp, startTask, stopTask, deleteTask, sapidList } from '@/api/exec/cosco'
...
export default {
	...
	async created() {
		...
		this.searchOption = {
			addFilter: {
				...
				searchInputGroup: [
					{ key: 'companyCode', value: '', showType: '8' },
					{ key: 'taskMode', value: '', showType: '9' },
				...
				filterGroups: {
					companyCode: { label: '分公司', type: 'select', showType: '8', selectOptions: [] },
					taskMode: { label: '订舱方式', type: 'select', prop: 'coscoTaskMode', showType: '9' },
					porCityId: {
						label: '起始地',
						type: 'remoteSelect',
						showType: '1',
		...
		this.tableOption.columns = [
			...
			{ label: '订舱方式', prop: 'taskMode', propInDict: 'coscoTaskMode', type: 'select' },
	...
	methods: {
		...
		addConfigDialog(row) {
			...
			this.$nextTick(() => {
				this.$refs.configTaskDialog.init(row)
			})
		},
		// 详情
		showDetail(row) {
		...
		cityQuery(queryString, item) {
			...
			cityList({ keywords: queryString }).then(res => {
		...
		// 取消优先级
		cancelHp() {
			this.validateRows(cancelHp, '恭喜你，取消优先级成功')
		},
		// 启动任务
		startTask() {
			this.validateRows(startTask, '恭喜你，启动任务成功')
		},
		// 暂停任务
		stopTask() {
			this.validateRows(stopTask, '恭喜你，暂停任务成功')
		},
		// 删除任务
		deleteTask() {
			if (!this.multipleSelection.length) return this.$msgErrClose('请先勾选要操作的数据')

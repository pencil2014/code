<template>
  ...
  >
    ...
    <div v-if="cmaData.show">
      <CmaTaskDialog :cmaData="cmaData" @cancel="closeCmaDialog" @submit="cmaSubmit" />
    </div>
...
import CmaTaskDialog from '@/views/exec/bkg/components/cmaTaskDialog'
import {
  cmaPort,
  cmaStart,
  cmaStop,
...
export default {
  ...
  methods: {
    ...
    addTask() {
      this.cmaData.source = ''
      this.cmaData.show = true
    },
    // 关闭新建弹窗
    closeCmaDialog() {
      this.cmaData.show = false
    },
    // 提交新建弹窗
    cmaSubmit(source) {
      const {
    ...
    handleDelete($index, row, option) {
      this.cmaData.source = JSON.parse(JSON.stringify(row))
      this.cmaData.source.taskId = ''
      this.cmaData.source.bookId = ''
      this.cmaData.show = true
    },
    // 任务详情
    showDetail(row) {

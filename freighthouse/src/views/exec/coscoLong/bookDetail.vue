<template>
	<div class="edit-container-exec">
		<div class="edit-row">
			<div class="row-tit">
				<span class="tit">远期订舱产品/产品详情</span>
		...
		<div class="edit-row">
				...
				<section class="order-section">
							...
							<el-table-column header-align="center" align="left" label="出发地" width="140px">
								<template slot-scope="scope">
									<div class="shippingLines">
										...
										<!-- 预期交货时间 -->
										<div style="width: 100px; text-align: center">{{ scope.row.estimatedTransitTimeInDays || 0 }}天</div>
										<!-- 有效期 -->
                    ...
                    <div class="symbol"></div>-->
										...
										<div style="width: 140px; text-align: center">
											{{ scope.row.scheduleData.vesselName ? scope.row.scheduleData.vesselName + scope.row.scheduleData.voyageNo + scope.row.direction + '/' + scope.row.scheduleData.serviceCode : '无' }}
										</div>
									</div>
								</template>
							</el-table-column>
			...
			<div class="submitButton">
				<el-button style="float: right; margin-top: -22px" type="primary" size="mini" ref="btnCancel" :loading="disabledBtn" :disabled="disabledBtn" @click="payOrder" plain>立即下单</el-button>
			</div>
		</div>
...
import { bookInfoLong, extraChargeFeeListLong, bookingOrder, intermodalServiceLong, getPackageType } from '@/api/exec/cosco'
...
export default {
	...
	methods: {
		...
		init() {
			...
			bookInfoLong({ productId, sapid }).then(res => {
			...
			intermodalServiceLong({ productId, sapid }).then(({ data }) => {
		...
		getExtraFeeList() {
			//获取附加费明细数据
			extraChargeFeeListLong({ productId: this.createItem.productId, sapid: this.$route.query.sapid }).then(res => {

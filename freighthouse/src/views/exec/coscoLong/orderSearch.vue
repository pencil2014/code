...
import { orderListLong, sapidList } from '@/api/exec/cosco'
...
export default {
	...
	async created() {
		...
		this.tableOption.operationBtns.data = [
			{ label: '订舱', showFn: row => row.brNoInfos.length === 0, action: 'Edit' },
			{ label: '详情', show: true, action: 'showDetail' }
		]
		...
		this.tableOption.columns = [
			...
			},
			{
				...
				type: 'text',
				formatter: row => {
					return row.vesselName ? row.vesselName + '/' + row.voyageNo + row.direction : '无'
				}
			},
			// {
			//   label: '线上支付状态',
	...
	methods: {
		...
		handleEdit(row) {
			...
			this.isJumpRouteFromPage = true
				let sapid=this.sapid?this.sapid:''
			this.$router.push({
				path: `orderBook?orderNo=${row.orderNo}&sapid=${sapid}`
			})
		}
	}
}

<template>
  <BaseDialog :config="dialogConfig" :callback="dialogCallback" id="overseasFreight-addOffer">
    <el-form ref="form" :rules="createItem.rules" label-width="0" :model="createItem" :show-message="false">
      <el-row>
        <el-col :span="8">
          <el-form-item label="名称" label-width="80px" prop="templateName">
            <el-input v-model="createItem.templateName" clearable size="mini" maxlength="60" style="width:100%" placeholder="请输入内容"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
      ...
      <el-table :data="createItem.list" :max-height="240">
        ...
        <el-table-column align="center" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-button @click="handleDelete(scope)" :disabled="createItem.list.length<=1" type="text" class="red" size="mini">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-form>
...
import { surchargeInfo, airSurchargeTemplatePageList } from '@/api/charge'
...
export default {
  ...
  created() {
    if (this.detailData) {
      this.dialogConfig.title = '附加费模板编辑'
      this.createItem.templateName = this.detailData.templateName
      this.createItem.list = this.detailData.surcharges
      this.createItem.templateId = this.detailData.templateId
    }

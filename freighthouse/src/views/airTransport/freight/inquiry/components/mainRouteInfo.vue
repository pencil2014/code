          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="起运港" prop="polPortCode">
                <el-select size="mini" :disabled="disabledState" style="width:100%" clearable v-model="createItem.polPortCode" filterable remote reserve-keyword placeholder="请选择起运港" :remote-method="(val)=>{ polPortRemoteMethod(val,createItem.polPortCode)}" @visible-change="polPortRemoteMethod('',createItem.polPortCode)" @change="handleChangePolPortCode">
                  <el-option v-for="item in airPortArr" :key="item.value" :disabled="item.value===createItem.podPortCode" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="目的地" prop="podPortCode">
                <el-select size="mini" v-model="createItem.podPortCode" filterable remote clearable :disabled="disabledState" style="width: 100%" placeholder="请选择目的地" :remote-method="(val)=>{ podPortFilterMehod(val,createItem.podPortCode)}" @visible-change="podPortFilterMehod('',createItem.podPortCode)" @change="handleChangePodPortCode">
                  <el-option v-for="item in destinationArr" :disabled="item.value===createItem.polPortCode" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                </el-select>
              </el-form-item>
            ...
            <el-col :span="8">
              <el-form-item label="航空公司" prop="airLine">
                <el-select style="width:100%" :disabled="disabledState" size="mini" clearable v-model="createItem.airLine" filterable remote reserve-keyword placeholder="请选择航空公司" :remote-method="val => remoteMethod(val,createItem.airLine)" @focus="remoteMethod('',createItem.airLine)">
            ...
            <el-col :span="8">
              ...
              <el-form-item label="有效日期" prop="beginDate">
                <el-date-picker v-model="createItem.beginDate" size="mini" style="width:100%" type="daterange" value-format="yyyy-MM-dd" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
                </el-date-picker>
              </el-form-item>
              <el-form-item label="维护人" prop="maintainerId" class="popover-class">
                <el-select size="mini" v-model="createItem.maintainerId" :disabled="!oQuery.id || disabledState" filterable remote style="width: 100%" placeholder="请输入" :remote-method="maintainerFilterMethod" @visible-change="maintainerVisibleChange" @change="handleChangeMaintainer">
            ...
            <el-col :span="24">
              <el-form-item label="货型（泡比）">
                <el-input clearable size="mini" v-model="createItem.cargoType" :disabled="disabledState" maxlength="128"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </div>
...
export default {
  ...
  methods: {
    ...
    maintainerQuerySearch(queryString) {
      let params = {
        name:
          this.createItem.maintainerName && !queryString
            ? this.createItem.maintainerName
            : queryString,
        roleCode: 'pricing'
      }
    ...
    handleChangePolPortCode(val) {
      ...
      if (!val) {
        ...
        this.$set(this.createItem, 'polCountryCname', portItem.countryCname)
        this.$set(this.createItem, 'polCountryEname', portItem.countryEname)
        this.$set(this.createItem, 'polCountryCode', portItem.countryCode)
      }
    }
  }
}

      ...
      <el-row :gutter="10">
        <el-col :span="4">
          <el-form-item label="提单类型" prop="airBl.blType">
            ...
            <el-select v-model="createItem.airBl.blType" size="mini" clearable Atholder="请选择" disabled @change="handleChangeBlType" style="width: 100%">
          ...
          <el-row :gutter="10">
            <el-col :span="24">
              <el-form-item label="Nature and Quantity of Goods (incl.Dimensions or Volume)">
                <el-input type="textarea" :disabled="isDraftConfirm" rows="2" size="mini" resize="none" v-model="createItem.airBlCargoStat.goodsNature"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
...
export default {
  ...
  computed: {
    ...
    airBlType() {
      ...
      //   return this.dictMap.airBlType.filter((e) => e.value == 'mawb')
      // } else {
      //   return this.dictMap.airBlType.filter((e) => e.value == 'hawb')
      // }
      ...
      handler: function (val) {
        if (val === 'mawb') {
          if (
            ...
            this.airOrderInfo.realVoyageList[1].carrierSupplierName &&
            !this.createItem.airBl.airWayBill
          ) {
            getCarrier({
              itat: this.airOrderInfo.realVoyageList[1].carrierSupplierName
            }).then((res) => {
  ...
  methods: {
    ...
    getOrderInfo() {
      ...
      orderInfo(params).then((res) => {
        ...
        if (
          data.realVoyageList[1] &&
          data.realVoyageList[1].carrierSupplierName
        ) {
          this.queryAircarrierList(data.realVoyageList[1].carrierSupplierName)

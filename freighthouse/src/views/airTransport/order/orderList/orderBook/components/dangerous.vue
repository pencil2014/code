<template>
	<div class="edit-row">
			...
			<div class="row-tit so">
				...
				<div class="row-tit-operate">
					...
					<div class="el-button-group">
						<el-button size="mini" type="default" class="ml10" @click="handleAdd" :disabled="isBd">新增申报</el-button>
						<!-- <el-button size="mini" type="primary" class="ml10" @click="handleCreateSerial">申报完成</el-button> -->
					</div>
				</div>
			</div>
			<div class="row-form table-com-layout" style="padding:0 24px">
				<el-table ref="dangerousTable" stripe :data="tableList" @selection-change="handleSelectionChange">
					...
					<!-- <el-table-column prop="address" label="附件信息" align="center"> </el-table-column> -->
		...
		<div class="agentDetail" v-show="showDetail">
			...
			<el-form ref="saveForm" :model="createdItem" :rules="rules">
        ...
        <div style="float:right;">
          <el-checkbox style="margin-right: 10px;" v-model="isReport">是否申报</el-checkbox>
				...
				<div class="row-flex flex-cargo ml10">
					<el-form-item label="委托单号" prop="intrustNo" style="width:16.5%;padding:0 14px 10px 0;">
						<el-input size="mini" disabled placeholder="请输入" v-model="createdItem.intrustNo" clearable></el-input>
					</el-form-item>
					<el-form-item label="委托公司" prop="supplierId" style="width:16.5%;padding:0 14px 10px 0;">
						<el-select
							size="mini"
							style="width: 100%"
							placeholder="请选择"
					...
					<el-form-item label="联系人姓名" prop="supplierContact" style="width:16.5%;padding:0 14px 10px 0;">
						<el-input size="mini" placeholder="请输入" v-model="createdItem.supplierContact" clearable></el-input>
					</el-form-item>
					<el-form-item label="联系人电话" prop="supplierContactPhone" style="width:16.5%;padding:0 14px 10px 0;">
						<el-input size="mini" placeholder="请输入" v-model="createdItem.supplierContactPhone" clearable></el-input>
					</el-form-item>
					<el-form-item label="邮箱" style="width:16.5%;padding:0 14px 10px 0;">
			...
			<div class="edit-container so">
				<div class="row-tit so">
					...
					<div class="row-tit-operate">
						<div class="el-button-group">
							<el-button size="mini" type="default" class="ml10" @click="handleAddEnclosure('')" :disabled="isBd || createdItem.declareStatus === 'declared'">添加附件</el-button>
						</div>
					</div>
				</div>
				<div class="row-form table-com-layout" style="padding:0 24px">
					<el-table :data="createdItem.dangerousDeclareContainerList">
						...
						<el-table-column prop="dangerousCode" label="联合国危险品编码" align="center"> </el-table-column>
		...
		<el-dialog title="危险品申报附件上传" :visible.sync="uploadVisible" :lock-scroll="true" width="780px">
			<el-form ref="createUploadItem" :rules="rulesCreateItem" :model="createUploadItem" label-position="right" label-width="120px">
				<el-form-item label="箱号" prop="cn" style="width: 100%">
					<el-select v-model="createUploadItem.cn" size="mini" style="width: 100%" placeholder="请选择" @change="handleSelectCn">
						<el-option v-for="(item, index) in containerList" :label="item.cn" :value="item.cn" :key="index"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="品名" prop="commodityName" style="width: 50%">
					<el-input size="mini" placeholder="请输入" v-model="createUploadItem.commodityName" clearable></el-input>
				</el-form-item>
				<el-form-item label="危险品等级" prop="dangerousGrade" style="width: 50%">
				...
				<el-form-item label="联合国危险品编码" prop="dangerousCode" style="width: 50%">
					<el-select
						size="mini"
			...
			<div slot="footer" class="dialog-footer">
				<el-button size="small" type="primary" @click="submitForm('createUploadItem')" :disabled="isBd"> <i class="el-icon-copy-document"></i> 保存 </el-button>
...
export default {
	...
	methods: {
		...
		handleSelectCn(val) {
			...
			this.createUploadItem.containerId = sItem.oid
		},
		// 获取列表
		getList() {
		...
		handleChangeSupplier(val) {
			...
			//   return o.value === val
			// })
			// this.createdItem.custName = item[0].label
		},

<template>
  <BaseDialog :config="dialogConfig" :callback="dialogCallback">
    <el-form ref="refuseForm" :rules="rules" :model="createdItem" label-position="right" label-width="120px" class="create-form">    
...
export default {
  ...
  methods: {
    ...
    // 发起拒绝
    handleRefuse(done) {
      let code = sessionStorage.getItem('serviceCode')
      let serviceInfoListStr = sessionStorage.getItem('serviceInfoListStr')
      console.log(code)
      if (serviceInfoListStr) {
        let serviceInfoList = JSON.parse(serviceInfoListStr)
        let list = serviceInfoList.filter(item => item.serviceCode === code)
        console.log(list[0])

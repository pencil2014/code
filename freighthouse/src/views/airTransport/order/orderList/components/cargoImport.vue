      ...
      <div style="padding: 8px 0 5px">
        <el-form class="row-flex flex-so" label-width="70px" ref="airCargo" :model="cargoData" :show-message="false">
          <div class="form-box" v-for="(el, index) in cargoData.formData" :key="index">
            <i class="el-icon-circle-plus-outline plus-minus-outline" @click="handleAddContainerList(index)" v-if="!(disabledAddOpState || isDisabled)"></i>
            <i class="el-icon-remove-outline plus-minus-outline" @click="handleDeleteContainerList(index)" v-if="index>0&&!(disabledAddOpState || isDisabled)"></i>
            <div class="air-cargo-form" :class="{'air-cargo-form':!(disabledAddOpState || isDisabled),'air-cargo-form100':disabledAddOpState || isDisabled}">
              <el-row>
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="HS Code" :prop="'formData.'+index+'.hscode'" :rules="cargoData.rules.hscode">
                    <el-tooltip v-if="disabledAddOpState || isDisabled" class="item" effect="dark" :content="el.hscode" placement="top">
                      <el-select size="mini" v-model="el.hscode" style="width: 100%" placeholder="请选择HS Code" disabled>
                        <el-option v-for="(item, index) in el.hscodeOptions || []" :key="index" :label="item.label" :value="item.value">
                        </el-option>
                      </el-select>
                    </el-tooltip>
                    <el-select v-else size="mini" v-model="el.hscode" clearable style="width: 100%" remote filterable placeholder="请选择HS Code" @change="handleChangeHscode(el.hscode,index)" :disabled="disabledAddOpState || isDisabled" :remote-method="(val) => queryCargoHscode(val,index)">
                      <el-option v-for="(item, index) in el.hscodeOptions || []" :key="index" :label="item.label" :value="item.value">
                      </el-option>
                    </el-select>
                  </el-form-item>
                ...
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="木质包装" :prop="'formData.'+index+'.isWoodenPackage'" :rules="cargoData.rules.isWoodenPackage">
                    <el-select size="mini" v-model="el.isWoodenPackage" style="width: 100%" clearable placeholder="请选择木质包装" :disabled="(disabledAddOpState || isDisabled) && isBreakEdit">
                ...
                <el-col class="cname-ename" :xl="{span: 'cargo-short-item'}">
                  ...
                  <el-form-item class="form-item-height" label="中文品名" :prop="'formData.'+index+'.cname'" :rules="cargoData.rules.cname">
                    <el-tooltip v-if="disabledAddOpState || isDisabled" class="item" effect="dark" :content="el.cname" placement="top">
                      <el-input size="mini" v-model="el.cname" maxlength="1024" clearable placeholder="请输入" disabled>
                      </el-input>
                    </el-tooltip>
                    <el-input v-else size="mini" v-model="el.cname" maxlength="1024" clearable placeholder="请输入" :disabled="disabledAddOpState || isDisabled">
                    </el-input>
                  </el-form-item>
                </el-col>
                <el-col class="cname-ename" :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="英文品名" :prop="'formData.'+index+'.ename'" :rules="cargoData.rules.ename">
                    <el-tooltip v-if="disabledAddOpState || isDisabled" class="item" effect="dark" :content="el.ename" placement="top">
                      <el-input size="mini" v-model="el.ename" maxlength="1024" placeholder="请输入" disabled>
                      </el-input>
                    </el-tooltip>
                    <el-input v-else size="mini" v-model="el.ename" maxlength="1024" @input="checkEname(el.ename,'英文品名')" clearable placeholder="请输入" :disabled="disabledAddOpState || isDisabled">
                    </el-input>
                  </el-form-item>
                </el-col>
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="货物性质" :prop="'formData.'+index+'.property'" :rules="cargoData.rules.property">
                    <el-select size="mini" v-model="el.property" clearable placeholder="请选择" style="width:100%" :disabled="disabledAddOpState || isDisabled">
                ...
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="货品分类">
                    <el-select size="mini" v-model="el.commodityTypeCode" filterable style="width:100%" clearable placeholder="请选择" :disabled="disabledAddOpState || isDisabled">
                ...
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="件数" :prop="'formData.'+index+'.quantity'" :rules="cargoData.rules.quantity">
                    <el-input size="mini" v-model="el.quantity" clearable placeholder="请输入" :disabled="disabledAddOpState || isDisabled" @input="checkIntegerQuantity(el.quantity,index)">
                    </el-input>
                  </el-form-item>
                </el-col>
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="毛重" :prop="'formData.'+index+'.weight'" :rules="cargoData.rules.weight">
                    <el-input size="mini" v-model="el.weight" @input="checkNumInputMaxVal(el.weight,'毛重',index,'weight')" clearable placeholder="请输入" :disabled="disabledAddOpState || isDisabled">
                ...
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="体积" :prop="'formData.'+index+'.volume'" :rules="cargoData.rules.volume">
                    <el-input size="mini" v-model="el.volume" clearable placeholder="请输入" :disabled="disabledAddOpState || isDisabled" @input="checkNumInputMaxVal(el.volume,'体积',index,'volume')">
                ...
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="包装" :prop="'formData.'+index+'.packageInfo'" :rules="cargoData.rules.packageInfo">
                    <el-select size="mini" v-model="el.packageInfo" filterable clearable style="width:100%" placeholder="请选择" :disabled="disabledAddOpState || isDisabled">
                ...
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="长" :prop="'formData.'+index+'.length'" :rules="cargoData.rules.length">
                    <el-input size="mini" v-model="el.length" @input="checkNumInput(el.length,'长',index,'length')" clearable placeholder="请输入" :disabled="disabledAddOpState || isDisabled">
                ...
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="宽" :prop="'formData.'+index+'.width'" :rules="cargoData.rules.width">
                    <el-input size="mini" v-model="el.width" @input="checkNumInput(el.width,'宽',index,'width')" clearable placeholder="请输入" :disabled="disabledAddOpState || isDisabled">
                      <template slot="append">M</template>
                    </el-input>
                  </el-form-item>
                </el-col>
                ...
                <el-col :xl="{span: 'cargo-short-item'}">
                  <el-form-item class="form-item-height" label="高" :prop="'formData.'+index+'.height'" :rules="cargoData.rules.height">
                    <el-input size="mini" v-model="el.height" @input="checkNumInput(el.height,'高',index,'height')" clearable placeholder="请输入" :disabled="disabledAddOpState || isDisabled">
                      <template slot="append">M</template>
                    </el-input>
                  </el-form-item>
                </el-col>
              ...
              <el-row>
                <el-col :xl="{span: 'cargo-middle-item'}">
                  <el-form-item class="" label="唛头" :prop="'formData.'+index+'.mark'" :rules="cargoData.rules.mark">
                    <el-input type="textarea" maxlength="512" @input="checkEname(el.mark,'唛头')" :autosize="{ minRows: 1, maxRows: 1 }" size="mini" placeholder="请输入" v-model="el.mark" :disabled="disabledAddOpState || isDisabled"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :xl="{span: 'cargo-middle-item'}">
                  <el-form-item class="" label="货描" :prop="'formData.'+index+'.cargoDesc'" :rules="cargoData.rules.cargoDesc">
                    <el-input type="textarea" maxlength="512" :autosize="{ minRows: 1, maxRows: 1 }" size="mini" placeholder="请输入" v-model="el.cargoDesc" :disabled="disabledAddOpState || isDisabled"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
...
const dateObj = {
  ...
  rules: {
    ...
    cargoDesc: [
      { max: 512, message: '货描最大长度限制512位', trigger: 'blur' }
    ],
    ...
    'createItem.serviceList': {
      handler(newVal) {
        ...
        if (newVal.includes('pod_comm_clearance')) {
          this.cargoData.rules.isWoodenPackage = [
            { required: true, message: '请选择', trigger: 'change' }
          ]
          if (
            this.isPricing &&
  ...
  methods: {
    ...
    checkNumInput(value, type, index, prop) {
      ...
      timer = setTimeout(() => {
        ...
        // else {
        if (
          this.cargoData.formData[index].quantity &&
          this.cargoData.formData[index].length &&
          this.cargoData.formData[index].width &&
          this.cargoData.formData[index].height
        ) {
          this.cargoData.formData[index].volume = Number(
            this.cargoData.formData[index].quantity *
              this.cargoData.formData[index].length *
              this.cargoData.formData[index].width *
              this.cargoData.formData[index].height

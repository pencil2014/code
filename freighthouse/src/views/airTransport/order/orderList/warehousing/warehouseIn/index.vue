<template>
	...
	<div id="warehouseInplan" class="edit-row">
					...
					<el-table-column label="操作" width="180" align="center" fixed="right">
						<template slot-scope="scope">
							<div class="operate-group">
								<el-button @click="handleDetail(scope.row)" type="text" size="mini" :disabled="!scope.row.batchNo">详情</el-button>
								<el-button @click="handleOutPlan(scope.row)" type="text" size="mini" :disabled="((isBd && !isImportJoint) || isDisabledEdit) || !scope.row.batchNo">生成出仓计划</el-button>
							</div>
						</template>
					</el-table-column>
				</el-table>
		...
		<template v-if="detailShow">
			<!-- <Detail :detailData="formData" :isDisabledEdit="isDisabledEdit" :saveBarFixed="saveBarFixed" @close="close" @handleConfirm="handleSave" v-if="isExport"></Detail> -->
			<ImportDetail :detailData="formData" :isDisabledEdit="(isBd && !isImportJoint) || isDisabledEdit" :saveBarFixed="saveBarFixed" @close="close" @handleConfirm="handleSave"></ImportDetail>
		</template>
		<!-- 生成出仓计划 -->
		<template v-if="outShow">
			<!-- <OutPlanDialog :detailData="outplan_formData" @close="close" @handleConfirm="handleOutPlanBack" v-if="isExport"></OutPlanDialog> -->
			<ImportOutPlanDialog :detailData="outplan_formData" @close="close" @handleConfirm="handleOutPlanBack"></ImportOutPlanDialog>
		</template>
	</div>
</template>

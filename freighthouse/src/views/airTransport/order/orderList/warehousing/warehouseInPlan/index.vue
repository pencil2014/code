<template>
	...
	<div id="warehouseInplan" class="edit-row">
					...
					<el-table-column label="操作" width="180" align="center" fixed="right">
						<template slot-scope="scope">
							<div class="operate-group">
								...
								<el-button @click="handleDelete(scope.row)" type="text" size="mini" :disabled="((isBd && !isImportJoint) || isDisabledEdit) || !scope.row.planNo">删除</el-button>
								<el-button @click="handleIn(scope.row)" type="text" size="mini" :disabled="((isBd && !isImportJoint) || isDisabledEdit) || !scope.row.planNo">实际进仓</el-button>
							</div>
						</template>
					</el-table-column>
				</el-table>
		...
		<template v-if="detailShow">
			<!-- 出口整箱，进口整箱 -->
			<!-- <Detail :detailData="formData" :saveBarFixed="saveBarFixed" :isDisabledEdit="isDisabledEdit" @close="close" @handleConfirm="handleSave" v-if="isExport"></Detail> -->
			<ImportDetail :detailData="formData" :saveBarFixed="saveBarFixed" :isDisabledEdit="(isBd && !isImportJoint) || isDisabledEdit" @close="close" @handleConfirm="handleSave"></ImportDetail>
		</template>
		<!-- 实际进仓 -->
		<template v-if="inShow">
			<!-- <InDialog :detailData="in_formData" @close="close" @handleConfirm="handleInBack" v-if="isExport"></InDialog> -->
			<ImportInDialog :detailData="in_formData" @close="close" @handleConfirm="handleInBack"></ImportInDialog>
		</template>
...
export default {
	...
	created() {
		...
		this.airServiceList = JSON.parse(serviceListStr)
		// this.getList()
	},
	mounted() {},
	...
	methods: {
		...
		handleInPlan() {
			...
			let { custid, custName } = this.airOrderInfo

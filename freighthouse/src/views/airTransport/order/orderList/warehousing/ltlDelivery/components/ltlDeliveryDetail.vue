	...
	<div class="ltlDelivery-detail">
		<div :class="['row-tit', 'h32', 'mt10']">
			...
			<div class="row-tit-operate">
				...
				<el-button size="mini" type="primary" class="ml10" @click="handleSave" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit">保存</el-button>
			</div>
		</div>
		...
		<el-form ref="form" class="ltlDelivery-detail-body" :rules="rules" :model="formData" label-width="100px">
			<el-row>
				<el-col :span="6">
					...
					<el-form-item label="快捷输入">
						<div class="box-address-flex">
							...
							<el-button size="mini" type="primary" class="box-address-btn" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit" @click="handleQuickInput('unload')">填充</el-button>
						</div>
					</el-form-item>
				...
				<el-col :span="6">
					...
					<el-form-item label="快捷输入">
						<div class="box-address-flex">
							...
							<el-button size="mini" type="primary" class="box-address-btn" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit" @click="handleQuickInput('box')">填充</el-button>
						</div>
					</el-form-item>
				</el-col>
				<el-col :span="6">
				...
				<el-col :span="6">
					...
					<el-form-item label="签收附件" prop="signFileName" :show-message="false">
						<upload-input v-model="formData.signFileName" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit" @downloadFile="handleDownload" @handleView="handleView" @uploadFile="handleAdd" @delFile="delFile" />
					</el-form-item>
				</el-col>
			</el-row>
			<div class="row-tit between h32">
				...
				<div class="row-tit-operate">
					<el-button size="mini" type="primary" @click="handleCopy('actual')" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit || requestDisabled">复制实际货物</el-button>
					<el-button size="mini" type="primary" @click="handleCopy('bl')" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit || requestDisabled">复制制单货物</el-button>
					<el-button size="mini" type="primary" @click="handleAddCargo" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit">添加货品</el-button>
				</div>
			</div>
			<el-table ref="cargo" fit highlight-current-row style="width: 100%" class="outPlanBoxTable" :data="tableData" show-summary :summary-method="getSummaries">
				<el-table-column prop="hscode" label="HS code" align="center">
					<template slot-scope="scope">
						<el-select size="mini" v-model="scope.row.hscode" style="width: 100%" remote filterable clearable placeholder="请选择海关编号" @change="val => handleChangeHscode(val, scope.row)" :remote-method="val => queryCargoHscode(val)">
				...
				<el-table-column prop="cname" label="中文品名" align="center">
					<template slot-scope="scope">
						<el-input disabled size="mini" v-model="scope.row.cname"></el-input>
					</template>
				</el-table-column>
				<el-table-column prop="ename" label="英文品名" align="center">
					<template slot-scope="scope">
						<el-input disabled size="mini" v-model="scope.row.ename"></el-input>
					</template>
				</el-table-column>
				<el-table-column prop="mark" label="唛头" align="center">
					<template slot-scope="scope">
						<el-input size="mini" placeholder="请输入唛头" v-model="scope.row.mark" clearable maxlength="128"></el-input>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" width="150" align="center" fixed="right">
					<template slot-scope="scope">
						<div class="operate-group">
							<el-button @click="handleDelete(scope.row, scope.$index)" type="text" size="mini" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit">删除</el-button>
							<el-button @click="handleFile(scope.row)" type="text" size="mini" :disabled="(!isOp && !isEcs && (!isCs || !isImport) && !isImportJoint) || isDisabledEdit || !scope.row.oid">上传</el-button>
						</div>
					</template>
				</el-table-column>
			</el-table>
...
import { cargoHscodeList, fclTruckIntrustRegion } from '@/api/base'
...
import { airCustomsClearanceGetOrderCargoInfo } from '@/api/base'
...
export default {
	...
	data() {
		return {
			...
			rules: {
				...
				consigneeEmail: [{ required: false, message: '', trigger: 'blur' }],
	...
	computed: {
		...mapState({
			...
			orderInfo: state => state.airTransport.airOrderInfo,
			userId: state => state.user.userId,
			roles: state => state.user.roles
		}),
		// 20220509特殊处理为进口协同单的服务项权限都放开
		isImportJoint() {
			return ['air_import'].includes(this.orderInfo.businessType) && this.$route.query.source === 'jointList'
		}
	},
	...
	created() {
		...
		// // 进口单非必填
		// if (['air_import'].includes(this.orderInfo.businessType)) {
	...
	methods: {
        ...
        orderNo: this.$route.query.orderNo,
				...
				if (res && res.data) {
					let cargosCopy = []
					if (cargoPurpose === 'actual') {
            cargosCopy = JSON.parse(JSON.stringify(res.data.actualCargos))
					} else if (cargoPurpose === 'bl') {
						cargosCopy = JSON.parse(JSON.stringify(res.data.blCargos))
					}
					cargosCopy.map((item) => {

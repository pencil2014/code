<template>
	<div class="ltlDelivery-detail">
		<div :class="['row-tit', 'h32', 'mt10']">
			<div class="ft14 tit">委托编号:{{ formData.intrustNo }}</div>
		...
		<el-form ref="form" class="ltlDelivery-detail-body" :rules="rules" :model="formData" label-width="120px">
			<el-row>
				<el-col :span="6">
					...
					<el-form-item label="快捷输入">
						<div class="quick-class">
							...
							<el-button size="mini" type="primary" class="ml10" @click="handleQuickInput('unload')">填充</el-button>
						</div>
					</el-form-item>
				...
				<el-col :span="6">
					...
					<el-form-item label="快捷输入">
						<div class="quick-class">
							<el-input size="mini" clearable v-model="quickInputModelBox" placeholder="请依次输入姓名、电话、地址，中文逗号隔开"></el-input>
							<el-button size="mini" type="primary" class="ml10" @click="handleQuickInput('box')">填充</el-button>
						</div>
					</el-form-item>
					<el-form-item :label="createItem.intrustType_cn + '计划编号'" prop="outPlanNo">
						<el-input size="mini" placeholder="请输入" v-model="formData.outPlanNo" disabled clearable></el-input>
					</el-form-item>
					<el-form-item :label="createItem.intrustType_cn + '仓库'" prop="whName">
						<el-input size="mini" placeholder="请输入" v-model="formData.whName" disabled clearable></el-input>
					</el-form-item>
				...
				<el-col :span="6">
					...
					<el-form-item label="海外分公司" prop="overseasCompanyCode">
						<el-select v-model="formData.overseasCompanyCode" size="mini" placeholder="请选择" style="width: 100%" @change="val => changeOverseasCompany(val)">
			...
			<div class="row-tit between h32">
				...
				<div class="row-tit-operate">
					<el-button size="mini" type="primary" @click="handleAddCargo" :disabled="!isOp || isDisabledEdit">添加货品</el-button>
				</div>
			</div>
				...
				<el-table-column label="操作" width="150" align="center" fixed="right">
					<template slot-scope="scope">
						<div class="operate-group">
							<el-button @click="handleDelete(scope.row, scope.$index)" type="text" size="mini" :disabled="!isOp || isDisabledEdit">删除</el-button>
							<el-button @click="handleFile(scope.row)" type="text" size="mini" :disabled="!isOp || isDisabledEdit || !scope.row.oid">上传</el-button>
						</div>
					</template>
				</el-table-column>
			</el-table>
...
import { cargoHscodeList, getForeignCity, fclTruckIntrustRegion } from '@/api/base'
...
export default {
	...
	components: { UploadInput, FileUpload, AddressPop, FileUploadCargo },
	created() {
		this.init()
		//获取国家城市级联框数据
		...
		// 进口单非必填
		if (['air_import'].includes(this.airOrderInfo.businessType)) {
	...
	methods: {
		...
		init() {
			if (Array.isArray(this.createItem.loadInfoList) && this.createItem.loadInfoList.length) {
				...
				this.formData.intrustType_cn = data.intrustType_cn

				this.getSupplierList(data.overseaAgentName)
				this.getCarList(data.supplierName)

				// this.formData.loadPlace = this.formData.loadCity ? [this.formData.loadProvince, this.formData.loadCity] : [this.formData.loadProvince]
				// this.formData.unloadPlace = this.formData.unloadCity ? [this.formData.unloadProvince, this.formData.unloadCity] : [this.formData.unloadProvince]

				//反显装货卸货地点
				let loadPlace = []
				let unloadPlace = []
				if (this.formData.loadCountry === '中国') {
					loadPlace = [this.formData.loadCountry, this.formData.loadProvince, this.formData.loadCity, this.formData.loadDistrict]
				} else {
					loadPlace = this.formData.loadProvince ? [this.formData.loadCountry, this.formData.loadProvince] : [this.formData.loadCountry]
				}
				if (this.formData.unloadCountry === '中国') {
					unloadPlace = [this.formData.unloadCountry, this.formData.unloadProvince, this.formData.unloadCity, this.formData.unloadDistrict]
				} else {
					unloadPlace = this.formData.unloadProvince ? [this.formData.unloadCountry, this.formData.unloadProvince] : [this.formData.unloadCountry]
				}
				this.formData.loadPlace = loadPlace
				this.formData.unloadPlace = unloadPlace
			}
		},
		//装货地点

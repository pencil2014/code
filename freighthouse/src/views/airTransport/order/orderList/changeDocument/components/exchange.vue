<template>
	<BaseDialog :config="dialogConfig" :callback="dialogCallback">
		<el-form ref="exchangeForm" :model="createdItem" label-position="right" label-width="120px" class="create-form">
			...
			<el-form-item v-if="!exchangeData.isShipImport" label="海外分公司" prop="overseasCompanyCode">
				<el-select v-model="createdItem.overseasCompanyCode" size="mini" placeholder="请选择" style="width: 100%" @change="val => changeOverseasCompany(val)">
...
import { airBlExchangeUpdateStatus } from '@/api/order/podService/blExchange'
...
export default {
	...
	methods: {
		handleSelectSupplierName(val) {
			...
			if (item) this.createdItem.overseasAgentName = item.label
		},
		...
		changeOverseasCompany(val) {
			...
			if (item) this.createdItem.overseasCompanyName = item.companyName
		},

<template>
  <div>
    <div class="edit-container">
      <ComOrderInfo ref="first" :isPricing="isPricing" :isBd="isBd" :pricingAuditEmployeeDisabled="pricingAuditEmployeeDisabled" :createItem="createItem" :orderStatus="airOrderInfo.orderStatus" :serviceTypeOptions="serviceTypeOptions" :serviceCodeOptions="serviceCodeOptions" @getHeight="getShipInfoHeight" @updateCust="updateCust">
        <div class="operateBtn-group">
          <div class="btn-groups">
            <!-- airOrderInfo.bdManageEmployeeId===userId && createItem.orderStatus==='pre_submit' -->
            ...
            <template v-else-if="createItem.orderStatus !== 'pre_submit'">
              ...
              <template v-if="isOp">
                ...
                <!-- <el-button size="mini" type="danger" @click="handRefuseOp" v-if="createItem.bkgStatus!=='BOOK_RELEASE'&&createItem.bkgStatus!=='REFUSE'&&$route.query.source !== 'jointList'" :disabled="isDisabled">拒绝</el-button> -->
                ...
                <el-button size="mini" type="default" @click="handShipment" :disabled="isDisabled" v-if="!['draft', 'refuse','submit'].includes(createItem.orderStatus)">货物到港</el-button>
              </template>
              <!-- 商务 -->
              <template v-if="isPricing&&$route.query.source !== 'jointList'">
                ...
                <el-button size="mini" type="primary" :disabled="pricingAuditEmployeeDisabled" @click="saveFunc('save')" v-if="['submit'].includes(createItem.orderStatus) || (['submit','pass'].includes(createItem.orderStatus)&&createItem.bkgStatus==='REFUSE')">保存</el-button>
                <el-button @click="saveFunc('pass')" :disabled="(orderBookInfoData.jointAuditEmployeeId&&!airOrderInfo.auditEmployeeId) || (airOrderInfo.auditEmployeeId&&(airOrderInfo.auditEmployeeId!==userId))" type="success" size="mini" style="margin-left: 10px;" v-if="['submit'].includes(createItem.orderStatus) || (['submit','pass'].includes(createItem.orderStatus)&&createItem.bkgStatus==='REFUSE')">通过</el-button>
              ...
              <template v-if="showCareatOrderBtn">
                ...
                <!-- <el-button size="mini" type="danger" class="" @click="handleCancelRequest" :disabled="submitDisabled" v-if="!['complete', 'break', 'cancel', 'close'].includes(createItem.orderStatus) && !copyState && createItem.cancelApplyStatus !== 'applied'">取消</el-button> -->
        ...
        <el-form class="order-cont-form" size="mini">
          <div class="ship-cont air-cont" ref="shipInfoCont" :class="workAssignShow ? 'abs-toogle' : ''" :style="{ height: `${shipInfoHeight}px` }">
            ...
            <div class="row-cont mt15">
              ...
              <Cargo v-show="cargoShow" :isPricing="isPricing" :isBd="isBd" :isOp="isOp" v-model="cargoData" :orderStatus="createItem.orderStatus" :createItem="createItem" :isDisabled="isDisabled" ref="cargo" />
            </div>
            <!-- 物流信息 -->
            <div class="row-cont mt15">
              <div class="row-cont-tit">
              ...
              <Contact v-show="contactShow" :isPricing="isPricing" :isBd="isBd" :isOp="isOp" v-model="contactData" :orderStatus="createItem.orderStatus" ref="transport" />
            </div>
            <!-- 预计航程 -->
            <div class="row-cont mt10" v-if="!['draft', 'refuse','pre_submit','submit'].includes(this.createItem.orderStatus) || isPricing">
              <SwTransportInfo v-if="isRequire" :businessType="createItem.businessType" :isOp="isOp" :isPricing="isPricing" :pricingAuditEmployeeDisabled="pricingAuditEmployeeDisabled" v-model="swTransportInfoData" :orderStatus="createItem.orderStatus" />
            </div>
            <div class="row-cont edit-row ml10" style="margin: 5px 10px 0 !important" v-if="(!['draft', 'refuse','pre_submit','submit'].includes(createItem.orderStatus)&&!isPricing) || isOp">
              <!-- 实际航程 -->
      ...
      <div v-if="cancelOrderPopShow">
        <CancelOrder @close="cancelRequestPopClose" :orderNo="$route.query.orderNo" :serviceType="'st14'" :isOrderFeeCount="isOrderFeeCount" />
      </div>
      <!-- 协同订单弹窗 -->
...
import { orderFileList, orderFileDelete } from '@/api/order/list'
...
export default {
  ...
  computed: {
    ...
    isDisabled() {
      return ['complete', 'cancel', 'break'].includes(
        this.airOrderInfo.orderStatus
      )
    },
    airBkgStatus() {
  ...
  methods: {
    ...
    fillOrderInfo(data) {
      ...
      this.orderBookInfoData = {
        ...
        podPortCode: data.podPortCode, // 卸货港
      ...
      orderFeeCount({ orderNo: this.$route.query.orderNo }).then((res) => {
        ...
        this.$store.dispatch('airTransport/setOrderFeeCount', this.isOrderFeeCount)
      })
      // }
    ...
    getServiceTypes() {
      ...
      baseBusinessServiceList(param).then((res) => {
        ...
        data.map((item) => {
          ...
          if (this.isOp) {
            if (isFinishedArr.includes(item.serviceCode)) {
              item.disabled = true
            }
    ...
    handleSave(type) {
      this.validateAllForm(type)
        .then(() => {
          ...
          if (
            this.airOrderInfo.serviceType === 'st14' &&
    ...
    obOrderSave(formdata, type) {
      ...
      this.$store
        .dispatch('airTransport/orderSave', {
          uri: 'order/air/imports/order/self/save',
          params: formdata
        })
        .then((res) => {
          if (res.data.code === 0) {

      ...
      <ComOrderInfo ref="first" :isPricing="isPricing" :isBd="isBd" :createItem="createItem" :orderStatus="airOrderInfo.orderStatus" :serviceTypeOptions="serviceTypeOptions" :serviceCodeOptions="serviceCodeOptions" @getHeight="getShipInfoHeight" @updateCust="updateCust">
        <div class="operateBtn-group">
          <div class="btn-groups">
            ...
            <!-- 操作 服务项有订舱才展示，避免协同操作人员进来也能点保存 createItem.serviceList.includes('airbook') -->
      ...
      <el-form v-if="bkgFormStatus" :inline="true" :model="opFormData" :show-message="false" :rules="opRules" class="air-order-book-info" ref="orderBookRef">
        <div class="air-book-status">
          <el-form-item label="订舱状态" style="margin-bottom:0;margin-right:40px" prop="bkgStatus">
            <el-radio-group v-model="opFormData.bkgStatus" @change="changeBkgStatus" :disabled="!isOp || isDisabled ||!createItem.serviceList.includes('airbook')">
          ...
          <el-form-item label="交仓状态" style="margin-bottom:0" prop="poDealStatus">
            <el-radio-group v-model="opFormData.poDealStatus" :disabled="!isOp || isDisabled ||!createItem.serviceList.includes('airbook')">
              ...
              <el-form-item v-if="opFormData.poDealStatus==='PROCESSING'" label="" style="margin-left:-20px" prop="poDealProcessingReason">
                <el-select v-model="opFormData.poDealProcessingReason" :disabled="!isOp || isDisabled ||!createItem.serviceList.includes('airbook')" size="mini" placeholder="请选择处理原因" style="width:180px">
          ...
          <div class="bl-fin-status">
            <el-button size="mini" @click="handleOutputLetter" :disabled="!isOp || isDisabled ||!createItem.serviceList.includes('airbook')">输出托书</el-button>
            <!-- createItem.isTransportCheck === 'Y' -->
            <el-button size="mini" @click="handShipment" :disabled="!isOp ||isDisabled ||!createItem.serviceList.includes('airbook')">货物出运</el-button>
          </div>
        </div>
      </el-form>
      <div class="order-cont">
        <el-form class="order-cont-form" size="mini">
          <div class="ship-cont air-cont" ref="shipInfoCont" :class="workAssignShow ? 'abs-toogle' : ''" :style="{ height: `${airInfoHeight}px` }">
            ...
            <OrderBookInfo :bkgStatus="opFormData.bkgStatus" @getInfo="handleGetOrderInfo" :isPricing="isPricing" :isOp="isOp" :isBd="isBd" :formData="orderBookInfoData" :orderStatus="createItem.orderStatus" :createItem="createItem" @selectFile="selectFile" :isDisabled="isDisabled" @saveRemark="handleSaveRemark" ref="orderBookInfo" />
            ...
            <SoContainer class="mt10" v-model="soContainerData" :isPricing="isPricing" :isOp="isOp" :orderStatus="createItem.orderStatus" :isBd="isBd" ref="soContainer" />
            <!-- 货物信息 -->
            <div class="detail-goods-summary">
              ...
              <!-- <div class="summary-info">
                <div>计费重量：</div>
                <div>{{goodsTotal.chargeWeight}}KGS</div>
              </div> -->
              <div class="summary-info">
                <div>比重：</div>
            ...
            <div class="row-cont mt10" v-if="!['draft', 'refuse','submit'].includes(this.createItem.orderStatus) || isPricing">
              <SwTransportInfo v-if="isRequire" :orderBookInfoData="orderBookInfoData" :businessType="createItem.businessType" :isPricing="isPricing" :isOp="isOp" v-model="swTransportInfoData" :orderStatus="createItem.orderStatus" ref="swtransportInfo-form" />
            </div>
            <div class="row-cont edit-row ml10" style="margin: 5px 10px 0 !important" v-if="(!['draft', 'refuse','submit'].includes(createItem.orderStatus)&&!isPricing) || isOp">
              <!-- 实际航程 -->
              <OpTransportInfo v-if="isRequire" :isPricing="isPricing" :isOp="isOp" :isBd="isBd" :businessType="createItem.businessType" :bkgStatus="opFormData.bkgStatus" v-model="opTransportInfoData" :disabled="(!isOp && !isEpricing && !isEcs) || isDisabled ||!createItem.serviceList.includes('airbook')" ref="optransportInfo-form" />
            </div>
          </div>
          <!-- 参与人与关键点 -->
...
import OpRefuseDialog from './orderBook/components/refuseDialog.vue'
...
export default {
  data() {
    return {
      ...
      orderIsJoint: false, // 商务审核订单参与人接口，订单是否协同
  ...
  computed: {
    ...
    opId() {
      let { workAssign } = this.createItem
      let opItem = workAssign && workAssign.find((el) => el.jobCode === 'op')
    ...
    isDisabled() {
      return ['break', 'cancel', 'complete'].includes(
        this.airOrderInfo.orderStatus
      )
    },
    airBkgStatus() {
  ...
  methods: {
    ...
    leavePageSaveOrderInfo() {
      ...
      if (
        ...
        this.isPricing &&
        (['submit'].includes(this.createItem.orderStatus) ||
          (['submit', 'pass'].includes(this.createItem.orderStatus) &&
            this.createItem.bkgStatus === 'REFUSE'))
      ) {
        this.saveFunc('save')
      } else if (
        this.isOp &&
        // this.createItem.serviceList.includes('airbook') &&
        ['pass'].includes(this.createItem.orderStatus)
      ) {
        this.handleEdit()
      }
    ...
    auditPass(type) {
      ...
      let params = {
        ...
        // blNo: this.orderBookInfoData.blNo
      }
      let blNoArr = this.orderBookInfoData.blNo.split('-')
      if (blNoArr[0] && blNoArr[1].trim()) {
        params.blNo = this.orderBookInfoData.blNo
      }
    ...
    fillOrderInfo(data) {
      ...
      this.orderBookInfoData = {
        ...
        podPortCode: data.podPortCode, // 目的地CODE
        ...
        mblPrefix: '',
        mblSuffix: '',
        airCustomsTicketNum: data.airCustomsTicketNum,
        bdRemark: data.bdRemark
      }
      let arr = data.blNo.split('-')
      if (arr.length === 2) {
        this.orderBookInfoData.mblPrefix = arr[0]
        this.orderBookInfoData.mblSuffix = arr[1]
      }
      ...
      if (data.ship) {
        ...
        let { polPortCode, podPortCode, transitPortCode } = ship
        ...
        if (this.createItem.ship.podPortCode) {
          this.$store
            ...
            .then((data) => {
              ...
              this.podCountryCode = portItem ? portItem.countryCode : ''
            })
        }
        // 获取起运港polCountryCode
        if (this.createItem.ship.polPortCode) {
          this.$store
            .dispatch('dict/basePortListByCodes', {
              portCodes: this.createItem.ship.polPortCode
            })
            .then((data) => {
              let portItem = data.find(
              ...
              // console.log(portItem)
              this.polCountryCode = portItem ? portItem.countryCode : ''
            })
        }
      }
    ...
    opValidateAllForm(bkgStatus, flag) {
      ...
      return new Promise((resolve, reject) => {
        let formList = [
          ...
          this.$refs['swtransportInfo-form'].validateForm(),
        ...
        if (bkgStatus === 'INIT') {
          ...
          Promise.all([this.$refs['swtransportInfo-form'].validateForm()])
            .then(() => {
              resolve()
            })
            .catch((err) => {
    ...
    handleEdit(flag) {
      if (
        this.opTransportInfoData.formData &&
        this.opTransportInfoData.formData[1] &&
        this.opTransportInfoData.formData[1].etdTime &&
      ...
      this.opValidateAllForm(this.opFormData.bkgStatus, flag)
        .then(() => {
          ...
          let params = {
            ...
            etdTime: this.orderBookInfoData.etdTime,
            airLine: this.orderBookInfoData.airLine,
            airCustomsTicketNum: this.orderBookInfoData.airCustomsTicketNum,
            transport: {
              ...this.contactData,
              transportTerm: this.orderBookInfoData.transportTerm,
              customsReleaseTime: this.orderBookInfoData.customsReleaseTime,
          ...
          if (
            !this.hasDestinationShow &&
            this.createItem.orderStatus === 'pass'
          ) {
            console.log(
    ...
    validateAllForm(type) {
      ...
      if (type === 'draft') {
        ...
        if (this.$refs['swtransportInfo-form']) {
          this.$refs['swtransportInfo-form'].clearValidate() // 预计航程
        }
        return new Promise((resolve, reject) => {
          resolve()
        })
      ...
      return new Promise((resolve, reject) => {
        ...
        if (this.$refs['swtransportInfo-form']) {
          formList.push(this.$refs['swtransportInfo-form'].validateForm())
        }
        Promise.all(formList)
          .then(() => {
            resolve()
    ...
    validatePricingAllForm(type) {
      ...
      return new Promise((resolve, reject) => {
        let formList = [
          this.$refs.orderBookInfo.validateForm(), //基础信息
          this.$refs.soContainer.validateForm(), //客户信息
          this.$refs['swtransportInfo-form'].validateForm(), //预计航程
          this.$refs.cargo.validateForm(), //货物信息
          this.validateForm() // 合计信息
        ]
        Promise.all(formList)
      ...
      this.validateAllForm(type)
        .then(() => {
          ...
          let params = {
            ...
            // unloadingPortCode: this.orderBookInfoData.unloadingPortCode,

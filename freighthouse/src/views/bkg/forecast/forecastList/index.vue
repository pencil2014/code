...
import { basePortList, baseSystemLineList, baseShippingCarrierList, baseCurrencyList, portSystemLineInfo, portList } from '@/api/base'
		...
		this.searchOption1 = {
			addFilter: {
				...
				filterGroups: {
					...
					etdDate: { label: 'ETD周期', type: 'daterange', showType: '3', format: 'yyyy-MM-dd' },
				},
			},
		}
		this.init1()
	...
	methods: {
		init1 () {
			...
			let operationBtns = Object.assign({}, this.option.operationBtns, {
				width: '80px',
				data: [
					{ label: '修改', action: 'Edit', showFn: (row) => row.verifyStatus !== 'pass' },
					{ label: '删除', action: 'Delete', showFn: (row) => row.verifyStatus !== 'pass' },
				],
			})
			...
			let columns = [
				...
				{ prop: 'remark', label: '预报备注' },
				{ prop: 'verifyStatus', label: '商务状态', type: 'select' },
				{ prop: 'refuseReason', label: '拒绝备注', isTooltip: true, width: 135 },
				{ prop: 'verifierName', label: '商务' },
		...
		getData (isSearch) {
			...
			this.searchOption1.addFilter.searchInputGroup.forEach((item) => {
				...
				if (typeof value !== 'object') {
					data.query.push({ column: key, type: 'eq', value: value })
				} else {
					data.query.push({ column: 'etdBeginTime', type: 'ge', value: value[0] })
					data.query.push({ column: 'etdEndTime', type: 'le', value: value[1] })
				}
			})
		...
		//新建
		handleAdd () {
			Object.assign(this.form, { etdTime: '' }, defaultCreateQuery)
			this.title = '新建预报'
			this.isDetail = false
			this.dialogVisible = true
			this.$nextTick(() => this.$refs.form.clearValidate())
		...
		initSystemLine (key) {
			...
			portSystemLineInfo(value).then((response) => {
		...
		polCodeSearch (portName, portAttribute, state) {
			if (portName === true) {
				portName = ''
			}
			portList({ portName, portAttribute, state }).then((res) => {
		...
		getEmployeeList () {
			...
			let data = {
				sysLineCode: this.form.sysLineCode,

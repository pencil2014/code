...
export default {
		...
		return {
			...
			alterDialog: {
				...
				formItems: [
					...
					{ label: '手机号', type: 'input', showVerify: true, prop: 'mobileNo', rules: mobileRule },
					{ label: '验证码', type: 'input', prop: 'verificationCode', rules: verifyCodeRule },
	...
	methods: {
		...
		sendCode(val) {
			console.log('🚀 ~ val', val)
			if (!this.alterDialog.form.mobileNo) return this.$message.error('请输入手机号!')
			const mobileRule = /^1[3-9]\d{9}$/
			if (!mobileRule.test(this.alterDialog.form.mobileNo)) return this.$message.error('请输入有效手机号!')

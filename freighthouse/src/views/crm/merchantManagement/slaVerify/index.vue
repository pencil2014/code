  ...
  <div class="info-page">
    <div class="info-title">
      ...
      <div>
        ...
        <!-- 系统配置项的审核人可审核 -->
        <template v-if="['submit'].includes(slaInfo.qualityVerifyStatus)">
...
//   props: ['custid', 'orgid', 'baseInfo'],
  ...
  methods: {
     ...
     */
    confirmFn (tag) {
      if (!tag) return (this.noPassDialog.show = false,this.noPassDialog.form.refuseReason='')
      this.checkRefuseApi()
    },
    /**
    ...
    checkRefuseApi () {
      ...
      slaApi['refuse'](data).then((res) => {
        if (res.code === 0) {
          this.noPassDialog.form.refuseReason=''

<template>
  ...
  >
    ...
    >
      <!-- 公共 -->
      <el-form-item label="客户名称:" prop="custName" class="normal-input">
        <el-select v-model="form.custName" value-key="custid" filterable clearable remote
      ...
      <!-- obd选择的跟进人 -->
      <el-form-item key="bdEmployeeIds" label="跟进人:" prop="bdEmployeeIds" v-if="isObd">
        <el-select
          v-model="form.bdEmployeeIds"
          value-key="employeeId"
          placeholder="请选择"
        >
          <el-option
            v-for="(item, index) in bdEmployeeList"
      ...
      <!-- bd选择的跟进人 -->
      <el-form-item key="bdEmployeeId" label="跟进人:" prop="bdEmployeeId" v-else>
        <el-select
          ...
          <div class="title">客情描述</div>
            ...
            <div class="item-cnt">
              <el-form-item  label="姓名:" key="custContact" :prop="`custDescription.${custIndex}.custContact`"
                ...
                <!-- <el-button size="mini" type="primary" class="addbtn"  @click="showDialog(custIndex)" v-if="form.custName">新增</el-button> -->
...
export default {
  ...
  watch: {
    ...
    'form.bdEmployeeIds'(val) {
      let name = val.bdEmployeeName
      const data = {
        currPage: 1,
        pageSize: 3000,
        query: [
          { column: 'name', type: 'eq', value: name},
          { column: 'roleCode', type: 'in', value: 'bd'}
        ]
      }
      listByRole(data)
        .then((res) => {
          if (res.code === 0) {
            ...
            let lastEmployee = chooseList.find(item=>item.employeeId ===val.bdEmployeeId)
            this.form.bdEmployeeId = lastEmployee
          }
        })
        .catch(() => {})
    }
  ...
  methods: {
    ...
    changeComanyName (str) {
      this.currentCustid = str
      if (str) {
        this.initData(str);
        this.getContractData();
        let findObj = this.customerList.find(item => item.custid === str)
        this.form.companyAddress = findObj.detailAddr
        ...
        if (this.isObd) {
          //获取该客户名下的下单和潜在维护人
          CUSTOMER.getOPMaintainer({custid:str}).then(res=>{
            this.bdEmployeeList = res.data || []
          })
          // this.getSearchBd(this.currentCustid, sysUserId)
        } else {
    ...
    //获取客户基本信息
    initData(custid){
      CUSTOMER.verifyInfo({custid:custid}).then((res) => {
    ...
    getContractData(type,oid,index){
      CONTACT.customerList({custid:this.currentCustid}).then((res) => {
    ...
    submit() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          this.form.custid = this.currentCustid
          this.form.flag = true
          // 拜访
          if (commuType === 'visit') {

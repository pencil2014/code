...
const formItem = [
  ...
  { label: '银行信息表', type: 'file', prop: 'fileList', r: true, show: true},
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      let fileList = this.addDialog.formItems.find(item => item.prop === 'fileList')
      if(this.isSupplier){
        // 供应商银行信息表暂时不填
        this.$set(fileList, 'show',false)
      }else{
        this.$set(fileList, 'show',false)
      }
      ...
      ACCOUNT[method](data).then((res) => {
        this.sourceList = res.data.map((item) => {
          // item._isInvalid = (item.status === 'no_effect' && item.verifyStatus !== 'submit')

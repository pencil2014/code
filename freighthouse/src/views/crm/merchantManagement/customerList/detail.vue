<template>
  ...
  >
    ...
    <!-- 修改客户等级 -->
    <CrmDialog :option="vipOption" @close="changeLevel" />
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      dialogDelayLeader: {
        ...
        text: '申请',
        labelWidth: '100px',
        conversation:'说明：若申请下单延期审核通过，则在申请延期时间内，命中下单的规则也不会掉入公海，若申请拜访延期审核通过，则在申请延期时间内，命中拜访规则也不会掉入公海',
        formItems: [
          {
            label: '审核人',
            type: 'select',
            ...
            change: (val) => {
              ...
              if (val === 'order_delay') {
                ...
                this.dialogDelayLeader.formItems.push(item)
                  ...
                  this.dialogDelayLeader.formItems = this.dialogDelayLeader.formItems.slice(0,3)
                  const item = {
                    label: '下单延长时间至',
                    type: 'date',
                    prop: 'mtDate',
               ...
               //请求参数，获取当前是不是已经成功过订单延期
                let delayTime = {}
                CUSTOMER.getLatestDelay(data).then(res=>{
                  if(res.data&&res.data.mtDate){
                    delayTime = {
                      text: `最近审核通过的下单延期时间为:${res.data.mtDate}`,
                      type: 'text',
                    }
                    this.dialogDelayLeader.formItems.push(delayTime)
                  }else{
                    delayTime = {}
                  }
                })
                ...
                CUSTOMER.getLatestDelay(data).then(res=>{
                  if(res.data&&res.data.mtDate){
                    delayTime = {
                      text: `最近审核通过的拜访延期时间为:${res.data.mtDate}`,
                      type: 'text',
                    }
                    this.dialogDelayLeader.formItems.push(delayTime)
                  }else{
                ...
                // if(Object.keys(delayTime).length>0){
                //   this.dialogDelayLeader.formItems.push(delayTime)
                // }
              }
            }
      ...
      vipOption: {
        ...
        showClear: true,
        title: '修改级别',
        form: { custGrade: '' ,applyReason:''},
        formItems: [
          {
            label: '调整后等级',
            prop: 'custGrade',
            type: 'select',
            propInDict: 'custLevelNew',
            rules: this.defaultRules
          },
          {
            label: '调整理由',
            ...
            rules:
              [
                { max: 512, message: '理由最大长度限制512位', trigger: 'blur' },
              ]
          },
        ]
      },
  ...
  methods: {
    ...
    changeLevel(tag) {
      ...
      CUSTOMER.updateCustLevel({
        beforeCustGrade:this.custGrade,
        custid: this.custid,
        ...this.vipOption.form
      }).then((res) => {

		...
		<div class="orderbook-info-table">
				...
				<div class="cont" :class="this.isBkg ? 'op-cont' : this.isBd ? 'bd-cont' : ''">
					...
					<div class="flex-form-item">
						...
						<el-form-item required label="托书附件" prop="rail.bookFileName" :show-message="false">
							...
							<upload-input class="lcl-file-upload" v-model="createItem.rail.bookFileName" isAllowDel @downloadFile="handleDownload" @handleView="handleView" @uploadFile="handleAdd" :disabled="disabledState || isPricing" />
						</el-form-item>
					</div>
				</div>
			</div>
			<div class="table-td">
				<div class="cont" :class="this.isBkg ? 'op-cont' : this.isBd ? 'bd-cont' : ''">
					...
					<div class="flex-form-item">
						...
						<el-form-item class="timeSpecial lcl-time-icon" required label="货好时间" prop="transport.cargoReadyTime" :show-message="false">
							<elDatePickerLimit
								size="mini"
						...
						<el-form-item
							...
							class="timeSpecial lcl-time-icon"
							label="截仓时间"
						...
						<el-form-item class="timeSpecial lcl-time-icon" required label="预计发车时间" prop="rail.etdTime" :show-message="false">
							<elDatePickerLimit
								size="mini"
								:field="'etdTime'"
						...
						>
							<el-input maxlength="128" show-word-limit clearable size="mini" placeholder="请输入" v-model="createItem.rail.vesselVoyage" :disabled="disabledState"></el-input>
						</el-form-item>

						<el-form-item prop="rail.bkgAgentSupplierId" :required="true" v-if="isSameUser || userEditRoles.includes('pricing') || userEditRoles.includes('bkg')" label="订舱代理" :show-message="false">
							<el-select
								:disabled="disabledState"
								size="mini"
						...
						<el-form-item v-if="isSameUser || userEditRoles.includes('pricing') || userEditRoles.includes('bkg')" :required="true" label="是否自拼" prop="rail.isSelf" :show-message="false">
							<el-select :disabled="disabledState" v-model="createItem.rail.isSelf" size="mini" clearable placeholder="请选择" style="width: 100%">
								<el-option v-for="(item, index) in isSelfOptions" :label="item.label" :value="item.value" :key="index"> </el-option>
							</el-select>
						</el-form-item>
			...
			<div class="table-td">
				<div class="cont" :class="this.isBkg ? 'op-cont' : this.isBd ? 'bd-cont' : ''">
					...
					<div class="flex-form-item">
						<el-form-item label="提单类型" prop="rail.blType" :show-message="false">
						...
						<el-form-item label="出单方式" prop="rail.blMode" :show-message="false">
						...
						<el-form-item label="委托备注" prop="intrustRemark" :show-message="false">
							<el-input
								@dblclick.native="showRemarkDetail(createItem.main.intrustRemark)"
						...
						<el-form-item v-if="isPricing || isBkg || (isBd && ['pass'].includes(orderStatus)) || noRoles" label="审单备注">
							<el-input
								@dblclick.native="showRemarkDetail(createItem.main.auditRemark)"
								:disabled="disabledState || isBkg || (isBd && ['pass'].includes(orderStatus)) || (isPricing && ['pass', 'complete', 'refuse'].includes(orderStatus))"
						...
						<el-form-item v-if="isBkg || (isBd && ['pass'].includes(orderStatus))" label="订舱备注" prop="rail.bkgRemark" :show-message="false">
							<el-input
								@dblclick.native="showRemarkDetail(createItem.rail.bkgRemark)"
								:disabled="disabledState"
								class="loadingBdRemark"
								type="textarea"
								clearable
						...
						<el-form-item label="备注" prop="main.bdRemark" v-if="$route.query.source !== 'jointList'">
							<el-input
								@dblclick.native="showRemarkDetail(createItem.main.bdRemark)"
								class="loadingBdRemark"
								type="textarea"
								clearable
								rows="4"
...
import { baseDeptParticipants, railDeptCode } from '@/api/base'
...
export default {
	...
	methods: {
		...
		fillBkg() {
			...
			console.log('🚀 ~ this.bkgEmployeeList', this.bkgEmployeeList)
		},
		// 订舱人
...
.upload-input-box.lcl-file-upload .upload-label .file-name {
	font-size: 12px;
}
.lcl-time-icon {
	.el-input__icon {

			...
			<div class="row-form mt10 table-com-layout">
				...
				>
					...
					<el-table-column prop="status" label="草单确认" align="center" type="">
						<template slot-scope="scope">
							<div class="operate-group">
								...
								<el-button @click="handleBlDraftConfirm(scope.row)" type="text" size="mini" :disabled="!isBkg || isView" class="underline" v-else> 未确认</el-button>
							</div>
						</template>
					</el-table-column>
					...
					<el-table-column prop="takeStatus" label="取单状态" align="center" type="">
						<template slot-scope="scope">
							<span v-if="scope.row.takeStatus === 'finish'">已取单</span>
							<span v-else-if="isBkg && scope.row.isDraftConfirm === 'y' && scope.row.blType === 'mbl'" @click="handleTake(scope.row)" style="cursor:pointer;color:#1890ff;">未取单</span>

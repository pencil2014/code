<template>
	<div class="row-flex ml10" id="put-info-rail">
		<el-form-item required label="提单类型" prop="blType" :show-message="false" class="flex-item">
			<el-select disabled v-model="createItem.blType" size="mini" clearable placeholder="请选择" style="width: 100%" @change="handleChangeBlType">
		...
		<el-form-item required label="提单号" prop="blNo" :show-message="false" class="flex-item">
			<el-input disabled size="mini" maxlength="60" clearable placeholder="请输入" v-model="createItem.blNo"></el-input>
		</el-form-item>
		...
		<el-form-item label="出单方式" class="flex-item">
			<el-select @change="handleChangeBlMode" disabled v-model="createItem.blMode" size="mini" clearable placeholder="请选择" style="width: 100%">
		...
		<div class="flex-item">
			...
			<el-input disabled size="mini" clearable placeholder="请填写正本数量" v-model.number="createItem.numOriginal" style="width: 45%" @input="numberInputCheck($event, 'numOriginal', '正本数量')"></el-input>
			<span class="seprate-line">/</span>
			<el-input disabled size="mini" clearable placeholder="请填写副本数量" v-model.number="createItem.numCopy" style="width: 45%" @input="numberInputCheck($event, 'numCopy', '副本数量')"></el-input>
		</div>
		...
		<el-form-item label="签单时间" class="flex-item">
			<elDatePickerLimit disabled size="mini" v-model="createItem.signTime" type="date" style="width: 100%" clearable placeholder="请选择" value-format="yyyy-MM-dd" />
		</el-form-item>
		<el-form-item label="签单地点" class="flex-item">
			<el-input disabled size="mini" clearable placeholder="请输入" v-model="createItem.signAddress"></el-input>
		</el-form-item>
		<el-form-item required prop="payMode" label="付款方式" :show-message="false" class="flex-item">
			<el-select disabled v-model="createItem.payMode" size="mini" clearable placeholder="请选择" style="width: 100%">
		...
		<el-form-item required prop="shipper" :show-message="false" class="flex-item col-3 shipper-rel">
			<span slot="label" style="position:relative;z-index:200">
				...
				<el-button disabled type="text" size="mini" @click="openContactPerson('发货人', '')">选择</el-button>
			</span>
			<TextAreaItem
				isNeedCheckRowLen
		...
		<el-form-item required prop="consignee" label="Consignee(收货人)" :show-message="false" class="flex-item col-3 shipper-rel">
			<span slot="label" style="position:relative;z-index:200">
				...
				<el-button disabled type="text" size="mini" @click="openContactPerson('收货人', '')">选择</el-button>
			</span>
			<TextAreaItem
				isNeedCheckRowLen
				...
				:rows="6"
				show-word-limit
				:value.sync="createItem.consignee"
			></TextAreaItem>
		</el-form-item>
		<div class="flex-item col-3">
			<div class="item-label notify">
				<div class="notify-flex-row">
					...
					<el-button disabled type="text" size="mini" @click="openContactPerson('通知人', notify)">选择</el-button>
				</div>
				<!-- <el-button disabled size="mini" type="text" class="same" @click="handleClickNotify">SAME AS CONSIGNEE</el-button> -->
			</div>
		...
		<el-form-item class="flex-item col-3 shipper-rel">
			...
			<TextAreaItem
				@dblclick.native="editDetail('forwardAgent')"
				disabled
				keys="forwardAgent"
				label="Forwarding Agent（运输代理人）"
				maxlength="512"
				:rows="6"
				show-word-limit
				:value.sync="createItem.forwardAgent"
			></TextAreaItem>
		</el-form-item>
		...
		<el-dialog class="edit-detail-dlg" :title="editTitle" :visible.sync="editVisible" width="50%">
			<TextAreaItem :regExp="editReg" disabled keys="editForm" :maxlength="editInputMaxLength.toString()" :rows="20" show-word-limit :value.sync="editValue"></TextAreaItem>
...
#put-info-rail .el-input__prefix {
	left: 0;
}
.train-number {
...
#put-info-rail .notify .lab.active {
	color: #1890ff;
	// font-weight: bold;
	position: relative;
	&::before {
...
.row-flex#put-info-rail .flex-item {
	padding-bottom: 0px !important;
}
.row-flex#put-info-rail .flex-item .plus {
	font-size: 16px;
	color: #1890ff;
	line-height: 20px;
	cursor: pointer;

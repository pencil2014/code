<template>
	<div class="row-flex ml10 put-detail-bl" id="blInfo">
		<el-row class="put-detail-bl-row">
			...
			<el-form-item :required="createItem.blType === 'mbl'" label="出单方式" prop="blMode" :show-message="false" class="flex-item">
			...
			<el-form-item required label="付款方式" prop="orderInfo.payMode" :show-message="false" class="flex-item">
			...
			<el-form-item label="合约号" prop="orderInfo.contractNo" :show-message="false" class="flex-item" v-if="!isBulkRo">
				<el-input size="mini" clearable placeholder="请输入" v-model="createItem.orderInfo.contractNo" :disabled="disabledState"></el-input>
			</el-form-item>
			<el-form-item label="合约公司" prop="orderInfo.contractCompany" :show-message="false" class="flex-item" v-if="!isBulkRo">
				<el-input size="mini" clearable placeholder="请输入" v-model="createItem.orderInfo.contractCompany" :disabled="disabledState"></el-input>
			</el-form-item>
		</el-row>
		<el-row class="put-detail-bl-row">
			<el-form-item :required="consigneeRequired" label="Shipper(发货人)" prop="blContact.shipper" :show-message="false" class="flex-item col-3 shipper-rel">
				...
				<TextAreaItem isNeedCheckRowLen @dblclick.native="editDetail('shipper')" :disabled="disabledState" :required="consigneeRequired" keys="shipper" label="Shipper(发货人)" :rows="6" :value.sync="createItem.blContact.shipper"></TextAreaItem>
			</el-form-item>
			<el-form-item :required="consigneeRequired" label="Consignee(收货人)" prop="blContact.consignee" :show-message="false" class="flex-item col-3 shipper-rel">
				<span slot="label" style="position:relative;z-index:200">
			...
			<div class="flex-item col-3">
				<div style="margin-bottom:8px;" class="item-label notify">
					<div class="notify-flex-row">
						<span class="lab" :class="notify === '1' ? 'active' : ''" @click="handleNotifyClick('1')"><em class="red" v-if="consigneeRequired">*</em>Notify Party(通知人)</span>
				...
				<el-form-item :required="consigneeRequired" prop="blContact.notify1" :show-message="false" v-show="notify === '1'" class="shipper-rel notify-rel">
					<TextAreaItem
						isNeedCheckRowLen
				...
				<el-form-item prop="blContact.notify2" :show-message="false" v-show="notify === '2'" class="shipper-rel notify-rel">
					<TextAreaItem isNeedCheckRowLen @dblclick.native="editDetail('notify2')" :disabled="disabledState" :required="consigneeRequired" keys="notify2" label="Also Notify Party" :rows="6" :value.sync="createItem.blContact.notify2"></TextAreaItem>
				</el-form-item>
			</div>
			<el-form-item label="Forwarding Agent（运输代理人）" prop="orderInfo.forwardAgent" :show-message="false" class="flex-item col-3 shipper-rel">
				...
				<TextAreaItem @dblclick.native="editDetail('forwardAgent')" :disabled="disabledState" keys="forwardAgent" label="Forwarding Agent（运输代理人）" :rows="6" :value.sync="createItem.orderInfo.forwardAgent"></TextAreaItem>
			</el-form-item>
		...
		<el-row class="put-detail-bl-row">
			...
			<el-form-item label="大船航次" prop="orderInfo.voyage" :show-message="false" class="flex-item">
				...
				<!-- <el-autocomplete
					...
					clearable
					v-model="createItem.orderInfo.voyage"
					:fetch-suggestions="(queryString, cb) => querySearchBookVoyage(queryString, cb, createItem.orderInfo.vessel)"
					:trigger-on-focus="false"
					placeholder="请输入"
					:disabled="disabledState"
			...
			<el-form-item label="船公司" prop="orderInfo.shipCarrierCode" :show-message="false" class="flex-item webUrl">
				<el-select
					size="mini"
				...
				<el-button disabled type="default" size="mini" class="webUrl-btn" style="margin-left: 5px;" @click="getShippingCarrierUrl(createItem.orderInfo.shipCarrierCode)">船司官网</el-button>
			</el-form-item>
			<el-form-item :required="isBarge" :label="isBarge ? '驳船收货地' : '收货地'" prop="orderInfo.bargePorPortName" :show-message="false" class="flex-item">
				<el-autocomplete
					:debounce="0"
					...
					placeholder="请输入"
					:disabled="disabledState"
					@select="value => handleSelectPort(value)"
					@clear="value => handleClearPort(value)"
				></el-autocomplete>
			</el-form-item>
			...
			<el-form-item label="预计到港时间" prop="orderInfo.etaTime" :show-message="false" class="flex-item">
				<elDatePickerLimit size="mini" v-model="createItem.orderInfo.etaTime" type="date" style="width: 100%" clearable placeholder="请选择" value-format="yyyy-MM-dd" :picker-options="pickerOptions" :disabled="disabledState" />
			</el-form-item>
			<el-form-item label="大船实际开船时间" prop="orderInfo.tdTime" :show-message="false" class="flex-item vessel-time">
				<elDatePickerLimit noCanFuture size="mini" v-model="createItem.orderInfo.tdTime" type="date" style="width: 100%" clearable :disabled="disabledState" placeholder="请选择" value-format="yyyy-MM-dd" />
			...
			<el-form-item label="驳船实际开船时间" prop="orderInfo.bargeTdTime" :show-message="false" class="flex-item" v-if="isBarge">
				<elDatePickerLimit noCanFuture size="mini" v-model="createItem.orderInfo.bargeTdTime" type="date" style="width: 100%" clearable :disabled="disabledState" placeholder="请选择" value-format="yyyy-MM-dd" />
			</el-form-item>
			<el-form-item required label="装船时间" prop="orderInfo.shipmentTime" :show-message="false" class="flex-item" v-if="isBulkRo">
		...
		<el-row class="put-detail-bl-row">
			<el-form-item label="唛头" prop="cargoMark" :show-message="false" class="flex-item col-2 shipper-rel">
				<TextAreaItem @dblclick.native="editDetail('cargoMark')" :regExp="cargoReg" :disabled="disabledState" keys="cargoMark" label="唛头" :rows="5" :value.sync="createItem.cargoMark"></TextAreaItem>
			</el-form-item>
			<el-form-item :required="markDescRequired" label="货物描述" prop="cargoDesc" :show-message="false" class="flex-item col-2 shipper-rel">
				<TextAreaItem @dblclick.native="editDetail('cargoDesc')" :regExp="cargoReg" :disabled="disabledState" :required="markDescRequired" keys="cargoDesc" label="货物描述" :rows="5" :value.sync="createItem.cargoDesc"></TextAreaItem>
			</el-form-item>
		</el-row>
		...
		<el-dialog class="edit-detail-dlg" :title="editTitle" :visible.sync="editVisible" width="50%">
			<TextAreaItem :regExp="editReg" :disabled="disabledState" keys="editForm" :maxlength="editInputMaxLength.toString()" :rows="20" :show-word-limit="editInputMaxLength ? true : false" :value.sync="editData"></TextAreaItem>
...
export default {
	...
	methods: {
		...
		handleClickNotify() {
			if (this.notify === '1') {
				this.$set(this.createItem.blContact, 'notify1', 'SAME AS CONSIGNEE')
			}
			if (this.notify === '2') {
				this.$set(this.createItem.blContact, 'notify2', 'SAME AS CONSIGNEE')
			}
		},
		// 签单人下拉搜索自定义查询
		...
		fillPolPort(portCodes) {
			...
			if (portCodes) {
				this.$store.dispatch('dict/basePortListByCodes', { portCodes }).then(data => {
					...
					this.createItem.orderInfo.bargePorPortName = this.getPortName(data, this.createItem.orderInfo.bargePorPortCode)
					this.createItem.orderInfo.polPortName = this.getPortName(data, this.createItem.orderInfo.polPortCode)
					this.createItem.orderInfo.unloadingPortName = this.getPortName(data, this.createItem.orderInfo.unloadingPortCode)
					this.createItem.orderInfo.podPortName = this.getPortName(data, this.createItem.orderInfo.podPortCode)
					this.createItem.orderInfo.transitPortName = this.getPortName(data, this.createItem.orderInfo.transitPortCode)

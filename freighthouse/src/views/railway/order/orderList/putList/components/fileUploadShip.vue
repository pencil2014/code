<template>
	<BaseDialog :config="dialogConfig" :callback="dialogCallback">
		<el-form ref="uploadFileForm" :model="createdItem" :rules="rules" label-position="right" label-width="90px" class="upload-ship-form">
			<el-form-item label="附件类型：" prop="fileType" :show-message="false">
				<el-select @change="handleSelect" size="mini" placeholder="请选择" v-model="createdItem.fileType" clearable filterable>
...
export default {
	...
	created() {
		if (this.upType) {
			// this.createdItem.fileType = this.upType
			if (this.upType === 'mbl_telex_original') {
				...
				switch (this.blMode) {
					case 'telex_release':
						this.createdItem.fileType = 'mbl_telex_original'
						this.blFileTypeOptions = [{ label: '船东电放件', value: 'mbl_telex_original' }]
						break
					case 'free_telex_release':
						...
						break
					case 'seaway_bill':
						this.createdItem.fileType = 'mbl_telex_swb'
						this.blFileTypeOptions = [{ label: '船东SWB', value: 'mbl_telex_swb' }]
						break
					case 'bill_at_destination':
						this.createdItem.fileType = 'mbl_telex_destination'
						this.blFileTypeOptions = [{ label: '船东目放', value: 'mbl_telex_destination' }]
						break
					case 'ibl':
						this.createdItem.fileType = 'mbl_telex_ibl'
						this.blFileTypeOptions = [{ label: '船东云提单', value: 'mbl_telex_ibl' }]
						break
				...
				switch (this.blMode) {
					case 'telex_release':
						this.createdItem.fileType = 'mbl_telex_release_guarantee'
						this.blFileTypeOptions = [{ label: '电放申请保函', value: 'mbl_telex_release_guarantee' }]
						break
					case 'free_telex_release':
						this.createdItem.fileType = 'mbl_telex_release_guarantee'
						this.blFileTypeOptions = [{ label: '电放申请保函', value: 'mbl_telex_release_guarantee' }]
						break
					case 'seaway_bill':
						this.createdItem.fileType = 'mbl_telex_guarantee_swb'
						this.blFileTypeOptions = [{ label: '船东SWB申请保函', value: 'mbl_telex_guarantee_swb' }]
						break
					case 'bill_at_destination':
						this.createdItem.fileType = 'mbl_telex_guarantee_destination'
						this.blFileTypeOptions = [{ label: '船东目放申请保函', value: 'mbl_telex_guarantee_destination' }]
						break
					case 'ibl':
						this.createdItem.fileType = 'mbl_telex_guarantee_ibl'
						this.blFileTypeOptions = [{ label: '船东云提单申请保函', value: 'mbl_telex_guarantee_ibl' }]
						break
	...
	methods: {
		...
		dialogCallback(action, done) {
			...
			if (action === 'Confirm') {
				if (this.upType === 'mbl_telex_release_guarantee' && fileList.length === 0) {
		...
		handleImport(done) {
			...
			let params = {
				data: {
					blId: this.$route.query.blId || this.param.blId,
					orderNo: this.$route.query.orderNo || this.param.orderNo,

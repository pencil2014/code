<template>
	<div class="si-detail">
		...
		<div v-if="activeFile === 'valid'" class="si-table">
			<el-table @selection-change="handleSelectionChange" fit highlight-current-row style="width: 100%" key="valid" :data="validFiles">
				...
				<el-table-column prop="putStatus" label="放单状态" align="center">
					<template slot-scope="scope">
						<span>{{ getPutStatus(scope.row) }}</span>
					</template>
				</el-table-column>
				...
				<el-table-column prop="putFinishTime" label="放单完成时间" align="center">
					<template slot-scope="scope">
						<span>{{ getPutTime(scope.row) }}</span>
					</template>
				</el-table-column>
				<el-table-column width="180" label="操作" align="center">
					<template slot-scope="scope">
						<div class="operate-group">
							...
							<el-button
								v-if="
									mblTelexBlArr.includes(scope.row.fileType) ||
										(['mbl_telex_release_guarantee', 'mbl_telex_guarantee_swb', 'mbl_telex_guarantee_ibl', 'mbl_telex_guarantee_destination'].includes(scope.row.fileType) && detailData.putStatus !== 'finish')
								"
								size="mini"
								type="text"
		...
		<div v-if="activeFile === 'invalid'" class="si-table">
			<el-table fit highlight-current-row style="width: 100%" key="invalid" :data="invalidFiles">
				<el-table-column type="selection" width="55"> </el-table-column>
				<el-table-column prop="fileType" label="附件类型" align="center">
					<template slot-scope="scope">
						{{ scope.row.fileType | fileTypeFilter }}
		...
		<div v-if="uploadShow">
			<FileUpload :fileType="curfileType" @close="uploadPopClose" />
		</div>
...
export default {
	...
	data() {
		return {
			...
			mblTelexBlArr: ['mbl_telex_original', 'mbl_telex_swb', 'mbl_telex_ibl', 'mbl_telex_destination']	// 船东电放件类型
		}
	},
	props: {
		detailData: {
	...
	mounted() {
		console.log('🚀 ~ this.dictMap.blFileType', this.dictMap.blFileType)
	...
	methods: {
		putRelation() {
			...
			for (let index = 0; index < this.multipleSelection.length; index++) {
				...
				if (![...telexList, ...tradeBdlist].includes(item.fileType)) {
					...
					// 出单方式为电放 附件为正本 财务同意 且客户同意放货或者特殊审批通过 不提示 v20220510
      ...
      this.expressRelationShow = false
		...
		getPutTime(row) {
			let text
			if (
				['mbl_telex_release_guarantee', 'mbl_telex_guarantee_swb', 'mbl_telex_guarantee_ibl', 'mbl_telex_guarantee_destination'].includes(row.fileType) ||
				(['mbl_original', 'hbl_original'].includes(row.fileType) && this.detailData.blMode === 'telex_release')
			) {
				// 电放保函 或 正本且出单方式为电放
				text = '--'
			} else {
				text = row.putFinishTime
			}
			return text
		},
		getPutStatus(row) {
			let text
			if (['release_guarantee', 'cust_guarantee', 'mbl_telex_release_guarantee', 'mbl_telex_guarantee_swb', 'mbl_telex_guarantee_ibl', 'mbl_telex_guarantee_destination'].includes(row.fileType)) {
				// 电放保函 放单保函
				text = '--'
			} else if (this.mblTelexBlArr.includes(row.fileType)) {
				// 电放
			...
			} else if (['mbl_original', 'hbl_original'].includes(row.fileType)) {
				// 正本
				if (this.detailData && this.detailData.blMode === 'telex_release') {
					//正本且出单方式为电放不显示状态
					text = '--'
			...
			this.$confirm('是否失效该条附件?', '警告', {
				...
				.then(response => {
					this.$message({
						...
						duration: 1000,
						onClose: () => {
							// 船东电放件失效成功后，船东完成放单复选框设置为未勾选，去掉disabled
							if (this.mblTelexBlArr.includes(row.fileType)) {
								this.$emit('getDetail')
								// this.$emit('updateTelexDownStatus')
							}
							this.getFileList()
						}
					})
		...
		handlePost() {
			...
			for (let index = 0; index < this.multipleSelection.length; index++) {
				...
				if (![...telexList, ...tradeBdlist].includes(item.fileType)) {
					...
					// 出单方式为电放 附件为正本 财务同意 且客户同意放货或特殊审批通过 不提示 v20220510
				} else if (((originList.includes(this.detailData.blMode) || telexList.includes(item.fileType)) && (['y', 'e'].includes(this.detailData.isAgreeRelease) || this.detailData.agreeReleaseStatus === 'pass')  && this.detailData.auditStatus === 'pass') || tradeBdlist.includes(item.fileType)) {
				} else {
					// 不满足条件则弹出提示
		...
		getFileList() {
			...
			takePutFiles(data).then(res => {
				this.validFiles = res.data.validFiles || []
				this.invalidFiles = res.data.invalidFiles || []
			})
		},
		// 预览
		handleView(row) {
		...
		uploadPopClose(action) {
			...
			if (action === 'Confirm') {
				this.getFileList()
				this.$emit('file', this.curfileType)
			}
		},
		// 上传前钩子获取file名
		getFile(file) {

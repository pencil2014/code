<template>
	<div class="popFileList">
		<BaseDialog :config="dialogConfig" :callback="dialogCallback">
			<div class="customsClearance-bewteen">
				<span class="ft12"> {{ param.blType === 'mbl' ? `MBL单号：${param.blNo}` : `HBL单号：${param.blNo}` }}</span>
				<div class="row-tit-operate">
					<!-- <el-button size="mini" type="primary" @click="handleUploadFile" v-if="param.putStatus !== 'finish'">上传附件</el-button> -->
				</div>
			</div>
		...
		<div v-if="uploadShow">
			<BlFileUpload :param="param" @close="uploadPopClose" />
		</div>
	</div>
</template>
<script>
...
import BlFileUpload from './fileUpload'
...
export default {
	filters: {
		fileTypeFilter(fileType) {
			...
			fileTypeOptions = [...fileTypeOptions, { label: '保险', value: 'bd' }]
...
.popFileList .operate-group .el-button {
	white-space: normal;
}
</style>

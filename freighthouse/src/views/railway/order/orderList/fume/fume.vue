	...
	<div class="edit-row fume-row" v-loading="lsLoading" :element-loading-text="eLoadingText" :element-loading-spinner="eLoadingSpinner" :element-loading-background="eLoadingBackground">
		<div class="table-com-layout" id="tableCont">
			<div class="between h28 mt04">
				...
				<div>
					<el-dropdown size="small" @command="handleCommand">
						<el-button size="mini" type="default" :disabled="!isBkg" class="mr10"> 输出<i class="el-icon-arrow-down el-icon--right"></i> </el-button>
			...
			<el-table ref="multipleTable" fit style="width: 100%" :data="list" @selection-change="handleSelectionChange" class="mt04">
				...
				<el-table-column prop="containers" label="箱型箱量" align="center" width="120" v-if="detailInfo.businessType === 'rail_export_fcl'"></el-table-column>
		...
		<div class="component-cont fume-detail" v-if="showDetail">
			...
			<div class="row-cont margin0 borderTop">
				...
				<el-form ref="fumeSave" :model="createItem" :rules="rules" class="row-flex" style="padding-left: 0; margin-top: 8px" label-width="90px">
					...
					<el-form-item label="箱型箱量" prop="containers" class="flex-item col-6" :show-message="false" v-if="detailInfo.businessType === 'rail_export_fcl'">
			...
			<!-- 申请人列表 -->
			<ApplyFume ref="applyFume" :isJoint="isJoint" :detailData="createItem" :fumeBls="createItem.fumeBls" :detailInfo="detailInfo" />
		</div>
	</div>
</template>
<script>
...
export default {
	...
	computed: {
		...
		disabledFume() {
			...
			return !this.isBkg || this.isJoint || this.isFinish
		},
		isBkg() {
		...
		isJoint() {
			...
			let findItem = this.orderInfo.serviceInfoList.find(item => item.serviceCode === 'fume')
	...
	methods: {
		...
		// 集装箱列表
		getContainerList() {
			this.$store.dispatch('orderService/getRailContainerList', {
		...
		// 提单号列表
		getBlNoList() {
			this.$store.dispatch('orderService/getRailBlNoList', {
				orderNo: this.oQuery.orderNo,
				jointNo: this.oQuery.jointNo
			})
		},
...
.edit-container .component-cont.fume-detail .row-flex .flex-item .el-form-item__content .el-date-editor .el-input__prefix{
	left: 0;
}

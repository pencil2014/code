	...
	<div class="take-detail">
		...
		<div class="take-file-info">
          ...
          <i class="hamburger el-icon-arrow-right" :class="{'is-active':true}"></i>
				...
				<el-form ref="takeInfo" class="detail-form" :rules="rules" :model="createItem" label-width="65px">
					<el-row>
						...
						<el-col :span="12" class="info-li">
							<span>HBL草单确认件：</span>
							<span @click="handleDownload(item)" v-for="(item, index) in hblFiles" :key="index" style="margin:0 6px;color:#1890ff;cursor:pointer;">{{ item.fileName }} 下载</span>
						</el-col>
				...
				<FileList @changeFileNos="changeFileNos" @getDetail="getDetail" :detailData="detailData" />
...
export default {
	...
	computed: {
		...
		hblFiles() {
			if (this.detailData.valids) {
				return this.detailData.valids.filter(item => item.fileType === 'hbl_draft')
			}
			return []
		}
	},

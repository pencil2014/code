<template>
	<div class="app-container">
		...
		<div v-if="uploadShow">
			<FileUpload :row="rowData" @close="uploadPopClose" />
		</div>
	</div>
</template>
<script>
...
const defaultTableQuery = {
	...
	scenesCode: 'RailwayTradeTakeList'
}
...
export default {
	...
	data() {
		return {
			...
			tableConfig: {
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.railway.trdeTakeList.columnsBase(this.handleOrderNoInfo),
				// 操作按钮组
				operationBtns: {
					minWidth: '120',
					fixed: 'right',
	...
	computed: {
		...mapState({
			...
			blTakeList: state => state.railway.trdeTakeList,
			defaultColumns: state => state.railway.trdeTakeList.defaultColumns,
			specialColumns: state => state.railway.trdeTakeList.specialColumns,
			columnsBase: state => state.railway.trdeTakeList.columnsBase()
		}),
		isAc() {
	...
	methods: {
		...
		getListQuery() {
			...
			let typeQuery = [{ column: 'takeType', type: 'eq', value: 'trade' }]
			// 合并状态参数
			let data = {}

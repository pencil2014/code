<template>
	<div class="app-container">
		...
		<div v-if="uploadShow">
			<FileUpload sourceType="RailwayHblTakeList" :row="rowData" @close="uploadPopClose" />
		</div>
	</div>
</template>
<script>
...
import FileUpload from './components/fileUploadTable'
...
export default {
	...
	data() {
		return {
			...
			tableConfig: {
				...
				configColumns: [],
				columns: [],
				allColumns: store.state.railway.hblTakeList.columnsBase(this.handleOrderNoInfo),
				// 操作按钮组
				operationBtns: {
					minWidth: '120',
					fixed: 'right',
	...
	computed: {
		...mapState({
			...
			blTakeList: state => state.railway.hblTakeList,
			defaultColumns: state => state.railway.hblTakeList.defaultColumns,
			specialColumns: state => state.railway.hblTakeList.specialColumns,
			columnsBase: state => state.railway.hblTakeList.columnsBase()
		}),
		isAc() {
	...
	methods: {
		...
		getListQuery() {
			...
			let typeQuery = [{ column: 'takeType', type: 'eq', value: 'hbl' }]
			// 合并状态参数
			let data = {}

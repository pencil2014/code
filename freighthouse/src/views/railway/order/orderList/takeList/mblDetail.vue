<template>
	<div class="take-container">
		...
		<component @getDetail="getDetail" :transferArr="transferArr" @transferChange="transferChange" @changeFileNos="changeFileNos" :is="activityKey" :createItem="createItem" :orderWorkOptions="detailData.workAssign" :detailData="detailData">
		</component>
	</div>
</template>
<script>
...
import Info from './components/mblTakeInfo'
...
export default {
	...
	computed: {
		...
		disabledPut() {
			return this.detailData.blMode === 'original' && this.detailData.validFiles.filter(item => item.fileType === 'mbl_original').length === 0
		},
		showCancel() {
		...
		getDetail() {
			blTakeInfo({
				...
				this.detailData.workAssign.forEach(item => {
					...
					item.disabled = true
					// MBL取单客服可以编辑
					if (['tc'].includes(item.jobCode)) item.disabled = false
					// 获取撤回人
					if (item.jobCode === 'doc') {
				...
				// this.getFileList()
				this.fileListData = this.detailData.valids

		...
		<div class="row-body">
			<el-form ref="form" :rules="rules" :model="formData" class="form-body" label-width="90px">
				...
				<div class="row-tit h32">
					...
					<div class="row-tit-operate">
						<el-button size="mini" type="primary" @click="handleAddBox" :disabled="isBd || isDisabledEdit">添加集装箱</el-button>
					</div>
				</div>
						...
						<el-table-column label="操作" width="150" align="center" fixed="right">
							<template slot-scope="scope">
								...
								<div class="operate-group" v-else>
									<el-button @click="handleAddCargo(scope.row, item)" type="text" size="mini" :disabled="isBd || isDisabledEdit" v-if="!scope.$index">添加货物箱号</el-button>
									<el-button @click="handleDelete(scope.row, scope.$index, index)" type="text" size="mini" :disabled="isBd || isDisabledEdit">删除</el-button>
								</div>
							</template>
						</el-table-column>
					</el-table>
...
export default {
	...
	watch: {
		detailData: {
			handler(newValue) {
				...
				let boxList = []
				console.log('🚀 ~ this.formData.cargoInfoList', this.formData.cargoInfoList)
	...
	computed: {
		...
		isBd() {
			return (!this.isBkg && !this.isEpricing && !this.isEcs) || this.isJoint
		},
		isJoint() {

<template>
	<div>
		<BaseDialog :config="dialogConfig" :callback="dialogCallback">
			<el-form id="podAddWarehouseOutPlan" ref="form" :rules="rules" :model="formData" label-width="90px">
				<el-row>
					<el-col :span="6">
            ...
            <el-input size="mini" placeholder="请输入" v-model="formData.whBatchNo" clearable></el-input>
				...
				<div>
					<el-table ref="outPlanBox" border fit highlight-current-row stripe style="width: 100%" class="outPlanBoxTable" :data="tableData" show-summary :summary-method="getSummaries">
						...
						<el-table-column prop="quantity" label="实际出仓件数" align="center" width="100">
						...
						<el-table-column prop="lastWeight" label="剩余库存毛重(KGS)" align="center" width="150">
							<!-- <template slot-scope="scope">
              <el-input disabled size="mini" placeholder="请输入毛重" v-model="scope.row.lastWeight" clearable @blur="validateNum('lastWeight', scope.row)"></el-input>
            </template> -->
						</el-table-column>
						<el-table-column prop="weight" label="实际出仓毛重(KGS)" align="center" width="150">
							<template slot-scope="scope">
						...
						<el-table-column prop="volume" label="实际出仓体积(CBM)" align="center" width="150">
							<template slot-scope="scope">
						...
						<el-table-column prop="packageType" label="实际出仓包装" align="center" width="120">
							<template slot-scope="scope">
...
import { outReqno } from '@/api/order/podService/warehouse'
...
export default {
	...
	data() {
		return {
			...
			formData: {
				...
				outBatchNo: this.detailData.outBatchNo || '',
	...
	computed: {
		...
		isImportJoint() {
			return ['ship_import_fcl', 'ship_import_lcl', 'ship_oversea_wh'].includes(this.orderInfoDataObj.businessType) && this.$route.query.source === 'jointList'
		}
	},
	components: {
		BaseDialog,
	...
	methods: {
		...
		createSerialNo() {
			outReqno().then(res => {
				this.formData.outBatchNo = res.data.seqNo
			})
		},
		// 选择货品
		handleAddCargo() {
			...
			if (action === 'Confirm') {
				this.$refs.form.validate(valid => {
					if (valid) {
						...
						if (!this.detailData.outPlanTime) return this.$message.error('请先填写预计出仓时间!')

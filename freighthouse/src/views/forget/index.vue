<template>
  <div class="finance-page reset-page" v-if="showPage">
    <div class="user-header">
      ...
      <img v-if="!isReset" src="./img-email.png" alt />
      <img v-else src="./img-lock.png" alt />
      <div class="user-wish">{{isReset?'设置密码':'邮箱验证'}}</div>
    </div>
    <div class="bg-ball ball-3"></div>
    <div class="bg-ball ball-4"></div>
    <div v-if="!isReset" class="middle-title">如果您忘记密码，可通过登录账号、公司邮箱重设登录密码</div>
    <div v-else class="middle-title">密码需至少包含数字、字母或特殊字符的两种，且长度在8-16位之间</div>
    <el-form v-if="!isReset" ref="emailForm" :model="emailForm" :rules="emailRules">
      ...
      <el-button :loading="loading" type="primary" @click="handleEmail" style="height:32px;background:#3E80F5;border:none;">确定验证</el-button>
    </el-form>
    <el-form v-else ref="loginForm" :model="loginForm" :rules="loginRules" label-position="right">
      <el-form-item label="新密码" prop="passWordNew">
        <el-input :type="pwdType[1]" v-model="loginForm.passWordNew" placeholder="请输入新密码" @input="passWordNewInput">
      ...
      <el-form-item label="确认新密码" prop="passWordSure">
        <el-input :type="pwdType[2]" v-model="loginForm.passWordSure" placeholder="请再次输入新密码" @input="passWordSureInput">
          <i slot="suffix" @click="togglePwdType(2)">
            <svg-icon :icon-class="pwdType[2]=='password' ? 'eye' : 'eye-open'" />
          </i>
        </el-input>
      </el-form-item>
      <el-button :loading="loading" type="primary" @click="handleRest" style="height:32px;background:#3E80F5;border:none;">完成设置</el-button>
    </el-form>
...
import { sendResetPasswordEmail, resetPasswordByEmail, checkCodeExpired } from '@/api/base'
...
let rules2 = [
  rules,
  {
    validator: (rule, value, callback) => {
      value = value.trim()
      var regx = /^(?!([a-zA-Z]+|\d+|[`~!@#$%^&*()_+={};:'"|,<.>?]+)$)[a-zA-Z\d`~!@#$%^&*()_+={};:'"|,<.>?]{8,16}$/ // /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z\W_]{8,16}$/
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loginRules: { passWordNew: rules2, passWordSure: rules2 },
      loading: false,
  ...
  methods: {
    ...
    checkCodeExpired() {
      ...
      checkCodeExpired(data).then(res => {
        if (res.data === 'expired') {
    ...
    //   }
    //   checkCodeExpired(data).then(res => {
    //     if (res.data.isValid === 'no' && res.data.isActived === 'no') { // res.data === 'expired'
    //       window.alert('激活链接已过期，请重新申请')
    //       window.location.href="about:blank"
    //       window.close()
    //     } else if (res.data.isValid === 'no' && res.data.isActived === 'yes') {
    //       this.$message({
    //         message: '该账户已激活，将跳转至登录界面；如忘记密码，请通过右下方的忘记密码重置',
    //         type: 'error',
    //         showClose: true,
    //         duration: 5000
    //       })
    ...
    handleEmail() {
      this.$refs.emailForm.validate((valid) => {
        if (!valid) return
        this.loading = true
        sendResetPasswordEmail({ loginAccount: this.emailForm.username, email: this.emailForm.email })
          .then((res) => {
            this.loading = false
            this.rubbishConfirm()
          })
    ...
    passWordSureInput() {
      if (this.loginForm.passWordSure.length > 16) {
        this.$msgErrClose('最多输入16位')
        this.loginForm.passWordSure = this.loginForm.passWordSure.substring(0, 16)
      }
    },
    // 重置密码
    handleRest() {
      ...
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          ...
          this.loading = true
          resetPasswordByEmail({ tokenCode: this.code, newPassWord: sha256(this.loginForm.passWordNew) })
            .then((res) => {
    ...
    rubbishConfirm() {
      ...
      this.$msgbox({
        message: h('div', { style: 'font-size:12px;text-align:center' }, [
          ...
          h('div', { style: 'font-weight: bold;color:#222222;padding:8px 0' }, '发送成功！'),
          h('div', { style: 'color: #4A4A4A;line-height:18px;width:260px' }, [
            h('span', `已发送至您公司邮箱${this.emailForm.email}请点击`),
            h('a', { style: 'color: #3E80F5', attrs: { href: 'https://exmail.qq.com/login' } }, '登录该邮箱'),
            h('span', '根据邮件提示操作。'),
          ]),
        ]),

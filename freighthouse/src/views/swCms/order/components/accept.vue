<template>
  <el-dialog title="订单受理" :visible.sync="option.show" width="600px" :before-close="cancel">
    <div>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px" inline size="mini" class="accept-form">
        ...
        <el-form-item label="拒绝原因" prop="auditRefuseReason" v-if="form.acceptOrder === 'no'">
          <el-input
            type="textarea"
          ...
          api(data)
            .then((res) => {
              if (res.code === 0) {
                this.cancel()
                this.$parent.getData()
                let text = isAccept ? '订单受理成功' : '订单拒绝成功'

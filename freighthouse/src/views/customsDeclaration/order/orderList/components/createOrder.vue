<template>
	<BaseDialog :config="dialogConfig" :callback="dialogCallback">
		<el-form ref="createOrderForm" :rules="rules" :model="createdItem" label-position="right" label-width="80px" class="create-order-form">
			...
			<el-form-item label="关联工作单" prop="exportOrderNo" v-if="custClass === 'department'">
		...
		getBusinessType() {
			baseBusinessList().then(res => {
				...
				this.businessTypeOptions = data
					.filter(item => ['ship_customs','ship_export_customs','ship_import_customs'].includes(item.businessType))
					.map(item => {
				...
				this.businessTypeOptions.map(item => {
					if (item.businessType === val) {
						...
						if (this.serviceTypeOptions.length === 1) {
							this.createdItem.serviceType = this.serviceTypeOptions[0].serviceType
						}
					}
				})
				this.getSellers(val)

	...
	<div class="finance-table" :class="{ max__height__table: $attrs.autoMax }" :id="option.id || 'option1'">
			...
			<vxe-table
				...
				@sort-change="
					({ column, property, order }) => {
						...
						let prop = property
						option.sortChange && option.sortChange(column, prop, order, option)
					}
				"
				...
				row-id
			>
				<vxe-column v-if="option.selection && option.selection.show" type="checkbox" :width="option.selection.width || 40" :fixed="option.selection.fixed ? 'left' : ''" :resizable="option.selection.resizable === false ? false : true"></vxe-column>
				<vxe-column v-if="option.customColumns && option.customColumns.show" :width="30" :fixed="option.customColumns.fixed === false ? '' : 'left'" :resizable="true">
					<template #header>
					...
					<template #default="{ row }">
						<div class="td_tag_div" v-if="row._tagArr && row._tagArr.length && row._tagArr[0].text === '协'">
							<span class="finance-self-tag" v-for="tag in row._tagArr || []" :class="[tag.type]" :key="tag.type">{{ tag.text }}</span>
						</div>
					</template>
				</vxe-column>
				<vxe-column
					...
					:width="(option.index && option.index.width) || 50"
				...
				<!-- 多表头，只做了普通渲染和按钮 -->
				<vxe-table-colgroup :title="colgroup.title" v-for="(colgroup, index) in colgroups" :key="'colgroups' + index" :header-class-name="'colgroup_diy_' + colgroup.className">
					<vxe-column
				...
				<vxe-table-colgroup :title="colgroup.title" v-for="(colgroup, index) in colgroups2" :key="'colgroups' + index" :header-class-name="'colgroup_diy_' + colgroup.className">
					<vxe-column
						v-for="(item, index) in colgroup.columns.filter(subItem => subItem)"
						:key="item.prop + index"
						:title="item.label"
				...
				>
					<template #default="{ row, rowIndex }">
						<template v-for="(item, index) in option.operationBtns.data">
							<span :key="'operationBtns' + index" class="operationbtns-wrapper" v-if="item.show || item.showFn(row, rowIndex) || (!item.showFn && row.edit)">
		...
		<div class="finance-el-pagination" v-if="option.pagination && option.pagination.show" ref="refPagination">
			...
			<div class="pagination-container">
				<NewPagination v-if="option.pagination.isNew" :config="option.pagination" @size-change="handleSizeChangeNew" @current-change="handleCurrentChangeNew" />
				<el-pagination
					v-else
					size="mini"
					background
		...
		<component :is="currentComponent" :component-props="componentProps"> </component>
	</div>
</template>
<script>
...
export default {
	...
	data() {
		return {
			...
			COLORFUL: {
				...
				// 发票状态
				feeInvoiceStatus: { finish: 'green', nofinish: 'red', part_finish: 'yellow' },
				// 财务状态
	...
	methods: {
		...
		handleMulti(data) {
			...
			this.$emit(
				'send-multi',
				[...data].sort((a, b) => a.diyTimeStamp - b.diyTimeStamp),
				this.option
			)
		},

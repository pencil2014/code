  ...
  <div>
    <el-form :inline="true" :model="formData" label-width="100px" class="form" ref="form" size="mini" :hide-required-asterisk="isDisabled">
      ...
      <el-form-item label="出库号：">
        <el-input v-model="formData.outboundOrderNo" class="normalInput" disabled></el-input>
      </el-form-item>
      <el-form-item label="出库车牌：">
        <el-input v-model="formData.outboundLicensePlate" class="normalInput" disabled></el-input>
      </el-form-item>
      <el-form-item label="客户名称：">
        <el-input v-model="formData.orderCustName" class="normalInput" disabled></el-input>
      </el-form-item>
      <el-form-item label="货物预出总数：">
        <el-input v-model="formData.estimatedQuantity" class="normalInput" disabled></el-input>
      </el-form-item>
      <el-form-item label="货物实出总数：">
        <el-input v-model="formData.workQuantity" class="normalInput" disabled></el-input>
      </el-form-item>
      <el-form-item prop="workVolume" label="体积(CBM)：">
        <el-input size="mini" placeholder="" v-model="formData.workVolume" disabled class="normalInput"> </el-input>
      </el-form-item>
      <el-form-item prop="workWeight" label="重量(KGS)：">
        <el-input size="mini" placeholder="" v-model="formData.workWeight" disabled class="normalInput"> </el-input>
      </el-form-item>
      <!-- <el-form-item label="是否分唛：" prop="hasMarks" :rules="changeRule">
        <el-select v-model="formData.hasMarks" class="normalInput">
...
export default {
  ...
  methods: {
    ...
    setDispatch (data, isFirst) {
      ...
      let members = data?.dispatchDetail?.members || []
      if (data?.detailList?.length) {
        estimatedQuantity = data.detailList.map(item => item.estimatedQuantity ? Number(item.estimatedQuantity) : null).reduce((pre, cur) => {
          return pre + cur
        }, 0)
        workQuantity = (data.detailList.map(item => item.actualQuantity ? Number(item.actualQuantity) : null)).reduce((pre, cur) => {
          return pre + cur
        }, 0)
        actualVolume = (data.detailList.map(item => item.volume ? Number(item.volume) : null)).reduce((pre, cur) => {
          return pre + cur
        }, 0)
        actualWeight = (data.detailList.map(item => item.weight ? Number(item.weight) : null)).reduce((pre, cur) => {
          return pre + cur
        }, 0)
      }
      if (data) {
        ...
        this.formData.outboundOrderId = data.outboundOrderId
        this.formData.outboundOrderNo = data.outboundNo
        this.formData.inboundOrderId = data.inboundOrderId
        this.formData.inboundOrderNo = data.inboundNos
        this.formData.outboundLicensePlate = data.licensePlate
        this.formData.orderCustId = data.custId
        this.formData.orderCustName = data.custName
        ...
        // this.formData.members = this.activeName ? members : []
    ...
    handleSubmit () {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          return this.$message.error("请将派工信息必填内容填写完整！")
        }
      })
    },
    /**
    ...
    async validInfo () {
      ...
      if (errMap) {
        ...
        if (ids.length != uniqIds.length) {
          this.$message.error('派工信息中作业人员不能重复')
        } else {
          this.setTallyManId(data)
        }
      }

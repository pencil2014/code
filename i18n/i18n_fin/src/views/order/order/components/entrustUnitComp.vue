<template>
	...
	<span class="container">
		<span class="mr5">{{$t('OrderFee.client')}}:</span>
		<!-- 文字溢出才加tip -->
		<el-popover placement="top" width="200" trigger="hover" class="port-ename" popper-class="port-unit-popover">
			<div class="port-popover">
				<div>
					<span>{{ showName }}</span>
					<em @click="handleCopyPort(showName, $event)">{{$t('OrderFee.copy')}}</em>
				</div>
				<div class="red" v-if="detailForm.bdServiceMode">{{$t('OrderFee.bdServiceMode')}}：{{ detailForm.bdServiceMode | getDictLabelFilter('bdServiceMode') }}</div>
				<!-- 海运进口，空运进口才显示合同信息 -->
				<template v-if="['ship_import_fcl', 'ship_import_lcl', 'air_import'].includes(detailForm.businessType)">
					<div class="">{{$t('OrderFee.hasContract')}}：{{ detailForm.contract && detailForm.contract.contractNo ? $t('OrderFee.yes') : $t('OrderFee.no') }}</div><!-- '是' : '否' -->
					<div class="" v-if="detailForm.contract && detailForm.contract.contractNo">{{$t('OrderFee.DurationOfContract')}}：{{ detailForm.contract.effectStartDate }} 至 {{ detailForm.contract.effectEndDate }}</div>
				</template>
			</div>
		...
		<template v-if="isWhitelist && !isJoint">
			<span>
				...
				<span class="industry-class">{{ $lang === 'zh' ? detailForm.industry : detailForm.industryEName }}</span>
			</span>
		</template>
		<template v-else-if="detailForm.custLevel && !isJoint">
			<el-tooltip class="item" effect="dark" placement="bottom">
				<div slot="content">{{$t('OrderFee.CustomerVIPLevel')}}</div>
				<img :src="require(`@/views/crm/merchantManagement/customerList/images/icon-${detailForm.custLevel.toLowerCase()}.png`)" class="vip-img" />
			</el-tooltip>
		</template>
		<span class="ml5" v-if="!isJoint && detailForm.custClass">{{$t('OrderFee.custClass')}}: {{ detailForm.custClass.toLowerCase() | getDictLabelFilter('customerClass') }}</span>
		<slot />
	</span>
</template>

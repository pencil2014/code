<template>
	<div>
		...
		<el-dialog class="dashboard-sys-update" :title="$t('Common.update')" :visible.sync="open" :lock-scroll="true" width="780px">
			<div class="ql-editor update-image-wrap" v-html="contentHtml">
				{{$t('Common.information')}}
			</div>
			<div slot="footer" class="dialog-footer">
				<el-button size="mini" @click="cancel">{{$t('Common.close')}}</el-button>
			</div>
		</el-dialog>
	...
	data() {
		return {
			...
			noticeMenu: [
				{
					key: 'Todo',
					value: this.$t('Common.todo')
				},
				{
					key: 'Msg',
					value: this.$t('Common.notice')
				},
				// {
				// 	key: 'MsgSys',
				// 	value: '系统通知'
				...
				{
					key: 'Update',
					value: this.$t('Common.update')
				}
			],
			activityKey: 'Todo',
			noticeTypeOptions: {
				UPDATE: this.$t('Common.update'),
				MSG: this.$t('Common.notice'),
				TODO: this.$t('Common.todo'),
				MSG_SYS: this.$t('Common.sysNotice')
			}
		}
	},
	mounted() {

  ...
  <div class="edit-row">
    <div class="">
      <div class="msg-form-bd">
        ...
        <el-table class="dashboard-table-wrap" header-row-class-name="dashboard-msg-header" @selection-change="handleSelectionChange" :cell-class-name="cellClassName" :data="tableList" ref="msgTableLayout">
          ...
          <el-table-column show-overflow-tooltip align="left" label="通知内容">
            <template slot-scope="scope">
    ...
    <el-dialog :title="`[业务] ${detail.noticeTitle}`" :visible.sync="open" :lock-scroll="true" width="80%">
      <el-form class="msg-detail" label-position="right" label-width="120px">
        <el-form-item style="padding:12px 0;width: 100%;" label="内容:">
...
export default {
  ...
  methods: {
    ...
    async dynamicHeight(value) {
      let { msgTableLayout, msgTableFooter } = this.$refs
      let height = document.documentElement.clientHeight || document.body.offsetHeight

      let table = msgTableLayout.$el.getBoundingClientRect()
      let footer = msgTableFooter.$el.getBoundingClientRect()
      this.tableHeight = height - table.top - footer.height - 10 // 表格高度=文档高度-表格距顶部距离-表格分页-容器padding间距
    },
    async getList() {

...
<!-- auto-complete="on" -->
    ...
    <div class="login-container-modal" v-if="logoutJump">
    </div>
  </div>
</template>
        ...
        if (valid) {
          ...
          this.$store.dispatch('user/login', Object.assign({}, this.loginForm, {
            ...
            if (url && !url.includes('/login')) {
              ...
              this.$router.push({ path: this.redirect || '/finance/settlement/feeList' }) // '/'
              // location.reload(); // will come into occurring unexpected http status canceled
            }
          }).catch(() => {
            this.loading = false
          })

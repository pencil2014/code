<template>
  ...
  >
    <div @click="bodyClick" class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">{{$t('Payment.payDetail')}}</div>
        </div>
          ...
          <template v-if="payOrder.payWay !=='intrabranch'">
            <el-button-group>
              <el-button
                ...
                "
                >{{$t('Payment.addRecords')}}</el-button
              >
            </el-button-group>
            <el-button-group>
              <el-button
                ...
                >{{$t('PayApplyList.markWrongPay')}}</el-button
              >
            </el-button-group>
          </template>
          <el-button-group>
      ...
      <div class="payInfo-box" style="height: calc(100vh - 95px);">
        ...
        <div class="item-box border-bottom">
          <div class="title">{{$t('Settle.transferInfo')}}</div>
          <!-- :disabled="!typeOne" -->
          <el-form
            :inline="true"
            :model="payOrder"
          ...
          >
            ...
            <el-form-item :label="$t('PayApplyList.payWay')" prop="payWay" :rules="{ required: true, message: ' ', trigger: 'change' }" >
              <el-select v-model="payOrder.payWay" :placeholder="$t('Common.plSelect')" @change="payWayChange(payOrder)" :disabled='payOrder.payWriteoffStatus === "yes"'
              >
                <el-option
            ...
            <span v-show="!(payOrder.payWay == 'prepay_dedue' || payOrder.payWay == 'cash')" >
              <el-form-item :label="$t('Payment.shortName')" prop="payAccount" :rules="{ required: !['prepay_dedue','cash','cheque'].includes(payOrder.payWay), message: ' ', trigger: 'blur' }" >
                <el-select v-model="payOrder.payAccount" :placeholder="$t('Common.plSelect')"
              ...
              <el-form-item v-if="payOrder.payInfo.payBankCode==='B0249H231000001'&&(payOrder.payAccount&&payOrder.payAccount.hasFixedTransferFee==='yes')" :label="$t('Settle.feeDeductWay')" prop="payInfo.transferFeeDeductWay" :rules="{ required: true, message: ' ', trigger: 'change' }">
                <!-- v-if="payOrder.payWay==='hk_china'&&payOrder.payAccount.hasFixedTransferFee==='yes'" // 只有花旗定价手续费显示, B0249H231000001 -->
                <el-select v-model="payOrder.payInfo.transferFeeDeductWay" :placeholder="$t('Common.plSelect')"
                  :disabled='payOrder.payWriteoffStatus === "yes"'
              ...
              <el-form-item v-if="payOrder.payInfo.thirdPayChannelCode" :label="$t('Settle.channelCode')">
                <el-input disabled v-model="payOrder.payInfo.thirdPayChannelCode"></el-input>
              </el-form-item>
              <el-form-item :label="$t('PayApplyList.payAccountName')">
                <el-input disabled v-model="payOrder.payAccountName"></el-input>
              </el-form-item>
              <el-form-item :label="$t('PayApplyList.payAccountNo')" >
                <el-input disabled v-model="payOrder.payAccountNo"></el-input>
              </el-form-item>
            </span>
            <el-form-item :label="$t('Payment.bankslipFile')" prop="bankseqFileNo" :rules="[ {required: (payOrder.payWay==='chinese_mainland' || payOrder.payWay==='hk_china')&&payOrder.hasSpecialFee!=='yes',  message: $t('Payment.bankslipTips'),  trigger: 'blur'  }]">
            ...
            <el-form-item :label="$t('Collect.bankFlowNumber')" prop="bankSeqNo">
              <el-input
            ...
            <el-form-item :label="$t('FinBill.paymentDate')" prop="payTime">
              <el-date-picker
                :disabled="disableForm"
                align="right"
            ...
            <el-form-item :label="$t('PayApplyList.realPayAmt')" prop="realPayAmt">
              <el-input
                :disabled="disableForm"
            ...
            <template
              ...
              "
            >
              <el-form-item :label="$t('Payment.otherPayables')" prop="lessPayAmt">
                <el-input disabled v-model="payOrder.lessPayAmt"></el-input>
              </el-form-item>
              <el-form-item :label="$t('Payment.subjects')" prop="lessPayDealway">
                <el-select
                  :disabled="disableForm"
                  placeholder
            ...
            <template
              ...
              "
            >
              <el-form-item :label="$t('Payment.ourPayment')" prop="greatPayAmt">
                <el-input disabled v-model="payOrder.greatPayAmt"></el-input>
              </el-form-item>
              <el-form-item :label="$t('Payment.overpayment')" prop="greatPayDealway">
                <el-select
                  :disabled="disableForm"
                  placeholder
            ...
            <el-form-item :label="$t('OrderFee.payOrderNos')" key="payOrderNo">
              <div>{{payOrder.payOrderNo}}</div>
            </el-form-item>
            <el-form-item v-if="!['prepay_dedue','cash'].includes(payOrder.payWay)" :label="$t('PayApplyList.settleMode')" prop="settleMode"  :rules="{ required: !['prepay_dedue','cash','intrabranch'].includes(payOrder.payWay), message: ' ', trigger: 'blur' }" ><!-- change -->
              <el-select :disabled='payOrder.payWriteoffStatus === "yes"' v-model="payOrder.settleMode" clearable :placeholder="$t('Common.plSelect')">
                <el-option
            ...
            <el-form-item class="remark" :label="$t('Payment.transferNote')" prop="remark">
              <el-input
                :disabled="disableForm"
                :maxlength="500"
                show-word-limit
        ...
        <div @click.stop="listenClick" class="table-box top-table-box">
          <div class="title-box">
            <div class="title">{{$t('Payment.bankcharges')}}</div>
            <div class="btn">
              <el-button @click="addFee" type="default" v-show="checkAuth($route.name, 'btn-addFee') && !disableForm && payOrder.payWay !== 'prepay_dedue' && (payOrder.payInfo.hasFixedTransferFee !== 'yes' || payOrder.payInfo.payBankCode !== 'B0249H231000001')"
                >{{$t('Collect.addHandlingFee')}}</el-button
              >
        ...
        <template v-if="modelWithholdingTax">
          <div class="table-box">
            <div class="title-box">
              <div class="title">{{$t('Payment.platformFee')}}</div>
              <div class="btn">
                <el-button
                  ...
                  "
                  >{{$t('Payment.affiPlatform')}}</el-button
                >
              </div>
            </div>
            <div class="bill-box">
        ...
        <template v-if="option5.data">
          <div class="table-box" v-show="option5.data.length">
            <div class="title-box">
              <div class="title">{{$t('Payment.exchangePurch')}}</div>
              <div class="btn">
        ...
        <template v-if="payOrderPayWay">
          <div class="table-box">
            <div class="title-box">
              <div class="title">{{$t('Payment.prepaymentOff')}}</div>
              <div class="btn">
                <el-button
        ...
        <div class="item-box">
          ...
          <div class="info-box">
            <div class="base-item">
              <span class="sub-title">{{$t('Collect.avaiWfAmt')}}:</span>
              {{
            ...
            <!-- <div class="base-item">
								其他应付款:
								<el-select v-model="returnAmt" disabled :placeholder="$t('Common.plSelect')">
            ...
            <div class="base-item" v-if="writeoffExRateSummarys[0]">
              <span class="sub-title">{{$t('Collect.writeoffrate')}}:</span>
            ...
            <template>
              <div class="base-item-list" v-if="writeoffPay.writeoffReceipayNo" style="flex-wrap:wrap;">
                <div class="base-item">
                  <span class="sub-title">{{$t('Hedge.writeoffNo')}}:</span>
                  {{ writeoffPay.writeoffReceipayNo }}
                </div>
                <div class="base-item">
                  <span class="sub-title">{{$t('Payment.underwritType')}}:</span>
                  {{
        ...
        <!-- <div class="table-box">
          ...
          <div class="bill-box">
            ...
            <span>
              是否后补票:{{ payOrder.delayInvoice === 'yes' ? $t('Common.yes') : $t('Common.no') }}
            </span>
            <span>后补票金额:{{ payOrder.delayInvoiceAmt }}</span>
          </div>
          <FinanceTable :option="option2" v-if="showTable[0]" />
        </div> -->
        <div class="table-box">
          <div class="title-box">
            <div class="title">{{$t('Settle.paymentInfo')}}</div>
            <div class="btn">
          ...
          <div class="item-box" v-show="showTable[1]">
            <div class="info-box" v-if="payOrder.payInfo">
              <div class="base-item">
                <span class="sub-title">{{$t('Settle.payOrderNo')}}:</span>
                {{ payOrder.payOrderNo }}
              </div>
              <div class="base-item">
                <span class="sub-title">{{$t('Payment.paymentType')}}:</span>
                {{ dictMapObj.payOrderType[payOrder.orderType] }}
              </div>
              <div v-if="!option11.data.length" class="base-item">
                <span class="sub-title">{{$t('Charge.applicationNo')}}:</span>
                <el-tooltip
              ...
              <div class="base-item">
                <span class="sub-title">{{$t('PayApplyList.acceptanceStatus')}}:</span>
                {{ payOrder.confirmStatus == 'pass' ? $t('PayApplyList.accepted') : $t('PayApplyList.waitAccepted') }}
              </div>
              <div class="base-item">
                <span class="sub-title">{{$t('Payment.payOrderSta')}}:</span>
              ...
              <div class="base-item" v-if="payOrder.custBearTransferAmt">
                <span class="sub-title">{{$t('Payment.covered')}}:</span>
              ...
              <div class="base-item">
                <span class="sub-title">{{$t('Payment.shortName')}}:</span>
                <el-tooltip
              ...
              <div class="base-item">
                <span class="sub-title">{{$t('PayApplyList.payAccountName')}}:</span>
                <el-tooltip
              ...
              <div class="base-item">
                <span class="sub-title">{{$t('PayApplyList.payAccountNo')}}:</span>
                {{ payOrder.payInfo.payAccountNo }}
              </div>
              <div class="base-item">
                <span class="sub-title">{{$t('Common.createdBy')}}:</span>
                {{ payOrder.createdName }}
              </div>
              <div class="base-item">
                <span class="sub-title">{{$t('Common.createTime')}}:</span>
                {{ payOrder.createdTime }}
              </div>
              <div class="base-item">
                <span class="sub-title">{{$t('FinBill.payer')}}:</span>
                {{ payOrder.confirmName }}
              </div>
              <div class="base-item">
                <span class="sub-title">{{$t('FinBill.paymentDate')}}:</span>
              ...
              <div v-if="!option11.data.length" class="base-item">
                <span class="sub-title">{{$t('PayApplyList.recvBankName')}}:</span>
                {{ payOrder.payInfo.recvBankName }}
              </div>
              <div v-if="!option11.data.length && payOrder.payInfo.recvAccountPublicType!=='prv'" class="base-item">
                <span class="sub-title">{{$t('PayApplyList.recvAccountName')}}:</span>
                {{ payOrder.payInfo.recvAccountName }}
              </div>
              <div v-if="!option11.data.length && payOrder.payInfo.recvAccountPublicType!=='prv'" class="base-item">
                <span class="sub-title">{{$t('PayApplyList.recvAccountNo')}}:</span>
        ...
        >
          <div class="title">{{$t('Payment.errInfo')}}</div>
          <div class="info-box">
            <div class="base-item">
              <span class="sub-title">{{$t('PayApplyList.relevant')}} {{$t('Settle.payOrderNo')}}:</span><!-- 关联 -->
            ...
            <div class="base-item">
              <span class="sub-title">{{$t('OrderFee.applyPerson')}}:</span>
            ...
            <div class="base-item">
              ...
              {{ payOrder.errorDisposalStatus == 'yes' ? $t('PayApplyList.hasHandled') : $t('PayApplyList.notHandled') }}
            </div>
            <div class="base-item">
              <span class="sub-title">{{$t('PayApplyList.closeSrcPayOrder')}}:</span>
            ...
            <div class="btn">
              <el-button
                ...
                v-show="
                  ...
                  payOrder.payWriteoffStatus === 'no'
                "
                >{{$t('Collect.underwritingrate')}}</el-button
              >
              <el-button
    ...
    >
      ...
      <div
        ...
        v-show="
          ...
          payOrder.payStatus !== 'pay_close' &&
          payOrder.payWriteoffStatus === 'no' &&
          dialogTitle === $t('PayApplyList.writeoffExRate')
        "
      ><!-- '核销汇率' -->
    ...
    <el-dialog
      ...
      class="self-dialog"
      :title="$t('PayApplyList.addExPurchasing')"
      width="900px"
    >
      <div class="exchange-form-box">
        <el-form
          ...
          size="mini"
        >
          <el-form-item :label="$t('PayApplyList.exchangeCurrency')+':'" prop="currency">
            <el-select
          ...
          <el-form-item :label="$t('PayApplyList.exchangeAmt')+':'" prop="amt">
            <el-input v-model="exchangeData.amt"></el-input>
          </el-form-item>
          <el-form-item :label="$t('PayApplyList.payAccountShortName')+':'" prop="payAccountShortName"><!-- 我司银行简称 -->
            <el-select
              @change="chooseBank"
              :placeholder="$t('Common.plSelect')"
          ...
          <el-form-item :label="$t('PayApplyList.settleMode')+':'" prop="settleMode"  :rules="{ required: exchangeData.bankAccountSettleModeList.length, message: ' ', trigger: 'change' }" ><!-- 账户结算方式 -->
              <el-select v-model="exchangeData.settleMode" clearable :placeholder="$t('OrderFee.plsSelect')">
                <el-option
          ...
          <el-form-item :label="$t('PayApplyList.payAccountName')+':'" prop="payAccountName"><!-- 我司账户名 -->
            <el-input disabled v-model="exchangeData.payAccountName"></el-input>
          </el-form-item>
          <el-form-item :label="$t('PayApplyList.payBankName')+':'" prop="payBankName"><!-- 我司银行名称 -->
            <el-input disabled v-model="exchangeData.payBankName"></el-input>
          </el-form-item>
          <el-form-item :label="$t('PayApplyList.payAccountNo')+':'" prop="payAccountNo"><!-- 我司账号 -->
      ...
      <div class="dialog-footer" slot="footer">
        <el-button @click="submitExchange" size="mini" type="primary"
          >{{$t('Common.submit')}}</el-button
        >
        <el-button @click="closeExchange" size="mini">{{$t('Common.cancel')}}</el-button>
      </div>
    </el-dialog>
    <!-- 修改核销汇率 -->
    <el-dialog
      ...
      class="self-dialog"
      :title="$t('PayApplyList.modifyWriteoffRate')"
      width="600px"
    >
      ...
      <div class="dialog-footer" slot="footer">
        <el-button @click="submitExchangeRate" size="mini" type="primary"
          >{{$t('Common.submit')}}</el-button
        >
        <el-button @click="closeExchangeDialog" size="mini">{{$t('Common.cancel')}}</el-button>
      </div>
    </el-dialog>
    <!-- 关联预付抵账 -->
    <template v-if="relevancePrepayDialogShow">
      <el-dialog
        ...
        class="self-dialog"
        :title="relevanceMark === 'withholdingTax' ? $t('PayApplyList.associatedPayfee') : $t('Settle.advance')"
        width="1100px"
      ><!-- '关联支付平台代付手续费抵扣' : '关联预付抵账'" -->
        ...
        <span class="dialog-footer" slot="footer">
          ...
          >
          <el-button @click="closeRelevancePrepay" size="mini">{{$t('Common.close')}}</el-button>
        </span>
      </el-dialog>
  </template>
    ...
    return {
      ...
      transferFeeDialog: {
        ...
        tableOption: { pagination: false, selection: false, data: [],columns: [
            ...
            { prop: 'orderNo', label: this.$t('FeeList.orderNo'), type: 'text', minWidth: 150, },
  ...
  methods: {
    ...
    init() {
      ...
      const operationBtns = Object.assign({}, this.option.operationBtns, {
        data: [{ label: this.$t('Common.del'), type: 'text', action: 'Delete', showFn: row => {
      ...
      this.columns1_common = [ // this.columns1_hk_china = [ // columns1_fixed_trfFee
        ...
        { prop: 'amt', label: this.$t('FeeList.feeAmt'), type: 'input', key: 'amt',
      ...
      const columns3 = [
        ...
        {
          prop: 'settleAmt',
          label: this.$t('PayApplyList.availableWriteoffAmt'), // this.$t('Payment.originalAmt'),
          type: 'text',
          width: 170
        },
        {
          prop: 'writeoffFeeAmt',
          label: this.$t('PayApplyList.writeoffAmtOriginal'),
          type: 'input',
          width: 170,
      ...
      const columns4 = [
        ...
        {
          prop: 'writeoffExRate',
          label: this.$t('PayApplyList.writeoffExRate'), // this.$t('Collect.writeoffrate'),
          type: 'input',
          validateInput: (val, row, item, change) => {
            if (change) {
      ...
      const operationBtns5 = Object.assign({}, this.option.operationBtns, {
        data: [{ label: this.$t('Common.del'), type: 'text', show: true, action: 'Delete', style: 'color:#CD4130;' }],
        width: 100,
        callback: (action, $index, row, option) => {
          if (action === 'Delete') {
            this.option5.data = []
      ...
      const operationBtns7 = Object.assign({}, this.option.operationBtns, {
        data: [
          { label: this.$t('OrderFee.moveUp'), type: 'text', showFn: (row, index)=> index !== 0, action: 'Upper' }, // '上移'
          { label: this.$t('OrderFee.moveDown'), type: 'text', showFn: (row, index)=>  this.option7.data.length-1 !== index, action: 'Below' }, // '下移'
          { label: this.$t('Common.del'), type: 'text', show: true, action: 'Delete', style: 'color:#CD4130;' }
        ],
        width: 200,
        callback: (action, $index, row, option) => {
          if (action === 'Delete') {
      ...
      const columns7 = [
        ...
        { prop: 'payOrderNo', label: this.$t('Settle.payOrderNo'), type: 'button', 
          operationBtns: {
            show: true,
      ...
      const columns8 = [
        { prop: 'payApplyPrepayNo', label: this.$t('PayApplyList.prepayApplyNo'), type: 'text', minWidth: 170 }, // '预付款申请单号'
        { prop: 'settleCompName', label: this.$t('Settle.branchCompany'), type: 'text' },
        ...
        { prop: 'payTime', label: this.$t('FinBill.paymentDate'), type: 'text' },
      ]
      ...
      const columns9 = [
        ...
        // { prop: 'realpayAmt', label: this.$t('Settle.applyPayAmt'), type: 'text', key: 'realpayAmt',},
...
          this.payOrder.payBankAccountId = payOrder.payInfo.payBankAccountId
    ...
    addExchange() {
      const realPayAmt = this.payOrder.realPayAmt
      if (!realPayAmt) {
        return this.$msgErrClose(this.$t('PayApplyList.inputActualAmtPayFirst')) // 请先填写实付金额
      }
      const len = this.option5.data.length
      if (len) {
        return this.$msgErrClose(this.$t('PayApplyList.onlyAddOneExchange')) // 只能添加一条购汇记录，请删除后再添加！
      }
    ...
    submitExchange() {
      this.$refs.exchangeForm.validate(valid => {
        if (valid) {
          ...
          const reg = /^\d+(\.\d{0,2})?$/
          if (!reg.test(amt)) {
            return this.$msgErrClose(this.$t('OrderFee.inputCorrectNumber')) // 请填写正确的购汇金额
          }
          if (this.exchangeData.currency === this.payOrder.currency) {
            return this.$msgErrClose(this.$t('PayApplyList.exCurrPayCurrNotSame')) // 购汇币种与付款币种一致，请重新选择
          }
    ...
    markErr() {
      if (this.payOrder.errorStatus === 'yes') {
        return this.$msgErrClose(this.$t('PayApplyList.hasMarkNotRepeat')) // 已操作过标记付错,不能重复操作
      }
      if (this.payOrder.payStatus === 'pay_close') {
        return this.$msgErrClose(this.$t('PayApplyList.closedPayNotMarkError')) // 已关闭的付款单不能操作标记付错
      }
      if (this.payOrder.confirmStatus !== 'pass') {
        return this.$msgErrClose(this.$t('PayApplyList.unacceptedPayNotMarkError')) // 未受理的付款单不能操作标记付错
      }
    ...
    writeOffPreview() {
      ...
      if (Number(this.payOrder.lessPayAmt) && !this.payOrder.lessPayDealway) {
        return this.$msgErrClose(this.$t('PayApplyList.lessPayDealway')) // 少付处理科目不能为空
      }
      if (Number(this.payOrder.greatPayAmt) && !this.payOrder.greatPayDealway) {
        return this.$msgErrClose(this.$t('PayApplyList.greatPayDealway')) // 多付处理科目不能为空
      }
      if (!this.writeoffPay.writeoffDate) {
        return this.$msgErrClose(this.$t('PayApplyList.writeoffDateNotEmpty')) // 核销日期不能为空
      }
      // const realPayAmt = this.payOrder.realPayAmt
    ...
    writeOffPreviewFn() {
      ...
      if((payWayList.includes(this.payOrder.payWay)) && settleCompanyCode !== settleCompCode ){
        let str = this.$t('PayApplyList.writeoffAndPayBranchNotSame') // 您本次核销付款单中的付款银行账户所属分公司与付款单所属分公司不一致，系统将自动变更付款单及涉及的结算单为代付单，请再次确认本次核销！
        this.$confirmWarn({ body: str, confirm: this.$t('Common.confirm'), cancel: this.$t('Common.cancel') }, () => { // 确认
    ...
    changeRealPayAmt() {
      ...
      if (!/^\d+(\.\d{0,4})?$/.test(this.payOrder.realPayAmt)) {
        return this.$msgErrClose(this.$t('OrderFee.inputCorrectNumber')) // 请输入正确的实付金额
      }
      const realPayAmt = this.payOrder.realPayAmt 
      ...
      if (
        ...
        this.payOrder.greatPayAmt = this.payOrder.lessPayAmt = 0
        this.payOrder.lessPayDealway = this.payOrder.greatPayDealway = ''
        return this.$msgErrClose(this.$t('PayApplyList.prepayNotOverpaid')) // 预付抵账不能多付
      }
      // 内部来往
      if (this.payOrder.payInfo.payAccountIsIntrabranch === 'yes' && realPayAmt - this.allTransferFeeList > applyAmt) {
        ...
        this.payOrder.greatPayAmt = this.payOrder.lessPayAmt = 0
        this.payOrder.lessPayDealway = this.payOrder.greatPayDealway = ''
        return this.$msgErrClose(this.$t('PayApplyList.interTranNotOverpaid')) // 内部来往不能多付
      }
      if (
        orderType === 'prepay' &&
        realPayAmt - this.allTransferFeeList != applyAmt
        ...
        this.payOrder.greatPayAmt = this.payOrder.lessPayAmt = 0
        this.payOrder.lessPayDealway = this.payOrder.greatPayDealway = ''
        return this.$msgErrClose(this.$t('PayApplyList.prepayNotOverOrUnderpaid')) // 预付付款不能多付或少付
      }
    },
    // 增加手续费
    addFee() {
      if (this.info.payOrder) {
        ...
        if (this.payOrder.payWay === 'prepay_dedue') { // if (orderType === 'prepay_to_repay') {
          return this.$msgErrClose(this.$t('PayApplyList.prepayNotAddHandling')) // 预付抵账！无需添加手续费！
        }
        const payWay = this.info.payOrder.payWay
        if (payWay === 'cash') {
          return this.$msgErrClose(this.$t('PayApplyList.cashpayNotAddHandling')) // cashpayNotAddHandling
        }
    ...
    cutTypeChange(value, row, item) {
      ...
      if(row.cutType === "payment_channel_transfer"){
        ...
        if(arr.length > 1){
          row.cutType = ''
          return  this.$msgErrClose(this.$t('PayApplyList.hasPayPlatformHandling')) // 已有支付平台代付手续费，请删除后再添加！
        }
      }
      // 添加手续费已加
    ...
    changeWriteoffExRate() {
      if (!/^\d+(\.\d{0,10})?$/.test(this.writeoffExRate)) {
       return this.$msgErrClose(this.$t('PayApplyList.enterCorrectRate')) // 请输入正确的汇率
      }
    ...
    showRateDialog(type) {
      if (type) {
        this.dialogTitle = this.$t('PayApplyList.writeoffExRate') // '核销汇率'
        // 数组去重
        let list = []
    ...
    submitRate() {
      ...
      if (!writeoffExRate) {
        return this.$msgErrClose(this.$t('PayApplyList.enterCorrectRate')) // 请输入正确的核销汇率
      }
    ...
    getBankAccountList() {
      ...
      if (!currency) {
        return this.$msgErrClose(this.$t('PayApplyList.selectExBuyCurrency')) // 请选择购汇币种
      }
      if (currency === this.payOrder.currency) {
        return this.$msgErrClose(this.$t('PayApplyList.exAndPayCurrNotSame')) // 购汇币种与付款币种一致，请重新选择
      }
    ...
    submitExchangeRate() {
      ...
      this.option6.data.forEach(item => {
        if (item.originalCurrency === item.settleCurrency) {
          if (Number(item.writeoffExRate) !== 1) {
            status = true
            this.$msgErrClose(
              `${item.originalCurrency} - ${item.settleCurrency} ${this.$t('PayApplyList.upperAndLowerRateIs1')}`
            )
          }
        } else {
      ...
      this.$msgSucClose(this.$t('OrderFee.successOp')) // 汇率修改成功
    ...
    getListAmtSummary(val) {
        if (val.length) {
          ...
          console.log(data);
          const labelObj = {
            realpayAmtTotalListLabel: this.$t('PayApplyList.payAmt'), // '付款金额',
            balanceTotalListLabel: this.$t('PayApplyList.balance'), // '剩余可用金额'
          }
          const colorObj = {
    ...
    relevancePrepayConfirm() {
      ...
      if(this.relevanceMark === 'withholdingTax'){
        if (!len.length) {
          return this.$msgErrClose(this.$t('PayApplyList.checkPlatformDataFirst')) // 请先勾选支付平台代付的数据
        }
        ...
        if (!len.length) {
          return this.$msgErrClose(this.$t('PayApplyList.checkPrepayDataFirst')) // 请先勾选预付款数据
        }
        const payOrderNoList = this.option7.data.map(item => item.payOrderNo)
        this.multipleSelection8.forEach(ele => {
          if (!payOrderNoList.includes(ele.payOrderNo)) {

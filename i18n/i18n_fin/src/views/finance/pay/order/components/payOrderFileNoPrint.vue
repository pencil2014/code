    ...
    <div class="finance-payorderFileNo-print payorder-base-item-apply" v-if="infoList.length">
          ...
          <div :id="'PayOrderPrint'+index" :style="printIsNo ? 'margin-top:20px;': ''">
            <div class="finance-payorderFileNo-print-cnt">
              ...
              <div class="print-info-box">
                ...
                <div class="base-item">{{$t('PayApplyList.payWay')}}：{{ dictMapObj.payWay[item.payWay] }}</div>
                <div class="base-item"></div>
                <div class="base-item">{{$t('PayApplyList.recvAccountName')}}：{{ item.recvAccountName }}</div>
                <div class="base-item">{{$t('PayApplyList.recvBankBranch')}}：{{ item.recvBankBranch }}</div>
                <div class="base-item">{{$t('PayApplyList.recvAccountNo')}}：{{ item.recvAccountNo }}</div>
                <template v-if="item.orderType === 'special_refund'">
                  <div class="base-item">{{$t('PayApplyList.settleCurrency')}}：{{ item.settleCurrency }}</div>
                  <div class="base-item">{{$t('PayApplyList.settleAmt')}}：{{ item.settleAmt }}</div>
                  <div class="base-item">{{$t('PayApplyList.realPayAmt')}}：{{ item.realPayAmt }}</div>
                </template>
                <template v-else>
                  <div class="base-item">{{$t('PayApplyList.payAccountName')}}：{{ item.payAccountName }}</div>
                  <div class="base-item">{{$t('PayApplyList.payBankBranch')}}：{{ item.payBankBranch }}</div>
                  <div class="base-item">{{$t('PayApplyList.payAccountNo')}}：{{ item.payAccountNo }}</div>
                </template>
              </div>
              ...
              <div class="print-info-box">
                <template v-if="item.orderType === 'special_refund'">
                  ...
                  <div class="base-item">{{$t('PayApplyList.acceptor')}}：{{ item.confirmName }}</div>
                </template>
                <template v-else>
                  <div class="base-item">{{$t('PayApplyList.createdTime')}}：{{ item.createdTime }}</div>
            ...
            <div
              ...
              :key="index"
            >
              <span>{{$t('PayApplyList.nthPrint1')}}{{it.printCount - 1}}{{$t('PayApplyList.nthPrint2')}}</span><!-- 第{{it.printCount - 1}}次打印 -->
      ...
      <div class="carousel-bottom" v-if="total>1">
        <div>
          ...
          <span style="margin:0 32px;">{{$t('PayApplyList.nthPage1')}}{{currIndex}}{{$t('PayApplyList.nthPage2')}}/{{$t('PayApplyList.totalPage1')}}{{total}}{{$t('PayApplyList.totalPage2')}}</span>
          ...
          <div v-for="(item, index) in infoList" :key="index">
            <div class="finance-payorderFileNo-print-cnt">
              ...
              <div class="print-info-box">
                ...
                <div class="base-item"></div>
                <div class="base-item">{{$t('PayApplyList.recvAccountName')}}：{{ item.recvAccountName }}</div>
                <div class="base-item">{{$t('PayApplyList.recvBankName')}}：{{ item.recvBankName }}</div>
                <div class="base-item">{{$t('PayApplyList.recvAccountNo')}}：{{ item.recvAccountNo }}</div>
                <template v-if="item.orderType === 'special_refund'">
                  <div class="base-item">{{$t('PayApplyList.settleCurrency')}}：{{ item.settleCurrency }}</div>
                  <div class="base-item">{{$t('PayApplyList.settleAmt')}}：{{ item.settleAmt }}</div>
                ...
                <template v-else>
                  <div class="base-item">{{$t('PayApplyList.payAccountName')}}：{{ item.payAccountName }}</div>
                  <div class="base-item">{{$t('PayApplyList.payBankName')}}：{{ item.payBankName }}</div>
                  <div class="base-item">{{$t('PayApplyList.payAccountNo')}}：{{ item.payAccountNo }}</div>
                </template>
              </div>
              <div class="print-table-total">
              ...
              <div class="print-info-box">
                <template v-if="item.orderType === 'special_refund'">
                  <div class="base-item">{{$t('PayApplyList.createdTime')}}：{{ item.createdTime }}</div><!-- 申请时间 -->
                  ...
                  <div class="base-item">{{$t('PayApplyList.approver')}}：{{ item.confirmName }}</div><!-- 审核人 -->
                </template>
                <template v-else>
                  <div class="base-item">{{$t('PayApplyList.createdTime')}}：{{ item.createdTime }}</div><!-- 申请时间 -->
                  <!-- <div class="base-item">发票登记日期：{{ item.invoiceRegisterTime }}</div> 海外版不显示 -->

<template>
  <div>
    <el-dialog
      :title="$t('PayApplyList.prepayApply')"
    ...
    >
      <div class="dialog-prepay-edit">
        ...
        >
          <div class="dialog-add-dialog-item" style="border-bottom: 1px solid #D8D8D8;">
            <p class="dialog-add-dialog-item-title">{{$t('PayApplyList.recvInfo')}}</p>
            <el-form-item :label="$t('OrderFee.settleCorpName')" prop="recvCorpName">
              <el-autocomplete
                :disabled="disabled"
                style="width:151px"
            ...
            <el-form-item :label="$t('PayApplyList.recvBankBranch')">
              <el-input disabled v-model="form.recvBankBranch"></el-input>
            </el-form-item>
            <el-form-item :label="$t('PayApplyList.recvAccountNo')" prop="recvAccountNo">
              <!-- <el-autocomplete
                :disabled="disabled"
                style="width:151px"
                clearable
            ...
            <el-form-item :label="$t('PayApplyList.recvAccountName')">
          ...
          >
            <p class="dialog-add-dialog-item-title">{{$t('PayApplyList.myCompInfo')}}</p>
            ...
            <el-form-item
              ...
              :label="$t('PayApplyList.transferFeeDeductWay')+'：'"
              prop="transferFeeDeductWay"
              :rules="{ required: true, message: ' ', trigger: 'change' }"
              style
            >
              <el-select v-model="form.transferFeeDeductWay" :placeholder="$t('OrderFee.plsSelect')" style="width:151px">
                <el-option
            ...
            <el-form-item :label="$t('PayApplyList.payAccountNo')" prop="payAccountNo">
              <el-input disabled v-model="form.payAccountNo"></el-input><!-- placeholder="请输入我司开户行" -->
            </el-form-item>
            <el-form-item :label="$t('PayApplyList.payBankName')" prop="payBankName">
              <el-input disabled v-model="form.payBankName"></el-input><!-- placeholder="请输入我司开户行" -->
            </el-form-item>
            <el-form-item :label="$t('PayApplyList.payAccountName')">
              <el-input disabled v-model="form.payAccountName"></el-input>
            </el-form-item>
            <!-- <el-form-item label="是否开票">-原已注释
                    <el-select  v-model="form.hasInvoice" @change="handleShow" :placeholder="$t('Common.plSelect')" style="width:100%">
                        <el-option label="是" value="yes"></el-option>
                        <el-option label="否" value="no"></el-option>
                    </el-select>
            </el-form-item>-->

            <el-form-item :label="$t('OrderFee.remark')">
              <el-input
                :placeholder="$t('OrderFee.plsInput')"
                type="textarea"
                :maxlength="500"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" :loading="submitLoading" @click="submitForm">{{$t('OrderFee.confirm')}}</el-button>
        <el-button size="mini" @click="cancel">{{$t('OrderFee.cancel')}}</el-button>
      </span>
    </el-dialog>
    <!-- 海外版不显示
    <el-dialog
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="relevanceSubmit" size="mini">{{$t('Common.sure')}}</el-button>
        <el-button @click="invoiceOpne" size="mini">{{$t('Common.close')}}</el-button>
      </span>
    </el-dialog> -->
  </div>
</template>
  ...
  methods: {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          if (this.form.applyPayAmt <= 0) {
            this.$message({ type: 'error', message: this.$t('PayApplyList.payAmtNotLess0'), showClose: true })
            return
          }
    ...
    savePrepayApply(data) {
      ...
      savePrepayApply(data)
        .then((res) => {
          if (res.code === 0) {
            this.$message({ type: 'success', message: this.$t('OrderFee.successOp'), showClose: true }) // '提交成功!'

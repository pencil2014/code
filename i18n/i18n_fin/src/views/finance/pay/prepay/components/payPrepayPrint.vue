		...
		<div class="finance-payorder-print" v-if="info">
			<div id="printPayOrder">
				<div class="finance-payPrepayPrint-print-cnt">
					...
					<div class="title">
						<div class="base-item base-item-apply-no-left" >{{$t('PayApplyList.nthPrint1')}}{{ isPrinting ? info.printCount :  info.printCount - 1 }}{{$t('PayApplyList.nthPrint2')}}</div>
						<h1>{{$t('PayApplyList.prepayApplyOrder')}}</h1><!-- 预付款申请单 -->
						<div class="base-item base-item-apply-no">{{$t('PayApplyList.prepayApplyNo')}}：{{ info.applyNo }}</div><!-- 预付款申请号 -->
					</div>
					<div class="print-info-box-div"> 
						<table width="100%" border="0" cellspacing="1" cellpadding="0"> 
							<tr> 
								<td class="item-title-td">{{$t('OrderFee.settleCorpName')}}</td><!-- 结算单位 -->
								<td class="item-contnet-td">{{ info.recvCorpName }}</td> 
								<td class="item-title-td">{{$t('PayApplyList.payWay')}}</td><!-- 付款方式 -->
								<td class="item-contnet-td">{{ dictMapObj.payWay[info.payWay] }}</td> 
								<td class="item-title-td">{{$t('PayApplyList.applyPayAmt')}}</td><!-- 付款金额 -->
							...
							<tr> 
								...
								<td class="item-title-td">{{$t('PayApplyList.recvBankBranch')}}</td><!-- 对方开户行支行 -->
							...
							<tr> 
								<td class="item-title-td">{{$t('PayApplyList.payAccountName')}}</td><!-- 我司户名 -->
								<td class="item-contnet-td">{{ info.payAccountName }}</td> 
								<td class="item-title-td">{{$t('PayApplyList.payBankBranch')}}</td><!-- 我司开户行支行 -->
							...
							<!-- <tr> 
								<td class="item-title-td">预付发票</td>海外版不显示
					...
					<div class="print-info-box" style="margin-top:24px;">
						<template>
							...
							<div class="base-item">{{$t('PayApplyList.sendTime')}}：{{ info.sendPayOrderTime }}</div><!-- 推送时间 -->
							...
							<div class="base-item">{{$t('PayApplyList.sender')}}：{{ info.sendPayOrderUserName }}</div><!-- 推送人 -->
						</template>
					</div>
				</div>
			...
			<div class="print-btn-box">
				<!-- <el-checkbox v-model="checkedPrint"></el-checkbox> --><!-- 是否打印发票 --><!-- 不显示 -->
				<el-button
					class="finance-btn"
					type="primary"
					size="mini"
...
export default {
	...
	data () {
      ...
      isPrinting: false,
			...
			printObj: {
				id: "printPayOrder",
				popTitle: this.$t('PayApplyList.prepayApplyOrder'), // '预付款申请单',
				type: "html",
				extraCss: '',

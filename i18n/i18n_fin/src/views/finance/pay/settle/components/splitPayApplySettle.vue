<template>
   <el-dialog
    class=""
    :title="$t('PayApplyList.splitPayment')"
    :visible.sync="paySeparatelyDialog"
    :before-close="cancel"
    width="1000px">
        <el-form :model="paySeparatelyForm" ref="paySeparatelyForm" :inline="true" label-width="155px" size="mini" class="pay-separately-form">
            <el-form-item :label="$t('PayApplyList.settleCorpName')" prop="recvCorpName" >
            ...
            <el-form-item :label="$t('PayApplyList.currency')" prop="currency" >
                <el-input disabled v-model="paySeparatelyForm.currency"></el-input>
            </el-form-item>
            <el-form-item :label="$t('PayApplyList.payWay')" prop="payWay" >
                <el-select v-model="paySeparatelyForm.payWay"
                    :placeholder="$t('Common.plSelect')" disabled>
                        <el-option
            ...
            <el-form-item :label="$t('PayApplyList.remainSplitAmt')" prop="applyPayAmt" >
                <el-input disabled v-model="paySeparatelyForm.applyPayAmt"></el-input>
            </el-form-item>
            <div class='finance-pay-form-container'>
                <el-form-item :label="$t('PayApplyList.payBankName')" prop="payBankBranch" >
                    <el-input disabled v-model="paySeparatelyForm.payBankBranch"></el-input>
                </el-form-item>
                <el-form-item :label="$t('PayApplyList.payAccountName')" prop="payAccountName" >
                    <el-input disabled v-model="paySeparatelyForm.payAccountName"></el-input>
                </el-form-item>
                <el-form-item :label="$t('PayApplyList.payAccountNo')" prop="payAccountNo" >
        ...
        <div class="finance-table-container"  v-for="(item,index) in payInfoList" :key="index" style="margin: 6px 14px;">
            <div class="money-box" style="background-color: #E9E9E9;">
                ...
                <div class="money-title">{{$t('PayApplyList.split')}}{{index+1}}</div><!-- 拆分 -->
                </div>
                <div class="money-box-right">
                    <el-button class="finance-btn" @click="paySeparatelyAdd" v-show="index == 0" size="mini">{{$t('PayApplyList.addSplit')}}</el-button>
                    <el-button class="finance-btn" @click="paySeparatelyDelete(index)" v-show="index != 0" size="mini">{{$t('PayApplyList.delSplit')}}</el-button>
                    <el-button
                        type="text"
                        class="collapse-btn"
            ...
            <div class="settlement-detail-form-1" v-show="item.isShowT">
                <el-form :model="settlementForm" ref="settlementForm" :inline="true" size="mini" style="padding: 10px 0">
                    ...
                    <el-form-item v-if="item.payWay==='hk_china'" :label="$t('PayApplyList.transferFeeDeductWay')+'：'" prop="transferFeeDeductWay" :rules="{ required: true, message: ' ', trigger: 'blur' }" style="margin-left:10px;" >
                    ...
                    <el-form-item :label="$t('PayApplyList.splitAmt')" prop="applyPayAmt">
        ...
        <span slot="footer" class="dialog-footer">
            <el-button type="primary" @click="paySeparatelySubmit" size="mini">{{$t('Common.submit')}}</el-button>
    ...
    methods:{
        ...
        paySeparatelySubmit(){
            ...
            if(sumAmt > this.paySeparatelyForm.applyPayAmt){
                return this.$msgErrClose(this.$t('PayApplyList.sumSplitAmtNotGreaterTip')) // '拆分金额总和不能大于剩余可拆分金额!'
            }
            let list = []
            ...
            for (let i = 0; i < list.length; i++) {
                if (list[i].applyPayAmt == '' ) {
                    return this.$msgErrClose(this.$t('PayApplyList.splitAmtNotEmptyOr0')) // '拆分金额不能为空或为0!'
                }
                if (list[i].applyPayAmt == 0 ) {
                    return this.$msgErrClose(this.$t('PayApplyList.splitAmtNot0')) // '拆分金额不能为0!'
                }
                if (list[i].payWay==='hk_china' && !list[i].transferFeeDeductWay) {
                    return this.$msgErrClose(this.$t('OrderFee.plsSelect') + this.$t('OrderFee.enspace') + this.$t('PayApplyList.transferFeeDeductWay')) // '请选择手续费扣款方式!'
                }
            }
            let data = {
                "applyNo": this.applyNo,
            ...
            // }
            // if(isInvoice){
            //     this.$confirm('您有拆分付款未选择发票，请确认是否提交？', this.$t('Common.tips'), {
        ...
        splitPayApplySettleFn(data){
            splitPayApplySettle(data).then(res=>{
                if(res.code === 0 ){
                    this.cancel(true)
                    this.$msgSucClose(this.$t('OrderFee.successOp')) // '拆分提交成功！'
                }
            })
        },
        //增加拆分按钮

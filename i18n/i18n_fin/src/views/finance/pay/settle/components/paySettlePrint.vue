    ...
    <div class="finance-payorder-print finance-paySettlePrint-print" v-if="infoList.length">
      ...
      >
        ...
        <el-carousel-item v-for="(item,index) in infoList" :key="item.key">
          <div :id="'printPaySettle'+index" class="finance-paySettlePrint-print-cnt">
            <div class="logo">
            ...
            <div class="title">
              ...
              >{{$t('PayApplyList.nthPrint1')}}{{ isPrinting ? item.printCount : item.printCount - 1 }}{{$t('PayApplyList.nthPrint2')}}</div>
              <h1>{{$t('PayApplyList.paySettleOrder')}}</h1><!-- 结算付款申请单 -->
              <div
                v-if="item.payOrderNo"
                class="base-item base-item-apply-no"
              >{{$t('PayApplyList.payOrderNo')}}：{{ item.payOrderNo }}</div><!-- 付款单号 -->
            </div>
      ...
      <div id="printPaySettleList" v-show="isPrint">
        <div class="one_page_print" v-for="(list,i) in infoListArr" :key="i">
          <div class="finance-paySettlePrint-print-cnt" v-for="(item,index) in list" :key="index">
            <div class="logo">
              <img src="../../../../../assets/bill_logo.png" />
              <h1 class="pay_comp_name">{{item.payCompName}}</h1>
            </div>
      ...
      <div class="print-btn-box">
        <el-button
          ...
          @click="handlePrintFn"
          v-print="handlePrint"
        >{{$t('PayApplyList.printAll')}}</el-button><!-- 打印全部 -->
        <el-button
          class="finance-btn"
          type="primary"
          size="mini"
  ...
  data() {
    return {
      ...
      columns1: [
        ...
        {
          prop: 'receipayType',
          label: this.$t('OrderFee.receipayType'), // '类型',
          type: 'select',
        ...
        {
          prop: 'orderNo',
          label: this.$t('OrderFee.orderNo'), // '业务编号',
          type: 'text',
          width: 120,
        },
        {
          prop: 'putNo',
          label: this.$t('OrderFee.blId'), // '提单号',
          type: 'text',
          // width: 120,
        },
        {

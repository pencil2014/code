<template>
  ...
  >
          ...
          <div class="money-box-left">
            ...
            <el-button-group>
              <el-button
                ...
                type=""
                >{{$t('PayApplyList.cancelPush')}}</el-button
              >
            </el-button-group>
            <!-- 海外版不显示
            <el-button-group
          ...
          <div class="money-box-right">
            ...
            <el-button-group>
              <el-button
                ...
                @click="handleMerge"
                size="mini"
                >{{$t('PayApplyList.combinedPayment')}}</el-button
              >
              <el-button
                class="finance-btn"
                ...
                size="mini"
                >{{$t('PayApplyList.splitPayment')}}</el-button
              >
            </el-button-group>
            <el-button-group>
              <el-button
                ...
                size="mini"
                >{{$t('PayApplyList.presentedPaperBill')}}</el-button
              >
              <el-button
                class="finance-btn"
                ...
                size="mini"
                >{{$t('PayApplyList.cancelPaperBill')}}</el-button
              >
            </el-button-group>
            <!-- <el-button-group>
              <el-button
    ...
    />
    <el-dialog :title="$t('OrderFee.notPassed')" :visible.sync="rejectDialogVisible" width="600px">
      <el-form
        :model="query"
        ref="query"
        label-width="120px"
  ...
  /> -->
    <ApplyForPass :option="applyForPassOption" @close="applyForPassOptionClose" @sure="sure">
      ...
      <div slot="descript">
        ...
        >
          ...
          <el-form-item
            :label="$t('PayApplyList.recvAccountNo')"
            prop="recvAccountNo"
          ...
          <el-form-item :label="$t('PayApplyList.recvBankBranch')" v-show="recvData.payWay !== 'prepay_dedue'">
    ...
    >
      ...
      >
        <el-form-item
          :label="$t('PayApplyList.finRemark')"
          prop="finRemark"
          :rules="{
            required: true,
            message: ' ',
  ...
  data() {
    return {
      ...
      selfColumnsBase: [
        ...
        {
          prop: 'mblPutStatus',
          label: this.$t('PayApplyList.mblPutStatus'), // 'MBL放单状态',
          type: 'select',
        ...
        {
          prop: 'hblPutStatus',
          label: this.$t('PayApplyList.hblPutStatus'), // 'HBL放单状态',
          type: 'select',
          propInDict: 'finOrderPutStatus',
          width: 120
        },
        ...
        // { // 海外版先不显示
        //   prop: 'hasInvoice',
        //   label: this.$t('PayApplyList.hasInvoice'), // '开票状态',
        //   type: 'select',
        ...
        // { // 海外版先不显示发票号
        //   prop: 'invoiceNos',
        //   label: this.$t('PayApplyList.copy'), // '发票号',
        //   type: 'columnTip',
        //   component: 'ColumnTip',
        //   componentProps: {
        //     show: false,
        ...
        //   label: this.$t('PayApplyList.copy'), // '是否有发票查验件',
        //   type: 'select',
        //   width: 120,
        ...
        // {
        //   prop: 'delayInvoice',
        //   label: this.$t('PayApplyList.copy'), // '是否后补票',
        //   type: 'select',
        //   propInDict: 'yesNo',
        // },
        // {
        //   prop: 'delayInvoiceAmt',
        //   label: this.$t('PayApplyList.copy'), // '后补票金额',
        //   width: 110,
        //   type: 'text',
        // },
        {
        ...
        // {
        //   prop: 'invoiceRegTime',
        //   label: this.$t('PayApplyList.copy'), // '发票登记时间',
        //   width: 125,
        //   type: 'text',
        // },
        // {
        //   prop: 'isAgentDzgPay',
        //   label: this.$t('PayApplyList.isAgentDzgPay'), // '是否代大掌柜付款',
        //   width: 170,
        //   type: 'select',
        //   propInDict: 'yesNo',
        //   formatter: ({ row }) => {
        //     return row.dzgPayBillNo ? this.$t('OrderFee.yes') : this.$t('OrderFee.no')
        //   },
        // },
        // {
        //   prop: 'dzgFeeAmt',
        //   label: this.$t('PayApplyList.dzgFeeAmt'), // '代付大掌柜金额',
        //   width: 125,
        //   type: 'text',
        // },
        // {
        //   prop: 'dzgPayBillNo',
        //   label: this.$t('PayApplyList.dzgPayBillNo'), // '大掌柜付款单号',
        //   width: 125,
        //   type: 'text',
        // },
        {
        ...
        {
          prop: 'recvBankAddr',
          label: this.$t('PayApplyList.recvBankAddr'), // '收款银行地址',
          type: 'text',
          width: 140,
        },
        {
          prop: 'recvRegAddr',
          label: this.$t('PayApplyList.recvRegAddr'), // '收款方公司地址',
          type: 'text',
          width: 160,
        },
        {
          prop: 'recvSwiftCode',
          label: this.$t('PayApplyList.recvSwiftCode'), // '收款方SWIFT Code',
          type: 'text',
          width: 120,
        },
        {
          prop: 'transferFeeDeductWay',
          label: this.$t('PayApplyList.transferFeeDeductWay'), // '手续费扣款方式',
          type: 'select',
          width: 180,
        },
      ],
      ...
      applyForPassOption: {
        ...
        btnType: 'success',
        text_cancel: this.$t('OrderFee.cancel'), // '取消',
        titleText: '',
        width: '920px', // '720px',
      },
      handlePrintList: [
      ...
      exportTypeList: [
        { label: this.$t('OrderFee.list'), value: 'payApplySettleListExport' },
        { label: this.$t('OrderFee.detail'), value: 'payApplySettleFeeDetailExport' },
      ],
      ...
      pushCashierList: [
        ...
        { label: this.$t('PayApplyList.andPrintAttachment'), value: 'paySettleFileNoPrint' }, // '并打印+附件'
  ...
  created() {
    this.searchOption1 = {
      ...
      addFilter: {
        ...
        filterGroups: {
          //1
          allNo: {
            label: this.$t('PayApplyList.allNo'), // '各类单号',
            type: 'input',
            // placeholder: this.$t('Settle.multipleTips'),
            showType: '1',
          },
          applyNo: { label: this.$t('PayApplyList.payApplyNo'), type: 'input', showType: '1' }, // 付款申请号
          // invoiceNo: { label: this.$t('FeeList.invoiceNo'), type: 'input', showType: '1' }, // 海外先不显示
          // invoiceRegNo: { label: '发票登记号', type: 'input', showType: '1' }, // 海外先不显示
          ...
          //   showType: '2',
          // },
          // hasCheckInvoiceFile: { // 海外先不显示
          //   label: '是否有发票查验件',
          //   type: 'select',
          //   prop: 'payApplyHasCheckInvoiceFile',
          //   showType: '2',
          // },
          // payStatus: {  label: '付款状态',  type: 'select',prop:"payStatusType", multiple:true,  showType: '2'  }, // 原已未显示
          status: {
            label: this.$t('PayApplyList.status'), // '状态',
            type: 'select',
          ...
          //3
          recvCorpCode: {
            label: this.$t('OrderFee.settleCorpCode'), // '结算单位',
            type: 'remoteSelect',
            showType: '3',
            remote: true,
            multiple: true,
          ...
          custClass: {
            ...
            selectOptions: this.dictMap.customerClassNew.concat({
              label: this.$t('PayApplyList.independentDept'), // '独立结算部门',
              value: 'department',
            }),
          },
          // settleCompName: { label: this.$t('Settle.branchCompany'),  type: 'input',  showType: '3'},
          settleCompCode: {
            label: this.$t('OrderFee.settleCompCode'), // '所属分公司',
          ...
          // },
          //6
          // specialDeductRechtstraeger: { label: this.$t('Settle.bearer'), prop:'specialDeductRechtstraeger', type: 'select',  showType: '6'},
          applyDate: {
            label: this.$t('PayApplyList.createdTime'), // '申请时间',
            type: 'daterange',
            format: 'yyyy-MM-dd',
          ...
          // createdName: { label: this.$t('Settle.applicant'),  type: 'input',  showType: '6'},
          createdBy: {
            label: this.$t('PayApplyList.createdName'), // '申请人',
            prop: 'label',
            type: 'remoteSelect',
          ...
          recvSwiftCode: { label: this.$t('PayApplyList.recvSwiftCode'), type: 'input' },
          transferFeeDeductWay: {
            label: this.$t('PayApplyList.transferFeeDeductWay'), // '手续费扣款方式',
            type: 'select',
            prop: 'transferFeeDeductWay',
    ...
    this.passTableOption = Object.assign({}, this.option, {
      ...
      columns: [
        {
          prop: 'applyNo',
          label: this.$t('PayApplyList.payApplyNo'), // '付款申请单号',
          type: 'text',
          width: '130px',
        },
      ...
      ...this.selfColumnsBase.map((item) => {
        if (item.prop == 'custClass') {
          ...
          item.selectOptionsObj = {
            ...
            department: this.$t('PayApplyList.independentDept'), // '独立结算部门',
          }
        }
        return item
      }),
  ...
  methods: {
      ...
      if (val) {
        this.$refs.recvData.validate((valid) => {
          if (valid) {
            ...
            if (sta1) {
              this.$confirmWarn(this.$t('PayApplyList.confirmAuditRNotFully'), () => {
                // if (invoiceRecvAcctEqRestList.length) { // 海外版不判断发票相关
    ...
    handleApplyForPass() {
      ...
      if(multiData.find(v => v.status !== 'pending' && v.status !== 'approving')){
        return this.$msgErrClose(this.$t('OrderFee.selectRowToAudit'))
      }
      if (
        ...
        return this.$msgErrClose(
          this.$t('PayApplyList.batchPayPassCondition')
        )
      }
      // let str = multiData // 海外版不显示
    ...
    handleApplyFor(confirmStatus) {
      ...
      if (this.userSettleCompanyCode !== row.settleCompCode) {
        return this.$msgErrClose(
          `${this.$t('OrderFee.noPermisionOpData')}${row.settleCompName}${this.$t('OrderFee.switchBranchInSystem')}`
        )
      }
    ...
    identifyElecInvoice() { // 海外版不显示
      let list = this.multipleSelection1
      if (this.multipleSelection1.length === 0) {
        this.$message({
          message: this.$t('OrderFee.selectRow'), // '请勾选结算单需要登记的项！',
          type: 'error',
          showClose: true,
        })
        return
    ...
    getListAmtSummary(data) {
      queryPayApplySettlePageSummary(data).then((res) => {
        if (res.code === 0 && res.data) {
          ...
          let data = { payAmtSummaryList: res.data.payAmtSummaryList }
    ...
    // 所属员工下拉框 this.compCode无值，使用mixin内的方法
    ...
    handleMerge() {
      ...
      if (
        ...
        return this.$msgErrClose(this.$t('PayApplyList.handlingFeeDiffNotCombine')) // '手续费扣款方式不同，不可以合并付款！'
      }
      let applyNoList = this.multipleSelection1.map((item) => {
        return item.applyNo
      })
    ...
    handleWithdrawPayApplyAudit() {
      ...
      this.$confirm(this.$t('PayApplyList.confirmCancelPayAudit'), this.$t('OrderFee.tip'), { // '是否确认撤销付款审核？'
        cancelButtonText: this.$t('OrderFee.cancel'),
        confirmButtonText: this.$t('OrderFee.confirm'),
    ...
    handlePaySeparately() {
      ...
      // ) {
      //   return this.$msgErrClose(this.$t('PayApplyList.usdAndMore2InvoiceSplit')) // '付款币种为USD且关联两张以上发票时才能拆分！请重新选择！'
      // }
      if (this.multipleSelection1[0].confirmStatus !== 'pending') {
        return this.$msgErrClose(this.$t('PayApplyList.onlyAuditCanSplit')) // '待审核状态可被拆分付款！请选择待审核状态！'
      }
    ...
    presentPaperBill(val) {
      ...
      this.$confirm(`${this.$t('OrderFee.confirmTip')}${title}？`, this.$t('OrderFee.tip'), {
        confirmButtonText: this.$t('OrderFee.confirm'),
        cancelButtonText: this.$t('OrderFee.cancel'),
        type: 'warning',
      })
        .then(() => {
          ...
          presentedPaperBill(data).then((res) => {
            this.$message({ message: `${title}${this.$t('OrderFee.success')}`, type: 'success' }) // 成功
            this.getData()
          })
        })
        .catch((err) => {
    ...
    recvAccountNoChange(val) {
      ...
      if (obj) {
        this.recvData.recvBankName = obj.bankEname // obj.bankName
        this.recvData.recvAccountNo = obj.accountNo
        this.recvData.recvAccountName = obj.accountEname // obj.accountName
        this.recvData.recvBankBranch = obj.bankBranchEname // obj.bankBranch
    ...
    handleWithdrawApplyPayOrder() {
      ...
      this.$confirm(this.$t('PayApplyList.confirmCancelPush'), this.$t('OrderFee.tip'), {
        cancelButtonText: this.$t('OrderFee.cancel'),
        confirmButtonText: this.$t('OrderFee.confirm'),
        cancelButtonClass: 'btn-custom-cancel',
        type: 'warning',
    ...
    financeReceive(val) {
      ...
      console.log(data)
      this.$confirm('是否确认' + val.label + '？', this.$t('Common.tips'), {
        confirmButtonText: this.$t('Common.sure'),
        cancelButtonText: this.$t('Common.cancel'),
        type: 'warning',
      })
    ...
    receiveConfirm(data, val) { // 海外版不显示

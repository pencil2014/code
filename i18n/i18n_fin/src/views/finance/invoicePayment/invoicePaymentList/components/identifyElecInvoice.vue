      ...
      <!-- v-for="(item,index) in invoicePaymentList" :key="'invoicePaymentList'+index" -->
          ...
          >
            ...
            <div class="item-list-invoice-payment"><!-- v-show="item.show" -->
                ...
                <el-form-item label="发票税率" :prop="`taxRate`" :rules="{ required: true, message: ' ', trigger: 'blur' }">
                    <el-select v-model="item.taxRate" :placeholder="$t('Common.plSelect')" @change="changeAmt(item)" style="width:170px;">
                        <el-option
                ...
                <el-form-item label="税额" :prop="`taxAmt`" :rules="{ required: false, message: ' ', trigger: 'change' }">
                    <el-input v-model="item.taxAmt" maxlength="16" :placeholder="$t('Common.plEnter')"></el-input>
                </el-form-item>
                <el-form-item label="价税合计(CNY)" :prop="`amtCny`" :rules="{ required: false, message: ' ', trigger: 'change' }">
                    <el-input v-model.trim="item.amtCny" maxlength="16" :placeholder="$t('Common.plEnter')"></el-input>
        ...
        <div class="dialog-add-dialog-item-1">
            ...
            <el-form-item label="我司发票抬头" :prop="`title`">
                <el-input v-model="item.title" :placeholder="$t('Common.plEnter')"></el-input>
            </el-form-item>
            <el-form-item label="纳税人识别号" :prop="`taxNo`">
                <el-input v-model="item.taxNo" disabled :placeholder="$t('Common.plEnter')"></el-input>
            </el-form-item>
            <el-form-item label="我司开户行" :prop="`bankName`">
                <el-input :value="`${item.bankName ? item.bankName : ''}${item.bankBranchName?'(':''}${item.bankBranchName ? item.bankBranchName : ''}${item.bankBranchName?')':''}`" :placeholder="$t('Common.plEnter')" disabled></el-input>
            </el-form-item>
            <el-form-item label="我司账号" :prop="`accountNo`">
                <el-input v-model="item.accountNo" :placeholder="$t('Common.plEnter')" disabled></el-input>
            </el-form-item>
            <el-form-item label="我司电话" :prop="`tel`">
                <el-input v-model="item.tel" disabled :placeholder="$t('Common.plEnter')"></el-input>
            </el-form-item>
        ...
        <div class="dialog-add-dialog-item">
            ...
            <el-form-item label="发票抬头" :prop="`supplierTitle`">
                <el-input v-model="item.supplierTitle" maxlength="32" disabled :placeholder="$t('Common.plEnter')"></el-input>
            </el-form-item>
            <el-form-item label="纳税人识别号" :prop="`supplierTaxpayerNo`">
                <el-input v-model="item.supplierTaxpayerNo" maxlength="32" disabled :placeholder="$t('Common.plEnter')"></el-input>
            </el-form-item>
            <el-form-item label="银行账号" :prop="`supplierAccountNo`" :rules="{ required: true, message: ' ', trigger: 'blur' }">
                ...
                <!-- <el-input v-model="item.supplierAccountNo" :placeholder="$t('Common.plEnter')" ></el-input> -->
            </el-form-item>
            <el-form-item label="开户行" :prop="`supplierBankName`">
                <el-input :value="`${item.supplierBankName ? item.supplierBankName : ''}${item.supplierBankBranchName?'(':''}${item.supplierBankBranchName ? item.supplierBankBranchName : ''}${item.supplierBankBranchName?')':''}`" :placeholder="$t('Common.plEnter')" disabled></el-input>
            </el-form-item>
            <el-form-item label="电话" :prop="`supplierTel`">
                <el-input v-model="item.supplierTel" disabled :placeholder="$t('Common.plEnter')"></el-input>
            </el-form-item>
...
export default {
  ...
  methods:{
    ...
    init(){
      ...
      //   })
      // })
      this.payApplySettleNoList = this.$route.query.payApplySettleNoList // 会自动解析数组
      ...
      // this.sPayApplyQuery = { ...this.$router.query }
      // delete this.sPayApplyQuery.payApplySettleNoList
      // delete this.sPayApplyQuery.currency
      // delete this.sPayApplyQuery.supplierCorpName
      // delete this.sPayApplyQuery.supplierCorpCode
      // delete this.sPayApplyQuery.settleCompCode
      // delete this.sPayApplyQuery.recvAccountNo
      // delete this.sPayApplyQuery.recvBankAccountId
      // delete this.sPayApplyQuery.from
    ...
    submitAdd() {
      ...
      this.$refs['invoicePaymentList'+this.currIndex].validate((valid) => {
        if (valid) {
          ...
          if (this.payApplySettleNoList && this.payApplySettleNoList.length) {
            invoicePaymentAddForm.payApplySettleNoList = this.payApplySettleNoList
          }
          if (this.fromSettlePayApply) {
            ...
            let msg = ''
            if (!bool1 && !bool2) {
              msg = '发票的购方和销方信息与结算付款申请单收款方和付款方信息不一致'
            } else if (!bool1) {
              msg = '发票的购方信息与结算付款申请单付款方信息不一致'
            } else if (!bool2) {
              msg = '发票的销方信息与结算付款申请单收款方信息不一致'
            } else if (!bool3) {

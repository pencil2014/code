<template>
  <div class="finance-page"
    ...
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        ...
        <div class="money-box-right">
          <el-button-group v-if="isDetail && checkAuth($route.name, 'btn-voucherAll')">
          ...
          <el-button-group v-if="!isDetail">
            <el-button class="finance-btn" @click="submitAdd" type="primary" size="mini">{{$t('Common.submit')}}</el-button>
          </el-button-group>
        </div>
      </div>
    </div>
    ...
    >
      ...
      <div class="settlement-detail-form invoice-detail-form-border-bottom">
          ...
          <DetailDiv label="发票凭证号" isSlot isBold v-if="exVoucherNos">
            <MultiBtn :isEx="true" :btnStr="exVoucherNos||''" @generate="getDetail({ invoicePaymentId })"/>
          </DetailDiv>
        ...
        <div class="money-box">
          ...
          <div class="money-box-right">
            ...
            <el-button
              ...
              size="mini"
            >
              {{isShowT[type] ? $t('Common.collapse') : $t('Common.expand')}}
    ...
    <VoucherNosTable :dialogOption="voucherNosTableOption" @close="getDetail({ invoicePaymentId })" />
  </div>
</template>
<script>
...
import { invoicePaymentVoucherList } from '@/api/fin/voucher'
  ...
  methods: {
    init(){
      ...
      this.tableOption[1].columns = [
        ...
        { prop: 'orderType', label: this.$t('PayApplyList.payOrderType'), type: 'select',  propInDict: 'payOrderType' },
        { prop: 'recvCorpName', label: this.$t('FeeList.settleCorp'), type: 'text', width: 150,},
        ...
        { prop: 'applyAmt', label: this.$t('Settle.applyPayAmt'), type: 'text', },
      ...
      this.tableOption[2].columns = [
        ...
        { prop: 'writeoffSubType', label: this.$t('Payment.underwritType'), type: 'select', propInDict:"writeoffSubType"},
        ...
        { prop: 'writeoffTime', label: this.$t('FinBill.paymentDate'), type: 'text', width: 125, },
      ]
      this.selfColumnsBase = [
        { prop: 'orderNo', label: this.$t('PreClose.bizNo'), width: 125,
          type: 'button',
          operationBtns: {
            show: true,
        ...
        { prop: 'settleCorpName', label: this.$t('FeeList.settleCorp'), type: 'text', width: 150, },
        { prop: 'serviceName', label: this.$t('FeeList.serviceCodes'), type: 'text', width: 120,},
        // { prop: 'feeName', label: this.$t('FeeList.feeEname'), type: 'text', width: 120,},
        { prop: 'feeEnName', label: this.$t('FeeList.feeEname'), type: 'text', width: 120, },
        {
          prop: 'receipayType',
        ...
        { prop: 'feeUnit', label: this.$t('FeeList.feeUnit'), type: 'select', propInDict:"feeUnit", width: 100,},
        { prop: 'taxRate', label: this.$t('FeeList.taxRate'), type: 'text' },
        { prop: 'taxAmt', label: this.$t('FeeList.taxAmt'), type: 'text' },
        { prop: 'feeAmtWithoutTax', label: this.$t('FeeList.feeAmtWithoutTax'), type: 'text'},
        { prop: 'invoiceAmt', label: this.$t('Collect.invoicedAmt'), type: 'text'},
    ...
    getDetail(data) {
      ...
      info(data)
        .then((res) => {
          this.exVoucherNos = res.data.invoiceVoucherNo || ''
    ...
    previewAll() {
      invoicePaymentVoucherList({ invoicePaymentId: this.invoicePaymentId }).then((res) => {

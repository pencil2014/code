    ...
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom invoice-payment-list-reg-top header-active">
        ...
        <div class="money-box-right">
          ...
          <el-button :loading='loadingAdd' @click="submitAdd" type="success" size="mini">{{$t('Common.submit')}}</el-button>
          ...
          >
            ...
            <div v-show="item.show" class="item-list-invoice-payment">
              <!-- <div style="width:538px; float:right;">
                ...
                <el-form-item label="发票代码" :prop="`invoicePaymentList[${index}].invoiceCode`" :rules="[{ required: invoiceNoChange(item,index) ||　invoicePaymentAddForm.isRepeatInvoiceNo, message: ' ', trigger: 'change' },{ pattern:  /^[0-9]+$/, message: '必须数字' }]">
                  <el-input v-model="item.invoiceCode" maxlength="12" placeholder="请输入12位数字"></el-input>
                </el-form-item>
                ...
                <el-form-item label="发票号" :prop="`invoicePaymentList[${index}].invoiceNo`" :rules="[{ required: true, message: ' ', trigger: 'change' },{ pattern:  /^[0-9]+$/, message: '必须数字' }]">
                    ...
                    <el-input v-model="item.invoiceNo" maxlength="20" placeholder="请输入8位或20位数字"></el-input>
                </el-form-item>
                ...
                <el-form-item label="价税合计(CNY)" :prop="`invoicePaymentList[${index}].amtCny`" :rules="{ required: false, message: ' ', trigger: 'change' }">
                    <el-input v-model.trim="item.amtCny"  @input="changeRate(item)" @change="changeRates(item)" maxlength="16" :placeholder="$t('Common.plEnter')"></el-input>
                </el-form-item>
        ...
        if (valid) {
          let invoiceList = this.invoicePaymentAddForm.invoicePaymentList
            for (let i = 0; i < invoiceList.length; i++) {
                ...
                if (invoiceList[i].invoiceCode.length && invoiceList[i].invoiceCode.length !== 12) {
                  return this.$msgErrClose(`发票代码只能是12位数字或者不填写！`)
                }

    ...
    <div class="cancel-invoice-receivable-form">
      <el-dialog :title="'标记财务已收'" :visible.sync="dialogVisible" width="604px">
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" size="mini" @click="receiveConfirm">{{$t('Common.sure')}}</el-button>
...
export default {
  ...
  created() {
    this.searchOption = {
      ...
      addFilter:{
        ...
        filterGroups: {
          //1
          allNo: { label: this.$t('FeeList.allNo'), type: 'input', placeholder:"多个单号用空格或逗号分隔", showType: '1' },
          ...
          writeoffReceipayNo: { label: this.$t('FeeList.writeoffNo'),  type: 'input',  showType: '1'  },
          receiveNo: { label: '发票登记号',  type: 'input',  showType: '1'  },
          // sourceApplyNo: { label: this.$t('Settle.payApplyNo'),  type: 'input',  showType: '1'  },
          //2
          ...
          // supplierCorpName: {  label: this.$t('FeeList.settleCorp'),  showType: '6',  type: 'autocomplete',  multiple:true,
          ...
          //   }
          // },
          supplierBankName: { label: this.$t('PayApplyList.recvBankName'),  type: 'input',  showType: '6'  },
          ...
          bankName: { label: this.$t('PayApplyList.payBankName'),  type: 'input',  showType: '6'  },
          //7
          ...
          payTime: {  label: this.$t('FinBill.paymentDate'),  type: 'daterange',  showType: '7'},
          receiveBy: {
            label: '发票签收人',
            type: 'remoteSelect',
            clearable: true,
    ...
    let operationBtns = Object.assign({}, this.option.operationBtns, {
      ...
      data: [
        ...
        },
        {
          ...
          type: 'text',
          showFn: (row)=> row.lockAmt== 0 && row.writeoffAmt == 0 && row.invoiceVoucherNo && this.checkAuth(this.$route.name, 'btn-Edit'),
          action: 'Edit'
        }
      ]
    })
  ...
  methods: {
    ...
    delInovice() {
      ...
      this.$confirm('确认删除此发票吗？', this.$t('Common.tips'), {
    ...
    financeReceive(val) {
      ...
      this.$confirm(`是否确认${title2}？`, this.$t('Common.tips'), {
        confirmButtonText: this.$t('Common.sure'),
        cancelButtonText: this.$t('Common.cancel'),
        type: 'warning',
      }).then(() => {

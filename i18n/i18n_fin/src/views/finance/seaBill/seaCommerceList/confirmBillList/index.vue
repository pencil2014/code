  ...
  >
    ...
    <el-dialog :title="refuseOpentitle" :visible.sync="refuseOpen" width="600px">
      <el-form :model="refuseForm" ref="refuseForm" label-width="80px">
        ...
        >
          <el-select v-model="refuseForm.dnRefuseReason" :placeholder="$t('Common.plSelect')">
            <el-option
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button @click="closeRefuse" size="mini">{{$t('Common.cancel')}}</el-button>
      </span>
    </el-dialog>
    <el-dialog title="撤销确认" :visible.sync="cancelOpen" width="600px">
      <el-form :model="cancelForm" ref="cancelForm" label-width="80px">
        <el-form-item label="原因归类：" prop="cancelType" >
          <el-select v-model="cancelForm.cancelType" :placeholder="$t('Common.plSelect')">
            <el-option
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleCancelRemark" size="mini">{{$t('Common.sure')}}</el-button>
        <el-button @click="cancelOpen = false" size="mini">{{$t('Common.cancel')}}</el-button>
      </span>
    </el-dialog>
    <el-dialog
      title="供应商专项管理费"
    ...
    >
      <div class="add-bill-cnt">
        ...
        >
          ...
          <el-form-item label="币种:" prop="currency">
            <el-select v-model="addBillForm.currency" :placeholder="$t('Common.plSelect')" filterable>
              <el-option
          ...
          <div>
            <div v-show="showCoastTable && showTable[1]">
              <div class="cost-employed">
                ...
                <!-- <div class="cost-item">
                  <span>费用名称：</span>
                  <el-select v-model="priceFeeCode" :placeholder="$t('Common.plSelect')">
                    <el-option
                      v-for="(op, i) in specialFeeOption"
                      :key="'feeCode' + i"
                      :label="op.label"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitAddBill" size="mini">{{$t('Common.submit')}}</el-button>
        <el-button size="mini" @click="closeDialogAddBill">{{$t('Common.cancel')}}</el-button>
      </span>
    </el-dialog>
    ...
    this.searchOption = {
      ...
      addFilter: {
        ...
        filterGroups: {
          ...
          inputNo: { label: this.$t('PreClose.bizNo'), type: 'input', showType: '1' },
  ...
  methods: {
    ...
    shBillConfirmSuc() {
      ...
      shBillConfirm(data).then((res) => {
        ...
        if (this.checkAuth(this.$route.name, 'tab-confirmBillList', 'btn-addBillDialog')) {
          ...
          // 是否添加管理费
          this.$confirm('是否添加供应商专项费用？', this.$t('Common.tips'), {
    ...
    getAddLienFee() {
      if (this.multipleSelection.find((v) => v.signStatus === 'yes')) {
        ...
        this.$confirmWarn({ body: msg, confirm: this.$t('Common.yes'), cancel: this.$t('Common.no') }, () => {
          this.getAddLienFee2()
        })
      } else {
        this.getAddLienFee2()

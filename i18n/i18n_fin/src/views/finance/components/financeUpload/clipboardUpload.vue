<template>
  <div class="clipboard-container">
    <div class="clipboard-embed">
      <div class="clipboard-visible-btn" :style="{width: width + 'px'}" @click="handleVisible">{{$t('Finance.pasteUpload')}}</div>
    </div>
    <el-dialog
      :title="$t('Finance.pupload')"
      ...
      <div class="clipboard-upload">
        <div class="clipboard-left" @paste="handlePaste">
          ...
          <div class="clipboard-left-text">
            <div>
              <input id="clipboard-input" :placeholder="$t('Finance.screenshot')"/>
            </div>
            <!-- <p class="info">
        ...
        <div class="clipboard-preview-container">
          <div id="clipboard-preview"
            ...
            <div v-show="!pasted" class="clipboard-preview-text">{{$t('Finance.screenshotPre')}}</div>
        ...
        <div class="clipboard-footer-upload">
          ...
          >
            ...
            <div class="file-upload__text">
              <div class="txt">{{$t('Finance.importTips')}}</div>
            </div>
          </el-upload>
          <!-- finance-table-upload -->
        </div>
        <span class="clipboard-footer-btn">
          <el-button type="primary" @click="confirm" size="mini">{{$t('Common.sure')}}</el-button>
          <el-button @click="clipboardVisible = false" size="mini">{{$t('Common.cancel')}}</el-button>
        </span>
      </div>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    handlePaste(event) {
      ...
      if (!items || items.length === 0) {
        this.$message.error(this.$t('Finance.notsupport')); // 当前浏览器不支持本地
        return;
      }
      ...
      if (!file) {
        this.$message.error(this.$t('Finance.notimg'));
        return;
      }
    ...
    handleExceed(files, fileList) {
      this.$message.error(this.$t('Settle.onlyfiles', {limit: this.limit})) // Message.error
    },
		getFile(file, hideMsg) {
			...
			if (accept) {
				...
				if (!isAccept && !hideMsg) {
					this.$message.error(this.$t('Finance.imgformatTips', {type: this.acceptTypes.join('、')}));
				}
			}
    ...
    uploadAction() {
      if (this.fileList.length === 0) {
        this.$message({
          type: 'error',
          message: this.$t('Finance.uploadTips'),
          showClose: true
        })
        return
      }

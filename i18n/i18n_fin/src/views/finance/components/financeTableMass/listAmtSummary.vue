...
<div class="finance-list-amt-summary" :style="{'padding-bottom':isPlaceBottom&&place==='bottom'?'2px':0,height:listAmtSummary.isPartTop && place === 'top'?'51px':'auto'}">
  <template v-if="listAmtSummary.groups && place === 'bottom'">
    <div class="money-box list-amt-summary-default">
      <div class="money-box-left">
        <div class="money-title">{{$t('Common.totalAmt')}}</div>
      </div>
      <div class="money-box-right">
        <el-button type="text" class="collapse-btn" @click="handleCollapse('ListAmtSummary')" size="mini">
          {{showListAmtSummary ? $t('Common.collapse') : $t('Common.expand')}}<i :class="{'el-icon-arrow-up': showListAmtSummary, 'el-icon-arrow-down': !showListAmtSummary}"></i>
        </el-button>
      </div>
    </div>
    ...
    <div v-show="showListAmtSummary"
      ...
      <ul class="list-amt-summary">
        ...
        >
        <template v-if="listAmtSummary.labelObj && listAmtSummary.labelObj[key + 'Label']">
  ...
  <div v-if="listAmtSummary.groups && listAmtSummary.isPartTop && place === 'top'">
    <ul class="list-amt-summary" style="padding:5px 0 0;background:#FFF;border-top:1px solid #E8E9ED;">
      <li class="list-amt-summary-item" v-for="(value, key) in listAmtSummary.subData1" :key="'listAmtSummary' + key" style="margin-left:0;"
        ...
        @mouseleave="listAmtSummaryShowMore($event, value, key, 'hide')"
      >
        <div class="list-amt-summary-title" style="width:auto;min-width:148.08px;">
          {{$t('Common.total')}} {{listAmtSummary.labelObj && listAmtSummary.labelObj[key + 'Label']}}: 
        </div>
        <ul class="list-amt-summary-content">
          <li class="list-amt-summary-content-item" v-for="(item, index) in value.filter((item, index) => index < 2)" :key="key + index">
            <span>{{item.currency}}</span>
  ...
  <div v-if="!listAmtSummary.groups">
    <ul v-if="listAmtSummary.sortArr" class="list-amt-summary" style="padding:5px 0;background:#FFF;border-top:1px solid #E8E9ED;min-height:31px;"> 
      <li class="list-amt-summary-item" v-for="key in listAmtSummary.sortArr" :key="'listAmtSummary' + key" style="margin-left:0;"
        @mouseenter="listAmtSummaryShowMore($event, listAmtSummary.data[key], key)"
        @mouseleave="listAmtSummaryShowMore($event, listAmtSummary.data[key], key, 'hide')"
      >
        <div class="list-amt-summary-title" style="width:auto;">
          {{$t('Common.total')}} {{listAmtSummary.labelObj && listAmtSummary.labelObj[key + 'Label']}}: 
        </div>
        <ul class="list-amt-summary-content">
          <li class="list-amt-summary-content-item" v-for="(item, index) in listAmtSummary.data[key].filter((item, index) => index < 2)" :key="key + index">
            <span>{{item.currency}}</span>
            <span :style="{color: listAmtSummary.colorObj && listAmtSummary.colorRgbObj[listAmtSummary.colorObj[key + 'Color']]}">
              {{item.amt}}
            </span>
    ...
    <ul v-else class="list-amt-summary" style="padding:5px 0;background:#FFF;border-top:1px solid #E8E9ED;min-height:31px;"> 
      <li class="list-amt-summary-item" v-for="(value, key) in listAmtSummary.data" :key="'listAmtSummary' + key"
        @mouseenter="listAmtSummaryShowMore($event, value, key)"
        @mouseleave="listAmtSummaryShowMore($event, value, key, 'hide')"
      ><!-- style="margin-left:0;" -->
        <div class="list-amt-summary-title" style="width:auto;">

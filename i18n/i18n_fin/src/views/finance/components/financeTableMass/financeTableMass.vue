<template>
  <div class="finance-table" :class="{'max__height__table':$attrs.autoMax}" :id="option.id || 'option1'" :style="option.styleCfg||{}">
    ...
    <div class="finance-el-table" :style="option.style||{}">
      ...
      <!-- scrollY.gt || (option.edit ? 100 : 50)  编辑状态下虚拟渲染大于100条激活，非编辑状态下虚拟渲染大于46(50以下为了50分页时就激活)即可-->
      <vxe-table
        v-if="showXTable && option.columns"
        ref="xTable1"
        resizable
        :empty-text="option.emptyText || $t('Common.noData')"
        ...
        :scroll-y="{enabled: option.scrollY ? (option.scrollY.enabled === false ? false : true) : true , gt: option.scrollY && option.scrollY.gt || (option.edit ? 100 : 46)}"
      ...
      >
        ...
        <vxe-column
          ...
          :resizable="true">
          <template #header>
            <el-tooltip :content="$t('Common.custTableHead')" placement="right">
          ...
          <template #default="{ row }">
            <div class="td_tag_div" v-if="row._tagArr && row._tagArr.length && (row._tagArr[0].text===$t('OrderFee.joint') || option.tagArrShow)">
              <span
        ...
        <vxe-column
          ...
          :title="option.index.title || $t('Common.index') "
        ...
        <vxe-column
          ...
          :title="item.label"
          :field="item.prop"
          :min-width="option.tdMinWidth || item.minWidth || 120"
          :width="item.forceWidth || (columns.length < 15 && ((parseInt(item.width) < 80 && parseInt(item.width) > 60) || parseInt(item.width) >= 120) ? '' : item.width)"
          :params="item"
        ...
        <!-- :show-overflow="(item.showOverflow === false ? false : true) && item.type !== 'columnTip'" -->
          ...
          <template #default="{ row, column, rowIndex }">
            <el-tooltip placement="top" v-if="item.isTool" >
              <div slot="content" v-if="item.newline">
                <div v-for="(i, k) in row.isTool" :key="k">{{i}}</div>
              </div>
              <div slot="content" v-else>{{row.isTool || item.isTool}}</div>
        ...
        <vxe-column
          ...
          :field="item.prop"
          :min-width="option.tdMinWidth || item.minWidth || 120"
          :width="columns2.length < 15 && ((parseInt(item.width) < 80 && parseInt(item.width) > 60) || parseInt(item.width) >= 120) ? '' : item.width"
          :params="item"
        ...
        <vxe-column
          ...
          :field="item.prop"
          :min-width="option.tdMinWidth || item.minWidth || 120"
          :width="columns3.length < 15 && ((parseInt(item.width) < 80 && parseInt(item.width) > 60) || parseInt(item.width) >= 120) ? '' : item.width"
          :params="item"
          :fixed="item.fixed ? item.fixed : ''"
          :formatter="item.formatter || formatterCommon"
          :sortable="option.sortable && item.sortable !== false"
        ...
        <vxe-column
          ...
          :title="option.operationBtns.title || $t('Common.operate')"
        ...
        >
          <template v-if="option.operationBtns.headerSign" #header>
            <span style="margin-right:4px;">{{ option.operationBtns.title || $t('Common.operate') }}</span>
            <el-tooltip v-if="option.operationBtns.headerSign.text" :content="option.operationBtns.headerSign.text || ''" placement="top">
              <i class="icon-custom-columns" :class="option.operationBtns.headerSign.iconClass || 'el-icon-question'"></i>
            </el-tooltip>
          </template>
          <template #default="{ row, rowIndex }">
            <template v-for="(item, index) in option.operationBtns.data">
              ...
              <!-- v-if="row.edit || item.show || item.showFn(row)" -->
                <el-button
                  type="text"
                  :style="item.style||getOperationColor(item)||{color:btnColorCfg[item.colorType]||'#1890ff'}"
  ...
  data() {
    return {
      ...
      COLORFUL: {
        ...
        feeitemReceipayGroup: {receive: 'red', pay: 'green', receivepay: 'yellow', },
        // 收付类型 
        ...
        payWriteoffStatus: {yes: 'green', no: 'red' },
        // 发票状态 
        feeInvoiceStatus: {finish: 'green', nofinish: '#4a4a4a', part_finish: 'yellow', }, // nofinish: 'red'
        receiveInvoiceApplyStatus: {finish: 'green', nofinish: '#4a4a4a', part_finish: 'yellow', }, // nofinish: 'red'
        // 财务状态 
        ...
        voucherYesNoPart: {yes: 'green', no: 'red', part_invoice: 'yellow', },
        // 退款付款状态
        payStatusType: {pay_done: 'green', no_pay: 'red', paying: 'yellow', },
      },
      btnColorCfg: {edit: '#EDB534',del: '#33B18A',pass: '#33B18A',cancel: '#604BB0',},
      ...
      if (prop === 'currency') {
        ...
        if (dict && !forceSelection) {
          if(multi){
            ...
            let regx = /^\,*|\,*$/g // 去掉首尾多余逗号
            return (cellValue+'').replace(/\b(\w+)\b/g, (item) => ((dict && dict[item]) || (noDict?'':item))).replace(/(,+)/g,',').replace(regx,'') // return (cellValue+'').replace(/\b(\w+)\b/g, (item) => ((dict && dict[item]) || (noDict?'':item))).replace(/(,+)/g,',')
          }
          // TODO 暂时按需求改，不必全局配置，待全局改再激活配置
      ...
      if (this.tips && !this.option.rootTipsHide) {
        Object.assign(
          this.tips,
          {
            ...
            text: data.filter(v => !v.invisible).length ? this.$t('Common.selectTips', {length: data.filter(v => !v.invisible).length}) : '',
          }
        )
      }
    ...
    rowClassNameFunc({ row, rowIndex }) {
      ...
      if (row._isBlack) {
				return 'finance-table-row-black'
			}
      if (row.invisible) {
      ...
      if (row._isDanger) {
        return 'finance-table-row-danger'
      }
      if (row.redMark) {
...
.finance-table .vxe-table--render-default th {
  padding: 12px 0 !important; // 3px 0 !important;
  height: 46px; // 30px !important;
  color: #333!important;
}
.finance-table .vxe-table--render-default th .vxe-cell {
  justify-content: center;
}
#app .finance-table .vxe-table--render-default tr td {
  text-align: center;
  padding-top: 12px; // 3px;
  padding-bottom: 12px; // 3px;
  color: #4a4a4a;
  height: 46px; // 26px !important;
}
...
.finance-table .finance-self-tag.black {
...
.finance-table .finance-table-row-black {
...
.finance-table .finance-table-row-danger {

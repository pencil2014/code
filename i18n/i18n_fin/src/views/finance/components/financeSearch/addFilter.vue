<template>
  ...
  >
    ...
    <div class="el-row">
      <div class="el-col filter-left">
        ...
        <el-select
          v-model="item.key"
          size="mini"
          :placeholder="item.placeholder || $t('Common.plSelect')"
          filterable
          :disabled="item.isFixedCreatedDate"
          @change="handleSelectQuery"
        >
          <template v-for="(child, key) in filterGroups">
            <el-option
              :key="key"
              :label="child.label"
              :value="key"
      ...
      <div
        ...
        v-if="rightShow"
        :class="{scope__select:filterGroups[item.key] && filterGroups[item.key].scope,'filter-right-noborder':filterGroups[item.key] && ['lsDaterange','lsDatetimerange'].includes(filterGroups[item.key].type)}"
      >
        <el-select
        ...
        <div class="multi-select-tag" v-else-if="filterGroups[item.key]">
          ...
          <el-autocomplete
            ...
            :placeholder="filterGroups[item.key].placeholder || $t('OrderFee.plsInput') + filterGroups[item.key].label"
          ...
          <el-date-picker
            ...
            :clearable="filterGroups[item.key].clearable || true"
            :range-separator="filterGroups[item.key]['range-separator']"
            :start-placeholder="filterGroups[item.key]['start-placeholder'] || $t('Common.startTime')"
            :end-placeholder="filterGroups[item.key]['end-placeholder'] || $t('Common.endTime')"
            :placeholder="filterGroups[item.key].placeholder || $t('Common.plSelect') + filterGroups[item.key].label"
            :format="filterGroups[item.key].format || 'yyyy-MM-dd'"
            :align="filterGroups[item.key].align"
            :popper-class="filterGroups[item.key]['popper-class']"
          ...
          <el-cascader
            ...
            v-model="item.value"
            collapse-tags
            @change="(value) => {filterGroups[item.key].change && filterGroups[item.key].change(value);cascaderVisibleChange();}"
    ...
    <!-- 至少保留一个搜索条件 -->
    <i
      v-if="searchInputGroup.length>1&&!item.isFixedCreatedDate"
...
export default {
  ...
  data() {
    return {
      ...
      scopeCfg: [
        { label: this.$t('Common.include'), value: '0' },
        { label: this.$t('Common.notInclude'), value: '1' },
      ],
    }
  },
  props: {
  ...
  methods: {
    ...
    minChange(item) {
      ...
      if (min !== '' && max !== '' && min - max > 0) {
        this.$msgWarnClose(this.$t('Common.minTips'))
        item.min = ''
      }
    },
    maxChange(item) {
      let { min, max } = item
      if (min !== '' && max !== '' && min - max > 0) {
        this.$msgWarnClose(this.$t('Common.maxTips'))
        item.max = ''
      }
    },
    rangeEnter(item) {
...
.search-filter {
  ...
  ::v-deep {
    ...
    .el-input__suffix .el-input__suffix-inner > i {
      line-height: 30px !important; // 20px !important;

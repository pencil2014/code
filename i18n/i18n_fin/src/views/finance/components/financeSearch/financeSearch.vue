  ...
  >
      ...
      <el-form ref="form" :inline="true" size="mini">
        ...
        >
          <el-input
            ...
            :placeholder="item.placeholder || $t('Common.plEnter') + item.label"
            clearable
          ></el-input>
          <el-date-picker
          ...
          <el-date-picker
            ...
            :start-placeholder="item.startPlaceholder || $t('Common.startTime')"
            :end-placeholder="item.endPlaceholder || $t('Common.endTime')"
          ></el-date-picker>
          <el-select
          ...
          <el-select
            v-if="item.type === 'select' && item.remote"
            v-model="searchOption.data[item.prop]"
            :placeholder="$t('Common.plEnter') + item.label"
            filterable
            remote
            clearable
            :multiple="item.multiple"
        ...
        <template v-if="searchOption.addFilter && !addFilterHide">
          <AddFilter
        ...
        <div class="search-btns" v-if="!searchOption.hideBtns">
          <el-button type="primary" @click="search('btn')" size="mini" icon="el-icon-search">{{$t('Common.search')}}</el-button>
          <el-button @click="cancel" size="mini">{{$t('Common.reset')}}</el-button>
          <i
    ...
    <div class="search-template" v-show="templateShow && searchOption.saveShow">
      <div class="fl template-list">
        <span>{{$t('Common.selectTemplate')}}：</span>
        <span
          class="template-li"
      ...
      <div class="fr template-operate">
        <span class="mr10" v-show="(searchOption.conditions||[]).length > 5">
          {{$t('Common.moreTemplates')}}
          <el-select
            style="width: 180px !important"
            size="mini"
            v-model="composeKey"
            :placeholder="$t('Common.moreTemp')"
        ...
        >
          <div class="flex">
            <span class="span-text">{{$t('Common.tempName')}}</span>
            <el-input
              type="text"
              size="mini"
              maxlength="32"
          ...
          <div style="text-align: right; margin: 8px 0 0">
            <el-button size="mini" type="default" @click="popoverShow=false" class="ui-mini-btn">{{$t('Common.cancel')}}</el-button>
            <el-button
              type="primary"
              size="mini"
          ...
          <el-button
            ...
            size="mini"
            slot="reference"
          >{{(searchOption.conditions||[]).length && composeKey ? $t('Common.saveas') : $t('Common.save')}}</el-button>
        </el-popover>
        <el-button
          type="text"
          size="mini"
    ...
    <div class="search-fieldValue-cont" v-if="searchOption.contentShow">
      <div class="label">{{$t('Common.queryCriteria')}}：</div>
...
export default {
  ...
  methods: {
    ...
    handleFixCreatedDate() {
      if (this.searchOption.fixCreatedDate) {
        let n = this.searchOption.fixCreatedNMonth || 3
        // this.addFilterHide = true
        let searchInputGroup = this.searchOption.addFilter.searchInputGroup
        // let searchInputGroup = JSON.parse(JSON.stringify(this.searchOption.addFilter.searchInputGroup))
        let index = searchInputGroup.findIndex(item => item.key === this.searchOption.fixCreatedKey)
        if (index > -1) {
          let find = searchInputGroup[index]
          if (!find.isFixedCreatedDate) {
            Object.assign(find, { value: [this.getLastNMonth(n).last, this.getLastNMonth(n).now], range: { min: '', max: '' }, isFixedCreatedDate: true, fixCreatedNMonth: n })
            // searchInputGroup.splice(index, 1)
            // searchInputGroup.push({
            //   key: this.searchOption.fixCreatedKey, value: [this.getLast3Month(Date.now()).last, this.getLast3Month(Date.now()).now], range: { min: '', max: '' }, isFixedCreatedDate: true
            // })
          }
          //  else if (index !== searchInputGroup.length - 1) {
          //   let ct = searchInputGroup.splice(index, 1)
          //   if(ct && ct[0]) searchInputGroup.push(ct[0])
          // }
        } else {
          searchInputGroup.push({
            key: this.searchOption.fixCreatedKey, value: [this.getLastNMonth(n).last, this.getLastNMonth(n).now], range: { min: '', max: '' }, isFixedCreatedDate: true, fixCreatedNMonth: n
          })
        }
        let cdFilter = this.searchOption.addFilter.filterGroups[this.searchOption.fixCreatedKey]
        if (cdFilter) {
          cdFilter.pickerOptions = {
            shortcuts: [{
              text: this.$t('Common.last3months'),
              onClick(picker) {
                const end = new Date();
              ...
              text: this.$t('Common.last6months'),
              onClick(picker) {
                const end = new Date();
                const start = new Date();
              ...
              onClick(picker) {
                const end = new Date();
                const start = new Date();
                start.setMonth(start.getMonth() - 240);
                picker.$emit('pick', [start, end]);
              }
            }]
          }
        ...
        // this.$nextTick(() => {
        //   this.addFilterHide = false
        //   this.searchOption.addFilter.searchInputGroup = searchInputGroup
        // })
      }
    },
    getLastNMonth(n) {
      ...
      return { now: dateFormat('yyyy-mm-dd', end), last: dateFormat('yyyy-mm-dd', start) }
    },
    getLast3Month() {
      const end = new Date();
      const start = new Date();
  ...
  　　　// 开始时间所在月的总天数<本月总天数，比如当前是5月30日，在2月中没有30，则取下个月的第一天(3月1日)为开始时间
    ...
    cancel() {
      ...
      if (filters) {
        ...
        filters.searchInputGroup.forEach((item) => {
          ...
          if (item.isFixedCreatedDate) {
            let n = item.fixCreatedNMonth || 3
            item.value = [this.getLastNMonth(n).last, this.getLastNMonth(n).now]
          }
          if (item.range) {
    ...
    // 保存
    handleSaveTemplate() {
      this.$confirmWarn(this.$t('Payment.saveTemp'), () => {
    ...
    handleSaveAsTemplate() {
      ...
      if (!this.newComposeKey && !this.composeKey) return this.$msgErrClose(this.$t('Payment.templateName'))
      if (this.searchOption.conditions.find((v) => v.composeKey.trim() == this.newComposeKey.trim())) {
        this.popoverShow = false
        return this.$confirmWarn(this.$t('Payment.overwritten'), () => {
          this.searchConditionSave()
        })
      }
      this.popoverShow = false
    ...
    handleConditionDelete(item, index) {
      this.$confirmWarn(this.$t('Payment.delTemp', {composeKey: item.composeKey}), () => {
  ...
  // watch: {
  //   'searchOption.addFilter.searchInputGroup'(val){
  //     console.log(val);
  //   }
  // },
  computed: {
...
.finance-search-form ::v-deep .el-range-separator {
  ...
  line-height: 30px !important; // 16px !important;
}
// 搜索按钮区域
.search-btns {
  float: right;
...
.search-fieldValue-cont {
  overflow: hidden;
  padding: 0 8px 8px 8px; // 0 8px 4px 8px
  margin-top: -4px;
  .label {
    float: left;
    display: inline-block;
  ...
  .filedValue-list {
    ...
    line-height: 20px;
    height: 20px;
    background-color: #f1f6fd; // #c6e2ff;
    margin-right: 10px;
    cursor: pointer;
    margin-top: 4px;
    border-radius: 2px;

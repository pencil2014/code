...
export default {
  ...
  props: {
    ...
    defaultOption: {
      ...
      default: () => {
        return {
          columns: [
            {
              label: i18n.t('FeeList.settleCorp'),
              prop: 'settleCorpName',
              type: 'select',
              filterable: true,
              selectOptions: [],
            ...
            {
              label: i18n.t('FeeList.currency'),
              prop: 'currency',
              type: 'select',
              propInDict: 'noOriginalCurrency',
            },
            {
              label: i18n.t('FeeList.feeEname'),
              prop: 'feeEnName',
              type: 'select',
              filterable: true,
              selectOptions: [],
            ...
            {
              label: i18n.t('PreClose.bizNo'),
              prop: 'sourceBizNo',
              type: 'input',
            },
            // {
            //   label: i18n.t('Settle.invoiceStatus'),
            //   prop: 'invoiceStatus',
            //   type: 'select',
            //   propInDict: 'feeInvoiceStatus',
            // },
            {
              label: i18n.t('FeeList.settleComp'),
              prop: 'settleCompName',
              type: 'select',
              filterable: true,
              selectOptions: [],
    ...
    extendOption: {
      ...
      default: () => ({
        columns: [
          // { // 海外版不显示
          //   label: i18n.t('FeeList.invoiceableAmt'),
          //   prop: 'canInvoiceAmt',
          //   type: 'range',
          //   inputType: 'number',
          // },
  ...
  watch: {
    list: {
      handler(val) {
        let feeNameList = [...new Set(val.map((v) => v.feeEnName))].map((v) => ({ label: v, value: v }))
  ...
  methods: {
    ...
    search(val) {
      ...
      let { settleCorpName, currency, sourceBizNo, invoiceStatus, settleCompName, feeEnName, canInvoiceAmt } = val || {} // feeName
      let { min, max } = canInvoiceAmt || {}
      if (settleCorpName || currency || sourceBizNo || invoiceStatus || settleCompName || feeEnName || min || max) { // feeName
        console.log('/有值搜索')
        ...
        this.list.forEach((item) => {
          ...
          if (
            (feeEnName && feeEnName !== item.feeEnName) || // feeName && feeName !== item.feeName

	...
	<div class="finance-table" :class="{ max__height__table: $attrs.autoMax }">
		<div class="finance-el-table" :style="option.style || {}">
			...
			>
				...
				<el-table-column
					:label="option.index.title || $t('Common.index')"
					type="index"
					width="40px"
					align="center"
					...
					>
						<template slot-scope="scope">
							...
							<el-input
								...
								:placeholder="item.placeholder || $t('Common.plEnter')"
								clearable
              ...
              <template v-else>
                ...
                >{{scope.row[item.prop]}}</el-button>
							...
							<div v-else-if="option.edit && scope.row.edit && !item.readOnly" @click.stop>
								<el-input v-if="item.type === 'number'" type="number" :disabled="item.notAllowed" :placeholder="$t('Common.plEnter')" v-model="scope.row[item.prop]"></el-input>
								<el-input
								...
								<div v-if="item.type === 'datePicker'" class="finance-table-datepicker">
									<el-date-picker v-model="scope.row[item.prop]" type="date" :placeholder="$t('Common.plSelect')" :value-format="item.valueFormat"></el-date-picker>
								</div>
								<div v-if="item.type === 'dateRange'" class="finance-table-datepicker">
									<el-date-picker v-model="scope.row[item.prop]" type="daterange" :value-format="item.valueFormat || ''" range-separator="-" :start-placeholder="$t('Common.startDate')" :end-placeholder="$t('Common.endDate')"></el-date-picker>
								</div>
								<div v-if="item.type === 'upload'">
									<el-upload
										...
										"
									>
										<el-button type="text" class="noselect" v-show="showUploadBtn" size="mini">{{$t('Hedge.clickUpload')}}</el-button>
									</el-upload>
								</div>
				...
				<el-table-column :label="$t('Common.operate')" :align="option.operationBtns.align || 'center'" :width="option.operationBtns.width" :fixed="option.operationBtns.fixed" v-if="option.operationBtns && option.operationBtns.show">
					<template slot-scope="scope">
					...
					<div class="money-box-left">
						<div class="money-total" v-for="(value, key) in option.listAmtSummary.data" :key="'listAmtSummary' + key">
							<span>
								...
								<span v-if="value && !value.length">{{$t('Common.none')}}</span>
							</span>
						</div>
					</div>
				</div>
			...
			Object.assign(
				...
				this.option.rootTipsHide
					...
					: {
							...
							text: data.length ? this.$t('Common.selectTips', {length: data.length}) : ''
					  }
			)

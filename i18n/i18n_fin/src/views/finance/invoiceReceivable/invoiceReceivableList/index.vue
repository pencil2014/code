          ...
          >
            ...
            >
              <el-input
                ...
                :placeholder="`请输入${runshCancel}说明`"
                type="textarea"
                style="width: 650px;"
                maxlength="500"
                show-word-limit
...
export default {
  ...
  data() {
    return {
      ...
      selfColumnsBase: [
        ...
        { prop: 'taxRate', label: this.$t('FeeList.taxRate'), type: 'select', propInDict: 'taxRateInfo', width: 80 },
        {
          prop: 'taxAmt',
        ...
        { prop: 'employeeName', label: this.$t('OrderFee.salesman'), width: 100 },
        ...
        { prop: 'applyName', label: this.$t('Settle.applicant'), type: 'text', width: 100 },
        { prop: 'applyTime', label: this.$t('Settle.applicationDate'), type: 'text', width: 130 },
    ...
    this.searchOption = {
      ...
      addFilter: {
        ...
        filterGroups: {
          ...
          noTaxAmt: { label: this.$t('FeeList.feeAmtWithoutTax'), type: 'range' },
          ...
          applyTime: { label: this.$t('Settle.applicationDate'), type: 'daterange', format: 'yyyy-MM-dd' },
          updatedBy: {
            label: '更新人',
            type: 'remoteSelect',
            remoteMethod: (queryString, item) => this.getEmployeeList2(queryString, item),
  ...
  methods: {
    ...
    sendInvoiceFile() {
      ...
      let rowData = this.multipleSelection1[0]
      if (this.notOk(rowData.invoiceType !== 'electronic', '请选择电子发票！')) {
        return
      }
      this.$prompt('请确认是否发送发票至以下邮箱？', ' ', {
        confirmButtonText: this.$t('Common.sure'),
        cancelButtonText: this.$t('Common.cancel'),
        inputValue: rowData.emailAddrs,
        inputType: 'textarea',
        ...
        .then(({ value }) => {
          sendInvoiceFile({
            invoiceReceivableId: rowData.invoiceReceivableId,
            emailAddrs: value.replace(/，/g, ','),
          }).then((res) => {
    ...
    relateSettlement() {
      ...
      this.rowData = {...rows[0], invoiceReceivableIds: rows.map(v => v.invoiceReceivableId).join(',')}
    ...
    handleEdit(row) {
      this.$confirm('确定进行发票打印？', this.$t('Common.tips'), {
        confirmButtonText: this.$t('Common.sure'),
        cancelButtonText: this.$t('Common.cancel'),
        type: 'warning',
      }).then((res) => {
    ...
    handleExport() {
      ...
      //查询对象的配置编码
      data.query.push({ column: 'queryDataModuleCode', value: 'invoiceReceivableListExport' })
      this.btnLoading = true
      this.$store
        .dispatch('finance/downloadExcelFile', { data: data })
        .then((res) => {
          this.aTagClick(window.URL.createObjectURL(res), '应收发票列表.xlsx')
        })
        .finally(() => {

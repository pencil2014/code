  ...
  >
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        ...
        <div class="money-box-right">
          ...
          <el-button-group v-if="!isApply && checkAuth($route.name, 'btn-voucherAll')">
            <el-button type="primary" class="finance-btn" size="mini" @click="previewAll">查看全部关联凭证</el-button>
          </el-button-group>
    ...
    <el-form :model="invoiceForm" ref="form" :inline="true" size="mini" label-width="100px">
      <div class="settlement-detail-form">
        ...
        <div>
          ...
          />
          <DetailDiv label="发票凭证号：" isSlot isBold v-if="exVoucherNos">
            <MultiBtn :isEx="true" :btnStr="exVoucherNos||''" @generate="getData" />
          </DetailDiv>
          <div v-if="invoiceRates1 && invoiceRates1.length > 0" class="settlement-exchange-rates">
            ...
            >
              <el-button @click="showMoreRate" type="text" slot="reference">{{$t('Common.more')}}</el-button>
            </div>
          </div>
      ...
      <div class="settlement-detail-form">
        <h3>客户信息</h3>
        <div>
          <DetailDiv :label="$t('FeeList.settleCorp')" :content="invoiceForm.unitName" :isTooltip="true" />
      ...
      <div class="settlement-detail-form">
        <h3>我司信息</h3>
        <div>
          <DetailDiv :label="$t('Settle.branchCompany')" :content="invoiceForm.settleCompName" :isTooltip="true" />
          <DetailDiv
            label="银行简称："
            :content="invoiceForm.saleCompAccountShortName"
            :isTooltip="true"
          />
      ...
      <div class="money-box">
        <div class="money-box-left">
          ...
          <div class="money-total">
            ...
            <span
              v-html="`${option1.data&&option1.data.length ? invoiceForm.invoiceCurrency + ' ' +invoiceForm.amt : ''}`"
            ></span>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="moreRateDialogVisible = false" size="mini">{{$t('Common.cancel')}}</el-button>
      </span>
    </el-dialog>
...
import { invoiceReceivableVoucherList } from '@/api/fin/voucher'
...
export default {
  ...
  data() {
    return {
      ...
      remarksOptions: [
        { label: this.$t('FeeList.orderNo'), value: 'orderNo' },
      ...
      dialogUpdateInfo: {
        ...
        formItems: [
          ...
          { label: this.$t('FeeList.invoiceNo'), type: 'input', prop: 'invoiceNo', rules: { required: true, message: ' ' } },
  ...
  methods: {
    ...
    renderData(res) {
      ...
      this.exVoucherNos = res.data.voucherNo || ''
    ...
    initInvoiceTableOption(index) {
      ...
      option.columns = [
        ...
        { prop: 'count', label: this.$t('FeeList.quantity'), type: 'input', inputType: 'number' },
        { prop: 'price', label: this.$t('FeeList.unitPrice'), type: 'input' },
        // 和后台确定是含税的金额，但预览发票时要显示不含税的金额，要计算
        { prop: 'amt', label: this.$t('FeeList.feeAmt'), type: 'text' },
        { prop: 'amtWithoutTax', label: '金额(不含税)', type: 'text' },
        { prop: 'taxRate', label: this.$t('FeeList.taxRate'), type: 'select', propInDict: 'taxRateInfo' },
        { prop: 'taxAmt', label: this.$t('FeeList.taxAmt'), type: 'text' },
        { prop: 'invoiceNo', label: this.$t('FeeList.invoiceNo'), type: 'text', width: 100 },
        {
          prop: 'fileName',
          label: '开票附件',
          type: 'button',
    ...
    showMoreRate() {
      ...
      let columns = [
        { prop: 'src', label: this.$t('Settle.orignalCurrency'), type: 'text' },
    ...
    previewAll() {
      invoiceReceivableVoucherList({ invoiceReceivableId: this.invoiceReceivableId }).then((res) => {
  ...
  activated() {
    ...
    this.selfColumnsBase = [
      ...
      // { prop: 'orderNo', label: this.$t('PreClose.bizNo'), type: 'text', width: 130 },
      ...
      { prop: 'serviceName', label: this.$t('FeeList.serviceCodes'), type: 'text', width: 100 },
      // { prop: 'feeName', label: this.$t('FeeList.feeEname'), type: 'text', width: 100 },
      ...
      { prop: 'currency', label: this.$t('FeeList.currency'), type: 'text', width: 70 },
      { prop: 'unitPrice', label: this.$t('FeeList.unitPrice'), type: 'text', width: 70 },
      { prop: 'unitCount', label: this.$t('FeeList.quantity'), type: 'text', width: 70 },
      { prop: 'feeAmt', label: this.$t('FeeList.feeAmt'), type: 'text', width: 70 },
      { prop: 'feeUnit', label: this.$t('FeeList.feeUnit'), type: 'select', width: 70 },
      { prop: 'taxRate', label: this.$t('FeeList.taxRate'), type: 'select', propInDict: 'taxRateInfo', width: 70 },
      // { prop: 'feeAmtWithTax', label: '含税费用金额', type: 'text' },
      { prop: 'taxAmt', label: this.$t('FeeList.taxAmt'), type: 'text', width: 70 },
      { prop: 'feeAmtWithoutTax', label: this.$t('FeeList.feeAmtWithoutTax'), type: 'text', width: 70 },
      // { prop: 'writeoffAmt', label: '已核销金额', type: 'text' },
      { prop: 'invoiceAmt', label: this.$t('Collect.invoicedAmt'), type: 'text', width: 70 },
    ...
    let columns2 = [
      ...
      { prop: 'count', label: this.$t('FeeList.quantity'), type: 'input' },
      { prop: 'price', label: this.$t('FeeList.unitPrice'), type: 'input' },
      // 和后台确定是含税的金额，但预览发票时要显示不含税的金额，要计算
      { prop: 'amt', label: this.$t('FeeList.feeAmt'), type: 'text' },
      { prop: 'amtWithoutTax', label: '金额（不含税）', type: 'text' },

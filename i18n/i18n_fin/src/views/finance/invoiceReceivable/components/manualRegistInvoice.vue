      ...
      <div class="money-box money-box-border-bottom">
        ...
        <div class="money-box-right">
          <el-button-group style="margin-right: 4px;">
            <el-button class="finance-btn" @click="submitRegist" type="primary" size="mini">{{$t('Common.submit')}}</el-button>
          </el-button-group>
          ...
          <el-button-group>
            <el-button class="finance-btn" @click="cancel(true)" size="mini">{{$t('Common.cancel')}}</el-button>
          </el-button-group>
        </div>
      </div>
    </div>
    <div class="finance-container-scroll manual-regist-invoice">
      ...
      >
        ...
        <el-form-item label="纳税人识别号" prop="taxNo">
          <el-input v-model="manualRegistInvoiceForm.taxNo" :placeholder="$t('Common.plEnter')" disabled></el-input>
        </el-form-item>
        <el-form-item label="客户发票抬头" prop="title" style="width: 50%;">
          <el-input v-model="manualRegistInvoiceForm.title" :placeholder="$t('Common.plEnter')" disabled></el-input>
        </el-form-item>
        <el-form-item label="开票方式" prop="invoiceWay">
          <el-select v-model="manualRegistInvoiceForm.invoiceWay" :placeholder="$t('Common.plSelect')" disabled>
            <el-option
        ...
        <el-form-item label="发票明细类型" prop="itemGoupType">
          <el-select v-model="manualRegistInvoiceForm.itemGoupType" :placeholder="$t('Common.plSelect')" disabled>
            <el-option
        ...
        <el-form-item label="开票类型" prop="invoiceType">
          <el-select v-model="manualRegistInvoiceForm.invoiceType" :placeholder="$t('Common.plSelect')" disabled>
            <el-option
        ...
        <el-form-item label="开票币种" prop="currency">
          <el-input v-model="manualRegistInvoiceForm.currency" :placeholder="$t('Common.plSelect')" disabled></el-input>
        </el-form-item>
        <el-form-item label="开票金额" prop="amt">
          <el-input v-model="manualRegistInvoiceForm.amt" :placeholder="$t('Common.plEnter')" disabled></el-input>
        </el-form-item>
        <el-form-item label="是否发送邮件" prop="isEmail" :rules="{required:true, message: ' '}">
          <el-select v-model="manualRegistInvoiceForm.isEmail " :placeholder="$t('Common.plSelect')">
            <el-option
        ...
        <el-form-item label="邮箱信息" prop="emailAddrs" style="width: 100%;">
          <el-input type="textarea" v-model="manualRegistInvoiceForm.emailAddrs" :placeholder="$t('Common.plEnter')"></el-input>
        </el-form-item>
      </el-form>
      <div
  ...
  methods: {
    ...
    initInvoiceTableOption(index) {
      ...
      option.columns = [
        ...
        { prop: 'count', label: this.$t('FeeList.quantity'), type: 'text', width: 60 },
        { prop: 'price', label: this.$t('FeeList.unitPrice'), type: 'text', width: 88 },
        // 和后台确定是含税的金额，但预览发票时要显示不含税的金额，要计算
        { prop: 'amt', label: this.$t('FeeList.feeAmt'), type: 'text', width: 88 },
        { prop: 'amtWithoutTax', label: '金额（不含税）', type: 'text', width: 110 },
        { prop: 'taxRate', label: this.$t('FeeList.taxRate'), type: 'select', width: 60, propInDict: 'taxRateInfo', readOnly: true },
        { prop: 'taxAmt', label: this.$t('FeeList.taxAmt'), type: 'text', width: 88 },
        {
          prop: 'invoiceNo',

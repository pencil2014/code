  ...
  >
    <div class="modifyfee-detail-info">
      <el-form ref="form" :inline="true" size="mini">
        ...
        <el-form-item label="结算单号">
          <el-input type="text" :placeholder="$t('Common.plEnter')" v-model="form.settleOrderNo" clearable />
        </el-form-item>
        <el-form-item label="结算单类型">
          <el-select v-model="form.orderType" :placeholder="$t('Common.plSelect')" clearable>
            <el-option
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button
        ...
        size="mini"
      >关联结算单</el-button>
      <el-button @click="closeDialog" size="mini">{{$t('Common.cancel')}}</el-button>
    </span>
  </el-dialog>
</template>
<script>
...
export default {
  ...
  created() {
    ...
    let columns = [
      { prop: 'settleOrderNo', label: this.$t('FeeList.settleOrderNo'), width: 130 },
      { prop: 'settleCorpName', label: this.$t('FeeList.settleCorp'), width: 160 },
      { prop: 'orderType', label: this.$t('Settle.orderType'), type: 'select', propInDict: 'settleOrderType', width: 100 },
      { prop: 'settleCompName', label: this.$t('FeeList.settleComp'), width: 160 },
      { prop: 'receivePayType', label: this.$t('FeeList.receipayType'), type: 'select', propInDict: 'feeitemReceipayGroup', width: 80 },
      // { prop: 'currency', label: '核销币种' },
      ...arr,
      { prop: 'createdName', label: this.$t('Common.createdBy'), width: 100 },
      { prop: 'createdTime', label: this.$t('Common.createTime'), width: 130 },
    ]
  ...
  methods: {
    ...
    getData() {
      ...
      let { settleCorpName, invoiceReceivableId, invoiceReceivableIds } = this.rowData
      this.form.settleCorpName = settleCorpName
      method({ ...this.form, invoiceReceivableId, invoiceReceivableIds }).then((res) => {
    ...
    relate() {
      ...
      relateInvoiceFee({ invoiceReceivableIds: this.rowData.invoiceReceivableIds, settleOrderIds })
        .then((res) => {

        ...
        <div class="settlement-detail-form">
          ...
          <div>
            ...
            <el-form-item label="开票方式：" prop="invoiceWay">
              <el-select v-model="applyForm.invoiceWay" :placeholder="$t('Common.plSelect')">
                <el-option
        ...
        <div class="settlement-detail-form">
          ...
          <div>
            ...
            <el-form-item label="客户发票抬头：" prop="title">
              <el-input disabled type="text" v-model="applyForm.title" :placeholder="$t('Common.plEnter')" clearable></el-input>
            </el-form-item>
            ...
            <el-form-item v-if="!isCannotChange" label="纳税人类型：" prop="taxpayerType">
              <el-select v-model="applyForm.taxpayerType" :placeholder="$t('Common.plSelect')">
                <el-option
...
export default {
  ...
  created() {
    ...
    this.selfColumnsBase = [
      ...
      { prop: 'currency', label: this.$t('FeeList.currency'), type: 'text', width: 55 },
      { prop: 'unitPrice', label: this.$t('FeeList.unitPrice'), type: 'text', width: 55 },
      { prop: 'unitCount', label: this.$t('FeeList.quantity'), type: 'text', width: 55 },
      { prop: 'feeAmt', label: this.$t('FeeList.feeAmt'), type: 'text', width: 70 },
      { prop: 'feeUnit', label: this.$t('FeeList.feeUnit'), type: 'select', width: 80, readOnly: true },
      { prop: 'taxRate', label: this.$t('FeeList.taxRate'), type: 'text', width: 55, propInDict: 'taxRateInfo' },
      { prop: 'taxAmt', label: this.$t('FeeList.taxAmt'), type: 'text', width: 70 },
      { prop: 'feeAmtWithoutTax', label: this.$t('FeeList.feeAmtWithoutTax'), type: 'text', width: 100 },
      { prop: 'invoiceStatus', label: '开票状态', width: 100, type: 'select', propInDict: 'feeInvoiceStatus', readOnly: true },
      { prop: 'invoiceAmt', label: this.$t('Collect.invoicedAmt'), type: 'text', width: 100 },
  ...
  methods: {
    ...
    initInvoiceTableOption(index, canDelete) {
      ...
      option.columns = [
        ...
        { prop: 'price', label: this.$t('FeeList.unitPrice'), type: 'text', width: 90, inputType: 'number', positive: true },
        ...
        { prop: 'taxRate', label: this.$t('FeeList.taxRate'), type: 'select', propInDict: 'taxRateInfo', width: 55, notAllowed: true },
    ...
    validateApply(index) {
      ...
      if (this.applyForm.invoiceCurrency !== 'CNY') {
        if (this.totalInvoiceAmtApplyCNY - invoiceSum !== 0) {
          return this.$msgErrClose('服务项费用和总开票金额不一致！')
        }
      } else {
      ...
      this.$refs.form.validate((res) => {
        ...
        if (!this.dealInvoiceList()) {
          if (this.applyForm.unitName !== this.applyForm.title) {
            this.$confirmWarn('结算单位与客户发票抬头不一致，是否确认此操作？', () => {
              this.sureSave()
            })
          } else {
            this.sureSave()
    ...
    sureSave() {
      ...
      invoiceApplySave(data)
        .then((res) => {
          ...
          this.$msgSucClose(this.CommitStatus === 'no' ? this.$t('FinBill.saveSuc') : '提交成功！')

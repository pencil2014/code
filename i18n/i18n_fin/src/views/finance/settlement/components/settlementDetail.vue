  ...
  >
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">{{isDetailPage ? $t('Settle.statementDet') : $t('Settle.statementGen')}}</div>
        </div>
        <div class="money-box-right">
          ...
          >
            <el-button class="finance-btn" size="mini" @click="checkExchangeRage('lock')">{{$t('Settle.modifySettle')}}</el-button>
          </el-button-group>
          <!-- <el-button-group
          ...
          >
            <el-button
              ...
              type="primary"
              size="mini"
            >{{$t('Settle.assInvoicePay')}}</el-button>
          </el-button-group> -->
          <el-button-group
          ...
          >
            <el-button
              ...
              type="primary"
              size="mini"
            >{{$t('FeeList.relationReg')}}</el-button>
          </el-button-group>
          <!-- <el-button-group
          ...
          >
            <!-- type="warning" -->
            <el-button class="finance-btn" @click="releaseSettleOrder" size="mini">{{$t('Settle.releaseSettleOrder')}}</el-button>
          </el-button-group>
          <el-button-group
            v-if="isDetailPage && !finLockOrUseOrClose&& settlementDetailForm.confirmStatus != 'yes' && checkAuth($route.name, 'btn-delSettle')"
          >
            <el-button type="danger" class="finance-btn" @click="settlementDelete" size="mini">{{$t('Settle.closeSettlement')}}</el-button>
          </el-button-group>
          <el-button-group
          ...
          >
            <el-button
              ...
              size="mini"
              type="purple"
            >{{$t('Settle.withdrawconf')}}</el-button>
          </el-button-group>
          <el-button-group
          ...
          >
            <el-button type="primary" class="finance-btn" @click="handleCommand" size="mini">{{$t('FeeList.exportStatement')}}</el-button>
          </el-button-group>
          <el-button-group v-if="isDetailPage && checkAuth($route.name, 'btn-billList')">
            <el-button class="finance-btn" @click="openBillList" size="mini">{{$t('Settle.statementList')}}</el-button>
          </el-button-group>
          <el-button-group
          ...
          >
            <el-button class="finance-btn" @click="interTransApply" size="mini">{{$t('Settle.internalHedg')}}</el-button>
          </el-button-group>
          <!-- TODO 状态判断 -->
          <!-- !isWriteoff -->
          ...
          <!-- <el-dropdown
            v-if="isDetailPage && (['receive', 'receivepay'].includes(settlementDetailForm.receivePayType) || settlementDetailForm.offsetPayType === $t('FeeList.receivable')) && invoiceApplyAvaliable && checkAuth($route.name, 'btn-invoiceConfirm')"
            class="ls__dropdown"
            trigger="click"
            @command="invoiceConfirm"
          >
            <el-button class="finance-btn" size="mini">
              {{$t('Settle.confirmInvoic')}}
          ...
          <el-button-group v-if="isDetailPage">
            ...
            <!-- <el-button
              v-if="settlementDetailForm.confirmStatus === 'yes' && invoiceApplyAvaliable && (['receive', 'receivepay'].includes(settlementDetailForm.receivePayType) || settlementDetailForm.offsetPayType === $t('FeeList.receivable')) && checkAuth($route.name, 'btn-applyInvoice')"
              class="finance-btn"
              @click="invoiceApply"
              size="mini"
              type="primary"
          ...
          <el-button-group v-if="isDetailPage && checkAuth($route.name, 'btn-uploadOrder')">
            ...
            <el-button
              ...
              @click="uploadOrder"
              size="mini"
            >{{$t('Settle.uploadWater')}}</el-button>
          </el-button-group>
          <el-dropdown
    ...
    <div class="settlement-detail-form settlement-detail-form-own">
      <el-form
        ...
        label-suffix=":"
      >
        <h3 style="padding-bottom: 8px;margin-top: 0px;">{{$t('Settle.settlementInfo')}}</h3>
        <DetailDiv
          v-if="finLockOrUseOrClose || !canEdit"
          :label="$t('FeeList.settleComp')"
          :content="settlementDetailForm.settleCompName"
          :isTooltip="true"
        />
        <el-form-item v-else :label="$t('FeeList.settleComp')" prop="settleCompName">
          <el-select v-model="settlementDetailForm.settleCompObj" :placeholder="$t('Common.plSelect')">
            <el-option v-for="op in settleCompList" :key="op.value" :label="op.label" :value="op"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('FeeList.settleOrderNo')" v-if="isDetailPage" prop="settleOrderNo">
        ...
        <DetailDiv
          ...
          :label__bold="true"
        />
        <el-form-item v-else :label="$t('FeeList.settleCorp')" prop="settleCorpName" class="label__bold">
          <el-select
        ...
        <br />
        <!-- <el-form-item :label="$t('FeeList.settleCorpType')" prop="settleOrderStatus" v-if="isDetailPage">
          <div
        ...
        <DetailDiv
          v-if="finLockOrUseOrClose || !canEdit"
          :label="$t('Settle.settleCycle')"
          :content="dictMapObj['settleCycle'][settlementDetailForm.settleCycle]"
          :isLabelBold="true"
        />
        <el-form-item v-else :label="$t('Settle.settleCycle')" prop="settleCycle" class="label__bold">
          <el-select
        ...
        <el-form-item :label="$t('Settle.orderType')" prop="orderType">
          <div
            class="el-form-item-text"
            v-html="renderPropLabel('settleOrderType', settlementDetailForm.orderType)"
          ></div>
        </el-form-item>
        <el-form-item :label="$t('Settle.statementStatus')" prop="settleOrderStatus" v-if="isDetailPage">
          <div class="el-form-item-text" v-html="settlementDetailForm.settleOrderStatusLabel"></div>
        </el-form-item>
        <el-form-item
          :label="$t('FeeList.lockReason')"
          prop="finLockReason"
        ...
        <el-form-item :label="$t('Settle.statementConf')" prop="confirmStatus" v-if="isDetailPage">
          <div class="el-form-item-text" v-html="settlementDetailForm.confirmStatusLabel"></div>
        </el-form-item>
        <DetailDiv
          :label="$t('Settle.agentCompName')"
          v-if="settlementDetailForm.agentStatus === 'yes'"
          :content="settlementDetailForm.agentCompName"
          :isTooltip="true"
        />
        <DetailDiv
         :label="$t('Settle.agentStatus')"
          v-if="isDetailPage"
          :content="dictMapObj['yesNo'][settlementDetailForm.agentStatus] || $t('Common.no')"
        />
        <el-form-item :label="$t('Common.createdBy')" prop="createdName" v-if="isDetailPage">
          <div class="el-form-item-text" v-html="settlementDetailForm.createdName"></div>
        </el-form-item>
        <el-form-item :label="$t('Common.createTime')" prop="createdTime" v-if="isDetailPage">
        ...
        ></div>
        <el-form-item :label="$t('Settle.settleCurrency')" prop="settleCurrency" class="label__bold">
          <el-select
        ...
        <DetailDiv
          v-if="!isGedgingHide"
          :label="$t('Settle.rolloverResult')"
          :content="settlementDetailForm.offsetPayType ? `<span style='color:${settlementDetailForm.offsetPayType === $t('FeeList.receivable') ? 'rgb(230, 57, 35)' : 'rgb(51, 177, 138)'}'>${settlementDetailForm.offsetPayType}</span>` : '0'"
          :isLabelBold="true"
        />
        <DetailDiv
          v-if="!isGedgingHide"
          :label="$t('Settle.totalrollover')"
        ...
        <div v-if="!isGedgingHide">
          <div class="settle_rate_list" v-if="exchangeRateList.length">
            <span class="settle_rate_list_title">{{$t('Settle.systemrate')}}：</span>
            ...
            <el-button type="text" size="mini" @click="checkExchangeRage('system')">{{$t('Common.more')}}</el-button>
          </div>
          <div class="settle_rate_list" v-if="exchangeRateList.filter(v=>!v.invisible).length">
            <span class="settle_rate_list_title">{{$t('Settle.settleExRate')}}：</span>
            <!-- <span
            ...
            <el-button type="text" size="mini" @click="checkExchangeRage('diy')">{{$t('Common.more')}}</el-button>
          </div>
        </div>
        <template
          v-if="isDetailPage && (['receive', 'receivepay'].includes(settlementDetailForm.receivePayType) || settlementDetailForm.offsetPayType === $t('FeeList.receivable')) && invoiceApplyTableOption.data.length"
        >
          <div class="devide-line"></div>
          <!-- <h3>{{$t('Settle.invoicereq')}}</h3>
          <FinanceTable :option="invoiceApplyTableOption" /> -->
        </template>
        <!-- <template v-if="isDetailPage && invoiceTableOption.data.length">
          <div class="devide-line"></div>
          <h3>{{$t('Settle.relevance')}}</h3>
          <FinanceTable :option="invoiceTableOption" />
        </template> -->
        <div v-if="isForeign && isDetailPage" class="devide-line"></div>
        <h3 v-if="isForeign && isDetailPage">{{$t('Settle.waterbill')}}</h3>
        <FinanceTable v-if="isForeign && isDetailPage" :option="orderTableOption" />
        <h3 v-if="badDebtList.data.length">{{$t('Charge.baddeList')}}</h3>
        <FinanceTable v-if="badDebtList.data.length" :option="badDebtList" />
        <h3 v-if=" gendingApplyList.data.length">{{$t('Hedge.hedgInformation')}}</h3>
        <FinanceTable v-if="gendingApplyList.data.length" :option="gendingApplyList" />
        <h3 v-if="payApplySettleList.data.length">{{$t('Settle.paymentInfo')}}</h3>
        <FinanceTable v-if="payApplySettleList.data.length" :option="payApplySettleList" />
        <h3 v-if="writeoffRecvList.data.length">{{$t('Charge.writeoffInfo')}}</h3>
        <FinanceTable v-if="writeoffRecvList.data.length" :option="writeoffRecvList" />
        <h3 v-if="writeoffPayList.data.length">{{$t('Charge.writeoffInfo')}}</h3>
    ...
    <div class="finance-table-container">
      <div class="money-box">
        <div class="money-box-left" style="line-height:20px;">
          <span class="money-box-title">{{$t('FeeList.expensesList')}}</span>
        ...
        <div class="money-box-right">
          ...
          >
            <el-button class="finance-btn" @click="autoCompulateSettleFee" size="mini">{{$t('Hedge.autoAllocation')}}</el-button>
          </el-button-group>
          <el-button-group
            v-if="(!isDetailPage || !finLockOrInvoiceOrCloseOrWriteOff) && canEdit && checkAuth($route.name, 'btn-addFee')"
          >
            <el-button class="finance-btn" type="primary" @click="openAddFeeDialog" size="mini">{{$t('Settle.addFee')}}</el-button>
          </el-button-group>
          <el-button-group
            v-if="(!isDetailPage || !finLockOrInvoiceOrCloseOrWriteOff) && canEdit && checkAuth($route.name, 'btn-addFee')"
          >
            <el-button class="finance-btn" type="danger" @click="delMulti" size="mini">{{$t('Settle.delFeeBtn')}}</el-button>
          </el-button-group>
        </div>
      </div>
      <DetailFeeSearch
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="agentReceipayDialogVisible = false" size="mini">{{$t('Common.cancel')}}</el-button>
        <el-button type="primary" @click="setAgentCompany" size="mini" :loading="btnLoading">{{$t('Common.submit')}}</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog :title="$t('FeeList.exportStatement')" :visible.sync="billDialog" width="500px">
        ...
        >
          ...
          <el-form-item :label="$t('FeeList.language')" prop="lang">
            <el-select v-model="billDialogForm.lang" :placeholder="$t('Common.plSelect')">
              <!-- <el-option :label="$t('FeeList.chinese')" value="CN"></el-option> 海外版屏蔽中文导出 -->
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="exportBill" size="mini">{{$t('Common.sure')}}</el-button>
        <el-button @click="billDialog = false" size="mini">{{$t('Common.cancel')}}</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    async handleData(res) {
      ...
      this.backupSettleCompObj = {
        settleCompName: data.settleCompName,
        settleCompCode: data.settleCompCode,
      }
      ...
      this.settlementDetailForm.confirmStatusLabel = data.confirmStatus === 'yes' ? this.$t('Common.confirmed') : this.$t('Common.unconfirmed')
      ...
      if (settleOrderFeeList.length > 0 && settleOrderFeeList.every((item) => item.settleCurrency === settleCurrency)) {
        ...
        this.$msgErrClose(this.$t('Settle.anomalyTips'))
      ...
      if (this.isDetailPage) {
        ...
        if (this.isWriteoff) {
          ...
          if (!optionColumns.find((item) => item.prop === 'writeoffAmt')) {
            optionColumns.push({ prop: 'writeoffAmt', label: this.$t('Collect.writtenoffAmount'), type: 'text' })
          }
          if (!optionColumns.find((item) => item.prop === 'noWriteoffAmt')) {
            optionColumns.push({ prop: 'noWriteoffAmt',label: this.$t('Collect.unwrittenAmount'), type: 'text' })
          }
        } else {
          // 已核销完 未核销
      ...
      if (this.adjustRateState && this.option1.columns[length - 1].prop !== 'diffSettleAmt' && !this.canEdit) {
        let arr = [
          { prop: 'newSettleExRate', label: this.$t('Settle.exchangerate'), type: 'text' },
          { prop: 'newExSettleAmt', label: this.$t('Settle.conversion'), type: 'text' },
          { prop: 'diffNoWriteoffAmt', label: this.$t('Settle.remittance'), type: 'text' },
          { prop: 'diffSettleAmt', label: this.$t('Settle.discountDif'), type: 'text' },
        ]

        // { prop: 'exchangeRate', label: this.$t('Settle.settleExRate'), type: 'text' },
        // { prop: 'exchangeRateFee', label: this.$t('Settle.exSettleAmt'), type: 'text' },
    ...
    dealRateList(origin = [], sys = [], useOrigin) {
      ...
      return origin.map((item) => {
        ...
        if (!item) {
          ...
          return this.$msgErrClose(this.$t('Settle.correspond',{ srcCurrency, toCurrency }))
        }
      ...
      let offsetFn = (list, sumStr, str) => {
        list
          ...
          .forEach((item, index) => {
            ...
            this.settlementDetailForm[sumStr] += item.currency + Math.abs(item.sumAmt) + (list.length > 1 ? ', ': ' ') // '&nbsp;&nbsp;'
          })
      }
      if (this.settlementDetailForm.offSetFeeSumStr === ' ') {
        this.settlementDetailForm.offSetFeeSumStr = ''
      }

      if (settleCurrency === 'original') {
        gedgingAmt = 0
        offsetFn(offSetRecFeeSumList, 'offSetFeeSumStr', this.$t('FeeList.receivable') + ' ')
        offsetFn(offSetPayFeeSumList, 'offSetFeeSumStr', this.$t('FeeList.payable') + ' ')
      } else {
    ...
    handleDelete($index, row, option) {
      if (this.option1.data.length <= 1) {
        return this.$msgErrClose(this.$t('Settle.onerecordTips'))
      }
    ...
    delMulti() {
      ...
      if(!rows.length){
        return  this.$msgErrClose(this.$t('Settle.delfeesTips'))
      }
      if (this.option1.data.length - rows.length < 1) {
        return this.$msgErrClose(this.$t('Settle.onerecordTips'))
      }
      this.$confirmWarn(this.$t('Settle.confirmWarn'), () => {
        let list = this.option1.data.filter((v) => !v.$isRowCheck)
        this.option1.data.splice(0, 3000, ...list)
        this.addDelFee()
      })
    },
    ...
    autoCompulateSettleFee() {
      ...
      autoCompulateSettleFee(data).then((res) => {
        this.$msgSucClose(this.$t('Settle.allocationSuc'))
    ...
    compConfirmValidate() {
      return new Promise((resolve, reject) => {
        let feeListCompList = this.option1.data.map(item => item.settleCompCode)
        feeListCompList = [...new Set(feeListCompList)]
        let list = [this.settlementDetailForm.settleCompObj.settleCompCode]
        feeListCompList = feeListCompList.concat(list)
        let data = {
          allCompanyCodeList: feeListCompList
        }
        confirmValidate(data).then(res => {
          if (res.code === 159024) {
  ...
  watch: {
    ...
    'option1.data': {
      ...
      handler(newVal, oldVal) {
        ...
        let backupSettleCompObj = this.backupSettleCompObj || {}
        let obj1 = {
          [backupSettleCompObj.settleCompCode]: {
            ...backupSettleCompObj,
            label: backupSettleCompObj.settleCompName,
            value: backupSettleCompObj.settleCompCode,
          },
        }
        let list1 = obj1[backupSettleCompObj.settleCompCode] ? [obj1[backupSettleCompObj.settleCompCode]] : []
        newVal.forEach((item) => {
          if (!obj1[item.settleCompCode]) {
            obj1[item.settleCompCode] = true
            list1.push({ ...item, label: item.settleCompName, value: item.settleCompCode })
          }
        })

        // } else if (arrSet[0] !== this.$store.state.user.userInfo.settleCompanyCode
        let userInfo = this.$store.state.user.userInfo
        if (!list1.find((v) => v.value == userInfo.settleCompanyCode)) {
          let settleCompName = userInfo.settleCompanyName
          let settleCompCode = userInfo.settleCompanyCode
          list1.unshift({ label: settleCompName, value: settleCompCode, settleCompCode, settleCompName })
        }
        this.settleCompList.splice(0, 1000, ...list1)
        this.settlementDetailForm.settleCompObj =
          this.settleCompList.find(({ settleCompCode }) => settleCompCode == this.settlementDetailForm.settleCompCode) || {}
        console.log(this.settleCompList)

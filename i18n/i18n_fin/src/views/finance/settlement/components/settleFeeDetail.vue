...
export default {
  ...
  data() {
    return {
      title: this.$route.name === 'ReceiLossSettleFeeDetail' ? this.$t('PreClose.breakdown') : this.$t('PreClose.hedgingcosts'),
      warnMessage:
        this.$route.name === 'ReceiLossSettleFeeDetail'
          ? this.$t('PreClose.reducedTips')
          : this.$t('PreClose.transaction'),
      keyDownActive: true,
  ...
  created() {
    ...
    let columns = [
      ...
      { prop: 'receipayType', label: this.$t('Settle.receipayType'), width: 80, type: 'select', st: 3 },
      { prop: 'settleCompName', label: this.$t('FeeList.settleComp'), minWidth: 200, type: 'text', st: 2 },
      { prop: 'settleCorpName', label: this.$t('FeeList.settleCorp'), minWidth: 200, type: 'text', st: 2 },
      { prop: 'feeEnName', label: this.$t('FeeList.feeEname'), width: 150, type: 'text', st: 2 },
      { prop: 'currency', label: this.$t('FeeList.currency'), type: 'text', width: 80, st: 3 },
      { prop: 'feeAmt', label: this.$t('FeeList.feeAmt'), type: 'text', width: 100 },
    ]
    if (this.$route.name !== 'ReceiLossSettleFeeDetail') {
      columns.push({ prop: 'finDate', label: this.$t('FeeList.finDate'), type: 'text', width: 130, st: 4 })
    }

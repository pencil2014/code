...
<div class="finance-bill-detail finance-page"
  ...
  <el-alert v-if="billChange" type="warning" :closable="false" class="orange">
    <template slot="title">
      <i class="tag-change">{{$t('OrderFee.modifySimple')}}</i>
      <span class="txt">
  ...
  <div class="finance-bill-detail-selectshow">
    <el-form ref="accountForm" :inline="true" size="mini">
      <el-form-item :label="$t('BillList.displayField')">
        <el-select v-model="showOptions"
          ...
          <el-option :label="$t('OrderFee.settleCompName')" value="settleCompNameShow"></el-option><!-- 所属公司 -->
          <el-option :label="$t('OrderFee.salesperson')" value="bdEmployeeNameShow"></el-option><!-- 业务员 -->
          <el-option :label="$t('FeeList.settleCorp')" value="settleCorpNameShow"></el-option>
          <el-option :label="$t('OrderFee.orderNo')" value="sourceBizNoShow"></el-option>
          ...
          <el-option :label="$lang==='en'?'Customer No':'客户委托号'" value="customerNoShow"></el-option><!-- 客户委托号 -->
          <el-option :label="$t('BillList.cn')" value="containerNoShow"></el-option>
          ...
          <el-option :label="$lang==='en'?'Route':'航线'" value="sysLienNameShow"></el-option><!-- 'Line' -->
          <el-option :label="$lang==='en'?'Vessel Voyage':'船名航次'" value="vesselVoyageShow"></el-option>
          <el-option :label="$lang==='en'?'Load':'起运港'" value="polEnameShow"></el-option>
          <el-option :label="$lang==='en'?'Dest':'目的港'" value="podEnameShow"></el-option>
          <el-option :label="$lang==='en'?'CNT NO.TYPE':'箱型箱量'" value="boxSizeShow"></el-option>
          <el-option :label="$lang==='en'?'ETD':'预计开航日期(ETD)'" value="etdShow"></el-option>
          <el-option :label="$lang==='en'?'ATD':'实际开航日期(ATD)'" value="atdShow"></el-option>
          <el-option :label="$lang==='en'?'ETA':'预计到港日期'" value="etaShow"></el-option>
          <el-option :label="$lang==='en'?'ATA':'实际到港日期'" value="ataShow"></el-option>
          ...
          <el-option :label="$lang==='en'?'Receipt Status':'签收状态'" value="signStatusShow"></el-option>
          <el-option :label="$lang==='en'?'Cargo Receipt Date':'签收时间'" value="signDateShow"></el-option>
      ...
      <el-form-item :label="$t('BillList.bankAccount')" prop="settleUnitBankAcctIds">
    ...
    <span class="finance-bill-switch-lang" v-if="$lang==='zh'&&!isDetailPage" @click="switchLang">{{lang === 'en' ? 'Switch Chinese' : '切换英文'}}</span>
    ...
    <div class="finance-bill-detail-info" style="position:relative;">
      <div class="detail-info-other">
        ...
        <div v-if="billType === 'sum'" class="finance-bill-detail-modeltype" style="position:absolute;right:20px;top:0;">
          <el-tooltip :content="$t('BillList.changeFeeModeTip')" placement="top">
            <el-button size="mini" @click="switchModelType">{{$t('BillList.changeFeeMode')}}</el-button>
          </el-tooltip>
        </div>
      </div>
    </div>
      ...
      <div>
        <el-form :model="form" :rules="rules" ref="form" label-width="80px" size="mini">
          <el-form-item :label="$t('BillList.mailTo')" prop="receive">
            <el-input v-model="form.receive" :placeholder="$t('OrderFee.plsInput')" show-word-limit maxlength="500">
            </el-input>
          </el-form-item>
          <el-form-item :label="$t('BillList.mailCc')" prop="copyto">
            <el-input v-model="form.copyto" :placeholder="$t('OrderFee.plsInput')" show-word-limit maxlength="500">
            </el-input>
          </el-form-item>
          <el-form-item :label="$t('BillList.billDocType')" prop="docType">
            <el-select :placeholder="$t('OrderFee.plsSelect')" v-model="form.docType" @change="changeDocType">
  ...
  <div class="finance-bill-detail-btns">
    <el-button type="primary" class="finance-btn" @click="confirmSettlementBill" size="mini">{{$t('OrderFee.confirm')}}</el-button>
    <el-button type="primary" class="finance-btn" @click="openBillEmailDialog" size="mini">{{$t('BillList.sendMail')}}</el-button>
    <el-button class="finance-btn" v-print="printObj" size="mini">{{$t('BillList.print')}}</el-button>
    ...
    <el-button class="finance-btn" @click="cancel" size="mini">{{$t('OrderFee.back')}}</el-button>
  </div>
</div>
</template>
<script>
    ...
    return {
      ...
      billI18n: {
        ...
        en: {
          ...
          SO: 'SO NO',
          vesselVoyage: 'Route', // 'Line', // 'Vessel/Flight',
  ...
  methods: {
    ...
    confirmSettlementBill() {
      ...
      confirm(data).then(res => {
        if (res.code === 0) {
          ...
          this.$message({
            type: 'success',
            message: this.$t('OrderFee.successOp'), // '结算对账单确认成功！',
            showClose: true
          })
        }
      })
    ...
    handleExport(command) {
      ...
      this.$store.dispatch('finance/downloadBlobFile', {
        ...
        link.download = this.$t('BillList.monthlyBill') + this.$t('OrderFee.enspace') + this.billDetailParams.settleOrderNo + '.' + command.toLowerCase() // '对账单'
        link.click()
      }).catch(err => {
        console.log(err)
      })
    ...
    openBillEmailDialog() {
      ...
      this.billFile = {
        name: this.$t('BillList.monthlyBill') + this.$t('OrderFee.enspace') + this.billDetailParams.settleOrderNo + '.' + command.toLowerCase(), // '对账单'
        callback
      }
    ...
    getColumns() {
      let labelObj = {
        ...
        sysLienNameShow: {en: 'Route', zh: '航线'}, // en: 'Line'
      ...
      // currencyTitle 有值表示使用币种和金额显示在一起的表头，值为币种的数组
      if (this.currencyTitle && this.currencyTitle.length) {
        this.currencyTitle.forEach(currency => {
          if (!this.receiveNegativeCurrencySet.includes(currency)) {
            let reciAmtProp = 'reciAmt' + currency
        ...
        let objList = [
          {prop: 'currency', en: 'Currency', zh: '币种'}, // en: 'Curr'

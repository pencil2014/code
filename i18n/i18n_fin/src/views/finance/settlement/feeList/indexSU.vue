  ...
  >
      ...
      <div class="finance-list-container">
        <div class="money-box">
          <div class="money-box-left">
            ...
            <!-- <el-checkbox
              v-if="(checkAuth($route.name, 'btn-createSettleMulti') || checkAuth($route.name, 'btn-createSettle')) && activeType === 'fee'"
              v-model="isSelectAll"
              :indeterminate="false"
              :disabled="!hasSearch"
            >全部查询结果</el-checkbox> -->
            ...
            <el-button-group>
              <el-button v-if="checkAuth($route.name, 'btn-export')" class="finance-btn" @click="handleCommand('settleUnit')" size="mini">{{$t('FeeList.export')}}</el-button>
              <!-- <div
            ...
            <!-- <el-button-group>
							<el-button class="finance-btn" @click="exportBtn" size="mini" v-if="checkAuth($route.name, 'btn-export')"
								>导出</el-button
							>
            </el-button-group>-->
          </div>
    ...
    <keep-alive v-if="relationRegDialogVisible.show">
      <RelationReg
        ...
        :settleOrderIds="multipleSelection1.map(v => v.feeIds).join(',').split(',').filter(v => v)"
...
import { setAgentCompany, cancelAgentCompany, surplusStatementExport, feeAgentValid } from '@/api/fin/fee'
import { groupBySettleCorpListPage, groupBySettleCorpExport, confirmFee, getListAmtSummary, groupSettleOrderApply, listReceivableRegForWriteoff } from '@/api/fin/fee'
...
export default {
  ...
  data() {
    return {
      ...
      pageSizeUrl: groupBySettleCorpListPage('geturl'),
      ...
      typesList: [
        {
          label: this.$t('FeeList.byFee'),
          name: 'fee'
        },
        {
          label: this.$t('FeeList.bySettleUnit'),
          name: 'settleUnit'
        },
      ],
    }
  ...
  created() {
    ...
    let filterKeys = [
      'allNo', 'settleCompCodes', 'settleCorpCodes', 'sourceBizNos', 'receipayType', 'finDate', 'bizDate', 'serviceType', 'businessTypes',
      'bdEmployeeId', 'opEmployeeId', 'bdDeptCode', 'pricingEmployeeId', 'shipCarrierCodes', 'writeoffStatuss', 'podPortCode', 'polPortCode', 'currency', 'receipayType', 'custIntrustNo', 'whPoNo'
      // 'invoiceNo','invoiceStatuss', 
    ] // 'settleOrderStatus'-20220803 zhangling delete
    let filterGroups = {}
    filterKeys.forEach(key => {
      let obj = this.filterGroups[key]
      filterGroups[key] = obj
    })
    this.searchOption1 = {
      saveShow: true,
      saveName: 'feeListBySettleUnit',
      contentShow: true,
      isNotAutoLoad: true,
  ...
  methods: {
    ...
    getData(isSearch, customerList) {
      ...
      return groupBySettleCorpListPage(data)
        .then((res) => {
          this.loading = false
          if (res.code === 0) {
            let { configColumns, list } = res.data
            configColumns = configColumns.filter(item => item !== 'currency' && item !== 'receipayType')
            ...
            list.forEach((item) => {
              ...
              item.receiveInvocieNos_multiValue = item.receiveInvocieNos ? item.receiveInvocieNos.split(',').map((subItem) => {
                let arr = subItem.split('/')
                return arr[1]
              }) : []
              item.receiveInvocieIds_multiValue = item.receiveInvocieNos ? item.receiveInvocieNos.split(',').map((subItem) => {
                let arr = subItem.split('/')
                return arr[0]
              }) : []
              item.payInvocieNos_multiValue = item.payInvocieNos ? item.payInvocieNos.split(',').map((subItem) => {
                let arr = subItem.split('/')
                return arr[1]
              }) : []
              item.payInvocieIds_multiValue = item.payInvocieNos ? item.payInvocieNos.split(',').map((subItem) => {
                let arr = subItem.split('/')
                return arr[0]
              }) : []
            })
    ...
    createSettlement(type) {
      ...
      if (!allResult) {
        ...
        //   }
        // }
        data.feeIds = this.multipleSelection1.map((v) => v.feeIds).join(',')
      } else {
        data.query = this.searchBackup
      }
      data.feeSourceType = 'group'
      this.loading = true
    ...
    openAgentReceipayDialog() {
      ...
      let data = {
        ...
        orderFeeIds: this.multipleSelection1.map((item) => item.orderFeeIds).join(',').split(',').filter(v => v),
      }
    ...
    handleCommand(command) {
      if (!this.option1.data.length) {
        return this.$message({ message: this.$t('FeeList.exportTips'), type: 'error', showClose: true })
      }
      let { prop, order } = this.sortParams
      if (prop === 'serviceCode') prop = 'serviceName'
      let data = {
      ...
      if (this.multipleSelection1.length) {
        data.query = [{ column: 'feeIds', type: 'in', value: this.multipleSelection1.map((v) => v.feeIds).join(',') }]
      }
      if (command === 'settleUnit') {
        groupBySettleCorpExport
        this.$store.dispatch('finance/downloadExcelFile', { data: data, uri: '/fin/fee/groupBySettleCorpExport' }).then((res) => {
          this.aTagClick(window.URL.createObjectURL(res), `${this.$t('FeeList.expensesList')}.xlsx`)
        })
      } else if (command === 'feeDetail') {
        //查询对象的配置编码
    ...
    sendMulti(data, option) {
      ...
      let ids = this.multipleSelection1.map((item) => item.feeIds).join(',')
      if (ids) {
    ...
    confirmFee() {
      ...
      this.$confirm(this.$t('FeeList.confirmSelectedTips'), this.$t('FeeList.tips'), {
        ...
        .then(() => {
          let feeIds = this.multipleSelection1.map((item) => item.feeIds).join(',').split(',').filter(v => v)
    ...
    regWriteoff() {
      ...
      let feeIdsA = rows.map((v) => v.feeIds.split(','))
      let feeIds = feeIdsA.reduce((prev, curr) => {
        return prev.concat(curr)
      }, [])
      let data = { feeIds: rows.map(v => v.feeIds).join(',').split(',').filter(v => v)}
      toQuickRecvWriteoff(data)
        .then((res) => {
    ...
    relationRegDialog() {
      ...
      listReceivableRegForWriteoff({
        feeIds: this.multipleSelection1.map((v) => v.feeIds).join(',').split(',').filter(v => v),
      })
        .then((res) => {
          if(!res.data?.length){
  ...
  computed: {
    ...mapState({
      defaultColumns: (state) => state.finance.columns.defaultColumnsFeeListSU,

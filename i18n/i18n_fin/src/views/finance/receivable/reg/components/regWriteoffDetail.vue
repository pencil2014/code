    ...
    <div class="finance-list-container" @click="saveDzg">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">{{$t('Collect.receiptReg')}}</div>
        </div>
        <div class="money-box-right">
          ...
          <el-button
            ...
            size="mini"
            type="primary"
          >{{$t('Collect.associatedcosts')}}</el-button>
      ...
      <div class="settlement-detail-form">
        <el-form
          ...
          label-suffix=":"
        >
          <h3>{{$t('Collect.receiptWriteoff')}}</h3>
          <el-form-item :label="$t('Collect.recvWay')" prop="recvWay">
            <el-select v-model="regInfoForm.recvWay" clearable :placeholder="$t('Common.plSelect')">
              <el-option
          ...
          <el-form-item :label="$t('Collect.abbreviat')" prop="compBankAccountId">
            <el-select
          ...
          <el-form-item :label="$t('Settle.settleMode')" prop="settleMode">
            <el-select v-model="regInfoForm.settleMode" clearable :placeholder="$t('Common.plSelect')">
              <el-option
          ...
          <el-form-item :label="$t('Settle.branchCompany')">
            <el-input disabled type="text" v-model="recvCompName"></el-input>
          </el-form-item>
          <el-form-item :label="$t('FeeList.settleCorp')">
          ...
          >
            <el-input v-model="regInfoForm.fixedTransferFee" disabled ></el-input>
          </el-form-item>
          <el-form-item :label="$t('Collect.bankFlowNumber')" prop="bankSeqNo" v-show="!recvNeedBankseqBoolean">
            <el-input type="text" v-model="regInfoForm.bankSeqNo" :placeholder="$t('Common.plEnter')"></el-input>
          </el-form-item>
          <el-form-item
          ...
          <el-form-item :label="$t('Collect.interTransferFee')" prop="innerTransferFee">
            <el-input
              type="text"
          ...
          <el-form-item :label="$t('Collect.externalFee')" prop="outerTransferFee">
            <el-input
              type="text"
          ...
          <el-form-item :label="$t('PreClose.residual')" prop="balanceDealType">
          ...
          <el-form-item :label="$t('Collect.transRemarks')" class="full_input" prop="recvRemark">
            <el-input
              type="textarea"
          ...
          <el-form-item :label="$t('Settle.accountName')" prop="payAccountName">
            <el-autocomplete
              size="mini"
              style="width: 100%"
          ...
          <el-form-item :label="$t('Settle.paymentBank')" prop="payBankName">
          ...
          <el-form-item :label="$t('Settle.accountType')" prop="payAccountPublicType">
          ...
          <el-form-item :label="$t('Settle.accountNo')" prop="payAccountNo">
            <el-input
              :disabled="!!selectedOptionItem1"
              type="text"
          ...
          <el-form-item label="SWIFT CODE" prop="payAccountSwiftCode">
            <el-input type="text" v-model="regInfoForm.payAccountSwiftCode" :placeholder="$t('Common.plEnter')"></el-input>
          </el-form-item>
          <el-form-item :label="$t('Collect.uploadSheets')">
            <FinUploadEmbed ref="FinUploadEmbed2" @send-file-list="sendFileList2" />
          </el-form-item>
          <el-form-item :label="$t('Settle.recvRegAddr')" class="full_input" prop="payAccountBankCompAddr">
            <el-input
              type="textarea"
...
export default {
  ...
  created() {
    ...
    this.autocomplete2 = {
      querySearch: (queryString, cb) => {
        let keyName = this.$lang === 'zh' ? 'cname' : 'ename'
        let data = {
          currPage: 1,
          pageSize: 30,
          query: [{ column: keyName, type: 'like', value: queryString }],
        }
        bankList(data).then((res) => {
          const list = res.data.list ? res.data.list : []
          cb(list.map((item) => ({ key: item.bankCode, value: item[keyName] })))
        })
      },
      select: (optionItem, row) => {
    ...
    init() {
      ...
      if (!this.source) {
        ...
        this.selfColumnsBase = [
          ...
          { prop: 'settleOrderNo', label: this.$t('FeeList.settleOrderNo'), type: 'text', key: 'settleOrderNo' },
    ...
    writeoffAction() {
      this.$refs.regInfoForm.validate((valid) => {
        ...
        if (this.recvCompCode !== this.$store.state.user.userInfo.settleCompanyCode && this.hasIntrabranch != 'yes') {
          str = (str ? str + this.$t('Common.and') : '') + this.$t('Collect.interBranch') + ','
        }
  ...
  components: { FinanceTable, CustomColumns, FinanceSearch,  TransferFee, FinUploadEmbed },
  computed: {

...
export default {
  ...
  created() {
    ...
    this.searchOption1 = {
      ...
      addFilter: {
        ...
        filterGroups: {
          sourceType: { label: '费用来源系统', type: 'select', placeholder:  this.$t('Common.plSelect'), showType: '1', propInDict: 'usaFeeCompareSourceType' },
          sourceSystem: { label: '委托号所在系统', type: 'select', placeholder:  this.$t('Common.plSelect'), showType: '1', propInDict: 'usaFeeCompareSourceSystem' },
          matchResult: { label: '是否匹配', type: 'select', placeholder:  this.$t('Common.plSelect'), showType: '1', selectOptions: [
            { label: this.$t('Common.yes'), value: 'yes' },
            { label: this.$t('Common.no'), value: 'no' }
          ] },
          custIntrustNo: { label: '客户委托号', type: 'input', placeholder:  this.$t('Common.plEnter'), showType: '2' },
          usaSettleCorpName: { label: '美国系统结算单位', type: 'input', placeholder:  this.$t('Common.plEnter'), showType: '3' },
          usaEtd: { label: '美国系统ETD', type: 'daterange', placeholder:  this.$t('Common.plSelect'), showType: '3' },
          usaInvoiceCreateDate: { label: '美国系统账单创建时间', type: 'daterange', placeholder:  this.$t('Common.plSelect'), showType: '3' },
          orderNo: { label: '订单号', type: 'input', placeholder:  this.$t('Common.plEnter'), showType: '4' },
          settleCorpCode: { label: this.$t('FeeList.settleCorp'), prop: 'settleCorpCode', placeholder:  this.$t('Common.plSelect'), type: 'remoteSelect', multiple :true, showType: '5',
          ...
          bizDate: { label: '业务时间', type: 'daterange', placeholder:  this.$t('Common.plSelect'), showType: '6' },
          // createdName: { label: this.$t('Common.createdBy'), prop: 'employeeId', type: 'remoteSelect', showType: '7',
          //   remoteMethod: this.getEmployeeList,
          //   remoteSelectList: this.employeeOptions,
          // },
          createdTime: { label: this.$t('Common.createTime'), type: 'daterange', placeholder:  this.$t('Common.plSelect'), showType: '8' },
        }
      }
    }
    ...
    let columns = [
      ...
      { prop: 'difAmt', label: this.$t('Supplier.balance'), type: 'text', fixed: 'left' },
      ...
      { prop: 'receipayType', label: this.$t('FeeList.receipayType'), type: 'select' },
      { prop: 'currency', label: this.$t('FeeList.currency'), type: 'text' },
      { prop: 'feeAmt', label: this.$t('FeeList.feeAmt'), type: 'text' },
      // { prop: 'createdName', label: this.$t('Common.createdBy'), type: 'text' }, // 都为系统，不展示
      { prop: 'createdTime', label: this.$t('Common.createTime'), type: 'text', minWidth: 110 },
  ...
  methods: {
    ...
    prevHandle(type) {
      ...
      this.$confirm(`是否确认${text}？`, this.$t('Common.tips'), {

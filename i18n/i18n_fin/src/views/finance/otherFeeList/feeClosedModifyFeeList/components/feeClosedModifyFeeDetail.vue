<template>
  <div class="finance-page">
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">{{$t('OrderFee.adjustmentDetail')}}</div>
        </div>
        <div class="money-box-right">
          ...
          <!-- <el-button
            ...
            type="purple"
          >撤回申请</el-button> -->
          <el-button @click="closeDetail" size="mini">{{$t('OrderFee.back')}}</el-button>
        </div>
      </div>
    </div>
    <div class="finance-approve-detail-main">
      <div class="finance-detail-info">
        <div class="detail-info-other">
          <span class="detail-info-item" @click="showOneNoDetail(modifyFeeData.sourceType, modifyFeeData.sourceBizNo)">{{$t('OrderFee.orderNo')}}：<span style="text-decoration:underline;cursor:pointer;color:#1890ff;">{{modifyFeeData.sourceBizNo}}</span></span>
          ...
          >{{$t('OrderFee.adjustReason')}}：{{renderPropLabel('feeAdjustReason', modifyFeeData.reason)}}</span>
          <span
            v-if="isAudited"
            class="detail-info-item"
          >{{$t('OrderFee.auditStatus')}}：{{renderPropLabel('auditStatus', modifyFeeData.bizAuditStatus)}}</span>
          <span
          ...
          <!-- 审核人始终显示 -->
          <span class="detail-info-item">{{$t('OrderFee.currAuditor')}}：{{modifyFeeData.bizAuditName}}</span><!-- 当前审核人 -->
          <span v-if="modifyFeeData.bizAuditTime" class="detail-info-item">{{$t('OrderFee.auditTime')}}：{{modifyFeeData.bizAuditTime}}</span>
          ...
          <div v-if="!(modifyFeeData.adjustType==='negative_recv_fee' && !modifyFeeData.remark)" class="detail-info-item" style="width:100%;margin-right:0;">
            <span class="detail-info-item-label">{{$t('OrderFee.adjustRemark')}}：</span>
            <span class="detail-info-item-content">{{modifyFeeData.remark}}</span>
          </div>
          <div class="detail-info-item" style="width:100%;margin-right:0;">
            <span class="detail-info-item-label">{{$t('OrderFee.adjustContent')}}：</span>
      ...
      <div class="finance-table-container" v-if="option2.data.length > 0">
        <div class="money-box">
          <div class="money-box-left">
            ...
            >
              {{item.currency}}{{$t('OrderFee.total')}}：
              <span
      ...
      <div class="finance-table-container">
        <div class="money-box">
          <div class="money-box-left">
            <div class="money-title">{{$t('OrderFee.auditLog')}}</div>
          </div>
        </div>
        <FinanceTable :option="option5" />
      </div>
    </div>
    <el-dialog :title="$t('OrderFee.auditRefuse')" :visible.sync="rejectDialogVisible" width="600px"><!-- 拒绝费用调整 -->
    ...
    <el-dialog :title="$t('OrderFee.edit')+$t('OrderFee.enspace')+$t('OrderFee.auditor')" :visible.sync="adjApproverVisible" width="600px" custom-class="fin-adjapprover-dial"><!-- 修改调费申请审批人 -->
      <el-form :model="adjApproverForm" ref="adjApproverForm" label-width="100px">
        <el-form-item
          :label="$t('OrderFee.newAuditor')"
          prop="employeeId"
          :rules="{required: true, message: ' ', trigger: 'change'}">
          <el-select v-model="adjApproverForm.employeeId" :placeholder="$t('OrderFee.plsSelect')"
            filterable
            remote
            clearable
        ...
        <el-form-item
          :label="$t('OrderFee.auditorRole')"
          prop="approverRoleDesc"
          :rules="{required: true, message: ' ', trigger: 'blur'}"
        >
          <el-input v-model="adjApproverForm.approverRoleDesc" :placeholder="$t('OrderFee.plsInput')" maxlength="500"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="adjApproverSubmit" size="mini">{{$t('OrderFee.confirm')}}</el-button>
        <el-button @click="adjApproverVisible = false" size="mini">{{$t('OrderFee.cancel')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      option2: {},
      option3: {},
      moneyTitle1: this.$t('OrderFee.feeAdd'),
      moneyTitle2: this.$t('OrderFee.feeDelete'),
      moneyTitle3: this.$t('OrderFee.feeModification'),
      moneyTotalReceive: [],
      comeType: 'edit',
      hedgingRecord: [],
      deleteSelection: [],
  ...
  created() {
    ...
    let arr2 = [
      ...
      { prop: 'modifyStatus', type: 'text', label: this.$t('OrderFee.modifyStatus'), width: 120 },
    ]
    columns.forEach((value) => {
      let obj = this.columnsBaseLocal.find((item) => {
        if (['settleCorpName', 'settleCompName'].includes(item.prop)) {
    ...
    let operationBtns = Object.assign(this.option.operationBtns, {
      data: [
        {
          label: this.$t('OrderFee.edit') + this.$t('OrderFee.enspace') + this.$t('OrderFee.auditor'), // '修改审批人',
          type: 'text',
          showFn: () => this.checkAuth(this.$route.name, 'btn-editAdjApprover'),
          action: 'Edit',
        },
        {
          label: this.$t('OrderFee.del') + this.$t('OrderFee.enspace') + this.$t('OrderFee.auditor'), // '删除审批人',
          type: 'text',
  ...
  methods: {
    ...
    preSubmit(type) {
      ...
      if (!this.option5.data.some(item => {
        ...
        return this.$message({
          type: 'error',
          message: this.$t('OrderFee.cannotApprove'), // `您不在当前审批人列表当中，不能进行审批`,
          showClose: true,
        })
      }
    ...
    handleDelete($index, row, option) {
      this.$confirm(this.$t('OrderFee.confirmTip') + this.$t('OrderFee.delete') + '?', this.$t('OrderFee.tip'), {
        confirmButtonText: this.$t('OrderFee.confirm'),
        cancelButtonText: this.$t('OrderFee.cancel'),
        type: 'warning',
      })
  ...
  computed: {
    finCloseStatus() {
      ...
      return (
        `${this.$t('OrderFee.closeStatus')}：` + // 总关账状态
        (cellValue === 'yes'
          ? `<span style="color:#CD4130;">${this.$t('OrderFee.yes')}</span>`
          : cellValue === 'no'
          ? `<span style="color:#33B18A;">${this.$t('OrderFee.no')}</span>`
          : cellValue)
      )
    },

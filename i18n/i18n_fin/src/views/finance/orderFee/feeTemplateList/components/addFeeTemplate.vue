  ...
  >
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">{{isDetail ? $t('OrderFee.feeTempDetail') : $t('OrderFee.addFeeTemp')}}</div>
        </div>
        <div class="money-box-right">
          <el-button-group>
            <el-button class="finance-btn" size="mini" type="primary" @click="handleSave">{{$t('Common.save')}}</el-button>
          </el-button-group>
          <el-button-group>
            <el-button class="finance-btn" size="mini" @click="cancel(false)">{{$t('Common.cancel')}}</el-button>
          </el-button-group>
        </div>
      </div>
    </div>
    <div class="settlement-detail-form">
      <el-form :model="formData" ref="form" :rules="rules" :inline="true" size="mini" label-width="150px">
        <el-form-item :label="$t('OrderFee.tempName')+': '" prop="tempName">
          <el-input v-model="formData.tempName" :placeholder="$t('OrderFee.plsInput')"></el-input>
        </el-form-item>
        <el-form-item :label="$t('OrderFee.businessType')+': '" prop="businessType">
          <el-select v-model="formData.businessType" value-key :placeholder="$t('OrderFee.plsSelect')" @change="queryBusinessServiceList">
            <el-option
        ...
        <el-form-item :label="$t('OrderFee.isEnabled')+': '" prop="enabled">
          <el-select v-model="formData.enabled" value-key :placeholder="$t('OrderFee.plsSelect')">
            <el-option
        ...
        <el-form-item :label="$t('OrderFee.sortNum')+': '" prop="sortNum">
          <el-input v-model="formData.sortNum" :placeholder="$t('OrderFee.sortNumTip')"></el-input>
        </el-form-item>
        <el-form-item :label="$t('OrderFee.tempLevel')+': '" prop="tempLevel">
          <el-select v-model="formData.tempLevel" value-key :placeholder="$t('OrderFee.plsSelect')">
            <el-option
        ...
        <el-form-item :label="$t('OrderFee.remark')+': '" prop="remark" class="long-form-item">
          <el-input
            maxlength="500"
            show-word-limit
            type="textarea"
    ...
    <el-tabs v-model="activeName" class="fin-el-tabs">
      <el-tab-pane v-for="(item, index) in tabsList" :key="'tabsList' + index" :label="item.label" :name="item.name" class="fin-el-tab-pane">
        <template>
          <div v-show="item.name === 'biz' && activeName === 'biz'">
            <div class="finance-table-container">
               <!-- v-if="!isSpecialTemp" -->
              <div class="money-box">
                <div class="money-box-left">
                  <div class="money-title">{{$t('OrderFee.receive')}}</div><!-- 应收 -->
                </div>
                <div class="money-box-right">
                  <el-button class="finance-btn" type="primary" size="mini" @click="addRow('option1')">{{$t('OrderFee.newRecv')}}</el-button>
                  <el-button class="finance-btn" size="mini" @click="handleCopy('receive')">{{$t('Common.copy')}}</el-button>
                  <el-button class="finance-btn" size="mini" @click="addRow('option2', true)">{{$t('OrderFee.addOpposite1')}}</el-button>
                  <el-button class="finance-btn" size="mini" type="danger" @click="handleDelete('receive')">{{$t('Common.del')}}</el-button>
                  <el-button v-if="isDetail" class="finance-btn" type="warning" size="mini" @click="handleEdit('receive')">{{$t('Common.modify')}}</el-button>
            ...
            <div class="finance-table-container">
              <div class="money-box">
                ...
                <div class="money-box-right">
                  <el-button class="finance-btn" type="primary" size="mini" @click="addRow('option2')">{{$t('OrderFee.newPay')}}</el-button>
                  <el-button class="finance-btn" size="mini" @click="handleCopy('pay')">{{$t('Common.copy')}}</el-button>
                  <el-button class="finance-btn" size="mini" @click="addRow('option1', true)">{{$t('OrderFee.addOpposite2')}}</el-button>
                  <el-button class="finance-btn" type="danger" size="mini" @click="handleDelete('pay')">{{$t('Common.del')}}</el-button>
                  <el-button v-if="isDetail" class="finance-btn" type="warning" size="mini" @click="handleEdit('pay')">{{$t('Common.modify')}}</el-button>
          ...
          <div v-show="item.name === 'special' && activeName === 'special'">
            <div class="finance-table-container">
              <div class="money-box">
                <div class="money-box-left">
                  <div class="money-title">{{$t('OrderFee.pay')}}</div>
                ...
                <div class="money-box-right">
                  <el-button class="finance-btn" type="primary" size="mini" @click="addRow('option3')">{{$t('OrderFee.newPay')}}</el-button>
                  <el-button class="finance-btn" size="mini" @click="handleCopy('pay')">{{$t('Common.copy')}}</el-button>
                  <el-button class="finance-btn" type="danger" size="mini" @click="handleDelete('pay')">{{$t('Common.del')}}</el-button>
                  <el-button v-if="isDetail" class="finance-btn" type="warning" size="mini" @click="handleEdit('pay')">{{$t('Common.modify')}}</el-button>
                  <el-button type="text" class="collapse-btn" @click="handleCollapse('Table3')" size="mini">
                    {{showTable3? $t('Common.collapse'):''}}
                    <i
...
export default {
  ...
  data() {
    return {
      ...
      tabsList: [
        {
          label: this.$t('OrderFee.commercialFee'), // '业务费用',
          name: 'biz'
        },
        {
          label: this.$t('OrderFee.specialMngFee'), // '专项管理',
          name: 'special'
        }
      ],
      activeName: 'biz',
  ...
  created() {
    ...
    this.option1.operationBtns.data = this.option2.operationBtns.data = this.option3.operationBtns.data = [{ ...obj2 },
        { label: this.$t('Common.copy'), type: 'text', show: true, action: 'Copy' },]
    this.serviceColumn = {
      ...
      visibleChange: (queryString, item, row) => {
        // this.queryBusinessServiceList({ queryString }, item)
        if (!row.feeCode) {
          return this.$message({ message: this.$t('OrderFee.feeNameBeforeService'), type: 'error' }) // message: '须先选择费用名称'
        }
        if (!this.formData.businessType) {
          return this.$message({ message: this.$t('FeeList.firstTips'), type: 'error' }) // message: '须先选择业务类型'
        }
    ...
    this.columns = [
      { prop: 'settleCorpType', label: this.$t('FeeList.settleCorpType'), type: 'select', propInDict: 'feeTemplateSettleCorpType',
      ...
      { prop: 'settleCorpCode', label: this.$t('OrderFee.fixedValue'), type: 'variableType', dependentProp: 'settleCorpType', 
      ...
      },
      {
        ...
        querySearch: (queryString, cb, row) => {
          ...
          if (!this.formData.businessType) {
            return this.$message({
              message: this.$t('OrderFee.bizTypeSelectFirst'), // 须先选择业务类型
              type: 'error',
              showClose: true
            })
          }
  ...
  methods: {
    ...
    saveFeeTemplate(reqMethod) {
      try {
        this.$refs.form.validate((res, rej) => {
          ...
          try {
            fbFeeVos.forEach(item => {
              if (item.settleCorpType === 'fixed' && !item.settleCorpCode) {
                this.$message({ message: this.$t('OrderFee.fixedValueSUTip'), type: 'error' }) // 结算单位类型为固定值时，须选择结算单位
          ...
          this.$confirm(this.$t('OrderFee.confirmSaveFeeTempTip'), this.$t('Common.tips'), { // 确认保存费用模板吗？
            ...
            .then(() => {
              ...
              reqMethod({ ...formData, feeTemplateItemList: fbFeeVos }).then((res) => { // ...this.formData
                this.$message({ message: this.$t('Settle.addSuc'), type: 'success' })
                this.cancel(true)
              })
            })
            .catch((err) => {
    ...
    addRow(optionId, reverse) {
      ...
      if(reverse) {
        ...
        if(!isPay) {
          if(!this.multipleSelection2.length) return this.$msgErrClose(this.$t('OrderFee.selectRow')) // '请选择需要转收入的费用！'
          selections = this.multipleSelection2
        } else {
          if(!this.multipleSelection1.length) return this.$msgErrClose(this.$t('OrderFee.selectRow')) // // '请选择需要转支出的费用！'

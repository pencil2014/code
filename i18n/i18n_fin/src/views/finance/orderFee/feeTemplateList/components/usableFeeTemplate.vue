      ...
      <div class="finance-list-container">
        <div class="money-box">
          <div class="money-box-left">
            <el-button @click="addFeeTemplate" size="mini" type="primary">{{$t('OrderFee.new')}}</el-button>
  ...
  created() {
    this.searchOption1 = {
      ...
      addFilter: {
        ...
        filterGroups: {
          tempNo: { label: this.$t('OrderFee.tempNo'), type: 'input', showType: '1' }, // '模板编号'
          tempName: { label: this.$t('OrderFee.tempName'), type: 'input', showType: '2' }, // '模板名称'
          businessType: { label: this.$t('FeeList.businessTypes'), type: 'select', showType: '3', prop: 'businessType' },
          createdTime: { label: this.$t('Common.createTime'), type: 'daterange', showType: '4' },
        }
      },
    }
    ...
    this.selfColumnsBase = this.columnsBaseLocal = [ // let columns =
      {
        prop: 'tempNo',
        label: this.$t('OrderFee.tempNo'), // '费用模板编号',
        type: 'button',
        width: 128,
        operationBtns: {
          show: true,
      ...
      {
        prop: 'tempName',
        label: this.$t('OrderFee.tempName'), // '费用模板名称',
        type: 'text',
        width: 120,
      },
      {
      ...
      {
        prop: 'tempType',
        label: this.$t('OrderFee.tempType'), // '模板类型',
        type: 'select',
      ...
      { prop: 'updatedName', label: this.$t('OrderFee.updatedName'), type: 'text', width: 110 }, // '更新人'
      { prop: 'updatedTime', label: this.$t('OrderFee.updatedTime'), type: 'text', width: 120 }, // '更新时间'
    ]
  ...
  methods: {
    ...
    confirmFeeTemplate() {
      if (this.multipleSelection1.length !== 1) {
        return this.$message({
          message: this.$t('OrderFee.selectRow'), // '请选择一个费用模板',
          type: 'error',
          showClose: true
        })
      }
    ...
    copyFeeTemplate() {
      if (this.multipleSelection1.length !== 1) {
        this.$message({
          message: this.$t('OrderFee.selectRow'), // '请选择一个费用模板进行复制',
          type: 'error',
          showClose: true
        })
      } else {
    ...
    modifyEnabled(val) {
      if (this.multipleSelection1.length === 0) {
        return this.$message({
          message: this.$t('OrderFee.selectRow'), // '至少选择一个费用模板进行启用禁用',
          type: 'error',
          showClose: true
        })
      }
      ...
      modifyEnabled(data).then(res => {
        this.$message({
          message: this.$t('OrderFee.successOp'), // val === 'yes' ? '启用' : '禁用',
          type: 'success',
          showClose: true
        })
    ...
    deleteTemplate() {
      if (this.multipleSelection1.length === 0) {
        return this.$message({
          message: this.$t('OrderFee.selectRow'), // '至少选择一个费用模板进行删除',
          type: 'error',
          showClose: true
        })
      }
      this.$confirm(this.$t('OrderFee.sureDelTemp'), this.$t('Common.tips'), { // 确认删除费用模板吗？
        ...
        .then(() => {
          ...
          deleteTemplate(data).then(res => {
            this.$message({
              message: this.$t('OrderFee.successOp'), // '删除模板成功',
              type: 'success',
              showClose: true
            })
            this.refreshListPage()
    ...
    sendMulti(data, option) {
      ...
      if (this.multipleSelection1.length > 1) {
        return this.$message({
          message: this.$t('OrderFee.selectOneRow'), // '只能选择一个费用模板',
          type: 'error',
          showClose: true
        })

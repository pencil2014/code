	...
	<div class="finance-container fin-order-fee-wrapper" style="width:100%">
		<div :class="{ 'finance-order-fee-cont': !feeEmbed, 'order-fee-cont-resize': orderInfoTopOpen }">
			...
			<template v-if="!feeEmbed">
				...
				<TopNavAir v-else-if="air && orderInfo.serviceType" :serviceType="orderInfo.serviceType" :activityKey="'OrderFee'" @routerPush="handleToRoute" />
			...
			<div :class="{ 'finance-order-fee-main-cont': !feeEmbed }">
				<template v-if="!feeEmbed">
					<!-- 原已注释 <LeftNavAir v-if="air" :orderInfo="orderInfo" :activityKey="'OrderFee'" @routerPush="handleToRoute" /> -->
					<!-- <span v-if="rail"></span> -->
				...
				<div :class="{ 'finance-order-fee-main': !feeEmbed }">
					<div v-if="!hideNoBox" class="order-no-box">
						<div class="order-no-one" style="font-weight:bold;"><!-- <div class="order-icon-box"></div> -->{{$t('OrderFee.orderNo')}}：{{ orderNo+(orderInfo.businessType?'('+(dictMapObj.businessType[orderInfo.businessType]||orderInfo.businessType)+')':'') }}</div><!-- dictMapObj['businessType'][orderInfo.businessType]||orderInfo.businessType -->
						<div class="order-no-one"><i></i>{{$t('OrderFee.bdDeptName')}}：{{ MainOrderCompany.bdDeptName }}</div>
						<div v-if="!jointNo && cust" class="order-no-one">
							<entrustUnitComp :options="custInfo" />
						</div>
						<div v-if="jointNo" class="order-no-one"><i></i>{{$t('OrderFee.client')}}：{{ client.unitName }}</div>
						<div class="order-no-one">
							{{$t('OrderFee.precloseStatus')}}：
							<span v-if="isFinCloseCombined" style="color:#33B18A;"><i class="el-icon el-icon-success" style="margin-right:3px;"></i>{{$t('OrderFee.closeYes')}}</span>
							<span v-else style="color:#EDB534;"><i class="el-icon el-icon-info" style="margin-right:3px;"></i>{{$t('OrderFee.closeNo')}}</span>
						</div>
						<div class="order-no-one order-no-one-finclosefail" style="color:red;" v-if="orderCloseInfo.prefinCloseFailList.length > 0" @click="openPreClose">
							<el-tooltip :content="orderCloseInfo.prefinCloseFailList[0].errorReason" placement="bottom-start" :visible-arrow="false" effect="dark">
								<span>{{$t('OrderFee.errorReason')}}：{{ orderCloseInfo.prefinCloseFailList[0].errorReason }}</span>
							</el-tooltip>
						</div>
						...
						<div class="order-no-box-workassign">
							<div class="order-status" v-if="['break', 'cancel', 'close'].includes(orderInfo.orderStatus)" style="margin-right:8px;">
								...
								<template v-if="!jointNo">
									<el-button type="warning" size="mini" v-if="orderInfo.orderStatus === 'cancel' || orderInfo.orderStatus === 'close'" @click="handleBreak" style="margin-left: 5px;">{{$t('OrderFee.toTermination')}}</el-button>
									<el-button type="warning" size="mini" v-if="orderInfo.orderStatus === 'break'" @click="handleCancel" style="margin-left: 5px;">{{$t('OrderFee.toCancel')}}</el-button>
								</template>
							</div>
							...
							<el-button class="finance-btn fin-workassign-btn" size="mini" type="text">{{$t('OrderFee.checkParticipants')}}</el-button>
							<!-- 参与人与关键点 -->
							<div class="fee-order-work-assign" v-if="workAssignShow">
								...
								<component
									...
									:origWorkOptions="orderWorkOptions"
              ...
              <el-button class="finance-btn" @click="cancel()" size="mini">{{$t('Common.close')}}</el-button>
            </div> -->
					</div>
					...
					<el-tabs v-model="activeName" class="fin-el-tabs">
						<el-tab-pane v-for="(item, index) in tabsList" :key="'tabsList' + index" :label="getLabel(item, isMatched, adjustCount)" :name="item.name" class="fin-el-tab-pane">
							<template>
								...
								<el-tooltip v-if="item.name === 'businessFee' && isMatched === 'no'" :content="$t('OrderFee.feeNotMatched')" slot="label">
		...
		<el-dialog :title="`申请负利润关账`" :visible.sync="dialogShow" width="600px" @close="dialogShow = false">
			<el-form :model="lossForm" ref="lossForm" :rules="lossRules" :label-width="'100px'" :inline="false" size="mini" style="padding-top: 16px">
				<el-form-item :label="$t('OrderFee.reasonType')" prop="reasonType">
					<el-select v-model="lossForm.reasonType" :placeholder="$t('OrderFee.selectReasonType')" clearable filterable style="width:100%;">
				...
				<el-form-item :label="$t('OrderFee.reason')" prop="reason">
					<el-input type="textarea" v-model="lossForm.reason" :placeholder="$t('OrderFee.inputReason')" maxlength="500" show-word-limit></el-input>
				</el-form-item>
			</el-form>
		...
		<!-- <el-dialog :visible.sync="orderStatusChangeShow" class="change-orderStatus-dialog" width="400px" @close="handleClose" title="订单状态修改">
			...
			<el-form ref="changeOrderStatusForm" :model="changeOrderStatusItem" label-position="right" label-width="70px" class="change-orderStatus-form">    
				<el-form-item label="业务日期" prop="bizDate" :show-message="false" required>
					<el-date-picker size="mini" v-model="changeOrderStatusItem.bizDate" type="date" style="width: 100%" clearable :placeholder="$t('Common.plSelect')" value-format="yyyy-MM-dd" />
				</el-form-item>
			</el-form>
			<span slot="footer" class="dialog-footer">
				<el-button type="primary" @click="handleChnageOrderStatus" size="mini">{{$t('Common.sure')}}</el-button>
...
export default {
	...
	data() {
		...
		return {
			...
			tabsList: [
				...
				// },
				{
					label: this.$t('OrderFee.businessFee'), // '全部费用',
					name: 'businessFee'
				},
				{
					label: this.$t('OrderFee.specialManagementFee'), // '专项管理',
					name: 'specialManagementFee'
				},
				{
					label: this.$t('OrderFee.managementFee'), // '管理费',
					name: 'managementFee'
				},
				// {
				// 	label: '异常费用', // 海外版异常费用，无翻译，暂不显示
				// 	name: 'exceptionFee'
				// },
				// {
				//   label: '费用代收付',
				...
				// },
				{
					label: this.$t('OrderFee.feesApprovalRecord'), // '费用审批记录',
					name: 'feeClosedModifyFee'
				},
				{
					label: this.$t('OrderFee.profitLossApply'), // '负利润审批',
	...
	mounted() {
		...
		FinOrderFee &&
			FinOrderFee.addEventListener('click', e => {
				if (e.target.innerText === this.$t('OrderFee.checkParticipants')) { // '查看参与人'
					e.stopPropagation()
	...
	methods: {
		...
		precloseByBill() {
			this.$confirm('是否确认预关账？', this.$t('Common.tips'), {
		...
		getCancel(values) {
			...
			this.cancelText = values.orderStatus === 'cancel' || values.orderStatus === 'break' ? this.$t('OrderFee.cancelText1') : this.$t('OrderFee.cancelText2') // '取消(终止)' : '申请取消'
		...
		// 变更订单状态
		handleBreak() {
			this.$confirm(this.$t('OrderFee.opTip'), this.$t('OrderFee.tip'), { // '是否将订单状态修改为终止？' '警告'
				...
				.then(() => {
					orderStatusChangeBreak({
						...
						this.$message({ type: 'success', message: this.$t('OrderFee.successOp') })
						this.getOrderInfo()
					})
				})
				.catch(err => {})
		...
		handleCancel() {
			this.$confirm(this.$t('OrderFee.opTip')+this.$t('OrderFee.feesHaveDelTip'), this.$t('OrderFee.tip'), { // '变更订单状态为取消？请确认预关账前费用是否已删除。' '警告'
				...
				.then(() => {
					orderStatusChangeCancel({ orderNo: this.$route.query.orderNo }).then(res => {
						this.$message({ type: 'success', message: this.$t('OrderFee.successOp') }) // '变更订单状态为取消成功'
						this.getOrderInfo()
					})
				})
				.catch(err => {})
...
.order-no-box {
	padding: 8px 222px 8px 14px; // 8px 250px 8px 15px
	line-height: 20px;
	background: #fff;

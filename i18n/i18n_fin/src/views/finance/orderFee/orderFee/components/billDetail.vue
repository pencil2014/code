...
<div class="finance-bill-detail finance-page">
  <el-alert v-if="billChange" type="warning" :closable="false" class="orange fin-bill-change">
    <template slot="title">
      <i class="tag-change">{{$t('OrderFee.modifySimple')}}</i><!-- 改 -->
      <span class="txt">
        {{$t('BillList.regenerateBill')}}
      </span>
    </template>
  </el-alert>
  <div class="finance-bill-detail-selectshow" style="width:994px;">
    <el-form ref="accountForm" :inline="true" size="mini">
      <el-form-item :label="$t('BillList.displayField')" style="margin-bottom: 8px;">
        <el-select v-model="showOptions"
          ...
          <el-option :label="$t('BillList.custName')" value="custNameShow"></el-option>
          <el-option :label="$t('BillList.custAddr')" value="custAddrShow"></el-option>
          ...
          <el-option :label="$t('OrderFee.orderNo')" value="orderNoShow"></el-option><!-- 工作号 -->
          <el-option v-if="ship" :label="$t('BillList.so')" value="soShow"></el-option>
          <el-option v-if="ship || rail" :label="$t('BillList.cn')" value="cnShow"></el-option>
          <el-option :label="$t('BillList.custIntrustNo')" value="custIntrustNoShow"></el-option>
          <el-option label="HBL" value="hblShow"></el-option>
          <el-option label="MBL" value="mblShow"></el-option>
          <el-option :label="$t('BillList.billDate')" value="billDateShow"></el-option>
          <el-option v-if="ship" :label="$t('BillList.bargeEtd')" value="bargeEtdShow"></el-option>
          <el-option v-if="ship" :label="$t('BillList.bargeAtd')" value="bargeAtdShow"></el-option>
          <el-option :label="$lang==='en'?$t('BillList.etd'):ship ? '大船ETD' : air ? '起飞时间' : rail ? '发车时间' : '开航日期'" value="etdShow"></el-option><!-- 开航日期 -->
          <el-option v-if="ship || air" :label="$lang==='en'?$t('BillList.atd'):ship ? '大船ATD' : '实际起飞时间'" value="atdShow"></el-option>
          <el-option v-if="ship || air" :label="$lang==='en'?$t('BillList.eta'):ship ? '大船ETA' : '到达时间'" value="etaShow"></el-option><!-- 到港日期 -->
          <el-option v-if="ship || rail" :label="$lang==='en'?$t('BillList.vessel'):ship ? '船名航次' : '班名班次'" value="vesselShow"></el-option>
          <el-option v-if="air" :label="$t('BillList.airLine')" value="airLineShow"></el-option>
          <el-option v-if="air" :label="$t('BillList.eflightNumber')" value="eflightNumberShow"></el-option>
          <el-option :label="$lang==='en'?$t('BillList.polPortName'):rail ? '起运站' : '起运港'" value="polPortCodeShow"></el-option>
          <el-option :label="$lang==='en'?$t('BillList.podPortName'):ship ? '目的港' : air ? '目的地' : rail ? '目的站' : '目的港'" value="podPortCodeShow"></el-option>
          <el-option v-if="air || rail" :label="$t('BillList.totalQuantity')" value="totalQuantityShow"></el-option>
          <el-option v-if="air || rail" :label="$lang==='en'?$t('BillList.totalWeight'):air?'毛重':'重量'" value="totalWeightShow"></el-option>
          <el-option v-if="air || rail" :label="$t('BillList.totalVolume')" value="totalVolumeShow"></el-option>
          <el-option v-if="air" :label="$t('BillList.chargeWeight')" value="chargeWeightShow"></el-option>
          <el-option :disabled="regAddrDisabled" :label="$t('BillList.useRegAddr')" value="regAddrShow"></el-option>
          <el-option :disabled="!regAddrDisabled" :label="$t('BillList.useOfficeAddr')" value="officAddrShow"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :label="$t('BillList.bankAccount')" prop="settleUnitBankAcctIds" style="margin-bottom: 8px;">
    ...
    <span class="finance-bill-switch-lang" v-if="$lang==='zh'" @click="switchLang">{{lang === 'en' ? 'Switch Chinese' : '切换英文'}}</span>
  </div>
  <div class="finance-bill-detail-main" id="printBillDetail" style="width:994px;break-after:page;">
    ...
    <div class="fin-bill-switch-remark" style="text-align:right;padding-right:107px;">
      ...
      <el-checkbox v-model="feeRemarkChecked" style="margin-right:16px;" @change="switchFeeRemark">{{$t('BillList.showRemark')}}</el-checkbox>
      <el-checkbox v-if="positiveBtnShow" v-model="positiveChecked" @change="switchPositive">{{$t('BillList.showPositive')}}</el-checkbox>
    </div>
    ...
    >
      <div>
        <el-form :model="form" :rules="rules" ref="form" label-width="100px" size="mini">
          <el-form-item :label="$t('BillList.mailTo')" prop="receive">
            <el-input v-model="form.receive" :placeholder="$t('OrderFee.plsInput')" show-word-limit maxlength="500"><!-- 请输入收件人邮箱（多个以英文逗号分隔） -->
            </el-input>
          </el-form-item>
          <el-form-item :label="$t('BillList.mailCc')" prop="copyto">
            <el-input v-model="form.copyto" :placeholder="$t('OrderFee.plsInput')" show-word-limit maxlength="500"><!-- 请输入抄送人邮箱（多个以英文逗号分隔） -->
            </el-input>
          </el-form-item>
          <el-form-item :label="$t('BillList.billDocType')" prop="docType">
            <el-select :placeholder="$t('OrderFee.plsSelect')" v-model="form.docType" @change="changeDocType"><!-- 请选择账单格式 -->
          ...
          <el-form-item :label="$t('BillList.mailSubject')" prop="subject">
            <el-input :placeholder="$t('OrderFee.plsInput')" v-model="form.subject" type="textarea" :rows="2" show-word-limit maxlength="500">
            </el-input>
          </el-form-item>
          <el-form-item :label="$t('BillList.mailContent')" prop="content">
            <el-input :placeholder="$t('OrderFee.plsInput')" v-model="form.content" type="textarea" class="finance-email-content" :rows="6" show-word-limit maxlength="500">
            </el-input>
          </el-form-item>
          <el-form-item :label="$t('BillList.mailSignature')" prop="signature">
            <el-input :placeholder="$t('OrderFee.plsInput')" v-model="form.signature" show-word-limit maxlength="500">
            </el-input>
          </el-form-item>
          <el-form-item :label="$t('BillList.attachment')" prop="file">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="sendBillEmail">{{$t('BillList.sendMail')}}</el-button>
        <el-button @click="dialogVisible = false" size="mini">{{$t('OrderFee.cancel')}}</el-button>
      </span>
    </el-dialog>
  </div>
  <div v-if="!isFromProp" class="finance-bill-detail-btns">
    <el-button v-if="checkAuth($route.name, 'btn-email')" type="primary" class="finance-btn" @click="openBillEmailDialog" size="mini">{{$t('BillList.sendMail')}}</el-button>
    <el-button v-if="checkAuth($route.name, 'btn-print')" class="finance-btn" v-print="printObj" size="mini">{{$t('BillList.print')}}</el-button>
    ...
    <el-button class="finance-btn finance-bill-detail-back" @click="cancel" size="mini">{{$t('OrderFee.back')}}</el-button>
  </div>
</div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        subject: '',
        content: '',
        signature: this.$t('BillList.notReplySystem'), // '此邮件由系统发出,请勿直接回复'
      },
      rules: {
        receive: [
          { required: true, message: ' ', trigger: 'blur' }, // 邮箱不能为空
          {
        ...
        docType: [
          { required: true, message: ' ', trigger: 'blur' } // 格式不能为空
        ],
        subject: [
          { required: true, message: ' ', trigger: 'blur' } // 主题不能为空
        ],
        content: [
          { required: true, message: ' ', trigger: 'blur' } // 内容不能为空
        ],
      },
      printObj: {
        id: "printBillDetail",
        popTitle: this.$t('OrderFee.bill'), // '账单',
        extraCss: '',
      ...
      billI18n: {
        ...
        en: {
          ...
          polPortName: 'POL', // 'POL Name',
          blNo: 'Bl No',
          podPortName: 'POD', // 'POD Name',
          cn: 'Container No',
  ...
  methods: {
    ...
    getData() {
      ...
      Fn(data).then(res => { // getBillDetail(data).then(res => {
        if (res.code === 0) {
          ...
          this.lang = this.billData.lang.toUpperCase() === 'US' ? 'en' : 'en' // 'zh' // 临时改
    ...
    handleExport(command, $this) {
      $this = $this ? $this : this
      $this.$message({
        message: this.$t('BillList.inExportTip'), // '导出中，请留意下载的区域',
        type: 'warning',
        showClose: true,
        duration: 5000
      })
      ...
      $this.$store.dispatch('finance/downloadBlobFile', {
        ...
        link.download = this.$t('BillList.bill') + this.$t('OrderFee.enspace') + this.billData.orderBillNo + '.' + command.toLowerCase() // '账单'
        link.click()
      })
    },
    ...
    openBillEmailDialog() {
      ...
      this.billFile = {
        name: this.$t('BillList.bill') + this.$t('OrderFee.enspace') + this.billData.orderBillNo + '.' + command.toLowerCase(), // '账单'
        callback
      }
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          .then((res) => {
            this.$message({
              type: 'success',
              message: this.$t('BillList.sendSuccess'), // '发送邮件成功!',
              showClose: true
            })
  ...
  watch: {
    ...
    ship(newVal) {
      if (newVal) {
        ...
        this.billI18n.en.polPortName = 'POL' // 'PolPort Name'
        this.billI18n.en.podPortName = 'POD' // 'PodPort Name'
      }
    },
    air(newVal) {
      if (newVal) {

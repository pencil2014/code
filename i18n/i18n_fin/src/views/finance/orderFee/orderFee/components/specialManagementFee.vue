      ...
      <div class="money-box">
        <div class="money-box-left">
          ...
          <div class="money-total" v-for="(item, index) in moneyTotalPay" :key="'moneyTotalPay' + index">
            {{item.currency}}合计：<span :class="{'money-pay-color': true}">{{item.sum}}</span>
          </div>
        </div>
        <div class="money-box-right">
          <el-button-group v-if="!isEditPay && !isFinCloseCombined">
            <el-button v-if="checkAuth(pageCode, 'tab-specialManagementFee', 'btn-edit')" class="finance-btn" @click="handleEdit('pay')" size="mini" type="warning">{{$t('OrderFee.edit')}}</el-button>
          </el-button-group>
          <el-button-group v-show="!hideSaveCnlBtn && isEditPay">
            <el-button class="finance-btn" @click="handleSave" type="primary" size="mini">{{$t('Common.save')}}</el-button>
          </el-button-group>
          <el-button-group v-show="!hideSaveCnlBtn && isEditPay">
            <el-button class="finance-btn" @click="handleCancel" size="mini">{{$t('Common.cancel')}}</el-button>
          </el-button-group>
          <!-- <el-button-group>
          ...
          <el-button-group v-if="!isFinCloseCombined">
            <el-button v-if="checkAuth(pageCode, 'tab-specialManagementFee', 'btn-addPay')" class="finance-btn" @click="addPay(null)" size="mini" type="primary">{{$t('OrderFee.newPay')}}</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="checkAuth(pageCode, 'tab-specialManagementFee', 'btn-confirmOrderFeePay')" class="finance-btn" @click="confirmOrderFeePrev('pay')" size="mini" type="success">{{$t('OrderFee.confirmPay')}}</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="checkAuth(pageCode, 'tab-specialManagementFee', 'btn-cancleConfirmOrderFeePay')" class="finance-btn" @click="cancleConfirmOrderFeePrev('pay')" size="mini" type="purple">{{$t('OrderFee.cancelConfirm')}}</el-button>
          </el-button-group>
          <el-button-group v-if="!isFinCloseCombined">
            <el-button v-if="checkAuth(pageCode, 'tab-specialManagementFee', 'btn-addPay')" class="finance-btn" @click="handleCopy('pay')" size="mini">{{$t('OrderFee.copy')}}</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="checkAuth(pageCode, 'tab-specialManagementFee', 'btn-createSettlement')&&$route.name!=='BookDetail'" class="finance-btn" size="mini" type="primary" @click="createSettlement">{{$t('OrderFee.createSettlement')}}</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="checkAuth(pageCode, 'tab-specialManagementFee', 'btn-feeClosedModifyFee') && isFinCloseCombined" class="finance-btn" size="mini" type="primary" @click="openModifyFeeDialog">{{$t('OrderFee.feeClosedModifyFee')}}</el-button>
          </el-button-group>
          <el-button-group>
          ...
          <el-button-group v-if="!isFinCloseCombined">
            <el-button v-if="checkAuth(pageCode, 'tab-specialManagementFee', 'btn-rowDelete')" class="finance-btn" @click="handleDelete('pay')" size="mini" type="danger">{{$t('OrderFee.del')}}</el-button>
          </el-button-group>
        </div>
      </div>
      <FinanceTable
      ...
      <div class="modifyfee-detail-info finance-wider-el-input">
        <el-form ref="feeTemplateForm" :model="feeTemplateForm" label-width="110px" size="mini">
          ...
          <el-form-item :label="$t('OrderFee.templateName')" prop="feeTemplateName"
            :rules="[{required: true, message: $t('OrderFee.templateName')+$t('OrderFee.required'), trigger: 'focus'}]">
            <el-input
              v-model="feeTemplateForm.feeTemplateName"
              size="mini"
              :placeholder="$t('OrderFee.plsInput')+$t('OrderFee.templateName')"
              maxlength="50"
              show-word-limit>
            </el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      option2: {},
      isEditPay: false,
      moneyTitle2: this.$t('OrderFee.pay'), // '应付',
      showTable1: true,
      showTable2: true,
      table1DataBackup: [],
      table2DataBackup: [],
  ...
  methods: {
    getData(params) {
      ...
      orderFeeList(data).then(res => {
        ...
        if (res.code === 0) {
          ...
          this.columns = this.columns.filter(item => this.overseaDelColumns.every(one => one !== item))
          let arr = []
    ...
    addPay(feeItem, isTemplate) {
      if (this.isFinCloseCombined) {
        this.$message({
          type: 'error',
          message: this.$t('OrderFee.closeAddFeeTip'), // '订单已预关账，不能新增费用!',
          showClose: true
        })
        return
      }
    ...
    handleEdit(type) {
      if (this.isFinCloseCombined) {
        this.$message({
          type: 'error',
          message: this.$t('OrderFee.closeNotModifyFee'), // '订单已关账，不能修改费用!',
          showClose: true
        })
        return
      }
    ...
    handleCopy(type) {
      if (this.isFinCloseCombined) {
        this.$message({
          type: 'error',
          message: this.$t('OrderFee.closeAddFeeTip'), // '订单已关账，不能复制费用!',
          showClose: true
        })
        return
      }
    ...
    getFeesCanEdit_del (data, showMessage = false, noSomeCanEdit) { // getFeesCanEdit_del, 不使用此方法，使用mixin中方法
      // this.option1.data.forEach(item => {
    ...
    openModifyFeeDialog() {
      ...
      if (!this.isFinCloseCombined) {
        this.$message({
          type: 'error',
          message: this.$t('OrderFee.notClosedNotAdjust'), // '订单未预关账，不能申请关账调费!',
          showClose: true
        })
        return
      }
    ...
    confirmOrderFeePrev(receipayType) {
      ...
      if (this.multipleSelection1.length === 0) {
        return this.$message({
          message: this.$t('OrderFee.selectRow'), // '请选择需要确认的费用！',
          type: 'error',
          showClose: true
        })
    ...
    validateFinDate(data) {
      try {
        data.forEach(item => {
          if (!item.finDate) {
            this.$message({ 
              message: this.$t('OrderFee.feesNoFinDate'),
              type: 'error',
              showClose: true
            })
            throw new Error('ending')
    ...
    cancleConfirmOrderFeePrev(receipayType) {
      if (receipayType === 'pay') {
        ...
        if (this.multipleSelection1.length === 0) {
          return this.$message({
            message: this.$t('OrderFee.selectRow'), // '需勾选费用进行撤销确认！',
            type: 'error',
            showClose: true
          })
        } else {

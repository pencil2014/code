...
<div class="finance-page">
  <div class="finance-list-container">
    <div class="money-box money-box-border-bottom">
      <div class="money-box-left">
        <div class="money-title">{{$t('OrderFee.feeClosedModifyFee')}}</div>
      </div>
      <div class="money-box-right">
        <el-button type="success" @click="submit" size="mini" :loading="loading">{{$t('OrderFee.submit')}}</el-button>
        <el-button @click="cancel('click')" size="mini">{{$t('OrderFee.cancel')}}</el-button>
      </div>
    </div>
  </div>
  <div class="finance-detail-info">
    <el-form :model="form" ref="form" :inline="true" size="mini" label-width="123px">
      <el-form-item :label="$t('OrderFee.adjustReason')"
        :rules="{required: true, message: ' ', trigger: 'change'}">
        <el-select v-model="reason" :placeholder="$t('OrderFee.plsSelect')" clearable
          @change="reasonChange">
          <el-option
           :key="'reason' + index"
           :label="item.label"
      ...
      <el-form-item :label="$t('OrderFee.dutyPerson')" label-width="133px"
        :rules="{required: true, message: ' ', trigger: 'change'}">
        <el-select v-model="dutyPersonVk" :placeholder="$t('OrderFee.plsSelect')"
      ...
      <el-form-item v-if="isOtherDuty" :label="$t('OrderFee.applyPerson2')" label-width="123px"
        :rules="{required: true, message: ' ', trigger: 'change'}">
        <el-select v-model="applyPersonId" :placeholder="$t('OrderFee.plsSelect')"
          :filterable="isFinBillFee"
          remote
          clearable
      ...
      <el-form-item :label="$t('OrderFee.adjustRemark')" class="modifyfee-detail-info-remark" style="width:100%;"
        :rules="{required: true, message: ' ', trigger: 'blur'}">
        <!-- :rules="{required: true, message: '备注不能为空', trigger: 'blur'}" -->
        <el-input
          :placeholder="$t('OrderFee.plsAdjustReasonDetail')"
          v-model="remark"
          type="textarea"
          clearable
          maxlength="500"
    ...
    <div class="money-box">
      <div class="money-box-left">
        ...
        <div class="money-total">
          <span style="color:#222;">{{$t('OrderFee.originalRTotal')}}：<span class="money-recei-color">{{selectionReceiEachSum}}</span></span>
          <span style="color:#222;margin-left:12px;">{{$t('OrderFee.originalPTotal')}}：<span class="money-pay-color">{{selectionPayEachSum}}</span></span>
          <!-- v-if="selectionPayEachSum" -->
        </div>
        <div class="money-total">
          <!-- <span style="color:#222;">{{addFeesSum}}</span> -->
          <span style="color:#222;">{{$t('OrderFee.newFeesRTotal')}}：<span class="money-recei-color">{{addFeesReceiEachSum}}</span></span>
          <span style="color:#222;margin-left:12px;">{{$t('OrderFee.newFeesPTotal')}}：<span class="money-pay-color">{{addFeesPayEachSum}}</span></span>
        </div>
        <!-- 先挂起 -->
        <div class="money-total" v-show="!isFinBillFee && showReduceSum && reduceSum < 0" style="margin-right:0px;">
          {{$t('OrderFee.profitChange')}}：<span class="money-pay-color">{{$t('OrderFee.reduce')}}</span> USD <span class="money-pay-color">{{reduceSum}}</span>
          <el-popover
            placement="bottom-start"
            trigger="hover">
            <div class="money-dialog money-dialog-profit" style="padding:0;">
              <div style="font-size:12px;">
                <div
                  v-html="`<span class='el-icon-info' style='color:#E9851F;margin-right:6px;'></span>${$t('OrderFee.grossMarginBefore')}CNY<span style='color:#E9851F'>${changeInfo.cnyProfitBefore}</span>(USD<span style='color:#E9851F'>${changeInfo.usdProfitBefore}</span>) ${$t('OrderFee.grossMargin')}：${changeInfo.grossProfitBeforeStr}`"
                ></div>
                <div
                  v-html="`<span class='el-icon-success' style='color:#33B18A;margin-right:6px;'></span>${$t('OrderFee.grossMarginAfter')}CNY<span style='color:#33b18a'>${changeInfo.cnyProfitAfter}</span>(USD<span style='color:#33b18a'>${changeInfo.usdProfitAfter}</span>) ${$t('OrderFee.grossMargin')}：${changeInfo.grossProfitAfterStr}`"
                ></div>
              </div>
            </div>
      ...
      <div class="money-box-right">
        <el-button v-if="!isFinBillFee" class="finance-btn" @click="addModify(null)" size="mini" type="primary">{{$t('OrderFee.newFee')}}</el-button><!-- 新增 -->
        <el-button v-if="!isFinBillFee" class="finance-btn" @click="alertTemplate" size="mini">{{$t('OrderFee.importTemplate')}}</el-button>
        <el-button v-if="!isFinBillFee" class="finance-btn" @click="handleCopy" size="mini">{{$t('OrderFee.copy')}}</el-button>
        <el-button class="finance-btn" @click="handleDelete" size="mini" type="danger">{{$t('OrderFee.del')}}</el-button>
  ...
  <div class="finance-table-container" v-if="comeType === 'edit'">
    <div class="money-box">
      <div class="money-box-left">
  ...
  <!-- <span slot="footer" class="dialog-footer">
    <el-button type="primary" @click="submit" size="mini" :loading="loading">{{$t('Common.save')}}</el-button>
  ...
  <div v-if="templateListVisible">
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="confirmFeeTemplate" size="mini" type="primary">{{$t('Common.sure')}}</el-button>
        <el-button @click="templateListVisible = false" size="mini">{{$t('Common.cancel')}}</el-button>
      </span>
    </el-dialog>
  </div>
</div>
...
export default {
  ...
  data() {
    return {
      ...
      option4: {},
      multipleSelection1: [],
      moneyTitle1: this.$t('OrderFee.fee') + this.$t('OrderFee.edit'), // '费用修改',
      moneyTitle2: this.$t('OrderFee.hedgeRecord'), // '红冲记录',
      moneyTotalReceive: [],
      comeType: 'edit',
      hedgingRecord: [],
      deleteSelection: [],
  ...
  created() {
    this.getProps()
    this.comeType = this.multipleSelection.length > 0 ? 'edit' : 'new'
    this.moneyTitle1 = this.comeType === 'edit' ? this.$t('OrderFee.fee') + this.$t('OrderFee.enspace') + this.$t('OrderFee.edit') : this.$t('OrderFee.fee') + this.$t('OrderFee.enspace') + this.$t('OrderFee.new') // '费用修改' : '费用新增'
  ...
  methods: {
    ...
    setColumns1() {
      let columns = ['receipayType', 'feeName', 'feeEnName', 'serviceName', 'settleCorpName', 'currency', 'feeUnit', 'unitPrice', 'unitCount', 'feeAmt', 'blId', 'taxRate', 'remark', 'negativeType', 'splitFromOrderNo', 'negativeFollowerId']
      if (this.isSpecialFee) columns = ['receipayType', 'feeName', 'feeEnName', 'serviceName', 'settleCorpName', 'currency', 'feeUnit', 'unitPrice', 'unitCount', 'feeAmt', 'taxRate', 'remark', 'negativeType', 'negativeFollowerId']
      if (this.isFinBillFee) columns = ['receipayType', 'feeName', 'feeEnName', 'settleCorpName', 'currency', 'feeUnit', 'unitPrice', 'unitCount', 'feeAmt', 'remark']
    ...
    setColumns2() {
      let columns2 = ['receipayType', 'feeName', 'feeEnName', 'serviceName', 'settleCorpName', 'currency', 'feeUnit', 'unitPrice', 'unitCount', 'feeAmt', 'blId', 'taxRate', 'remark']
      if (this.isSpecialFee) columns2 = ['receipayType', 'feeName', 'feeEnName', 'serviceName', 'settleCorpName', 'currency', 'feeUnit', 'unitPrice', 'unitCount', 'feeAmt', 'taxRate', 'remark']
      if (this.isFinBillFee) columns2 = ['receipayType', 'feeName', 'feeEnName', 'settleCorpName', 'currency', 'feeUnit', 'unitPrice', 'unitCount', 'feeAmt', 'remark']
      columns2 = columns2.filter(item => this.overseaDelColumns.every(one => one !== item))
      let arr2 = []
    ...
    handleDelete($index, row, option) {
      if (!this.multipleSelection1.length) {
        this.$message({
          type: 'error',
          message: this.$t('OrderFee.selectRow'), // '请至少选择一条费用进行删除',
          showClose: true
        })
        return
      }
    ...
    handleCopy() { // handleCopy(row, option)
      if (!this.multipleSelection1.length) {
        this.$message({
          type: 'error',
          message: this.$t('OrderFee.selectRow'), // '请至少选择一条费用进行复制',
          showClose: true
        })
        return
      }
    ...
    submit2() {
      ...
      if (this.isFinBillFee) {
        ...
        if (!this.validate(data, ['finBillNo', 'reason', 'dutyPerson', 'applyPersonId', 'remark'], { remark: this.$t('OrderFee.remark'), reason: this.$t('OrderFee.adjustReason'), applyPersonId: this.$t('OrderFee.applyPerson2') })) { // '备注' '调费原因' '发起审批人'
          return
        }
      } else {
        data.orderNo = this.useNo // 改成this.orderNo, 20220122-改回原样
        data.jointNo = this.jointNo // 20220124负数开发额外增加
        if (!this.validate(data, ['orderNo', 'reason', 'dutyPerson', 'applyPersonId', 'remark'], { remark: this.$t('OrderFee.remark'), reason: this.$t('OrderFee.adjustReason'), applyPersonId: this.$t('OrderFee.applyPerson2') })) { // '备注' '调费原因' '发起审批人'
          return
        }
      }
            ...
            if (item.receipayType === 'receive' && item.operation !== 'delete') {
              if (!item.isNoNeedBlFee && !item.isNoNeedBlSettleUnit && !this.air) {
                if (!item.blId.length && this.blList.length) {
                  ...
                  // this.$message({
                  //   type: 'error',
                  //   message: this.$t('OrderFee.recvNeedBl'), // `录入应收费用，请关联提单号`,
                  //   showClose: true,
                  //   offset: 162
                  // })
                  // throw new Error('ending')
            ...
            if (item.unitPrice < 0 && item.operation !== 'delete') {
              if (item.receipayType === 'receive') {
                if (!item.negativeType) {
                  this.$message({
                    message: this.$t('OrderFee.negRecvNeedInfo'), // `录入应收负数，须录入负数类型`,
                    type: 'error',
                    duration: 5000,
                    showClose: true,
                    offset: 162
                ...
                if (item.negativeType === 'order_split' && !item.splitFromOrderNo) {
                  this.$message({
                    message: this.$t('OrderFee.negRecvSplitNeedInfo'), // `录入应收负数且属于拆单，须输入原单号`,
                    type: 'error',
                    duration: 5000,
                    showClose: true,
                    offset: 162
                ...
                this.$message({
                  message: this.$t('OrderFee.negPayNeedInfo'), // `录入应付负数，须录入负数类型和跟进人`,
                  type: 'error',
                  duration: 5000,
                  showClose: true,
                  offset: 162
        ...
        try {
          data.fees.forEach(item => {
            ...
            if (item.feeCode === 'TZFY' && item.unitPrice >= 0) {
              this.$message({
                message: this.$t('OrderFee.adjustFeeEnterNeg'), // '“调整费用”须录入负数',
                type: 'error',
                showClose: true,
                offset: 108
              })

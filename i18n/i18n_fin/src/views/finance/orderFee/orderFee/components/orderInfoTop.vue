    ...
    <div class="header-info">
        ...
        <div class="align-right">
          <span class="btn" @click="show">
            {{isShow?$t('OrderFee.collapse'):$t('OrderFee.expand')}}
   ...
   data() {
     return {
       ...
       shipHeaderZX: [ // 海运整箱头部需要展示的字段
          { label: 'SO', value: 'so' }, // 'SO号'
          { label: 'MBL', value: 'mbl' },
          { label: 'HBL', value: 'hbl' },
       ],
       shipContainerZX: [ // 海运整箱展示的字段
          { label: this.$lang === 'en' ? 'PolPort' : '大船起运港', value: 'polPortName'},
          { label: this.$lang === 'en' ? 'TransitPort' : '中转港', value: 'transitPortName'},
          { label: this.$lang === 'en' ? 'UnloadingPort' : '卸货港', value: 'unloadingPortName'},
          { label: this.$lang === 'en' ? 'PodPort' : '目的地', value: 'podPortName'},
          { label: this.$lang === 'en' ? 'PorPort' : '驳船收货地', value: 'porPortName'},
          { label: this.$lang === 'en' ? 'Ship Carrier' : '船公司', value: 'shipCarrierCode'},
          { label: this.$lang === 'en' ? 'Vessel' : '大船船名', value: 'vessel'},
          { label: this.$lang === 'en' ? 'Voyage' : '大船航次', value: 'voyage'},
          { label: this.$lang === 'en' ? 'ETD' : '大船预计开船时间', value: 'etdTime'},
          { label: this.$lang === 'en' ? 'Cy CutOff' : '大船截关时间', value: 'cyCutOff'},
          { label: this.$lang === 'en' ? 'Container' : '箱型箱量', value: 'container'},
          { label: this.$lang === 'en' ? 'Quantity' : '总件数', value: 'quantity', unit: this.$t('OrderFee.piece') + this.$t('OrderFee.enspace'), show0: true},
          { label: this.$lang === 'en' ? 'Weight' : '总重量', value: 'weight', unit: 'KGS', show0: true},
          { label: this.$lang === 'en' ? 'Volume' : '总体积', value: 'volume', unit: 'CBM', show0: true},
          { label: this.$lang === 'en' ? 'Settle Mode' : '客户结算方式', value: 'settleMode', showValue: ''},
       ],
       shipHeaderPX: [ // 海运拼箱头部需要展示的字段
         { label: this.$lang === 'en' ? 'Po' : '入舱单号', value: 'po' },
         { label: 'MBL', value: 'mbl' },
         { label: 'HBL', value: 'hbl' },
       ],
       shipContainerPX: [ // 海运拼箱展示的字段
          { label: this.$lang === 'en' ? 'PolPort' : '起运港', value: 'polPortName'},
          { label: this.$lang === 'en' ? 'UnloadingPort' : '卸货港', value: 'unloadingPortName'},
          { label: this.$lang === 'en' ? 'PodPort' : '目的地', value: 'podPortName'},
          { label: this.$lang === 'en' ? 'Receipt Place' : '装货地', value: 'receiptPlaceName'},
          { label: this.$lang === 'en' ? 'ETD' : '大船预计开船时间', value: 'etdTime'},
          { label: this.$lang === 'en' ? 'Container' : '箱型箱量', value: 'container'},
          { label: this.$lang === 'en' ? 'Quantity' : '总件数', value: 'quantity', unit: this.$t('OrderFee.piece') + this.$t('OrderFee.enspace'), show0: true},
          { label: this.$lang === 'en' ? 'Weight' : '总重量', value: 'weight', unit: 'KGS', show0: true},
       ...
       airHeaderList: [ // 空运头部需要展示的字段
          { label: this.$lang === 'en' ? 'MBL' : '主单号', value: 'mbl'},
          { label: this.$lang === 'en' ? 'HBL' : '分单号', value: 'hbl'},
       ],
       ariContainerList: [ // 空运需要展示的字段
          { label: this.$lang === 'en' ? 'PolPort' : '起运港', value: 'polPortCode'},
          { label: this.$lang === 'en' ? 'PodPort' : '目的地', value: 'podPortCode'},
          { label: this.$lang === 'en' ? 'AirLine' : '航空公司', value: 'airLine'},
          { label: this.$lang === 'en' ? 'Transport Term' : '运输条款', value: 'transportTerm'},
          { label: this.$lang === 'en' ? 'Total Quantity' : '预计件数', value: 'totalQuantity', unit: this.$t('OrderFee.piece') + this.$t('OrderFee.enspace'), show0: true},
          { label: this.$lang === 'en' ? 'Total Weight' : '预计毛重', value: 'totalWeight', unit: 'KGS', show0: true},
          { label: this.$lang === 'en' ? 'Total Volume' : '预计体积', value: 'totalVolume', unit: 'CBM', show0: true},
          { label: this.$lang === 'en' ? 'Charge Weight' : '预计计费重量', value: 'chargeWeight', unit: 'KGS', show0: true},
          { label: this.$lang === 'en' ? 'Bl Total Quantity' : '制单计数', value: 'bltotalQuantity', unit: this.$t('OrderFee.piece') + this.$t('OrderFee.enspace'), show0: true},
          { label: this.$lang === 'en' ? 'Bl Total Weight' : '制单毛重', value: 'bltotalWeight', unit: 'KGS', show0: true},
          { label: this.$lang === 'en' ? 'Bl Total Volume' : '制单体积', value: 'bltotalVolume', unit: 'CBM', show0: true},
          { label: this.$lang === 'en' ? 'Bl Charge Weight' : '制单计费重量', value: 'blchargeWeight', unit: 'KGS', show0: true},
          { label: this.$lang === 'en' ? 'Customer Class' : '客户类型', value: 'custClass',showValue:''},
          { label: this.$lang === 'en' ? 'Service Type' : '服务类型', value: 'serviceType',showValue:''},
          { label: this.$lang === 'en' ? 'Service List' : '服务项', value: 'serviceList',showValue:''},
          { label: this.$lang === 'en' ? 'Settle Mode' : '客户结算方式', value: 'settleMode', showValue: ''},
       ],
       ...
       railContainerList: [ // 铁路需要展示的字段
          { label: this.$lang === 'en' ? 'PorPort' : '收货地', value: 'porPortName'},
          { label: this.$lang === 'en' ? 'PolPort' : '起运站', value: 'polPortName'},
          { label: this.$lang === 'en' ? 'PodPort' : '目的站', value: 'podPortName'},
          { label: this.$lang === 'en' ? 'Final PodPort' : '最终目的地', value: 'finalPodPortName'},
          { label: this.$lang === 'en' ? 'ETD' : '预计发车时间', value: 'etdTime'},
          { label: this.$lang === 'en' ? 'Container' : '箱型箱量', value: 'container'},
          { label: this.$lang === 'en' ? 'Quantity' : '总件数', value: 'quantity', unit: this.$t('OrderFee.piece') + this.$t('OrderFee.enspace'), show0: true},
          { label: this.$lang === 'en' ? 'Weight' : '总重量', value: 'weight', unit: 'KGS', show0: true},
          { label: this.$lang === 'en' ? 'Volume' : '总体积', value: 'volume', unit: 'CBM', show0: true},

  ...
  <div class="work-cont">
    ...
    <template v-if="orderIsJoint">
      <el-collapse  v-model="workerActives">
       <el-collapse-item name="1">
        <template slot="title">
          <div class="worker-type-label">{{$t('OrderFee.coParticipants')}}</div>
        </template>
        <div class="work-assign-item" v-for="(item, index) in jointWorkAssign" :key="item.jobCode" :id="'work-'+item.jobCode" :class="{'warning': item.isUpdatePricing || item.isUpdateBkg}">
          ...
          <div class="item-value invalid" v-if="pricingHide(item)">
            ...
            <el-select v-if="item.show" size="mini" clearable :filterable='true' :disabled="workDisabled(item) || true" v-model="item.employeeName" style="" :placeholder="$t('Common.plSelect')" @visible-change="(val) => {visibleChange(val, index)}" :filter-method="(val) => {filterMehod(val, index)}" @change="(val) => {handleChangeEmployee(item, val, index)}">
       ...
       <el-collapse-item name="2">
        <template slot="title">
          <div class="worker-type-label">{{$t('OrderFee.origiParticipant')}}</div>
        </template>
        <div class="work-assign-item" v-for="(item, index) in orderWorkOptions" :key="item.jobCode" :id="'work-'+item.jobCode" :class="{'warning': item.isUpdatePricing || item.isUpdateBkg}">
          ...
          <div class="item-value" v-if="pricingHide(item)" :class="workDisabled(item) ? 'invalid' : ''">
            <span v-if="!item.show">{{item.employeeName}}</span>
            <el-select v-if="item.show" size="mini" clearable :filterable='true' :disabled="workDisabled(item) || selectedDisabled" v-model="item.employeeName" style="" :placeholder="$t('Common.plSelect')" @visible-change="(val) => {visibleChange(val, index)}" :filter-method="(val) => {filterMehod(val, index)}" @change="(val) => {handleChangeEmployee(item, val, index)}">
              <el-option v-for="o in item.data" :key="o.employeeId" :label="o.cname" :value="o.employeeId">
              </el-option>
            </el-select>
          </div>
    ...
    <div class="mt10">
      <div class="joint-work-item" v-if="jointAuditWorkData && Object.keys(jointAuditWorkData).length">
        <div class="item-label">{{jointAuditWorkData.jobEname}}</div><!-- {{jointAuditWorkData.jobCname}} -->
      ...
      <div class="joint-work-item" v-if="jointWorkData && jointWorkData.length">
        <div class="item-label">{{$t('OrderFee.coPositionAssign')}}</div>
        <div class="item-value joint-work">

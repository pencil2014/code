    ...
    <div class="finance-table-container">
      <div class="money-box">
        ...
        <div class="money-box-right">
          ...
          <el-button-group>
            <el-button v-if="checkAuth(pageCode, 'tab-bill', 'btn-createSettlement')" class="finance-btn" @click="createSettlement" size="mini">{{$t('OrderFee.createSettlement')}}</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="checkAuth(pageCode, 'tab-bill', 'btn-customerConfirm')" class="finance-btn" @click="customerConfirm" size="mini">{{$t('OrderFee.customerConfirm')}}</el-button>
          </el-button-group>
        </div>
      </div>
      <FinanceTable
...
export default {
  ...
  data() {
    ...
    return {
      ...
      option2: {},
      isEdit: false,
      moneyTitle2: this.$t('OrderFee.bill'), // '账单',
      showTable1: true,
      showTable2: true,
      table1DataBackup: [],
      table2DataBackup: [],
  ...
  methods: {
      ...
      billList(data).then(res => {
        ...
        if (res.code === 0) {
          ...
          let columns = [
            ...
            {
              prop: 'createdTime',
              label: this.$t('OrderFee.createdTime'), // '生成日期',
              type: 'text',
              width: 120
            },
            {
              ...
              type: 'text',
              formatter: ({ cellValue }) => {if (cellValue === 'yes') {return this.$t('OrderFee.yes')} else if (cellValue === 'no') {return this.$t('OrderFee.no')}},
              width: 100
            },
            {
              prop: 'orderBillNo',
            ...
            },
            {
              ...
              width: 125,
              headerSign: {
                text: this.$t('OrderFee.DNOrCN'), // 'DN号、CN号的统称's
              }
            },
            {
              prop: 'settleCorpName',
            ...
            {
              prop: 'receipayType',
              label: this.$t('OrderFee.receipayType'), // '收付类型',
              type: 'select',
    ...
    createSettlement() {
      if (this.multipleSelection1.length === 0) {
        this.$message({
          message: this.$t('OrderFee.selectRow'), // '请选择需要生成结算单的账单！',
          type: 'error',
          showClose: true
        })
        return
      ...
      if (this.multipleSelection1.length > 1) {
        this.$message({
          message: this.$t('OrderFee.selectOneRow'), // '一次只能选择一个账单！',
          type: 'error',
          showClose: true
        })
        return
      ...
      if (this.multipleSelection1[0].confirmStatus === 'no') {
        this.$message({
          message: this.$t('OrderFee.confirmBillFirst'), // '请先确认账单！',
          type: 'error',
          showClose: true
        })
        return

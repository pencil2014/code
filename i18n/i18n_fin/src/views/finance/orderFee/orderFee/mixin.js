	...
	data () {
		return {
			option: {
				edit: false,
				columns: [
					{ prop: 'settleCompName', label: this.$t('FeeList.settleComp'), type: 'text', key: 'settleCompName', width: 160 },
					{ prop: 'settleCorpName', label: this.$t('FeeList.settleCorp'), type: 'input', width: 160, key: 'settleCorpName', disabled: true },
					{ prop: 'serviceCode', label: this.$t('FeeList.serviceCodes'), type: 'select', key: 'serviceCode', width: 100, disabled: true },
					// { prop: 'feeName', label: this.$t('FeeList.feeEname'), type: 'input', key: 'feeName', width: 110, disabled: true },
					{ prop: 'feeEnName', label: this.$t('FeeList.feeEname'), type: 'text', key: 'feeEnName', width: 130 },
					{ prop: 'unitCount', label: this.$t('FeeList.quantity'), type: 'input', key: 'unitCount', width: 70, disabled: true },
					{ prop: 'unitPrice', label: this.$t('FeeList.unitPrice'), type: 'input', key: 'unitPrice', disabled: true },
					{ prop: 'feeUnit', label: this.$t('FeeList.feeUnit'), type: 'select', key: 'feeUnit', width: 80, disabled: true },
					{ prop: 'feeAmt', label: this.$t('FeeList.feeAmt'), type: 'input', key: 'feeAmt', disabled: true },
					...
					// { prop: 'taxRate', label: this.$t('FeeList.taxRate'), type: 'text', key: 'taxRate', width: 70, disabled: true, formatter: (row, pItem) => { if (row.taxRate) { return Number(row.taxRate) * 100 + '%' } else { return row.taxRate } } },
					// { prop: 'taxAmt', label: this.$t('FeeList.taxAmt'), type: 'text', key: 'taxAmt' },
					// { prop: 'feeAmtWithoutTax', label: this.$t('FeeList.feeAmtWithoutTax'), type: 'text', key: 'feeAmtWithoutTax', width: 100 },
					{ prop: 'remark', label: this.$t('FeeList.expenseRemarks'), type: 'input', key: 'remark', width: 130 }
				],
				data: [],
				operationBtns: {
					width: '80px',
      ...
      configColumns: [], // 传给自定义表头当前用的字段
			...
			eLoadingText: this.$t('OrderFee.eLoadingText'),
      ...
      descColumns: [],
			ascColumns: [],
			overseaDelColumns: ['invoiceNos', 'taxRate', 'taxAmt', 'feeAmtWithoutTax', 'isIssueInvoice', 'invoiceStatus', 'invoiceAmt', 'notInvoiceAmt', 'isAgentDzgPay', 'dzgFeeAmt', 'dzgPayBillNo'], // 海外版发票、大掌柜、税率相关的不显示 // , 'feeEnName'
    }
  },
  created() {
    ...
    this.columnsBaseLocal.forEach(item => {
      ...
      if (item.prop === 'settleCorpName') {
        item = Object.assign(item, {
          ...
          querySearch: (queryString, cb, row, item) => {
            ...
            if (!row.receipayType) {
              return this.$message({
                message: this.$t('OrderFee.plsSelect') + this.$t('OrderFee.receipayType'), // '须先选择收付类型',
								type: 'error',
								showClose: true,
								offset: 162
              })
              ...
              if (!this.isFinBillFee && (!row.serviceCode || !row.feeCode)) {
                return this.$message({
                  message: this.$t('OrderFee.addPay1stFeeService'), // '应付的结算单位，须先选择费用名称、服务项',
									type: 'error',
									duration: 5000,
									showClose: true,
									offset: 162
      ...
      } else if (item.prop === 'feeName' || item.prop === 'feeEnName') {
        item = Object.assign(item, {
					type: 'autocomplete',
          clearable: true,
          ...
          select: (optionItem, row) => {
            ...
            if (this.$route.name === 'FinBillListDetail' && row.receipayType === 'pay') {
              this.getTaxRate && this.getTaxRate(row)
							...
							row.serviceName = this.$lang === 'zh' ? orderServices[0].serviceName : orderServices[0].serviceEname
							row.serviceCode = orderServices[0].serviceCode
							if (row.receipayType === 'pay') {
								this.serviceNamePay = this.$lang === 'zh' ? row.serviceName : row.serviceEname
								this.serviceCodePay = row.serviceCode
								this.getTaxRate && this.getTaxRate(row)
							} else if (row.receipayType === 'receive') {
								this.serviceNameRecei = this.$lang === 'zh' ? row.serviceName : row.serviceEname
        ...
        item = Object.assign(item, {
            ...
            if (row.feeCode) {
              ...
              if (orderServices) {
                orderServices = orderServices.map(item => {
                  return {
                    value: this.$lang === 'zh' ? item.serviceName : item.serviceEname,
                    key: item.serviceCode
                  }
								})
								if (orderServices.length === 0) {
                  this.$message({
                    message: this.$t('OrderFee.feeNotAddNoService'), // '该费用项无对应服务项，无法添加该费用',
                    type: 'error',
										showClose: true,
										offset: 162
                  })
          ...
          focus: (row) => {
            if (!row.feeCode || row.feeCode === '') {
              this.$message({
                message: this.$t('OrderFee.feeNameBeforeService'), // '须先选择费用名称，再选择服务项',
								type: 'error',
								duration: 5000,
								showClose: true,
        ...
        item = Object.assign(item, {
          validateInput: this.validateInput,
				...
				item = Object.assign(item, {
					clearable: true,
					placeholder: this.$t('OrderFee.plsSelect'), // '请选择',
					...
					visibleChange: (item, row) => {
						if (row.unitPrice >= 0) {
							this.$message({
								message: this.$t('OrderFee.posNoNeedNegType'), // '录入负数，才需选择负数类型',
								type: 'error',
								showClose: true,
								offset: this.isSpecialFee ? 108 : 162
							})
				...
				item = Object.assign(item, {
					remote: true,
					clearable: true,
					placeholder: this.$t('OrderFee.plsSelect'), // '请输入',
					remoteSelectList: [],
					remoteMethod: this.employeeSelectlistMx,
					visibleChange: (queryString, item, row) => {
						if (!row.unitPrice || row.unitPrice >= 0) {
							this.$message({
								message: this.$t('OrderFee.posNoNeedFollower'), // '录入应付负数，跟进人必填；应收负数，跟进人非必填',
								type: 'error',
								showClose: true,
								offset: 162
							})
				...
				item = Object.assign(item, {
					clearable: true,
					placeholder: this.$t('OrderFee.plsInput'), // '请输入',
					focus: (row, item) => {
						if (row.receipayType !== 'receive' || row.unitPrice >= 0 || row.negativeType !== 'order_split') {
							this.$message({
								message: this.$t('OrderFee.negRecvSplitNeedInfo'), // '录入应收负数且属于拆单，才需输入原单号',
								type: 'error',
								showClose: true,
								offset: 162
							})
				...
				item = Object.assign(item, {
					...
					visibleChange: (queryString, item, row) => {
						if (!row.receipayType) {
							return this.$message({
                message: this.$t('OrderFee.plsSelect') + this.$t('OrderFee.receipayType'), // '请先选择收付类型',
								type: 'error',
								showClose: true,
								offset: 162
              })
							...
							this.$message({
								message: this.$t('OrderFee.payNotBl'), // '应付费用无需关联提单号',
								type: 'error',
								showClose: true,
								offset: 162
							})
      ...
      } else if (item.prop === 'transferExVoucherNos') {
        // 会计转字凭证号
        item = Object.assign(item, {
          type: 'columnTip',
          width: 130,
          ...
          componentProps: {
            ...
            callback: (no, componentProps, index) => {
              let row = componentProps.row
              let voucherSeqNos = row.transferExVoucherSeqNos_multiValue
              let voucherSeqNo = voucherSeqNos[index]
              this.voucherView({ voucherSeqNo, isEx: true })
            }
          }
        })
      } else if (item.prop === 'exVoucherNos') {
        // 会计收付凭证号
        item = Object.assign(item, {
          type: 'columnTip',
          width: 130,
          ...
          componentProps: {
            ...
            callback: (no, componentProps, index) => {
              let row = componentProps.row
              let voucherSeqNos = row.exVoucherSeqNos_multiValue
              let voucherSeqNo = voucherSeqNos[index]
              this.voucherView({ voucherSeqNo, isEx: true })
            }
          }
    ...
    checkAuth(pageCode, moduleCode, subModuleCode) {
      return checkAuth(pageCode, moduleCode, subModuleCode)
		...
		validateMoney(amt, params = {}) {
			...
			if (isNaN(amt)) {
				this.$message({
					type: 'error',
					message: this.$t('OrderFee.numberError'), // '数字输入不正确！',
					showClose: true
				})
				return false
			}
			if (amt > 10000000000) {
				this.$message({
					type: 'error',
					message: this.$t('OrderFee.notBigger10Billion'), // '输入的数字不能大于100亿！',
					showClose: true
				})
				return false
			}
			...
			if (positive) {
				if (amt < 0) {
					this.$message({
						type: 'error',
						message: this.$t('OrderFee.enterPos'), // '请输入正数！',
						showClose: true
					})
					return false
				}
      ...
      let negative = typeof pItem.negative === 'function' ? pItem.negative(row) : pItem.negative
          ...
          if (!/^-\d+(\.\d{1,2})?$/.test(value)) {
            this.$message({
              type: 'error',
              message: pItem.errMsg || this.$t('OrderFee.enterPos'), // '请输入负数！',
              showClose: true
            })
            let res = /^-\d+(\.\d{1,2})?/.exec(row[pItem.prop])
            row[pItem.prop] = res && res[0]
      ...
      this.moneyTotalReceive = currencys.map(item => {
        return {
          ...
          sum: decimalFixed2(receiveData.reduce((sum, cur) => {
			...
			payOptionId = matchObj.pay && (parseInt(matchObj.pay.substring(6))) || 1
      ...
      let { isTab, from, isFromSea, bkgReqNo } = otherParams
          ...
          if (type === 'receive') {
                ...
                query: { invoicePaymentId: no, fromOrder: true }
					...
					this.$message({ message: this.$t('OrderFee.invoiceNotOpen'), type: 'error', showClose: true }) // '无收付类型，不支持打开此发票号！'
				}
            ...
            query: { settleOrderBaddebtId: no, fromOrder: true}
				...
				queryPayOrderByWriteoffNo(data).then(res => {
                  ...
                  query: { source: 'detail', writeoffNopayNo: no, fromOrder: true}
							...
							this.$message({ message: this.$t('OrderFee.numberNotOpen'), type: 'error', showClose: true }) // 不支持打开此编号
						}
					}
				}).catch(err => {
					console.log(err)
		...
		editGuard(data, showMessage = false) {
			try {
				data.forEach(item => {
					let msgPrefix = `"${this.$lang === 'zh' ? item.feeName : item.feeEnName}", ${this.$t('OrderFee.feeAmt2')}${item.feeAmt}` // 金额
					let message = ''
					if (item.settleStatus === 'finish') {
						message = `${this.$t('OrderFee.feeSettled2')}${msgPrefix}，${this.$t('OrderFee.notModifyFee')}`
						...
						message = `${this.$t('OrderFee.feePartialSettled')}${msgPrefix}，${this.$t('OrderFee.notModifyFee')}`
						item.editGuardMsg = message
						showMessage && this.$message({ type: 'error', message, showClose: true, duration: 5000 })
						throw new Error('ending')
					} else if (item.invoiceStatus === 'finish' || item.invoiceStatus === 'part_finish') {
						message = `${this.$t('OrderFee.feeInvoiced2')}${msgPrefix}，${this.$t('OrderFee.notModifyFee')}`
						item.editGuardMsg = message
						showMessage && this.$message({ type: 'error', message, showClose: true, duration: 5000 })
						throw new Error('ending')
					} else if (item.settleLock === 'lock') {
						message = `${this.$t('OrderFee.feeLocked2')}${msgPrefix}，${this.$t('OrderFee.notModifyFee')}`
						item.editGuardMsg = message
						showMessage && this.$message({ type: 'error', message, showClose: true, duration: 5000 })
						throw new Error('ending')
					} else if (item.confirmStatus === 'yes') {
						message = `${this.$t('OrderFee.feeConfirmed')}${msgPrefix}，${this.$t('OrderFee.notModifyFee')}`
						item.editGuardMsg = message
						showMessage && this.$message({ type: 'error', message, showClose: true, duration: 5000 })
						throw new Error('ending')
					} else if (item.precloseStatus === 'yes') {
						message = `${this.$t('OrderFee.feePreClosed')}${msgPrefix}，${this.$t('OrderFee.notModifyFee')}`
						item.editGuardMsg = message
						showMessage && this.$message({ type: 'error', message, showClose: true, duration: 5000 })
						throw new Error('ending')
					} else if (item.closeStatus === 'yes') {
						message = `${this.$t('OrderFee.feeClosed')}${msgPrefix}，${this.$t('OrderFee.notModifyFee')}`
						item.editGuardMsg = message
						showMessage && this.$message({ type: 'error', message, showClose: true, duration: 5000 })
						throw new Error('ending')
					} else if (item.viewGroupType === 'special' && this.isBusinessFee) {
						message = `${this.$t('OrderFee.specialModifyOnSelf')}`
						item.editGuardMsg = message
						showMessage && this.$message({ type: 'error', message, showClose: true, duration: 5000 })
						throw new Error('ending')
					} else if (item.jointStatus === 'yes' && item.jointMain === 'no') {
						message = `${this.$t('OrderFee.jointFee')}${msgPrefix}，${this.$t('OrderFee.notModifyFee')}，${this.$t('OrderFee.modifyFromOpponent')}(${item.jointToOrderNo})` // `协同费用${msgPrefix}，不能修改和删除，请从对手单号${item.jointToOrderNo}发起修改`
						item.editGuardMsg = message
						showMessage && this.$message({ type: 'error', message, showClose: true, duration: 5000 })
						throw new Error('ending')
					}
		...
		debounceLs(fn, time = 600) {
			...
			this.timerTr = setTimeout(() => {
				if (!fn || !(fn instanceof Function)) {
					this.$message({ message: this.$t('OrderFee.paramError'), type: 'error', showClose: true })
				} else {
					fn()
				}
			}, time)
		...
		validate(data, keys, labelObj) {
			...
			if (data instanceof Array) {
				if (data.length === 0) {
					this.$message({
						type: 'error',
						message: this.$t('OrderFee.dataNotEmpty'),
						showClose: true,
						duration: 4000
					})
					return false
				...
				if (typeof (keys) === 'string') {
					for (let i = 0; i < data.length; i++) {
						...
						if (value == null || value === '') {
							...
							this.$message({
								type: 'error',
								message: `${label}${this.$t('OrderFee.required')}`, // `${label}不能为空`
								showClose: true,
								duration: 4000
							})
							return false
					...
					if (data[i] instanceof Object) {
						for (let j = 0; j < keys.length; j++) {
							...
							if (value == null || value === '') {
								...
								let label = column ? column.label : key
								label = label + this.$t('OrderFee.required') // label + '不能为空'
								let specialKeys = ['settleCorpName', 'settleCorpCode', 'feeName', 'feeEnName', 'feeCode', 'serviceName', 'serviceCode']
								if (specialKeys.includes(key)) {
									label = (column ? column.label : key === 'serviceCode' ? this.$t('OrderFee.serviceName') : key) + this.$t('OrderFee.mustDropdown') // '服务项'
								}
								if (labelObj && labelObj[key]) {
									label = labelObj[key] + this.$t('OrderFee.required') // labelObj[key] + '不能为空'
								}
								this.$message({
									type: 'error',
									message: `${label}`,
			...
			if (this.defaultColumns) {
				this.selfColumnsBase = this.columnsBaseLocal.filter(item => {
					if (this.isFeeList && item.prop === 'acctVoucherNos') { // 在订单费用显示为“凭证号”，在费用列表显示为“实收付凭证号”
						item.label = this.$t('OrderFee.actualRPVoucher') // '实收付凭证'
					}
				...
				this.selfColumnsBase = this.columnsBaseLocal.filter(item => {
					...
					if (this.overseaDelColumns.some(subItem => subItem === item.prop)) {
						return false
					}
					return this.defaultColumnsRecei.some(subItem => subItem === item.prop)
				})
			}
		...
		handleCustomColumns2 () {
			...
			if (this.defaultColumnsPay) {
				this.selfColumnsBase = this.columnsBaseLocal.filter(item => {
					...
					return this.defaultColumnsPay.some(subItem => subItem === item.prop)
				})
			}
			this.customColumnsPopShow = true
		},
    ...
    previewInvoiceFromList(invoiceReceivableId){
      invoiceReceivableInfo({ invoiceReceivableId }).then(res => {
        ...
        this.otherInvoiceInfo = {
          ...
          taxRate: res.taxRate * 100 === 0 ? '免税' : res.taxRate * 100 + '%', // 海外版不显示
    ...
    */
    finCommonSearch(dataQuery = [], searchOption, hms, gle, multipleSelection, otherParams = {}){
      if (!searchOption || !searchOption.addFilter || !searchOption.addFilter.searchInputGroup){
        return console.error(`searchOption ${this.$t('OrderFee.paramError')}`) // searchOption参数有误
      }
    ...
    finCommonColumns(res, tableOption = 'option1', selfColumnsBase = 'selfColumnsBase', configColumns = 'configColumns', params) {
      ...
      if (!res || !res.data || !this[tableOption] || !this[selfColumnsBase] || !this[configColumns]){
        if (process.env.NODE_ENV === "development"){
          // 开发环境时页面直接报弹窗错误
          this.$msgErrClose(this.$t('OrderFee.customColParamError')) // '自定义表头处理参数有误！'
        }
        // 其它环境时，为不影响用户直观体验，只打印报错日志
        console.error(this.$t('OrderFee.customColParamError')); // '自定义表头处理参数有误！'
        return
      }
    ...
    dealPaginationSelect(option, resData, list, key){
      ...
      option.data = list
		...
		setUnitCount(row, containerCount) {
			if (row.feeUnit === 'bill' || row.feeUnit === 'count' || row.feeUnit === 'm' || row.feeUnit === 'rt') {
				...
				if (!this.air) {
					this.$message({
						type: 'error',
						message: this.$t('OrderFee.nonAirNotOptional'),
						showClose: true
					})
					row.feeUnit = ''
				} else {
		...
		employeeSelectlistMx(queryString, item, row, index, option) {
			if (queryString) {
				...
				let optionId = option && option.id
				option && option.data.forEach(one => {
      ...
      if (value) {
		...
		employeeSelectlistSimp(queryString) {
      let data = {
        "state": "valid",
        "name": queryString
      }
    ...
    printCallbackBeforeCommon(times, callback, fn, callbackFail){
      if(times - 1 > 0) {
        this.$confirmWarn(`${this.$t('OrderFee.sureToPrint1')}${times}${this.$t('OrderFee.sureToPrint2')}`, () => { // `确定进行第${times}次打印？`
          callback && callback()
          fn && fn()
		...
		$tData(data) {
			let businessTypeObj = {
				en: {
					ship_export_fcl: 'Maritime Export FCL'
				},
				zh: {
					ship_export_fcl: '海运出口整箱'
				}
			}
			return businessTypeObj[this.$lang][data]
		}
	},
	computed: {

...
let arr = [
  ...
  // 'exPreclosingList', // 外账转字凭证列表接口  功能移至凭证文件接口acct.js
]
let API = {}
arr.forEach(item => {
...
// 一键修改财务日期
API['setNextCloseDate'] = function (data) {
  return request({
    url: `/fin/sysDate/setNextCloseDate`,
    method: 'post',
    data
  })
};
// 外账-订单转字费用详情
API['exOrderPreclosingFees'] = function (data) {
  return request({

...
import { 	custBlPortList } from '@/api/order/bl'
...
const getDefaultState = () => {
	return {
		...
		dictMap: {
			...
			// 凭证状态
			voucherStatus: [
				{ label: i18n.t('Common.generated'), value: 'yes' },
				{ label: i18n.t('Common.notGenerated'), value: 'no' }
			],
      // 关账状态
      finCloseStatus: [
        { label: i18n.t('Common.closed'), value: 'yes' },
        { label: i18n.t('Common.unclosed'), value: 'no' }
      ],
      // 确认状态
      webConfirmStatus: [
        { label: i18n.t('Common.confirmed'), value: 'yes' },
        { label: i18n.t('Common.unconfirmed'), value: 'no' }
      ],
			// 邮件发送状态
			emailStatus: [
				{ label: i18n.t('Common.sent'), value: 'yes' },
				{ label: i18n.t('Common.noSent'), value: 'no' }
			],
			validCurrency: [],
			validCurrencyNR: [],
      noOriginalCurrency: [],
      // 有无状态
      hasNo: [
        { label: i18n.t('Common.have'), value: 'yes' },
        { label: i18n.t('Common.none'), value: 'no' }
      ],
      // 前端字典 收款登记类型
      regTypeWeb: [
        { label: i18n.t('Collect.receiptReg'), value: 'receivable_reg' },
        { label: i18n.t('Collect.preCollect'), value: 'receivable_prerev' },
      ],
		}
	}
}
...
const mutations = {
	SET_DICT: (state, dictList) => {
		...
		dictList.forEach(item => {
			...
			} else if (['domesticCity', 'foreignCity', 'feeitemFinGroup', 'voucherClassDict'].includes(item.groupCode)) {
				item.itemList.forEach(child => {
				 child.label = lang === 'zh' ? child.itemCname : child.itemEname
				})
				Object.assign(dictValue, { [item.groupCode]: item.itemList })
        if (!['feeitemFinGroup', 'voucherClassDict'].includes(item.groupCode) ) return
				let obj = {}
				item.itemList.forEach(child => {
			...
			let arr = item.itemList.map((child, index) => {
				if (child.itemCode) {
					obj[child.itemCode.toLowerCase()] = lang === 'zh' ? child.itemCname : child.itemEname
					obj[child.itemCode] = lang === 'zh' ? child.itemCname : child.itemEname
					console.log
					return {
						label: key === 'airTransportTerm' ? `${lang === 'zh' ? child.itemCname : child.itemEname}${child.descpt}` : (lang === 'zh' ? child.itemCname : child.itemEname), //空运运输条款需要展示 字典说明
					...
					return {
						label: lang === 'zh' ? child.itemCname : child.itemEname,
						value: child.itemCode,
						en: child.itemEname,
						parent: child.parent
					}
...
const actions = {
	...
	getDictList({ commit, state }) {
		return new Promise((resolve, reject) => {
			getDictList()
				.then(response => {
					...
					if (feePayNegativeType && feeRecvNegativeType) {
						...
						data.push({
							descpt: i18n.t('OrderFee.negativeType'),
							groupCname: i18n.t('OrderFee.negativeType'),
          ...
          // 费用单类型
          let finBillBillType = data.find(v => v.groupCode === 'fin_bill_bill_type')
          if (finBillBillType) {
            finBillBillType.itemList = finBillBillType.itemList.filter(i => !i.itemCode.includes('dzg_'))
          }
					...
					supplierAllList({}).then(res => {
						if (res.data) {
							...
							let crmCategoryList = res.data.category.map(item => fn(item.categoryCode, item.cname, item.ename, ''))
							let crmBiztypeList = res.data.biztype.map(item => fn(item.businessType, item.name, item.ename, ''))
							...
							commit('SET_DICT', [
								...
								{ groupCode: 'crm_service', itemList: res.data.service.map(item => fn(item.serviceCode, item.serviceName, item.serviceEname, item.businessType)) },
					...
					feeItemClassList({}).then(res => {
						if (res.data) {
							let itemList = res.data.map(v => ({ label: v.cname, value: v.feeClassCode, en: v.ename, itemCname: v.cname, itemEname: v.ename }))
							commit('SET_DICT', [{ groupCode: 'feeitemFinGroup', itemList }])
						}
					})
          // 凭证class
          voucherClassList({ name: '' }).then(res => {
            let itemList = res.data.list.map(v => ({ label: v.name, value: v.code, en: v.code, cname: v.name }))
            commit('SET_DICT', [{ groupCode: 'voucherClassDict', itemList }])
          })
					// 服务类型
					baseBusinessList({}).then(res => {
						let itemList = []
						res.data.forEach(item => {
							item.serviceTypeList.forEach(a => itemList.push({ itemCname: item.name + '-' + a.name, itemEname: item.ename + '-' + a.ename, itemCode: a.serviceType }))
						})
						...
						let shipItemList = []
						res.data.forEach(item => {
							item.serviceTypeList.forEach(a => item.business === 'ship' && shipItemList.push({ itemCname: item.name + '-' + a.name, itemEname: item.ename + '-' + a.ename, itemCode: a.serviceType }))
						})
						// 获取报关单服务类型
						let customsItemList = []
						res.data.forEach(item => {
							item.serviceTypeList.forEach(a => ['ship_customs','ship_export_customs','ship_import_customs'].includes(item.businessType) && customsItemList.push({ itemCname: item.name, itemEname: item.ename, itemCode: a.serviceType }))
						})
						let airItemList = []
						res.data.forEach(item => {
							item.serviceTypeList.forEach(a => item.business === 'air' && airItemList.push({ itemCname: item.name + '-' + a.name, itemEname: item.ename + '-' + a.ename, itemCode: a.serviceType }))
						})
						...
						let shipItem = []
						let railItem = []
						let feeListItem = [{ itemCname: '费用单', itemEname: 'Finbill', itemCode: 'finbill' }]
						res.data.forEach(item => {
							if (item.business === 'ship') {
								shipItem.push({ itemCname: item.name, itemEname: item.ename, itemCode: item.businessType })
							}
							if (item.business === 'rail') {
								railItem.push({ itemCname: item.name, itemEname: item.ename, itemCode: item.businessType })
							}
							feeListItem.push({ itemCname: item.name, itemEname: item.ename, itemCode: item.businessType })
						})
	...
	basePortList({ commit }, param) {
		let { queryString, portAttribute, state, isEn, systemCode } = param
		return new Promise((resolve, reject) => {
			let fun = systemCode === 'CUST' ? custBlPortList : basePortList
			fun(queryString, portAttribute, state)
				.then(response => {
					let results = []
		...
		return new Promise((resolve, reject) => {
			baseSystemLineList(name, state, sysLineGroupCode)
				.then(response => {
					const results = []
					const list = response.data
          let lang = localStorage.getItem('local') || 'en-US'

					list.forEach((item, index) => {
						if (item.sysLineCode) {
							results.push({
								key: item.sysLineCode,
								value: lang === 'en-US'? item.ename :  item.cname
							})
						}
					})
	...
	querySettleUnit({ commit }, payload, needAll) {
      ...
      if (payload.isForbid){
        data[payload.receipayType === 'receive' ? 'flitByRecvFeeForbid' : 'flitByRecvFeeForbid'] = 'yes'
      }
				...
				.then(response => {
					...
					data &&
						data.map(item => {
							...
							result.push(
								Object.assign(item, {
									label: item.code === 'ORIGINAL' ? lang === 'zh' ? '原币别' : 'ORIGINAL' : item.code,
									value: item.code
								})
							)
						})
				...
				.then(res => {
					...
					list.forEach(item => {
						...
						results.push(
							Object.assign(item, {
								key: item.feeCode,
								value: lang === 'zh' ? item.feeName : item.feeEnName // item.feeName
							})
						)
					})
					resolve(results)
			...
			queryFeeItemList(data)
				.then(res => {
					...
					list.forEach(item => {
						...
						results.push(
							Object.assign(item, {
								label: lang === 'zh' ? item.feeName : item.feeEnName, // params.lang === 'en' ? item.feeEnName : item.feeName,
								value: item.feeCode
							})
						)
					})
		...
		return new Promise((resolve, reject) => {
			...
			reqMethod(data)
				.then(res => {
					...
					list.forEach(item => {
						results.push({
							label: lang === 'zh' ? item.serviceName : item.serviceEname, // item.serviceName,
							...
							if (
								...
								results.push(
									Object.assign(subItem, {
										label: lang === 'zh' ? subItem.serviceName : subItem.serviceEname, // subItem.serviceName,

    ...
    return new Promise(resolve => {
      ...
      // } else {
        allMenuCheckAuth({}).then(res => {
          ...
          let pagesFront = []
          if (effect) {
            // 配置时，粘贴复制极易出现空格，不易找错，此处强制去掉空格(所有空格，后续如有特殊需求要保留空格，则改为递归去掉resourceCode内空格) // 废弃此处理，英文菜单含有空格，20221221
            // let str = JSON.stringify(res.data).replace(/\s/g, '')
            res.data = res.data.filter(item => item.resourceCode === 'Finance') // JSON.parse(str).filter(item => item.resourceCode === 'Finance') // 子系统只显示财务菜单
          ...
          commit('SET_ROUTES', asyncRoutes)
          commit('SET_MENU_ROUTES', menusFront.length ? menusFront[0].children : menusFront) // routes
...
function generateMenusFrontSpecific(menus, routesTiled, isSubMenu) {
  ...
  menus.forEach(menu => {
    ...
    if (menuFront) {
      ...
      menuFront.meta.icon = menuFront.meta.icon ? menuFront.meta.icon : '' // isSubMenu ? '' : menuFront.meta.icon
      menuFront.routeCode = menu.routeCode ? menu.routeCode : '' // menuFront.resourceUrl = ['sw', 'adm'].includes(menu.sysCode) ? menu.resourceUrl : ''
      if (menu.subMenu.length > 0) {
      ...
      //   menuFront.meta.icon = name === 'Wms' ? 'el-icon-wms' : ''
      //   menuFront.children = generateMenusFrontSpecific(menu.subMenu, routesTiled)
      //   menusFront.push(menuFront)
      // } else {
      //   if (menu.sysCode === 'wms') {
      //     menuFront = { path: '', name: menu.menuCode, meta: { title: menu.menuName } }
      //     menuFront.routeCode = menu.routeCode ? menu.routeCode : ''
      //     menusFront.push(menuFront)
      //   } else {

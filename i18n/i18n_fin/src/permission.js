...
const whiteList = ['/login', '/forget', '/order/order/sowoll/blList', '/order/order/sowoll/custBlCopyPush'] // no redirect whitelist
const whitePerm = ['/dashboard', '/finance/finGeneral/finWelcome'] // permission whitelist
...
router.beforeEach(async (to, from, next) => {
  ...
  if (!ISCHROMIUM) {
    next({ path: '/404' })
    NProgress.done()
    window.alert(i18n.t('Finance.browserTips'))
    return
  }
  // 设置title
  ...
  if (hasToken) {
    if (to.path === '/login') {
      ...
      if (hasRoles) {
        ...
        if (effect && whitePerm.indexOf(to.path) === -1) { // effect && to.path !== '/dashboard'
          ...
          if (isEffectModule && !checkAuth(to.name) && !isCRM) {
            NProgress.done()
            // 财务系统默认打开的是费用列表，无权限时处理一下
            if (to.name === 'SettlementFeeList') {
              next({ path: '/finance/finGeneral/finWelcome' })
            } else {
              // Message.error('无权限访问')
              // 上面这个测试环境报错了，暂时没找到原因，改用下面这个方式
              Message({
                message: i18n.t('Finance.noPermiss'),
                type: 'error',
                duration: 2000
              })
            }
        ...
        if (ELMSGBOXWRAP && ELMSGBOXWRAP.style.display !== 'none') {
          ...
          setTimeout(() => {
            if (ELMSGBOXWRAP && ELMSGBOXWRAP.style.display !== 'none') {
              Message({
                type: 'error',
                message: i18n.t('OrderFee.confirmOpBeforeLeave'), // 请先完成确定或取消操作，再切换页面
                showClose: true,
                duration: 5000
              })
              NProgress.done()
        ...
        try {
          ...
          // await store.dispatch('charge/getValueByKeys')

<template>
  <div class="navbar">
    ...
    >
      ...
      <span slot="footer">
        <el-button size="mini" type="primary" @click="alertSure">{{$t('Common.modify')}}</el-button>
        <el-button size="mini" @click="alertPwdShow=false">取消</el-button>
      </span>
    </el-dialog>
    <div v-if="emailBookPopShow">
      <emailBookPop @close="emailBookClose" />
    </div>
  </div>
</template>
...
import emailBookPop from './emailBookPop'
...
export default {
  ...
  data() {
    return {
      ...
      host: location.host,
      emailBookPopShow: false
    }
  },
  methods: {
    emailBook() {
      this.emailBookPopShow = true
    },
    emailBookClose(action) {
      this.emailBookPopShow = false
    },
    toggleSideBar() {

    ...
    <template v-if="hasOneShowingChild(item.children,item) && (!onlyOneChild.children||onlyOneChild.noShowingChildren)&&!item.alwaysShow">
      <app-link v-if="onlyOneChild.meta" :to="onlyOneChild.name" :route="onlyOneChild"><!-- :to="resolvePath(onlyOneChild.path)" -->
        <el-menu-item :index="onlyOneChild.name" :class="{'submenu-title-noDropdown':!isNest}" class="el-menu-item__title"><!-- :index="resolvePath(onlyOneChild.path)" -->
          ...
          <div class="overseas-freight-nu-read" v-if="item.name==='OverseasFreight' && unReadCount">{{unReadCount}}</div>
        </el-menu-item>
      </app-link>
    </template>
    <el-submenu v-else class="left-menu" ref="subMenu" :index="item.name"><!-- :index="resolvePath(item.path)" -->
      <template slot="title">
        <div>
          ...
          <div class="charge-overseas-freight-nu-read" v-if="item.name==='ChargeSearch' && unReadCount">{{unReadCount}}</div>
          <div class="order-overseas-freight-nu-read" v-if="item.name==='SeaShipping' && unReadCount">{{unReadCount}}</div>
        </div>
      </template>
      <sidebar-item
...
export default {
  ...
  computed:{
    ...mapState({
      unReadCount: (state) => state.charge.overseasFreight.unReadCount
    })
  },
  methods: {
...
.charge-overseas-freight-nu-read{
  position: absolute;
  top: 9px;
  left: 90px;
  height: 14px;
...
.order-overseas-freight-nu-read{
  position: absolute;
  top: 16px;
  left: 64px;
  height: 14px;

<template>
	<el-form-item v-if="!item.hide" class="el-form-block" :required="item.required" :label="item.label" :prop="item.prop" :show-message="false">
		...
		<div class="box-address-flex" v-if="item.type === 'address'">
			<el-input size="mini" :placeholder="$t('Common.plEnter')" v-model="form[item.key]" clearable :show-word-limit="item.showWordLimit" :maxlength="item.maxlength" class="box-address-input"></el-input>

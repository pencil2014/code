<template>
	<div class="operate-group">
		<template>
			<ButtonTip v-for="(item, key) in operationBtns.data.slice(0, getLen())" :key="key"
				v-show="displayOperationBtn(item, scope.row, key)"
				:id="item.id ? item.id + scope.$index : ''"
				:btnType="item.type || 'text'" 
				:btnDisabled="typeof item.disabled === 'function' ? item.disabled(item, scope.row) : item.disabled" 
				:btnText="item.label" 
				:tipContent="item.tipContent" 
				:class="['operateBtn-tip', item.className]" 
				:btnClassName="item.btnClassName"
				@click="handleOperate(scope, item, operationBtns, $event)">
			</ButtonTip>
			<!-- <el-button
				size="mini"

<template>
	<div class="orig-so-container">
		<div class="orig-cont">
			...
			<!-- 箱型箱量 -->
			<el-form-item id="so-list-form" class="so-row-cont" v-for="(o, index) in soContainerList" :key="index" prop="">
				...
				<div v-show="!o.containerListShowState" style="padding: 8px 0 5px">
					<el-form class="row-flex flex-so" v-for="(item, sIndex) in o.containerList" :key="sIndex" :model="soContainerList[index]" ref="containerList">
						<div class="row-flex-cont" :style="{width: isBkg ? '100%' : ''}">
							<div class="flex">
								...
								<el-form-item class="item-cell" label="箱量" :prop="`containerList.${sIndex}.containerNum`" :show-message="false" :rules="[{ required: true, trigger: 'blur' }]">
								...
								<el-form-item class="item-cell specialWidth" label="单柜件数" :prop="`containerList.${sIndex}.quantity`" :show-message="false">
									<el-input clearable size="mini" v-model.number="item.quantity" placeholder="请输入" style="width: 100px" :disabled="true"></el-input>
								</el-form-item>
								<el-form-item class="item-cell" label="单柜重量" :prop="`containerList.${sIndex}.weight`" :show-message="false" :rules="[{ required: true, trigger: 'blur' }]">
							...
							<div class="flex">
								<el-form-item class="item-cell specialWidth" label="特种长宽高" required v-if="item.containerType && (item.containerType.toUpperCase().indexOf('OT') !== -1 || item.containerType.toUpperCase().indexOf('FR') !== -1)">
									<el-col :span="8">
										<el-form-item :prop="`containerList.${sIndex}.specialLength`" :show-message="false" :rules="[{ required: true, trigger: 'blur' }]">
									...
									<el-col :span="8">
										<el-form-item :prop="`containerList.${sIndex}.specialWidth`" :show-message="false" :rules="[{ required: true, trigger: 'blur' }]">
									...
									<el-col :span="8">
										<el-form-item :prop="`containerList.${sIndex}.specialHeight`" :show-message="false" :rules="[{ required: true, trigger: 'blur' }]">
								...
								<div class="item-cell" v-if="item.containerType && (item.containerType.toUpperCase().indexOf('OT') !== -1 || item.containerType.toUpperCase().indexOf('FR') !== -1)">
									...
									<el-input clearable size="mini" v-model="item.sizeRight" placeholder="请输入" style="width: 110px;margin-right: 5px" :disabled="true">
										<template slot="prepend">右</template>
										<template slot="append">m</template>
									</el-input>
									<el-input clearable size="mini" v-model="item.sizeTop" placeholder="请输入" style="width: 110px;" :disabled="true">
										<template slot="prepend">高</template>
										<template slot="append">m</template>
									</el-input>
								</div>
								...
								<el-form-item
									...
									:show-message="false"
									:rules="[{ required: true, trigger: 'blur', item: item }]"
...
export default {
	...
	created() {
		// // bkg默认收起
		// if (this.isBkg) {
		// 	this.containerListShowState = true
		// }
	},
	mounted() {},
	computed: {
	...
	methods: {
		...
		handleToogleExpand(index) {
			let { containerListShowState } = this.soContainerList[index]
			this.$set(this.soContainerList[index], 'containerListShowState', !containerListShowState)

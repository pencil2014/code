						...
						<template v-else>
							<template v-if="$route.query.source !== 'jointList'">
								...
								<template v-if="isBd">
									...
									<!-- 代订舱的审核通过的 -->
									<el-button size="mini" type="default" class="submitCancel" @click="handleProxyCancel" :disabled="submitDisabled" v-if="createItem.orderStatus === 'pass' && createItem.serviceList.includes('proxy_whbook') && createItem.submitCancelStatus !== 'APPLIED'">撤回</el-button>
							...
							<template v-if="(isDoc|| isOp) && hasDestinationServices">
								...
								<template v-if="isOp">
									...
									<el-button size="mini" type="danger" class="" @click="handleOpCancelRequest('break')" :disabled="submitDisabled" v-if="['pass'].includes(createItem.orderStatus) && isOrderFeeCount && !['CANCEL','BREAK'].includes(createItem.cancelJointHandleStatus)">终止</el-button>
									<el-button size="mini" type="danger" class="" @click="handleOpCancelRequest('cancel')" :disabled="submitDisabled" v-if="['pass'].includes(createItem.orderStatus) && !isOrderFeeCount && !['CANCEL','BREAK'].includes(createItem.cancelJointHandleStatus)">取消</el-button>
			...
			<div class="order-cont">
				<el-form ref="orderSubmit" :model="createItem" :rules="rules" :show-message="false" class="order-cont-form lcl-form">
					...
					<div class="ship-cont" ref="shipInfoCont" :class="workAssignShow ? 'abs-toogle' : ''" :style="{ height: `${shipInfoHeight}px` }">
						...
						<OrderBookInfo
						 	@change="handleChangeIncludeFeeder" :includeFeeder="includeFeeder"
						...
						/>
						<OverseasInfo :createItem="createItem" :businessType="createItem.businessType" :bdId="bdId" :polCode="createItem.ship.polPortCode" :podCode="createItem.ship.unloadingPortCode" @updateGetInfo="updateGetInfo" :orderStatus="createItem.orderStatus" />
						<!-- 货物信息 -->

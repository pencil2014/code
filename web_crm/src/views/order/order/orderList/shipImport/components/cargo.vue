<template>
	<div class="cargo-form">
		<div class="row-flex flex-cargo" v-for="(o, index) in createItem.cargoList" :key="index">
			<div class="btn-plus-search" @click="handleAddCargo" v-if="!viewState && !(isOp && ordDetRole != 'op')"></div>
			<div class="btn-minus-search" @click="handleDeleteCargo(index)" v-if="index > 0 && !viewState && !(isOp && ordDetRole != 'op')"></div>
			<div class="cargo-item hscode" v-if="isOp">
				<div class="item-label"><span class="lab">货品简称</span></div>
				<div class="item-content">
					<el-input size="mini" v-model="o.shortCname" clearable placeholder="请输入" :disabled="viewState && isOp && ordDetRole != 'op'">></el-input>
				</div>
			</div>
			...
			<div class="cargo-item hscode" v-if="isOp">
				...
				<div class="item-content">
					<el-input size="mini" v-model.number="o.quantity" clearable placeholder="请输入" :disabled="viewState && isOp && ordDetRole != 'op'"></el-input>
				</div>
			</div>
			<div class="cargo-item hscode" v-if="isOp">
				<div class="item-label"><span class="lab">总毛重</span></div>
				<div class="item-content">
					<el-input size="mini" v-model="o.weight" clearable placeholder="请输入" :disabled="viewState && isOp && ordDetRole != 'op'"
						>>
			...
			<div class="cargo-item hscode" v-if="isOp">
				...
				<div class="item-content">
					<el-input size="mini" v-model="o.volume" clearable placeholder="请输入" :disabled="viewState && isOp && ordDetRole != 'op'"
						>>
			...
			<div class="cargo-item hscode" v-if="isOp">
				...
				<div class="item-content">
					<el-select size="mini" v-model="o.packageInfo" style="width: 100%" clearable placeholder="请选择" :disabled="viewState && isOp && ordDetRole != 'op'" filterable>
			...
			<div class="cargo-item hscode" v-if="isOp">
				...
				<div class="item-content">
					<el-input size="mini" v-model="o.length" clearable placeholder="请输入" :disabled="viewState && isOp && ordDetRole != 'op'"
						>>
			...
			<div class="cargo-item hscode" v-if="isOp">
				...
				<div class="item-content">
					<el-input size="mini" v-model="o.width" clearable placeholder="请输入" :disabled="viewState && isOp && ordDetRole != 'op'"
						>>
						<template slot="append">m</template>
					</el-input>
				</div>
			...
			<div class="cargo-item hscode" v-if="isOp">
				...
				<div class="item-content">
					<el-input size="mini" v-model="o.height" clearable placeholder="请输入" :disabled="viewState && isOp && ordDetRole != 'op'"
						>>
						<template slot="append">m</template>
					</el-input>
				</div>
			...
			<div class="cargo-item hscode" v-if="isOp" style="width: 47.4%">
				...
				<div class="item-content">
					<el-input size="mini" v-model="o.mark" clearable placeholder="请输入" :disabled="viewState && isOp && ordDetRole != 'op'">></el-input>
				</div>
			</div>
			<el-form-item class="cargo-item hscode" label="HS Code" :prop="`cargoList.${index}.hscode`" :show-message="false" :rules="{ required: isBd, message: '请选择海关编号', trigger: 'change' }">
				...
				<el-select
					...
					clearable
					placeholder="请选择海关编号"
					:disabled="(viewState || isOp) && ordDetRole != 'op'"
			...
			<el-form-item class="cargo-item hscode" label="中文品名" :prop="`cargoList.${index}.cname`" :show-message="false" :rules="{ required: isBd, message: '请输入中文品名', trigger: 'blur' }">
				...
				<el-input v-else size="mini" v-model="o.cname" clearable placeholder="请输入中文品名" :disabled="(viewState || isOp) && ordDetRole != 'op'" maxlength="128"></el-input>
			</el-form-item>
			<el-form-item class="cargo-item hscode" label="英文品名" :prop="`cargoList.${index}.ename`" :show-message="false" :rules="{ required: isBd, message: '请输入英文品名', trigger: 'blur' }">
				...
				<el-input v-else size="mini" v-model="o.ename" clearable placeholder="请输入英文品名" :disabled="(viewState || isOp) && ordDetRole != 'op'" maxlength="128"></el-input>
			</el-form-item>
			<div class="cargo-item hscode">
				<div class="item-label"><span class="lab">货品分类</span></div>
				<div class="item-content">
					<el-select size="mini" v-model="o.commodityTypeCode" style="width: 100%" filterable clearable placeholder="请选择货品分类" :disabled="(viewState || isOp) && ordDetRole != 'op'">
			...
			<el-form-item class="cargo-item hscode" label="货物性质" :prop="`cargoList.${index}.property`" :show-message="false" :rules="{ required: isBd, message: '请选择货物性质', trigger: 'change' }">
				<el-select size="mini" v-model="o.property" style="width: 100%" clearable placeholder="请选择货物性质" :disabled="(viewState || isOp) && ordDetRole != 'op'" @change="handleChangeProperty(o)" >
			...
			<el-form-item class="cargo-item hscode" label="装箱要求" :prop="`cargoList.${index}.boxRemark`" :show-message="false" :rules="{ required: isBd, message: '请输入装箱要求', trigger: 'blur' }">
				<el-input size="mini" v-model="o.boxRemark" @focus="handleClear(o)" @blur="handleBlur(o)" placeholder="如无装箱要求，填无" :disabled="(viewState || isOp) && ordDetRole != 'op'"> </el-input>
			</el-form-item>
			<el-form-item
			...
			<el-form-item class="cargo-item hscode" label="危险等级" v-if="o.property === 'dangerous'" :show-message="false" :prop="`cargoList.${index}.dangerousGrade`" :rules="{ required: isBd, message: '请选择危险等级', trigger: 'change' }">
				<el-select size="mini" v-model="o.dangerousGrade" style="width: 100%" clearable placeholder="请选择危险等级" :disabled="(viewState || isOp) && ordDetRole != 'op'">
			...
			<el-form-item class="cargo-item hscode" label="国际危码" :prop="`cargoList.${index}.dangerousCode`" :show-message="false" v-if="o.property === 'dangerous'" :rules="{ required: isBd, message: '请选择国际危码', trigger: 'change' }">
				<el-select size="mini" v-model="o.dangerousCode" style="width: 100%" clearable placeholder="请选择国际危码" :disabled="(viewState || isOp) && ordDetRole != 'op'" remote filterable :remote-method="val => queryCargoUnno(val)">
			...
			<div class="cargo-item hscode" v-if="o.property === 'dangerous'">
				...
				<div class="item-content">
					<el-input size="mini" v-model="o.packageGroup" maxlength="64" clearable placeholder="请输入包装组别" :disabled="(viewState || isOp) && ordDetRole != 'op'"></el-input>
				</div>
			</div>
			<el-row class="flex-item-commodityDesc">
				<el-form-item class="form-cont" label="货物描述" :prop="`cargoList.${index}.commodityDesc`">
					...
					<el-input v-else type="textarea" :autosize="{minRows: 1, maxRows: 4}" size="mini" v-model="o.commodityDesc" clearable placeholder="请输入货物描述" :disabled="(viewState || isOp) && ordDetRole != 'op'" maxlength="512"> </el-input>
				</el-form-item>
			</el-row>
		</div>
	</div>
...
export default {
	...
	computed: {
		...
		viewState() {
			...
			} else if (!this.ordDetRole || this.isDoc || this.isAc || this.isOpTruck || this.isCsc || this.isBkg) {
				return true
			} else {
				return false
			}

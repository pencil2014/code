<template>
	<div class="edit-row" id="bl" v-loading="lsLoading"
		...
		<div v-if="$route.query.source != 'SiListAddBl'" class="edit-container so" id="blTableCont">
			<div class="row-tit so">
				...
				<div class="row-tit-operate">
					<el-button @click="handleImportSubBl" v-if="['st01', 'st15', 'st09'].includes(orderInfoDataObj.serviceType) && showRelateBtn && isDoc" class="mr10" type="primary" size="mini">导入关联提单</el-button>
					<el-dropdown size="small" @command="handleCommand">
						<el-button size="mini" type="default" :disabled="isBd && !acIsExport" class="mr10"> 输出<i class="el-icon-arrow-down el-icon--right"></i> </el-button>
			...
			<div class="row-form mt10 table-com-layout">
				...
				>
					...
					<el-table-column label="操作" width="280" align="right" fixed="right" type="">
						<template slot-scope="scope">
...
export default {
		...
		return {
			...
			tableConfig: {
				...
				allColumns: [
					...
					{ label: '补料要求', prop: 'siRemark', width: 100 },
					...
					{ label: '附件', prop: 'blFileNum', width: 120 },
					{ label: '保险', prop: 'insuranceNum', width: 120 },
					{ label: '贸易单证', prop: 'documentNum', width: 120 },
	...
	methods: {
		...
		getBlList() {
			this.lsLoading = true
			blPageList({ orderNo: this.$route.query.orderNo, jointNo: this.$route.query.jointNo, columns: this.tableQuery.columns }).then(res => {
...
// }
#bl .table-com-layout .bl-table .operate-group .el-button--mini {
	&:last-child {
		margin-right: 0px;
	}
}

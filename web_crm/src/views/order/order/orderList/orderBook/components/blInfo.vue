	...
	<div class="row-flex ml10" id="blInfo">
		...
		<el-row>
			...
			<el-form-item label="提单备注(打印)" prop="remarkPrint" :show-message="false" class="flex-item col-3 shipper-rel">
				<el-input @focus="handleFocus('remarkPrint')" @dblclick.native="editDetail('remarkPrint')" type="textarea" :rows="6" maxlength="512" show-word-limit size="mini" placeholder="请输入" v-model="createItem.remarkPrint" :disabled="disabledState"></el-input>
			</el-form-item>
			...
			<el-form-item label="大船实际开船时间" prop="orderInfo.tdTime" :show-message="false" class="flex-item vessel-time">
				...
				<el-popover placement="top" width="280" trigger="click" class="vessel-time-popover" id="schedulePopover" ref="schedulePopover">
					<Schedule :vesselTime="vesselTime"
						:tdTimeDisabled="disabledState"
						:etaTimeDisabled="disabledState"
    ...
    updateVesselTime(val, key) {
      this.createItem.orderInfo[key] = val
		...
		getUserCompanyMethod() {
           // 获取当前登录账号所属公司信息
		   this.$store.dispatch('order/getUserCompanyInfoAction')
		   .then(signAddress => {
		...
		handleSearchVesselTime() {
			...
			this.$store
				.dispatch('dict/queryBookVessleTime', param)
				.then(data => {

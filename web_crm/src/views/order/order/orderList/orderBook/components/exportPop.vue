<template>
	<BaseDialog :config="dialogConfig" :callback="dialogCallback">
		<el-form ref="exportForm" :rules="rules" :model="createItem" label-position="right" label-width="120px" class="orderBlExport-form">
			...
			<el-form-item label="输出类型" prop="docType">
				<el-row>
					<el-col :span="8" v-for="(item, index) in docTypeOptions" :key="index">
						<el-radio :disabled="['hbl_telex_original', 'hbl_telex_original0', 'hbl_telex_original_supChain', 'hbl_telex_original0_supChain', 'usa_sign'].includes(createItem.fileType) && item.value !== 'pdf'" @change="handleChangeFileType" size="mini" v-model="createItem.docType" :label="item.value">
							{{ item.label }}
						</el-radio>
					</el-col>
				</el-row>
			...
			<el-form-item label="显示签名" prop="isSign" v-if="exportTypeQuery.value === 'hbl_confirm' && ['hbl_telex_original', 'hbl_telex_original0', 'hbl_telex_original_supChain', 'hbl_telex_original0_supChain', 'usa_sign'].includes(createItem.fileType)">
				<el-row>
					<el-col :span="4" v-for="(item, index) in mblShowOptions" :key="index">

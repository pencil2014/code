      ...
      <el-collapse-item name="2">
		...
		<div v-else class="work-assign-item" v-for="(item, index) in orderWorkOptions" :key="item.jobCode" :id="'work-' + item.jobCode" :class="{ warning: item.isUpdate}">
			<div class="item-label" v-if="pricingHide(item)">
				<span>{{ item.jobCname }}</span>
				<el-popover popper-class="emplpyeeName-pop" placement="left" width="240" @show="handleTogglePopOver(item.employeeId)" trigger="click">
					<div class="employeeName-list">
						<ul>
							<li v-if="employeeInfo.telephone">座机号：{{employeeInfo.telephone}}</li>
              <li v-else>手机号：{{employeeInfo.mobileNo}}</li>
							...
							<div class="joint-work-row" v-if="item.jobUserList && item.jobUserList.length">
								...
								<div class="val" v-for="(ele, b) in item.jobUserList" :key="'user' + b">
									<div class="label">
										...
										<el-popover popper-class="emplpyeeName-pop" placement="left" @show="handleTogglePopOver(item.employeeId)" width="240" trigger="click">
											<div class="employeeName-list">
												<ul>
													<li v-if="employeeInfo.telephone">座机号：{{employeeInfo.telephone}}</li>
                  				<li v-else>手机号：{{employeeInfo.mobileNo}}</li>

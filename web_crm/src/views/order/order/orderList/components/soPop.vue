<template>
	<div>
		<BaseDialog :config="dialogConfig" :callback="dialogCallback">
			<div v-if="['ship_export_fcl', 'ship_export_lcl'].includes(soListQuery.businessType)" class="ship-data">
				<el-table key="ship-data" fit highlight-current-row style="width: 100%" :data="tableData" @cell-mouse-enter="showTooltip" @cell-mouse-leave="hiddenTooltip" class="sopop-table">
			...
			<div v-if="['rail_export_fcl', 'rail_export_lcl'].includes(soListQuery.businessType)" class="rail-data">
				<el-table key="rail-data" fit highlight-current-row style="width: 100%" :data="tableDataRail" @cell-mouse-enter="showTooltip" @cell-mouse-leave="hiddenTooltip" class="sopop-table">
					<el-table-column prop="po" label="入仓单号" align="center"> </el-table-column>
					<el-table-column prop="containerInfo" label="箱型箱量" align="center"> </el-table-column>
...
import { blSoList, railblSoList } from '@/api/order/bl'
...
export default {
	...
	methods: {
		...
		getSoList() {
			...
			if (['rail_export_fcl', 'rail_export_lcl'].includes(this.soListQuery.businessType)) {
				railblSoList(data).then(res => {
					this.tableDataRail = res.data
				})
			}
			if (['ship_export_fcl', 'ship_export_lcl'].includes(this.soListQuery.businessType)) {

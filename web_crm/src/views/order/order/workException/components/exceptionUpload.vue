<template>
  <div class="upload-input-box" :class="{'disabled-input':disabled}">
    <label :for="refId" class="file-label" :class="{'has-files':!((!fetchFileList||JSON.stringify(fetchFileList)=='[]')&&localFileList.length===0)}">
      <input type="file" :id="refId" name="file" :disabled="disabled" multiple="multiple" @change="selectFile($event)" style="display:none">
      <p class="placeholder-text" v-if="(!fetchFileList||JSON.stringify(fetchFileList)=='[]')&&localFileList.length===0">{{placeholder}}</p>
    ...
    <ul class="file-list" v-if="!((!fetchFileList||JSON.stringify(fetchFileList)=='[]')&&localFileList.length===0)">
      <li class="file-item" v-for="(item) in fetchFileList" :key="item.fileNo">
        <div class="file-info"><span class="file-name" title="预览" @click="handleView(item.fileName,item.fileNo)">{{item.fileName}}</span>
          <i class="el-icon-close" v-if="!disabled" title="删除" @click="delFile(item.fileNo)"></i>
          <span class="file-download" title="下载" @click="downloadFile(item.fileName,item.fileNo)">下载</span>
        </div>
      </li>
      ...
      if (bigSizeFiles.length) {
        ...
        for (let i = 0; i < this.localFileList.length; i++) {
          if (this.localFileList[i].size > 0) {
            formdata.append('fileName', this.localFileList[i])
          } else {
        ...
        if (emptyFiles.length) {
          this.$message(
            `您选择的(${emptyFiles.join(',')})为空文件，文件不能上传！`
          )
        }

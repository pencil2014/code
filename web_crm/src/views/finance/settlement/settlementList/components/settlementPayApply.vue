...
<div class="finance-page settlement-pay-apply-detail"
  ...
  <div class="finance-detail-page">
    <div class="finance-list-container">
      <div class="money-box money-box-border-bottom">
        <div class="money-box-left">
          <div class="money-title">{{applyNo ? '付款申请详情' : '结算付款申请'}}</div>
        </div>
        <div v-if="!applyNo" class="money-box-right">
          <el-button-group>
            <el-button class="finance-btn" @click="submit" type="primary" size="mini">提交申请</el-button>
          </el-button-group>
          <el-button-group>
        ...
        <div v-if="applyNo" class="money-box-right">
          <el-button-group>
            <el-button type="success" v-if="!isApproved && checkAuth($route.name, 'btn-settlementPass')" class="finance-btn" @click="approvePass('pass')" size="mini">审批通过</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="!isApproved && checkAuth($route.name, 'btn-settlementRefuse')" class="finance-btn" @click="refuseDialog" type="danger" size="mini">审批拒绝</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="!isStatus && checkAuth($route.name, 'btn-handleWithdrawPayApplyAudit')" class="finance-btn" @click="handleWithdrawPayApplyAudit" size="mini" type="purple">撤销付款审核</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="!isApproved && checkAuth($route.name, 'btn-handleSettleSubmit')" class="finance-btn" @click="handleSettleSubmit" size="mini" type="">保存</el-button>
          </el-button-group>
          <el-button-group>
    ...
    <div class="settlement-detail-form">
      <h3 style="font-size:12px">基本信息</h3>
      <el-form v-show="!applyNo" label-width="100px" :model="payApplyDetailForm" ref="payApplyDetailForm" :inline="true" size="mini" :rules="rules" class="settlement-detail-form-1">
        <el-form-item label="结算单位：" prop="recvCorpCode" class="flex-item">
          <el-select
        ...
        <el-form-item v-if="blackType" label="黑名单类型：">
        ...
        <DetailDiv style="width:268px" label="应收核销状态：" :content="renderNoEdit(payApplyDetailForm, {prop: 'recvWriteoffStatus', propInDict: 'writeoffStatus', type: 'select'})"/>
        <DetailDiv style="width:268px" label="应收金额：" :content="payApplyDetailForm.orderReciSumAmt" :isTooltip="true"/>
        <DetailDiv style="width:268px" label="应收未核销金额：" :content="payApplyDetailForm.orderReciNoWriteOffAmt" :isTooltip="true"/>
        <DetailDiv style="width:268px" label="应收已核销金额：" :content="payApplyDetailForm.orderReciWriteOffAmt" :isTooltip="true"/>
        <!-- <el-form-item label="应收核销状态：" prop="recvWriteoffStatus">
          <el-input	:value="renderNoEdit(payApplyDetailForm, {prop: 'recvWriteoffStatus', propInDict: 'writeoffStatus', type: 'select'})" disabled	placeholder="请输入"></el-input>
        </el-form-item>
        <el-form-item label="应收金额：" prop="orderReciSumAmt">
          <el-input	v-model="payApplyDetailForm.orderReciSumAmt" disabled	placeholder="请输入"></el-input>
        </el-form-item>
        <el-form-item label="应收未核销金额：" prop="orderReciNoWriteOffAmt">
          <el-input	v-model="payApplyDetailForm.orderReciNoWriteOffAmt" disabled	placeholder="请输入"></el-input>
        </el-form-item>
        <el-form-item label="应收已核销金额：" prop="orderReciWriteOffAmt">
          <el-input	v-model="payApplyDetailForm.orderReciWriteOffAmt" disabled	placeholder="请输入"></el-input>
        </el-form-item> -->
        <el-form-item label="最晚付款时间：" :prop="!applyNo ? 'latestDate' : ``" >
          <el-date-picker
            size="mini"
        ...
        <el-form-item label="申请附件：" prop="recvFinishsProveFileNo">
        ...
        <el-form-item label="申请备注：" label-width="100px" class="flex-item" prop="">
          <el-input
            type="textarea"
            autosize
            v-model="payApplyDetailForm.remark"
            placeholder="申请备注，最多500个字"
            maxlength="500"
            show-word-limit>
          </el-input>
        </el-form-item>
      ...
      <div v-show="applyNo">
        <DetailDiv  label="付款申请号：" :content="payApplyDetailForm.applyNo"/>
        <DetailDiv  label="结算单号：" :content="payApplyDetailForm.settleOrderNo" :isTooltip="true"/>
        <DetailDiv  label="结算单位：" :content="payApplyDetailForm.recvCorpName" :isTooltip="true"/>
        <DetailDiv  label="是否代收付：" :content="renderNoEdit(payApplyDetailForm, {prop: 'hasAgentPay', propInDict: 'yesNo', type: 'select'})"/>
        <DetailDiv  label="结算周期：" :content="renderNoEdit(payApplyDetailForm, {prop: 'settleCycle', type: 'select'})"/>
        <DetailDiv  label="结算币种：" :content="payApplyDetailForm.currency"/>
        <DetailDiv  label="航线：" :content="payApplyDetailForm.sysLineName" :isTooltip="true"/>
        <DetailDiv  label="申请人：" :content="payApplyDetailForm.createdName"/>
        <DetailDiv  label="申请时间：" :content="payApplyDetailForm.createdTime"/>
        ...
        <el-form label-width="100px" :model="payApplyDetailForm" :inline="true" ref="payApplyDetailForm" style="display:inline-block;">
        ...
        <DetailDiv v-if="payApplyDetailForm.payOrderRefuseReason"  label="出纳拒绝原因：" :content="payApplyDetailForm.payOrderRefuseReason" :isFull="true" />
        <!-- 拉黑说明---20221027改为不显示  -->
        ...
        <DetailDiv  label="申请备注：" :content="payApplyDetailForm.remark" :isFull="true"/>
        ...
        <div v-show="!isApproved" class="settlement-detail-form-remark">
          <el-form label-width="100px" :inline="true" style="width:100%" >
            <el-form-item label="审核备注：" label-width="100px" style="display:flex;" prop="">
              <el-input
                type="textarea"
                autosize
                v-model="payApplyDetailForm.auditOpinion"
                placeholder="审核备注，最多500个字"
                maxlength="500"
                show-word-limit>
              </el-input>
            </el-form-item>
        ...
        <div class="order-box">
          <h3 style="font-size:12px">应收放单</h3>
          <DetailDiv  label="应收核销状态：" :content="renderNoEdit(payApplyDetailForm, {prop: 'recvWriteoffStatus', propInDict: 'writeoffStatus', type: 'select'})"/>
          <DetailDiv  label="放单情况：" :content="renderNoEdit(payApplyDetailForm, {prop: 'putStatus', propInDict: 'finOrderPutStatus', type: 'select'})"/>
          <DetailDiv  label="应收金额：" :content="payApplyDetailForm.orderReciSumAmt" :isTooltip="true"/>
          <DetailDiv  label="应收核销金额：" :content="payApplyDetailForm.orderReciWriteOffAmt" :isTooltip="true"/>
          <DetailDiv  label="应收未核销金额：" :content="payApplyDetailForm.orderReciNoWriteOffAmt" :isTooltip="true"/>
        </div>
      </div>
    </div>
    ...
    <div class="finance-table-container" v-for="(item, index) in payInfoList" :key="index">
      <div class="money-box" style="background: #F8F9FD;">
        <div class="money-box-left">
          <div class="money-title size-12">{{item.currency}}付款信息</div>
        </div>
        <div class="money-box-right">
          ...
          <el-button v-if="checkAuth($route.name, 'btn-settlementAddInvoice')" type="primary" class="finance-btn" @click="identifyElecInvoice(item,index)" size="mini">电子发票识别</el-button>
          <el-button v-if="checkAuth($route.name, 'btn-settlementAddInvoice')" class="finance-btn" @click="addInvoice(item,index)" size="mini">发票登记</el-button>
          <el-button v-if="checkAuth($route.name, 'btn-settlementRelevance') && (applyNo || payApplyDetailForm.confirmStatus !== 'refuse')" class="finance-btn" @click="relevance(item)" size="mini">关联发票</el-button>
          <el-button type="text" class="collapse-btn" @click="handleCollapse(index)" size="mini">{{ item.showTable1 ? '收起' : '展开'}}<i :class="{'el-icon-arrow-up': item.showTable1, 'el-icon-arrow-down': !item.showTable1}"></i></el-button>
        </div>
      </div>
      <div v-show="item.showTable1" class="detail-info-other">
         <el-form :model="item" :ref="'payForm' + index" label-width="110px"  :inline="true" size="mini" style="display: inline-block;">
            <el-form-item label="所属分公司：" prop="payCompName" >
              <el-tooltip class="item" effect="dark" :content="item.payCompName" placement="top">
                <el-input	v-model="item.payCompName" disabled	placeholder="请输入"></el-input>
              </el-tooltip>
            </el-form-item>
            <el-form-item label="付款币种：" prop="currency">
              <el-input	v-model="item.currency" disabled	placeholder="请输入"></el-input>
            </el-form-item>
            <el-form-item label="付款金额：" prop="applyPayAmt">
              <el-input	:value="decimalFixed2(item.applyPayAmt)" disabled	placeholder="请输入"></el-input>
            </el-form-item>
             <el-form-item label="是否后补票：" prop="prepaySum">
              <el-input	:value="dictMapObj.yesNo[item.delayInvoice]" disabled	placeholder="请输入"></el-input>
            </el-form-item>
            <el-form-item label="后补票金额：" prop="prepaySum">
              <el-input	:value="decimalFixed2(item.delayInvoiceAmt)" disabled	placeholder="请输入"></el-input>
            </el-form-item>
            <el-form-item v-if="applyNo" label="专项类手续费：" prop="lessPayAmt">
              <el-input	:value="decimalFixed2(item.lessPayAmt)" disabled	placeholder="请输入"></el-input>
            </el-form-item>
            <!-- <DetailDiv  label="付款币种：" :content="item.currency"/>
            <DetailDiv label="付款金额：" :content="decimalFixed2(item.applyPayAmt)"/>
            <DetailDiv label="专项类手续费：" :content="decimalFixed2(item.lessPayAmt)"/> -->
               <!-- :rules="payWayRules" -->
               <!-- 提示时控制 -->
            <el-form-item v-if="!isApproved" label="付款方式：" prop="payWay" :rules="{ required: true, message: ' ', trigger: 'change' }" >
              <el-select v-model="item.payWay" placeholder="请选择"
            ...
            <el-form-item v-if="item.payWay==='hk_china'" label="手续费扣款方式：" prop="transferFeeDeductWay" :rules="{ required: true, message: ' ', trigger: 'change' }" style="margin-left:10px;" >
            ...
            <el-form-item v-if="isApproved" label="付款方式：" prop="payWay">
              <el-input	:value="renderNoEdit({payWay: item.payWay}, {prop: 'payWay', type: 'select'})" disabled	placeholder="请输入"></el-input>
            </el-form-item>
            <!-- <el-form-item v-if="!isApproved" label="账户结算方式：" prop="settleMode"  :rules="{ required: true, message: ' ', trigger: 'change' }" >
              <el-select v-model="item.settleMode" clearable placeholder="请选择账户结算方式">
                <el-option
            ...
            <!-- <el-form-item v-if="isApproved" label="账户结算方式：" prop="settleMode">
              <el-input	:value="renderNoEdit({payWay: item.bankAccountSettleMode}, {prop: 'settleMode', type: 'select'})" disabled	placeholder="请输入"></el-input>
            </el-form-item> -->
            <el-form-item  v-show="item.payWay === 'prepay_dedue'" label="预付款余额：" prop="prepaySum">
              <el-input	v-model="item.prepaySum" disabled	placeholder=""></el-input>
            </el-form-item>
            <el-form-item v-if="item.hasCustSpecialFee == 'yes' && !applyNo"  label="手续费承担人：" prop="specialDeductRechtstraeger"  :rules="{ required: true, message: ' ', trigger: 'change' }" >
              <el-select :disabled='applyNo' v-model="item.specialDeductRechtstraeger" clearable placeholder="请选择手续费承担人">
                <el-option
            ...
            <el-form-item v-if="applyNo && item.specialDeductRechtstraeger"  label="手续费承担人：" prop="specialDeductRechtstraeger"  >
              <el-select disabled v-model="item.specialDeductRechtstraeger" clearable placeholder="请选择手续费承担人">
                <el-option
                  v-for="item in dictMap['specialDeductRechtstraeger']"
                  :key="item.value"
                  :label="item.label"
            ...
            <el-form-item v-if="isApproved && item.settleMode" label="账户结算方式：" prop="settleMode">
              <el-input	:value="dictMapObj.bankAccountSettleMode[item.settleMode]" disabled	placeholder="请输入"></el-input>
            </el-form-item>
            <el-form-item v-if="item.hasCustSpecialFee==='yes' && item.hasThirdPayChannelCode" label="标识码：" prop="thirdPayChannelCode">
              <el-input	:value="item.thirdPayChannelCode" disabled	placeholder="请输入"></el-input>
            </el-form-item>
            <el-form-item v-if="item.hasCustSpecialFee==='yes' && !item.hasThirdPayChannelCode" label="标识码：" prop="thirdPayChannelCode" :rules="{ required: true, message: ' ', trigger: 'change' }">
              <el-select v-model="item.thirdPayChannelCode" placeholder="请选择"
            ...
            <!-- <div v-if="applyNo" style="display: block;">
              <el-form-item v-show="!isApproved && item.payWay !== 'intrabranch'" label="代大掌柜付款：" prop="internalFeeCode" :rules="{ required: item.payWay !== 'intrabranch' && (item.dzgFeeTaxRate !=='' || item.dzgFeeAmt !== '' || item.dzgPayBillNo !== '' || item.internalFeeCode !==''), message: ' ', trigger: 'blur' }">
                <el-select v-model="item.internalFeeCode" @change="internalFeeChange(item,index)" clearable placeholder="请选择">
                  <el-option
              ...
              <el-form-item v-show="!isApproved && item.payWay !== 'intrabranch'" label="代付大掌柜金额：" prop="dzgFeeAmt" :rules="{ required: item.payWay !== 'intrabranch' && (item.dzgFeeTaxRate !=='' || item.dzgFeeAmt !== '' || item.dzgPayBillNo !== '' || item.internalFeeCode !==''), message: ' ', trigger: 'change' }" >
                <el-input v-lsNumber="{value:'number', obj:item, key:'dzgFeeAmt'}"	v-model="item.dzgFeeAmt" @change="internalFeeChange(item,index)" @input="dzgFeeAmtChange(item)" maxlength="16"	placeholder="请输入"></el-input>
              </el-form-item>
              <el-form-item v-show="!isApproved &&  item.payWay !== 'intrabranch'" label="大掌柜付款单号：" prop="dzgPayBillNo" :rules="{ required: item.payWay !== 'intrabranch' && (item.dzgFeeTaxRate !=='' || item.dzgFeeAmt !== '' || item.dzgPayBillNo !== '' || item.internalFeeCode !==''), message: ' ', trigger: 'change' }" >
                <el-input	v-model="item.dzgPayBillNo" @change="internalFeeChange(item,index)"	placeholder="请输入"></el-input>
              </el-form-item>
              <el-form-item v-show="!isApproved && item.payWay !== 'intrabranch'" label="税率：" prop="dzgFeeTaxRate" :rules="{ required: item.payWay !== 'intrabranch' && (item.dzgFeeTaxRate !=='' || item.dzgFeeAmt !== '' || item.dzgPayBillNo !== '' || item.internalFeeCode !==''), message: ' ', trigger: 'blur' }" >
                <el-select v-model="item.dzgFeeTaxRate" @change="internalFeeChange(item,index)" clearable placeholder="请选择" >
                  <el-option
    ...
    <div v-if="applyNo" class="finance-table-container">
      <div class="money-box">
        <div class="money-box-left">
          <div class="money-title size-12">代大掌柜付款</div>
        </div>
        <div class="money-box-right">
          <el-button v-if="checkAuth($route.name, 'btn-handleInternalFee') && (applyNo) && (!isApproved && payInfoList[0].payWay !== 'intrabranch')" class="finance-btn" @click="handleInternalFee" size="mini">代大掌柜付款</el-button>
        </div>
      </div>
      <FinanceTableMass
        :option="option6"
    ...
    <div v-if="payApplyDetailForm.confirmStatus === 'pass'" class="finance-table-container">
      <div class="money-box">
        <div class="money-box-left">
          <div class="money-title size-12">转账信息</div>
        </div>
        <div class="money-box-right">
        </div>
      </div>
    ...
    <div class="finance-table-container">
      <div class="money-box">
        <div class="money-box-left">
          ...
          <div class="money-total" style="margin-right: 0;">
            <!-- class="money-title-1" -->
            应付结算汇总：
          </div>
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" :loading='loadingRelationInvoice' @click="relevanceConfirm" size="mini">确定</el-button>
    ...
    <el-dialog
      ...
      <FinanceTableMass
        ...
        <!-- <ClipboardUploadEmbed
          ...
          实验剪贴板 -->
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="relevancePrepayConfirm" size="mini">确定</el-button>
        <el-button @click="handleCancel" size="mini">关闭</el-button>
      </span>
    </el-dialog>
    <el-dialog
      ...
      class="relevance-invoice"
      width="600px">
      <el-form :inline="false" label-width="80px" ref="refuseForm" :model="refuseForm"> 
        <el-form-item prop="refuseReason" label="拒绝原因" class="el-form-item-payapply-remark" :rules="[{required: true, message: ' ', trigger: 'blur'}]" >
          <el-input
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="refuseBefore" size="mini">确定</el-button>
    ...
    <el-dialog
      ...
      width="850px">
      <el-form :model="internalFeeData" ref="internalFeeData" :inline="true" label-width="110px"> 
        <div v-for="(item,index) in internalFeeData.list" :key="index">
          ...
          >
            <div class="money-box" style="padding:4px;margin-bottom:8px">
              <div class="money-box-left">
                <div class="money-title size-12">代大掌柜付款{{index+1}}</div>
              </div>
              <div class="money-box-right">
                <el-button
                  v-if="index !== 0"
            ...
            <el-form-item label="代大掌柜付款：" :prop="`list[${index}].internalFeeCode`" :rules="{ required: true, message: ' ', trigger: 'blur' }">
              <el-select v-model="item.internalFeeCode" clearable placeholder="请选择">
                <el-option
                  v-for="it in dictMap['dzgCompCode']"
                  :key="it.value"
                  :label="it.label"
            ...
            <el-form-item label="代付大掌柜金额：" :prop="`list[${index}].dzgFeeAmt`" :rules="{ required: true, message: ' ', trigger: 'blur' }" >
              <el-input 
               v-lsNumber="{value:'number', form:internalFeeData,listStr:'list',rowIndex:index, key:'dzgFeeAmt', vm}"
              v-model="item.dzgFeeAmt" @input="dzgFeeAmtChange" maxlength="16"	placeholder="请输入"></el-input>
            </el-form-item>
            <el-form-item label="大掌柜付款单号：" :prop="`list[${index}].dzgPayBillNo`" :rules="{ required: true, message: ' ', trigger: 'blur' }" >
              <el-input	v-model.trim="item.dzgPayBillNo" 	placeholder="请输入"  ></el-input>
            </el-form-item>
            <el-form-item label="税率：" :prop="`list[${index}].dzgFeeTaxRate`"  :rules="{ required: true, message: ' ', trigger: 'blur' }" >
              <el-select v-model="item.dzgFeeTaxRate" clearable placeholder="请选择" >
                <el-option
                v-for="(op, j) in dictMap.taxRateInfo"
                :key="'currency' + j"
                :label="op.label"
       ...
       <el-button size="mini" @click="handleAddInternalFee" style="width:100%;border: 1px dashed;" > <i class="el-icon-plus"></i>添加代大掌柜付款信息</el-button>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" :loading='loadingInternalFeeSubmit' @click="internalFeeSubmit" size="mini">确定</el-button>
        <el-button @click="handleInternalFeeCancel" size="mini">取消</el-button>
      </span>
    </el-dialog>
    <ApplyForPass v-if="resultDialogOption.show" :option="resultDialogOption" @close="applyForPassOptionClose" @sure="sure">
      <div slot="descript">
        <el-form class="pass_dialog_form" :model="recvData" ref="recvData" :inline='true' size="mini" label-width="90px">
          ...
          <el-form-item label="收款账号" prop="recvAccountNo" :rules="{ required: true, message: ' ', trigger: 'blur' }" v-if="recvData.payWay !== 'prepay_dedue'">
            <el-select disabled v-model="recvData.recvAccountNo" filterable remote :remote-method="bankAccountList" @change="recvAccountNoChange" placeholder="请选择" style="width:100%">
          ...
          <el-form-item class="form_full" label="审核备注" prop="" >
...
export default {
  ...
  created() {
    ...
    // }
    this.columns0 = [
      {prop: 'recvCorpName', label: '结算单位', type: 'text'},
      // {
      ...
      //   remoteSelectList: []
      // },
      // {prop: 'payBankName', label: '付款银行', type: 'text'},
      // {prop: 'payAccountName', label: '付款账户名', type: 'text'},
      // {prop: 'payAccountNo', label: '付款账号',  type: 'text'},
      {
        prop: 'recvAccountNo',
        label: '收款账号',
        type: 'select',
      ...
      {prop: 'recvBankBranch', label: '收款银行支行', type: 'text'},
    ...
    let columns2 = [
      {prop: 'invoiceNo', label: '发票号', key: 'invoiceNo', width:125,
        type: 'button',
        width: 118,
        operationBtns: {
          show: true,
      ...
      {prop: 'invoiceType', label: '开票类型', type: 'select', key: 'invoiceType',propInDict:"invoiceType", width: 120},
      ...
      {prop: 'remainingAmount', label: '剩余可用金额', type: 'text', key: 'remainingAmount', width: 125,
        formatter: ({row}) => {
      ...
      {prop: 'createdTime', label: '登记时间', type: 'text', key: 'createdTime', width: 125},
    ]
    ...
    let columns4 = [
        ...
        { prop: 'payOrderNo', label: '付款单号', width: 146,
          type: 'button',
          operationBtns: {
            show: true,
            callback: (fn, index, row) => {
        ...
        { prop: 'bankSeqNo', label: '银行流水', type: 'text' },
    ...
    let columns5 = [
      {prop: 'settleCompName', label: '所属分公司', type: 'text', width:150},
    ...
    let columns6 = [
      { prop: 'internalFeeCode', label: '代大掌柜付款', type: 'select', propInDict:'dzgCompCode'},
      { prop: 'dzgFeeAmt', label: '代付大掌柜金额', type: 'text' },
      { prop: 'dzgPayBillNo', label: '大掌柜付款单号', type: 'text' },
      { prop: 'dzgFeeTaxRate', label: '税率', type: 'select', propInDict:"taxRateInfo" },
    ]
    ...
    this.optionCheck = Object.assign({}, this.option, {
      ...
      operationBtns: false,
      columns: [
        { prop: 'approverName', label: '审核人', type: 'text' },
        { prop: 'approverDesc', label: '审核人角色', type: 'text' },
        { prop: 'approvalResult', label: '审核状态', type: 'select', propInDict: 'auditStatus' },
        { prop: 'approveEndTime', label: '审核时间', type: 'text' },
        { prop: 'refuseReason', label: '审核拒绝原因', type: 'text' },
        { prop: 'approvalComments', label: '审核备注', type: 'text' }
      ]
    })

    this.getData()

    this.currencyAddOriginal = [{label: '原币种', value: 'original'}].concat(this.dictMap['currency'])
  ...
  methods: {
    ...
    getData() {
      ...
      queryPayApplySettle(data).then(res => {
        if (res.code === 0) {
          ...
          this.payInfoList.forEach((item, index) => {
            ...
            let columns1 = [
              ...
              {prop: 'invoiceType', label: '开票类型', type: 'select', key: 'invoiceType',propInDict:"invoiceType", width:125,},
              ...
              //   }
              // },
              {prop: 'relevanceAmt', label: '关联金额', type: 'text', key: 'relevanceAmt'},
              {prop: 'createdName', label: '登记人', type: 'text', key: 'createdName'},
              {prop: 'createdTime', label: '登记时间', type: 'text', key: 'createdTime', width:125,},
            ]
        ...
        if (this.payApplyDetailForm.hasBlack === 'yes') {
          let title = this.payApplyDetailForm.hasCustBlack === 'yes' ? '该结算单位已在黑名单客户列表被拉黑，请确认是否继续请款？' : this.payApplyDetailForm.hasSupplierBlack === 'yes' ? '该结算单位已在黑名单供应商列表被拉黑，该付款申请需要审计中心进行审核，请确认是否继续请款？' : ''
      ...
      queryPayApplySettleInfo(data).then(res => {
        if (res.code === 0) {
          ...
          this.payInfoList.forEach((item, index) => {
            ...
            //   //   remoteSelectList: []
            //   // },
            //   // {prop: 'payBankName', label: '付款银行', type: 'text', width:125,},
            //   // {prop: 'payAccountName', label: '付款账户名', type: 'text'},
            //   // {prop: 'payAccountNo', label: '付款账号', type: 'text'},
            //   {
            //     prop: 'recvAccountNo',
            ...
            let columns8 = [
              {prop: 'settleCompName', label: '所属分公司', width:150, type: 'text'},
              {prop: 'payOrderNo', label: '付款单号', width:125, type: 'select'},
              {prop: 'settleCorpName', label: '结算单位', width:150, type: 'text'},
              {prop: 'currency', label: '付款币种', type: 'text'},
              {prop: 'realpayAmt', label: '付款金额', type: 'text'}, // balance
              {prop: 'balance', label: '剩余可用金额', type: 'text', // remainingAmount
                // formatter: (row, pItem) => {
    ...
    sure(val){
      if(val){
        if(this.payApplyDetailForm.recvWriteoffStatus !== 'finish' && this.payApplyDetailForm.hasCustSpecialFee === 'yes'){
          this.$confirm('应收未收清，是否继续审核通过？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            if(this.payApplyDetailForm.invoiceRecvAcctEqRest === 'no'){
              this.$confirm( this.payApplyDetailForm.invoiceRecvAcctNotEqWarningMsg ? this.payApplyDetailForm.invoiceRecvAcctNotEqWarningMsg :'发票登记账号和付款账号不一致,是否审核通过', '提示', {
                  cancelButtonText: '取消',
                  confirmButtonText: '确定',
    ...
    submit() {
      ...
      for (let i = 0; i < data.payInfoList.length; i++) {
        if(data.payInfoList[i].specialDeductRechtstraeger === "order" && data.payInfoList[i].specialDeductHasApproval === 'yes'){
          tip = '本次付款申请的装车拼箱费扣减手续费后利润不达标，需要总裁办审批。涉及订单号：'+data.payInfoList[i].specialProfitLowOrderNo +  ' 是否确认提交申请？'
        }
      }
      if (hasBlack === 'yes') {
         tip = `${recvCorpName} 已是黑名单，该付款申请需要进入审计中心审核，是否确认提交？`
      }
      this.$confirm( tip, '', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      ...
      for (let i = 0; i < this.payInfoList.length; i++) {
        ...
        if (this.payInfoList[i].hasCustSpecialFee === 'yes' && option0Data.recvAccountPublicType === 'prv') {
          if (!option0Data.recvIdNo || !option0Data.recvMobileNo) { // !this.payInfoList[i].recvIdNo || !this.payInfoList[i].recvMobileNo
            this.$message({
              message: '对私账户的必填字段“身份证号”、“手机号”为空，请在CRM核实',
              type: 'error',
              showClose: true
            })
            return
    ...
    handleDelayInvoice(type){
      if(this.payApplyDetailForm.delayInvoice===type){
        return this.$msgErrClose('请不要重复操作！')
      }
    ...
    relevance(item) {
      ...
      if(this.applyNo){
        ...
        if(this.payApplyDetailForm.needRelationInvoiceAmt === 0){
          return this.$message({  message: '结算付款申请单'+this.payApplyDetailForm.applyNo+'只有押金类费用，不支持关联应付发票',  type: 'error',  showClose: true })
        }
      }else{
        for (let i = 0; i < this.payInfoList.length; i++) {
          if(this.payInfoList[i].canInvoiceAmt === 0){
            return this.$message({  message: '付款申请单只有押金类费用，不支持应付发票登记',  type: 'error',  showClose: true })
          }
        }
      }
      // 已隐藏按钮
    ...
    payInfoRelationInvoice(tempInvoiceData, selection){
      if (this.applyNo) {
        ...
        this.payInfo.option1.data = tempInvoiceData
        this.$message({
          message: "已选取"+selection.length+"张发票",
          type: 'success',
          showClose: true
        })
      }
    ...
    identifyElecInvoice(item, index) {
      if (this.applyNo) {
        if (this.payApplyDetailForm.needRelationInvoiceAmt === 0) {
          return this.$message({  message: '结算付款申请单'+this.payApplyDetailForm.applyNo+'只有押金类费用，不支持应付发票登记',  type: 'error',  showClose: true })
        }
      } else {
        for (let i = 0; i < this.payInfoList.length; i++) {
          if (this.payInfoList[i].canInvoiceAmt === 0) {

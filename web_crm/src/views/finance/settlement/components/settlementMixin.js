...
export const settlementMixin = {
  ...
  methods: {
    ...
    init() {
      ...
      this.badDebtList = {
        ...
        columns: [
          ...
          { prop: 'badReason', label: '应收冲减原因', type: 'select', propInDict: 'baddebtReason' },
          ...
          { prop: 'witeoffSumRemark', label: '应收冲减金额' },
    ...
    saveSettlement1() {
      ...
      console.log(arrSet, this.$store.state.user.userInfo, this.sourceType === 'express_bill', this.$route.query.settleCompCode)
      if (arrSet.length > 1) {
    ...
    isDetailPageChange() {
      if (this.isDetailPage) {
        ...
        settleOrderFeeList.forEach((item, index) => {
          let feeItem = list[index]
          if (item.feeId !== feeItem.feeId || item.settleAmt != feeItem.settleAmt || item.settleCurrency !== feeItem.settleCurrency || item.exchangeRate !== feeItem.exchangeRate) {

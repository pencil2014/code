<template>
  ...
  <el-dialog :title="option.title" :visible.sync="option.show" width="500px">
    <el-form
      ...
      label-width="100px"
    >
      <el-form-item label="付款账户名" prop="payName">
        <el-input v-model="option.form.payName" placeholder="请输入付款账户名" size="mini" clearable></el-input>
      </el-form-item>
      <el-form-item label="原水单附件" v-if="option.oldFile&&option.oldFile.fileName">
        <el-button @click="openOldFile" size="mini" type="text">{{option.oldFile.fileName}}</el-button>
      </el-form-item>
      <el-form-item prop="file" label="上传水单附件" v-if="option.show">
    ...
    <div slot="footer" class="dialog-footer">
      <el-button @click="submitForm('ruleForm')" size="mini" type="primary" >确定</el-button>

...
>
    ...
    <div class="finance-list-container">
      <div class="money-box">
        <div class="money-box-left">
          <el-button type="success" class="finance-btn" v-if="checkAuth($route.name, 'tab-settleSpecial', 'btn-settlePass')" @click="handleApplyForPass('pass')" size="mini">审核通过</el-button>
          <el-button type="danger" class="finance-btn" v-if="checkAuth($route.name, 'tab-settleSpecial', 'btn-settleRefuse')" @click="handleApplyFor('refuse')" size="mini">审核拒绝</el-button>
        ...
        <div class="money-box-right">
          <el-button-group>
            <el-button class="finance-btn" v-if="checkAuth($route.name, 'tab-settleSpecial', 'btn-settleMerge')" @click="handleMerge" size="mini">合并付款</el-button>
            <el-button class="finance-btn" v-if="checkAuth($route.name, 'tab-settleSpecial', 'btn-settlePaySeparately')" @click="handlePaySeparately" size="mini">拆分付款</el-button>
          </el-button-group>
          <!-- <el-button-group>
    ...
    <div slot="descript">
      <el-form class="pass_dialog_form" :model="recvData" ref="recvData" :inline="true" size="mini" label-width="90px">
        ...
        <el-form-item label="收款账号" prop="recvAccountNo" :rules="{ required: true, message: ' ', trigger: 'blur' }" v-if="recvData.payWay !== 'prepay_dedue'">
          <el-select v-model="recvData.recvAccountNo" filterable remote :remote-method="bankAccountList" @change="recvAccountNoChange" placeholder="请选择" style="width:100%">
  ...
  <el-dialog
    ...
    width="600px">
    <el-form :model="remark" ref="remark" label-width="68px" class="el-form-item-payapply-remark" required>
    <el-form-item label="财务备注"
      prop="finRemark"
      :rules="{required: true, message: '备注原因不能为空', trigger: 'blur'}">
      <el-input type="textarea" placeholder="请输入备注" maxlength="350" show-word-limit v-model="remark.finRemark"></el-input>
    </el-form-item>
    </el-form>
    <span slot="footer" class="dialog-footer">
      <el-button type="primary" :loading="remarkLoading" @click="remarkSubmit" size="mini">确定</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      exportTypeList: [
				{ label: '列表', value: 'payApplySettleSpecialListExport' }, // payApplySettleListExport
  ...
  created() {
    this.searchOption1 = {
      ...
      addFilter:{
        ...
        filterGroups: {
          //1
          allNo: { label: '各类单号', type: 'input', showType: '1' },
          ...
          delayInvoice: {  label: '是否后补票',  type: 'select',prop:"yesNo",  showType: '2'  },
          // payStatus: {  label: '付款状态',  type: 'select',prop:"payStatusType", multiple:true,  showType: '2'  },
          status: {  label: '状态',  type: 'select',prop:"payApplySettleStatus", multiple:true,  showType: '2'  },
          recvFinishStatus: {  label: '应收核销状态',  type: 'select',prop:"writeoffStatus", multiple:true,  showType: '2'  },
          // confirmStatus: {  label: '审核状态',  type: 'select',prop:"payApplySettleStatus", showType: '2'  },
          payWay: {  label: '付款方式',  type: 'select', prop:"payWay", multiple: true, showType: '2' },
          settleOrderType: {  label: '结算类型',  type: 'select', selectOptions: this.dictMap.settleOrderType.filter((v) => (v.value === 'customer_special' || v.value === 'supplier_special' || v.value === 'lien')), multiple:true, },
          ...
          settleCycle: { label: '结算单位结算周期', type: 'select', multiple:true, prop: 'settleCycle' },
          ...
          delayInvoiceAmt: {  label: '后补票金额',  type: 'range',  showType: '5'  },
          dzgFeeAmt: {  label: '代付大掌柜金额',  type: 'range',  showType: '5'  },
          dzgPayBillNo: {  label: '大掌柜付款单号',  type: 'input',  showType: '5'  },
          //6
          applyDate: {  label: '申请时间',  type: 'daterange', format: 'yyyy-MM-dd', showType: '6'  },
          latestDate: {  label: '最晚付款时间',  type: 'daterange', format: 'yyyy-MM-dd', showType: '6'  },
          sendPayOrderUserName: { label: '推送人',  type: 'input',  showType: '6'},
          sendPayOrderTime: {  label: '推送时间',  type: 'daterange', format: 'yyyy-MM-dd', showType: '6'},
          createdName: { label: '申请人',  type: 'input',  showType: '6'},
          remark: { label: '申请备注',  type: 'input',  showType: '6'},
          confirmDate: {  label: '审核时间',  type: 'daterange', format: 'yyyy-MM-dd', showType: '6'},
          confirmName: { label: '审核人',  type: 'input',  showType: '6'},
          auditOpinion: { label: '审核备注',  type: 'input',  showType: '6'},
          payConfirmName: { label: '付款人',  type: 'input',  showType: '6'},
          payTime: { label: '付款时间',  type: 'daterange',  format: 'yyyy-MM-dd', showType: '6'},
          ...
          polCountryCname: { label: '起运港国家',  type: 'input',  showType: '6'},
          // polCountryCode: {
          //   label: '起运港国家',
          //   type: 'remoteSelect',
          //   multiple:true,
    ...
    this.passTableOption = Object.assign({}, this.option, { operationBtns: false, selection: {show: false},
      ...
      { prop: 'applyNo', label: '付款申请单号', type: 'text', width: '130px'},
      { prop: 'applyPayAmt', label: '金额', type: 'text', width: '90px' },
      { prop: 'latestDate', label: '最晚付款时间', type: 'datePicker', width: '120px' },
      // { prop: 'latestDate', label: '最晚付款时间', type: 'datePicker', width: '120px', required: true },
      { prop: 'auditOpinion', label: '审核备注', type: 'input', minWidth: '320px'},
    ],})
  ...
  methods: {
        ...
        this.$refs.recvData.validate((valid) =>{
          ...
          if(valid){
            ...
            if(this.multipleSelection1.find(item => item.recvWriteoffStatus !== 'finish' && item.hasCustSpecialFee === 'yes')){
              this.$confirm('含有应收未收清的申请单，是否继续审核通过？', '提示', {
                ...
                if(invoiceRecvAcctEqRestList.length){
                  this.$confirm( invoiceRecvAcctEqRestList[0].invoiceRecvAcctNotEqWarningMsg ? invoiceRecvAcctEqRestList[0].invoiceRecvAcctNotEqWarningMsg :'发票登记账号和付款账号不一致,是否审核通过', '提示', {
                    cancelButtonText: '取消',
                    confirmButtonText: '确定',
    ...
    handleApplyForPass(){
      ...
      if(!this.selectedValueSame(multiData, ['recvCorpCode', 'recvAccountName', 'currency', 'payWay'])){
        return this.$msgErrClose('批量操作需要满足的条件：同结算单位、收款账户相同、币种相同、付款方式相同！')
      }
    ...
    handleApplyFor(confirmStatus) {
      ...
      if(this.userSettleCompanyCode !== row.settleCompCode){
        return this.$msgErrClose(`您没有权限操作${row.settleCompName}的数据，请在系统右上角切换分公司！`)
      }
    ...
    handlePaySeparately(){
      ...
      if (this.multipleSelection1[0].hasGroupAgent === 'yes') {
        return this.$msgErrClose('集团付款申请单不允许进行拆分付款！')
      }
      ...
      if (this.multipleSelection1[0].confirmStatus !== 'pending') {
        return this.$msgErrClose('待审核状态才可进行拆分付款！请选择待审核状态！')
      }
      if (this.multipleSelection1[0].hasCustSpecialFee === 'yes') {
        return this.$msgErrClose('客户专项类付款申请单不允许进行拆分付款！')
      }
    ...
    financeReceive(val) {
      ...
      console.log(data);
      this.$confirm('是否确认财务'+val.label+'？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => {

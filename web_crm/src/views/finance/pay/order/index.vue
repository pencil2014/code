    ...
    <div class="finance-list-container">
      <div class="money-box">
        <div class="money-box-left">
          <el-button-group>
            <el-button type="primary" v-if="checkAuth($route.name, 'tab-orderList', 'btn-orderWriteOffPreview')" @click="writeOffPreview('refuse')" class="finance-btn" size="mini">付款核销</el-button>
          </el-button-group>
          <el-button-group>
            <el-button type="primary" v-if="checkAuth($route.name, 'tab-orderList', 'btn-orderPrintCreate')" @click="batchPrintCreate" class="finance-btn" size="mini">标记受理</el-button>
          </el-button-group>
          <el-button-group>
            <el-button type="purple" v-if="checkAuth($route.name, 'tab-orderList', 'btn-orderCanclePayOrder')"  @click="canclePayOrder" class="finance-btn" size="mini">撤销标记受理</el-button>
          </el-button-group>
          <el-button-group>
        ...
        <div class="money-box-right">
          <el-button-group>
            ...
            <el-button v-if="checkAuth($route.name, 'tab-orderList', 'btn-orderRefusePay')" type="danger" @click="refusePay" class="finance-btn" size="mini">拒绝付款</el-button>
          </el-button-group>
          <el-button-group>
            <el-button class="finance-btn" v-if="checkAuth($route.name, 'tab-orderList', 'btn-orderDownLoadInvoice')" @click="handleDownLoadInvoice" size="mini">批量下载水单</el-button>
          </el-button-group>
          <el-button-group>
            <el-button class="finance-btn" v-if="checkAuth($route.name, 'tab-orderList', 'btn-bankseqFileFn')" @click="handleBankseqFileFn" size="mini">发送水单</el-button>
          </el-button-group>
          <el-button-group>
            <el-button v-if="checkAuth($route.name, 'tab-orderList', 'btn-orderMerge')" @click="handleMerge" class="finance-btn" size="mini">合并付款</el-button>
            <!-- <el-button class="finance-btn" @click="handlePaySeparately" size="mini">拆分付款</el-button> -->
          </el-button-group>
          <el-button-group>
            <el-button v-if="checkAuth($route.name, 'tab-orderList', 'btn-orderRefund')" @click="handleRefund" class="finance-btn" size="mini">预付退款</el-button>
          </el-button-group>
          <!-- <el-button-group>
...
export default {
  ...
  data() {
    return {
      ...
      selfColumnsBase: [
        ...
        { prop: 'bankseqFileSendStatus', label: '发送水单状态', width: 100, formatter: ({row}) => {
    ...
    this.searchOption1 = {
      ...
      addFilter: {
        ...
        filterGroups: {
          ...
          settleOrderType: {  label: '结算类型',  type: 'select', prop:"settleOrderType", multiple:true, showType: '2'  },
          ...
          // 5
          payAmt: { label: '付款金额', type: 'range', showType: '5' },
          writeoffAmt: { label: '已核销金额', type: 'range', showType: '5' },
          noWriteoffAmt: { label: '未核销金额', type: 'range', showType: '5' },
          // 6
          applyDate: { label: '创建时间', type: 'daterange', showType: '6' },
          payDate: { label: '付款时间', type: 'daterange', showType: '6' },
          // 7
          orderType: {
            label: '付款单类型',
            type: 'select',
          ...
          payApplyConfirmName: { label: '付款单审核人',  type: 'input',  showType: '6'},
          applyEmpName: { label: '申请人',  type: 'input',  showType: '6'},
          errorStatus: {
            label: '付错标识',
            type: 'select',
            prop: 'yesNo',
          ...
          confirmTime: { label: '受理时间', type: 'daterange', showType: '8' },
        }
      }
    }
  ...
  methods: {
    ...
    convertRes(data, multi) {
      return data.map(item => {
        ...
        item.errorDisposalType =
          this.dictMapObj.payErrorDisposalType[item.errorDisposalType] || ''
        item.errorDisposalStatus =
    ...
    handleExport(code) {
      ...
      let text = code === 'payOrderListExport' ? '是否确认导出付款单?' : '是否确认导出对外付款汇总?'
      let title = code === 'payOrderListExport' ? '导出付款单' : '导出对外付款汇总'
        ...
        .then(() => {
          ...
          this.$store
            ...
            .then(res => {
              ...
              let name =  code === 'payOrderListExport' ? '付款单.xlsx' : '对外付款汇总.xlsx'
              link.download = name
              link.click()
            })
            .catch(() => {

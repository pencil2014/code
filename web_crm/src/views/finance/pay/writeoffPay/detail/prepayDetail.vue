  ...
  <div class="order-detail">
    <div class="finance-approve-detail">
      <div class="finance-approve-btns-box">
        <div class="finance-approve-btns-left">{{ dictMapObj.payOrderType[payOrder.orderType] }}核销</div>
      ...
      <div class="finance-approve-detail-main">
        ...
        <div class="finance-detail-info">
          ...
          <div class="detail-info-other">
            <span class="detail-info-item-max">{{ dictMapObj.payOrderType[payOrder.orderType] }}单号：{{payOrder.sourceApplyNo}}</span>
            <!-- <span class="detail-info-item">退款请款编号：{{}}</span> -->
        ...
        <div class="finance-detail-info" v-show="transferFees.length">
          ...
          <div v-for="(item,index) in transferFees" :key="index">
              ...
              <span class="detail-info-item">本次分摊：
                <!-- 本次分摊必须为负数-->
                <el-input v-model="item.allotAmt" disabled v-lsNumber="{value:'number', obj:item, key:'allotAmt'}" type="text" size="mini"></el-input>
              </span>
          </div>
        </div>
        <div class="finance-detail-info">
          ...
          <span class="detail-info-item">核销汇率：
            <el-input v-model="infoData.writeoffExRate" v-lsNumber="{value:'number', obj:infoData, key:'writeoffExRate',dot:11}" type="text" size="mini" ></el-input> 
          </span> -->
          <span class="detail-info-item">结算币种：{{payOrder.currency}}</span>
          <span >其他应付：
            <span>
              <el-select size="mini" filterable v-model="query.otherPayables" disabled placeholder="请选择">
                <el-option
              ...
              <el-input style="margin-left:12px" v-model="query.otherPayAmt" disabled type="text" size="mini" ></el-input>
            </span>
          </span>
        </div>
...
export default {
  ...
  methods: {
    getData() {
      queryPayWriteoff({writeoffReceipayNo:this.writeoffReceipayNo}).then(res => {
        if (res.code === 0) {
          ...
          this.settleExRateSummarys = settleExRateSummarys&&settleExRateSummarys.length ? settleExRateSummarys : settleExRateSummarysData
          this.writeoffRateSummarys = settleExRateSummarys&&settleExRateSummarys.length ? settleExRateSummarys : settleExRateSummarysData
        }
      }).catch(err=>{
        console.log(err)
        this.delVisitedView()
    ...
    submit(){
      if (!this.validateMoney(this.query.otherPayAmt,{positive:true})) {
        return
      }
      let transferFees = this.transferFees
      for (let i = 0; i < transferFees.length; i++) {
        if (!this.validateMoney(transferFees[i].allotAmt,{positive:true})) {
          return
        }
      }
      let data ={
...
.order-detail .detail-info-other{
  margin:12px 0;
}
.detail-info-item-1 {
  display: block;

  ...
  <div class="order-detail">
    <div class="finance-approve-detail">
      <div class="finance-approve-btns-box">
        <div class="finance-approve-btns-left">结算付款核销</div>
        <div class="finance-approve-btns-right">
          <el-button @click="cancel" size="mini">返回</el-button>
          <el-button @click="submit" :disabled="payInfo.payWriteoffStatus !== 'yes'" size="mini">核销</el-button>
      ...
      <div class="finance-approve-detail-main">
        ...
        <div class="finance-detail-info">
          ...
          <div class="detail-info-other">
            ...
            <span class="detail-info-item">银行流水号：{{payOrder.bankSeqNo}}</span>
            <span class="detail-info-item">付款币种：{{payOrder.currency}}</span>
            <span class="detail-info-item">付款金额：{{payOrder.applyAmt}}</span>
        ...
        <div class="finance-detail-info" v-show="transferFees.length">
          ...
          <div v-for="(item,index) in transferFees" :key="index">
              ...
              <span class="detail-info-item">本次分摊：
                <el-input v-model="item.allotAmt" type="text" :disabled="payInfo.payWriteoffStatus !== 'yes'" v-lsNumber="{value:'number', obj:item, key:'allotAmt'}" size="mini" ></el-input>
              </span>
          </div>
        </div>
        <div class="finance-detail-info">
          ...
          <span >其他应付：
            <span>
              <el-select size="mini" filterable v-model="query.otherPayables" :disabled="payInfo.payWriteoffStatus !== 'yes'" placeholder="请选择">
                <el-option
                  v-for="(o, key) in dictMap['lessPayDealway']"
                  :key="'lessPayDealway'+key"
                  :label="o.label"
                  :value="o.value"
                ></el-option>
              </el-select>
              <el-input style="margin-left:12px" placeholder="请输入其他应付金额" v-model="query.otherPayAmt" :disabled="payInfo.payWriteoffStatus !== 'yes'" v-lsNumber="{value:'number', obj:query, key:'otherPayAmt'}" type="text" size="mini" ></el-input>
            </span>
          </span>
          <!-- <div class="detail-info-item">
            <span>结算汇率：</span>
            <span v-for="(item,index) in settleExRateSummarys" :key="index">
          ...
          <!-- <el-form class="detail-info-item1" size="mini">
            <div style="width:270px">
              <div class="">
                <div class="settlement-exchange-rates-title">结算汇率</div>
                  <el-form-item
                    v-for="(item, index) in settleExRateSummarys"
                    :key="'settlementRate' + index"
                    :label="item.originalCurrency+'-'+item.settleCurrency">
                    <div
                      v-html="item.settleExRate"
                      placeholder="请输入核销汇率">
                    </div>
                  </el-form-item>
              </div>
          </div>
          <div style="width:270px;display: block;">
                <div class="">
                  <div class="settlement-exchange-rates-title">核销汇率</div>
                    <el-form-item
                      v-for="(item, index) in writeoffRateSummarys"
                      :key="'settlementRate' + index"
                      :label="item.settleCurrency+'-'+item.writeoffCurrency">
                      <el-input
                        v-model="item.writeoffExRate"
                        v-lsNumber="{value:'number', obj:item, key:'writeoffExRate',dot:11}"
                        placeholder="请输入核销汇率">
          ...
          <!-- <span class="detail-info-item">核销汇率：
            <span v-for="(item,index) in settleExRateSummarys" :key="index">
              <span>{{item.settleCurrency}}</span>-<span>{{item.writeoffCurrency}}</span>
              <el-input v-model="infoData.writeoffExRate" placeholder="请输入核销汇率" type="text" size="mini" v-lsNumber="{value:'number', obj:form, key:'applyPayAmt'}"></el-input> 
            </span>
...
export default {
  ...
  created() {
    ...
    let columns1 = [
      ...
      {prop: 'writeoffExRate', label: '核销汇率', type: 'input',  key: 'writeoffExRate', notAllowed: true, inputType:"number", dot:11},
      {prop: 'writeoffFeeAmt', label: '核销金额', type: 'input',  key: 'writeoffFeeAmt', notAllowed: true, inputType:"number"},
      {prop: 'serviceName', label: '服务项', type: 'text', key: 'serviceName'},
      {prop: 'feeUnit', label: '计费单位', type: 'text',
        formatter: (row, pItem) => { 
  ...
  methods: {
    ...
    getData() {
      queryPayWriteoff({writeoffReceipayNo:this.writeoffReceipayNo}).then(res => {
        if (res.code === 0) {
          ...
          let list = []
          settleExRateSummarys.forEach(item=>{
            let data ={
    ...
    submit(){
      let list = this.transferFees
      for (let i = 0; i < list.length; i++) {
        if(Number(list[i].allotAmt) > 0){
          this.$message({
            type: 'success',
            message: '本次分摊不能大于0!',
            showClose: true
      ...
      let data ={
        ...
        otherPayables:this.query.otherPayables,
        otherPayAmt:this.query.otherPayAmt,
    ...
    writeOffPreview() {
      ...
      confirmPayWriteoff({writeoffReceipayId:this.writeoffReceipayId }).then(res => {
        if (res.code === 0) {
          this.$message({
            type: 'success',
            message: '提交成功!',

<template>
  <div class="finance-tab-page"
      ...
      <div class="finance-list-container">
        <div class="money-box">
          <div class="money-box-left">
            <el-button-group>
              <el-button type="success" class="finance-btn" v-if="checkAuth($route.name, 'tab-payApplyReceiveRefund', 'btn-payApplyReceiveRefundPass')" @click="handleApplyFor('pass')" size="mini">申请通过</el-button>
            </el-button-group>
            <el-button-group>
              <el-button  type="danger" class="finance-btn" v-if="checkAuth($route.name, 'tab-payApplyReceiveRefund', 'btn-payApplyReceiveRefundRefuse')" @click="handleApplyFor('refuse')" size="mini">申请拒绝</el-button>
            </el-button-group>
            <el-button-group>
    ...
    width="600px">
      <el-form :model="formQuery" ref="formQuery" label-width="80px" class="el-form-item-payapply-remark" required>
        <el-form-item label="拒绝原因"
        prop="refuseReason"
        :rules="{required: true, message: '拒绝原因不能为空', trigger: 'blur'}">
          <el-input type="textarea" placeholder="请输入拒绝原因" maxlength="200" show-word-limit v-model="formQuery.refuseReason"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" :loading='loadingSubmit' @click="handleSubmit" size="mini">确定</el-button>
    ...
    this.searchOption1 = {
      ...
      addFilter:{
        ...
        filterGroups: {
          ...
          applyNo: { label: '退款申请号',  type: 'input',  showType: '1'  },
          receivableRegNo: { label: '收款登记号',  type: 'input',  showType: '1'},
          payOrderNo: { label: '关联付款单号',  type: 'input',  showType: '1'},
          //2
          settleCorpCode: {
            label: '结算单位',
            type: 'remoteSelect',
          ...
          //3
          refundCurrency: {  label: '币种',  type: 'select', selectOptions: this.dictMap.currency.filter((v) => v.value != 'ORIGINAL'), showType: '3'  },
          
          //4
          confirmStatus: {  label: '审核状态',  type: 'select', prop:"confirmStatus", multiple:true, showType: '4'  },
          refundType: {  label: '退款类型',  type: 'select', prop:"refundType", multiple:true, showType: '4'  },
          payStatus: {  label: '退款付款状态',  type: 'select', prop:"payStatusType", multiple:true, showType: '4'  },
          payOrderConfirmStatus: {  label: '付款审核状态',  type: 'select', prop:"auditStatus", showType: '4'  },
          
          //5
          realRefundAmt: {  label: '实退金额',  type: 'range',  showType: '5'  },
          //6
          recvBankName: {  label: '收款银行',  type: 'input',  showType: '6'  },
          recvAccountName: {  label: '收款名称',  type: 'input',  showType: '6'  },
          recvAccountNo: {  label: '收款账号',  type: 'input',  showType: '6'  },
          //7
          createdTime: {  label: '申请时间',  type: 'daterange',  showType: '7', format: 'yyyy-MM-dd'},
          createdName: {  label: '申请人',  type: 'input',  showType: '7'  },
          confirmTime: {  label: '审核时间',  type: 'daterange',  showType: '7', format: 'yyyy-MM-dd'},
          confirmName: {  label: '审核人',  type: 'input',  showType: '7'  },
          payDate: { label: '付款时间',  type: 'daterange',  showType: '7', format: 'yyyy-MM-dd'},
    ...
    handleApplyFor(confirmStatus) {
      ...
      if(confirmStatus ==="pass" ){
        ...
        this.$confirm('您是否确认申请通过?', "申请通过", {
          ...
          this.$message({message: '申请通过提交成功', type: 'success'});
          this.getData()
        }).catch(function() {});

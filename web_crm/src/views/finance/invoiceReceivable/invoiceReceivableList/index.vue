...
export default {
  ...
  methods: {
    ...
    relateSettlement() {
      ...
      if (!(this.multipleSelection1[0].status == 'normal' && this.multipleSelection1[0].invoiceGroup == 'normal')) {
        this.$msgErrClose('正常发票才能进行关联！')
        return
      }
      if (this.multipleSelection1[0].lockAmt == 0) {
        this.$msgErrClose('可关联费用金额为0，不能进行关联！')
        return
      }
      this.rowData = this.multipleSelection1[0]
      this.btnType = 'relate'

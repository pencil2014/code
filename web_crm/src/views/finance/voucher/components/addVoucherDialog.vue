          ...
          { prop: 'dimensions', label: '核算维度', type: 'add', minWidth: 130 },
          {
            ...
            change: (value, row) => {
              if (this.form.finDate) {
                getKdExchangeRate({ srcCurrency: value, finMon: this.form.finDate.slice(0, 7) }).then((res) => {
    ...
    'form.finDate'(val) {
      if (val) {
        this.tableOption.data.forEach((item) => {
          if (item.srcCurrency) {
            getKdExchangeRate({ srcCurrency: item.srcCurrency, finMon: val.slice(0, 7) }).then((res) => {

<template>
  <el-dialog
    ...
    class="follow-add"
    :title="`${options.isEdit ? '编辑' : '新增'}跟进记录`"
    width="1050px"
  >
    ...
    >
      ...
      <!-- qq wechat email telephone -->
      <template v-if="showQQ || showTelephone || showEmail || showWechat">
        ...
        <el-form-item label="长帆目标占比:"  prop="goalProportion"  key="goalProportion">
          <el-input  v-model="form.goalProportion"  placeholder="请填写" maxlength="32"></el-input>
        </el-form-item>
        <!-- 客户联系人 -->
        <div class="outer-box">
          <div class="title">客情描述</div>
            ...
            <div class="item-cnt">
              <el-form-item  label="姓名:" key="custContact" :prop="`custDescription.${custIndex}.custContact`"
                ...
                <el-button size="mini" type="success" class="addbtn"  @click="showEditDialog(custIndex,custItem.custContact)" v-if="custItem.custContact">编辑</el-button>
              </el-form-item>
          ...
          <div class="outer-item">
            ...
            >
              <div class="item-cnt">
                ...
                >
                  <el-select v-model="item.tradeCode" placeholder="请选择" 
                  :style="(item.tradeCode=='dap'||item.tradeCode=='ddu'||item.tradeCode=='ddp')?'width:30%':'width:100%'">
                    <el-option
                      :label="v.label"
                      :value="v.value"
                      :key="v.value"
                ...
                <el-input
                 v-if="item.tradeCode=='dap'||item.tradeCode=='ddu'||item.tradeCode=='ddp'"
                  style="width:70%"
                  v-model="item.shippingAddr"
                  placeholder="请输入收货地址"
                  :disabled="isDisabled"
                > </el-input>
                </el-form-item>
      ...
      <!-- qq wechat email telephone -->
      <template v-if="showQQ || showTelephone || showEmail || showWechat ||showQuotation">
        <el-form-item
          key="nextPlan"
          label="下一步计划:"
          prop="nextPlan"
...
export default {
  ...
  created() {
    ...
    if (this.options.isEdit || this.options.form.commuId && ['bilateral', 'other'].includes(this.options.form.relationFlag)) {
      ...
      if(this.form.custDescription?.length){
        this.form.custDescription.forEach((item) => {
          if(item.custContact&&item.custContact.includes('#')||item.contactId){
      ...
      if (['unilateral'].includes(this.options.form.relationFlag)) {
        ...
        this.form.visitors = [...this.options.form.accompanyNameLists]
        if(this.options.visitPlanNo){
          this.form.visitPlanNo = this.options.visitPlanNo
        }
        if (this.form.custDescription.length) {
          this.form.custDescription.forEach(item => {
    ...
    // 控制公司规模
    // if (['peer'].includes(this.baseInfo.custClass)) { //国内同行 item.value.includes("dp") || item.value.includes("dc"),
      this.scaleList = this.dictMap['orgScale'].filter((item) => item.value.includes('dp')|| item.value.includes("dc"))
    // } else if (['cust'].includes(this.baseInfo.custClass)) {
    //   this.scaleList = this.dictMap['orgScale'].filter((item) => item.value.includes('dc'))
    // }
  },
  watch: {
    'form.commuType'(val) {
      ...
      if (val === 'visit') {
        ...
        this.rules.isCooperation.required =
          ...
          this.isTrue =
            sta
        this.isQQ = false
      } else if (['qq', 'email', 'wechat', 'telephone'].includes(val)) {
        this.rules.shipmentType.required = true //出货类型必填
        this.isQQ = true
        this.isTrue = false
      }else if(val === 'inquiry_quotation'){
        this.isQQ = false
      }
    },
  },
  ...
  methods: {
    ...
    // 跟进人
    getEmployeeList(str = '') {
      if((this.$store.state.user.userId !==this.form.bdEmployeeId) && !this.options.isEdit){
        str = this.$store.state.user.name
      }
      const data = {
        currPage: 1,
        pageSize: 100,
        ...
        .then((res) => {
          if (res.code === 0) {
            ...
            if (typeof this.form.bdEmployeeId !== 'object') {
              if((this.$store.state.user.userId !==this.form.bdEmployeeId) && !this.options.isEdit){
                 source = res.data.list.find(
                (item) => item.employeeId === this.$store.state.user.userId
                )
              }else{
                 source = res.data.list.find(
    ...
    //编辑显示弹框
    showEditDialog(index,val){
      this.getSelectOne(val,index)
    },
    checkNumber(val) {
      let obj = val
      obj = obj.toString()

		...
		<!-- :before-upload="getFile" 改在handleChangeFile里面调用getFile做相关处理，主要是为了autoUpload为false时做处理，否则用before-upload亦可 -->
			<div class="file-upload">
				...
				<div class="file-upload__text">
					...
					<p class="info">
						支持 {{this.comTypes}} 格式文件， 且文件大小限制在{{defaultFileSize}}M内
					</p>
				</div>
			</div>
		</el-upload>
...
export default {
	...
	methods: {
		...
		clearFile(){
			this.resFileList = []
			this.handleRemove(this.file,this.fileList,true)
			this.$emit('send-res-file-list',[],this.index)
		},
		handleImport (done) {
			this.handleProgress()
			// 根据后台需求数据格式
		...
		fileListEdit: {
			...
			handler(newVal) {
               if(newVal && newVal.length){
					...
					this.resFileList = newVal.length ? [...newVal] : []
				}
			}
		}
	}

...
const state = {
	...
	whJointList: {
		...
		specialColumns: whOrderList.specialColumns,
		columnsBase: handleOrderNoInfo => {
			return whJointList.columnsBase.concat(whOrderList.columnsBase(handleOrderNoInfo)).filter(item => !['custName'].includes(item.key)).map(item => {
				return { ...item, minWidth: item.width }
			})
		}
		// columnsBase: whOrderList.columnsBase
	...
	blInfoList: {
		...
		specialColumns: blList.specialColumns,
		columnsBase: handleOrderNoInfo => {
			return blInfoList.columnsBase
				.concat(blList.columnsBase(handleOrderNoInfo))
				.map(item => {
					return { ...item, minWidth: item.width }
				})
		}
	...
	orderList: {
		...
		// 订单详情可写入角色集合
		allEditRoleArr: ['bd', 'obd', 'asst', 'op', 'op_truck', 'op_customs','op_wh','doc', 'ac', 'eac', 'line_cs', 'bkg', 'csc', 'ecs', 'epricing'],
	...
	blFinList: {
		...
		// 表头库
		columnsBase(handleViewSo, handleViewCustName, handleOrderNo, handleToOrderFee, handleArrearAmt, handleOverQuotaAmt, handleViewWorkFlowActList) {
			return [
				{
					label: '是否特殊申请',
					key: 'isSpecial',
	...
	blFinApplyList: {
		...
		// 表头库
		columnsBase(handleViewSo, handleViewCustName, handleViewCustSettleUnit, handleOrderNo, handleToOrderFee, handleViewWorkFlowActList) {
			return [
				{
					label: '申请人',
					key: 'applyEmployeeName',
	...
	// 工厂不同意列表
	factoryDisagree: {
		defaultColumns: ['blNo','orderNo','custName','businessType','blMode','isDraftConfirm','siStatus','copyCheckStatus','isAgreeRelease','auditStatus','takeStatus','putStatus','agreeReleaseStatus','agreeApplyEmployeeName','agreeApplyReason','agreeAuditEmployeeName','agreeAuditReason'],
		specialColumns: [],
		// 表头库
		columnsBase(handleOrderNoInfo) {
			return [

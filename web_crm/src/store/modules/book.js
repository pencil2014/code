...
const state = {
	...
	bookList: {
		...
		occupy: {
			defaultColumns: bookList.defaultColumns.filter(item => !['orderNo', 'jointNo', 'orderStatus', 'isExternal', 'bdDeptName', 'containerWeight', 'orgAuditEmployeeName'].includes(item)),
			specialColumns: bookList.specialColumns,
			columnsBase: (handleBkgReqNoInfo, roles, type) => {
				return bookList
					.columnsBase(handleBkgReqNoInfo, roles, type)
